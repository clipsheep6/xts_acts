...
static void TestInstallerCallback(const uint8_t resultCode, const void *resultMessage)
{
    ...
    /* 当resultCode为OPERATION_DOING时bundle manage service也会调用callback，
       这里需要等待install/uninstall最终成功或者失败的结果，因此忽略直接返回 */
...
static BOOL BundleMgrTestSuiteSetUp(void)
{
    ...
    // 预先安装testdemo1.bin应用，GetBundleInfo、QueryAbilityInfo接口可以直接查询到应用信息
    char hapFile[DIR_PATH_MAX] = {0};
    sprintf_s(hapFile, sizeof(hapFile), "%s", PREPARED_APP_BIN_NAME);
...
LITE_TEST_CASE(BundleMgrTestSuite, testInstallLegal, Function | MediumTest | Level0)
{
    ...
    char hapFile[DIR_PATH_MAX] = {0};
    sprintf_s(hapFile, sizeof(hapFile), "%s", INSTALL_APP_BIN_NAME);

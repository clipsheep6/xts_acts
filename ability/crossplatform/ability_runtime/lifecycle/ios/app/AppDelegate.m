...
#import "FeatureMainViewController.h"
...
- (BOOL)handleOpenUrlWithBundleName:(NSString *)bundleName
    ...
    } else if ([moduleName isEqualToString:@"feature"] && [abilityName   isEqualToString:@"MainAbility"]) {
        NSString *instanceName = [NSString stringWithFormat:@"%@:%@:%@",bundleName, moduleName, abilityName];
        FeatureMainViewController *entryOtherVC = [[FeatureMainViewController alloc] initWithInstanceName:instanceName];
        entryOtherVC.params = params;
        subStageVC = (FeatureMainViewController *)entryOtherVC;

...
export default class Hap2MainAbility4 extends Ability {
  ...
  onWindowStageCreate(windowStage: window.WindowStage) {
    ...
    windowStage.loadContent('pages/Hap2MainAbility4_pages', null);
  }
  ...
  onForeground() {
    ...
    globalThis.fs.open(uri, globalThis.fs.OpenMode.READ_ONLY)
      .then((file) => {
    ...
    globalThis.fs.open(uri, globalThis.fs.OpenMode.READ_WRITE)
      .then((file) => {
        console.info('file fd: ' + file.fd);
    ...
    commonEvent.createSubscriber(subscribeInfo)
      .then(async (data) => {
        ...
        await commonEvent.subscribe(subscriber__1, async (err, data) => {

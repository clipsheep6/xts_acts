...
export default class EntryAbility extends UIAbility {
  onCreate(want, launchParam) {
    ...
    if (want.action === 'Acts_CardClickCallback_1700') {
      let errCode = '';
      try {
      ...
      try {
        formObserver.on('router', 'com.acts.cardclickcallback', callBack);
      } catch (err) {
        errCode += err.code;
      }
      try {
        formObserver.off('router', 'com.acts.cardclickcallback', callBack);
      } catch (err) {
        errCode += err.code;
        let commEventData = {
          parameters: {

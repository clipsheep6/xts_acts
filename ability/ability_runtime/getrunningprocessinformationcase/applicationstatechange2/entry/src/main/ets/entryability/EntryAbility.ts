...
const TAG = "app_test_2"
let applicationStateChangeCallback = {
  onApplicationForeground() {
    console.log(TAG, 'applicationStateChangeCallback onApplicationForeground');
    CommonEvent.publish("APP2_onApplicationForeground", () => {
      console.log(TAG, 'publish onApplicationForeground');
    });

  },
  onApplicationBackground() {
    console.log(TAG, 'applicationStateChangeCallback onApplicationBackground');
    CommonEvent.publish("APP2_onApplicationBackground", () => {
      console.log(TAG, 'publish onApplicationBackground');
    });
  }
};
export default class EntryAbility extends UIAbility {
  ...
  onCreate(want, launchParam) {
    ...
    let subscribeInfo = {
      events: ["CALL_APP2_TERMINATE_SELF"]
    };
    ...
    CommonEvent.createSubscriber(subscribeInfo).then((commonEventSubscriber) => {
      ...
      CommonEvent.subscribe(this.subscriber, (err, data) => {
        if (err.code) {
          ...
          if (data.event === "CALL_APP2_TERMINATE_SELF") {
            taht.terminateSelf()
          }
        }
      });
    ...
    globalThis.applicationContext.on('applicationStateChange', applicationStateChangeCallback);
  }
  ...
  onDestroy() {
    ...
    if (this.subscriber) {
      setTimeout(() => {
        globalThis.applicationContext.off('applicationStateChange', applicationStateChangeCallback);

...
export default class EntryAbility extends Ability {
  ...
  onForeground() {
    ...
    if (AppStorage.get<Want>("want")!.action == "Acts_RegisterJsErrorCallback_0100") {
      observer = {
        onUnhandledException(strMsg) {
          ...
          commonEvent.publish("ACTS_RegisterEvent_First", commonEventData, (err) => {
            ...
            setTimeout(() => {
              AppStorage.get<common.UIAbilityContext>("context")!.terminateSelf()
                .then(() => {
                  console.info("====>Acts_RegisterJsErrorCallback_0100 rely terminateSelf end");
                })
                .catch((err) => {
                  console.info("====>Acts_RegisterJsErrorCallback_0100 rely terminateSelf err: " + JSON.stringify(err));
                });
            }, 100);
          })
        }
    ...
    } else if (AppStorage.get<Want>("want")!.action == "Acts_RegisterJsErrorCallback_0200") {
      observer = {
        onUnhandledException(strMsg: String) {
          commonEventData.parameters.message = strMsg;
          console.info("====>Acts_RegisterJsErrorCallback_0200 onUnhandledException Message: " + strMsg);
          commonEvent.publish("ACTS_RegisterEvent_First", commonEventData, (err) => {
            console.info("====>Acts_RegisterJsErrorCallback_0200 onUnhandledException publish err: " + JSON.stringify(err));
          })
        },
        onException(errObject: Error) {
          ...
          commonEvent.publish("ACTS_RegisterEvent_Second", commonEventData, (err) => {
            console.info("====>Acts_RegisterJsErrorCallback_0200 onException publish err: " + JSON.stringify(err));
            setTimeout(() => {
              AppStorage.get<common.UIAbilityContext>("context")!.terminateSelf().then(() => {
                console.info("====>Acts_RegisterJsErrorCallback_0200 rely terminateSelf end");
              }).catch((err) => {
                console.info("====>Acts_RegisterJsErrorCallback_0200 rely terminateSelf err: " + JSON.stringify(err));
              })
            }, 100)
          })
        }
    ...
    } else if (AppStorage.get<Want>("want")!.action == "Acts_UnregisterJsErrorCallback_0800") {
      observer = {
        onUnhandledException(strMsg: String) {
          commonEventData.parameters.message = strMsg;
          console.info("====>Acts_UnregisterJsErrorCallback_0800 onUnhandledException Message: " + strMsg);
          commonEvent.publish("ACTS_RegisterEvent_First", commonEventData, (err) => {
            console.info("====>Acts_UnregisterJsErrorCallback_0800 onUnhandledException publish err: " + JSON.stringify(err));
          })
        },
        onException(errObject: Error) {
          commonEventData.parameters.message = errObject;
          ...
          commonEvent.publish("ACTS_RegisterEvent_Second", commonEventData, (err) => {
            console.info("====>Acts_UnregisterJsErrorCallback_0800 onException publish err: " + JSON.stringify(err));
            setTimeout(() => {
              AppStorage.get<common.UIAbilityContext>("context")!.terminateSelf()
                .then(() => {
                ...
                .catch((err) => {
                  console.info("====>Acts_UnregisterJsErrorCallback_0800 rely terminateSelf err: " + JSON.stringify(err));
                })
            }, 100)
          })
        }
      ...
      ErrorManager.off("error", observerId)
        ...
        .catch((err) => {
          console.info("====>Acts_UnregisterJsErrorCallback_0800 rely off err: " + JSON.stringify(err));
        })
      setTimeout(() => {
        AppStorage.get<common.UIAbilityContext>("context")!.terminateSelf();
      }, 300);

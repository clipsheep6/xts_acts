...
export default class MultipleAbility extends UIAbility {
  onCreate(want, launchParam) {
    console.info('====> [RelyMultipleAbility] MultipleAbility start');
    abilityWant = want;
    ...
    if (want.action === 'SUB_Ability_AbilityRuntime_terminateProcess_0100') {
      try {
        applicationContext.terminateProcess().catch((err: BusinessError) => {
          console.info('====> [RelyMultipleAbility] 0100 terminateApplication err: ' + JSON.stringify(err));
        });
      } catch (err) {
        console.info('====> [RelyMultipleAbility] 0100 terminateApplication throw err: ' + JSON.stringify(err));
      }
      ...
      this.context.startAbility({
        ...
        console.info('====>[RelyMultipleAbility] 0400 startAbility err:' + JSON.stringify(err));
      })
      ...
      try {
        applicationContext.terminateApplication().catch((err: BusinessError) => {
          console.info('====> [RelyMultipleAbility] 0700 terminateApplication err: ' + JSON.stringify(err));
        });
      } catch (err) {
        console.info('====> [RelyMultipleAbility] 0700 terminateApplication throw err: ' + JSON.stringify(err));
      }
  ...
  onDestroy() {
    ...
    console.info('====>[RelyMultipleAbility] onDestroy');
  }

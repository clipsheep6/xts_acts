...
const SQL_CREATE_TABLE = 'CREATE TABLE IF NOT EXISTS book' + '(contact_id INTEGER PRIMARY KEY AUTOINCREMENT, name TEXT NOT NULL, age INTEGER, introduction TEXT NOT NULL, salary FLOAT)';
let rdbStore: ESObject = undefined;
...
export default {
  ...
  insert(uri, valueBucket, callback) {
    ...
    rdbStore.insert(TABLE_NAME, valueBucket, (err, resultSet) => {
      console.log(TAG + "insert callback resultSet:" + resultSet + " ,json=" + JSON.stringify(resultSet) + ' ,err' + err);
  ...
  update(uri, valueBucket, predicates, callback) {
    ...
    err = "Error update";
    let rdbPredicates = dataAbility.createRdbPredicates(TABLE_NAME, predicates);
    rdbStore.update(valueBucket, rdbPredicates, (err, resultSet) => {
      console.log(TAG + "insert callback resultSet:" + resultSet + " ,json=" + JSON.stringify(resultSet) + ' ,err' + err);
      callback(err, defaultReturn);
    })
  },
  ...
  delete(uri, predicates, callback) {
    ...
    let rdbPredicates = dataAbility.createRdbPredicates(TABLE_NAME, predicates);
    rdbStore.delete(rdbPredicates, (err, resultSet) => {
      console.log(TAG + "insert callback resultSet:" + resultSet + " ,json=" + JSON.stringify(resultSet) + ' ,err' + err);
      callback(err, defaultReturn);
    })
  },
  ...
  batchInsert(uri, values, callback) {
    ...
    for (let j = 0; j < values.length; j++) {
      rdbStore.insert("EMPLOYEE", values[j], (err, ret) => {

...
export default {
    ...
    onShare(formId) {
        ...
        if(hasFormInfo){
            let newFormInfo = formInfoMap.get(formId);
            wantParams.formName = newFormInfo.formName;
            wantParams.tempFlag = newFormInfo.tempFlag;
            wantParams.updateCount = newFormInfo.updateCount;
            wantParams.temperature = "22";
            wantParams.time = "12:36:00";
            wantParams.imageSrc = "/common/hot.png";
            console.log(`FormMgrService FormAbility onShare success, formId = ${formId}`);
            console.log(`FormMgrService FormAbility onShare, want: ${JSON.stringify(wantParams)}`);
        } else {
            console.log(`FormMgrService FormAbility onShare failed, formId = ${formId}`);
        }
        return wantParams;
    },
    ...
    onUpdate(formId) {
        ...
        upDateForm(formId);
        console.log(`FormMgrService FormAbility onUpdate end, formId = ${formId}`);
    },
    ...
    onEvent(formId,message){
        ...
        if(commonEventData.parameters.message == '{"action":"message","params":{"message":"MessageEvent_0100"}}'){
            commonEvent.publish("ACTS_MESSAGE_ID1",commonEventData,(err)=>{
                console.debug(`ACTS_InAppCardMessageEvent_0400====>publish err:` + JSON.stringify(err));
            })
        }
        if(commonEventData.parameters.message == '{"action":"message","params":{}}'){
            commonEvent.publish("ACTS_MESSAGE_ID2",commonEventData,(err)=>{
                console.debug(`ACTS_InAppCardMessageEvent_0500====>publish err:` + JSON.stringify(err));
            })
        }
        if(commonEventData.parameters.message == '{"action":"message","params":{"message":""}}'){
            commonEvent.publish("ACTS_MESSAGE_ID3",commonEventData,(err)=>{
                console.debug(`ACTS_InAppCardMessageEvent_0600====>publish err:` + JSON.stringify(err));
            })
        }
    }
}

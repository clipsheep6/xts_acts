...
const TAG: string = '[testTag] EmbeddedUIExtAbilityB';
const DOMAIN = 0x0000;
const BUNDLE = 'EmbeddedUIExtAbilityB_';

export default class EmbeddedUIExtAbilityB extends EmbeddedUIExtensionAbility {
  onCreate() {
    ...
    commonEventManager.publish(
      'Rely_EmbeddedUIExtAbilityB_Create', commonEventPublishData, (err) => {
      if (err) {
        hilog.info(DOMAIN, TAG, BUNDLE + 'Rely_EmbeddedUIExtAbilityB_Create error');
      } else {
        hilog.info(DOMAIN, TAG, BUNDLE + 'Rely_EmbeddedUIExtAbilityB_Create successful');
      }
    })
  }
  ...
  onSessionCreate(want: Want, session: UIExtensionContentSession) {
    ...
    session.loadContent('pages/EmbeddedUIExtAbilityPageB', storage);
  }
  ...
  onConfigurationUpdate(newConfig: Configuration): void {
    ...
    commonEventManager.publish(
      'Rely_EmbeddedUIExtAbilityB_ConfigurationUpdate', commonEventPublishData, (err) => {
      if (err) {
        hilog.info(DOMAIN, TAG, BUNDLE + 'Rely_EmbeddedUIExtAbilityB_ConfigurationUpdate error');
      } else {
        hilog.info(DOMAIN, TAG, BUNDLE + 'Rely_EmbeddedUIExtAbilityB_ConfigurationUpdate successful');
      }
    })
  }
}
...
const TAG: string = '[testTag] EmbeddedUIExtAbilityA';
const DOMAIN = 0x0000;
const BUNDLE = 'EmbeddedUIExtAbilityA_';

export default class EmbeddedUIExtAbilityA extends EmbeddedUIExtensionAbility {
  onCreate() {
    ...
    commonEventManager.publish(
      'Rely_EmbeddedUIExtAbilityA_Create', commonEventPublishData, (err) => {
      if (err) {
        hilog.info(DOMAIN, TAG, BUNDLE + 'Rely_EmbeddedUIExtAbilityA_Create error');
      } else {
        hilog.info(DOMAIN, TAG, BUNDLE + 'Rely_EmbeddedUIExtAbilityA_Create successful');
      }
    })
  }
  ...
  onSessionCreate(want: Want, session: UIExtensionContentSession) {
    ...
    session.loadContent('pages/EmbeddedUIExtAbilityPageA', storage);
  }
  ...
  onConfigurationUpdate(newConfig: Configuration): void {
    ...
    commonEventManager.publish(
      'Rely_EmbeddedUIExtAbilityA_ConfigurationUpdate', commonEventPublishData, (err) => {
      if (err) {
        hilog.info(DOMAIN, TAG, BUNDLE + 'Rely_EmbeddedUIExtAbilityA_ConfigurationUpdate error');
      } else {
        hilog.info(DOMAIN, TAG, BUNDLE + 'Rely_EmbeddedUIExtAbilityA_ConfigurationUpdate successful');
      }
    })
  }
}
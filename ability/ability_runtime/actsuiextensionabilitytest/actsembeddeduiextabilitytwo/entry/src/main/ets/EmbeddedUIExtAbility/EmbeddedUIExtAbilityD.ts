...
const TAG: string = '[testTag] EmbeddedUIExtAbilityD';
const DOMAIN = 0x0000;
const BUNDLE = 'EmbeddedUIExtAbilityD_';

export default class EmbeddedUIExtAbilityD extends EmbeddedUIExtensionAbility {
  onCreate() {
    ...
    commonEventManager.publish(
      'Rely_EmbeddedUIExtAbilityD_Create', commonEventPublishData, (err) => {
      if (err) {
        hilog.info(DOMAIN, TAG, BUNDLE + 'Rely_EmbeddedUIExtAbilityD_Create error');
      } else {
        hilog.info(DOMAIN, TAG, BUNDLE + 'Rely_EmbeddedUIExtAbilityD_Create successful');
      }
    })
  }
  ...
  onSessionCreate(want: Want, session: UIExtensionContentSession) {
    ...
    session.loadContent('pages/EmbeddedUIExtAbilityPageD', storage);
  }
  ...
  onConfigurationUpdate(newConfig: Configuration): void {
    ...
    commonEventManager.publish(
      'Rely_EmbeddedUIExtAbilityD_ConfigurationUpdate', commonEventPublishData, (err) => {
      if (err) {
        hilog.info(DOMAIN, TAG, BUNDLE + 'Rely_EmbeddedUIExtAbilityD_ConfigurationUpdate error');
      } else {
        hilog.info(DOMAIN, TAG, BUNDLE + 'Rely_EmbeddedUIExtAbilityD_ConfigurationUpdate successful');
      }
    })
  }
}
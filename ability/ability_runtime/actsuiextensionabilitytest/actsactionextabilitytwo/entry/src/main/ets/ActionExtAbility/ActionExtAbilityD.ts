...
const TAG: string = '[testTag] ActionExtAbilityD';
const DOMAIN = 0x0000;
const BUNDLE = 'ActionExtAbilityD_';

export default class ActionExtAbilityD extends ActionExtensionAbility {
  onCreate() {
    ...
    commonEventManager.publish(
      'Rely_ActionExtAbilityD_Create', commonEventPublishData, (err) => {
      if (err) {
        hilog.info(DOMAIN, TAG, BUNDLE + 'Rely_ActionExtAbilityD_Create error');
      } else {
        hilog.info(DOMAIN, TAG, BUNDLE + 'Rely_ActionExtAbilityD_Create successful');
      }
    })
  }
  ...
  onSessionCreate(want: Want, session: UIExtensionContentSession) {
    ...
    session.loadContent('pages/ActionExtAbilityPageD', storage);
  }
  ...
  onConfigurationUpdate(newConfig: Configuration): void {
    ...
    commonEventManager.publish(
      'Rely_ActionExtAbilityD_ConfigurationUpdate', commonEventPublishData, (err) => {
      if (err) {
        hilog.info(DOMAIN, TAG, BUNDLE + 'Rely_ActionExtAbilityD_ConfigurationUpdate error');
      } else {
        hilog.info(DOMAIN, TAG, BUNDLE + 'Rely_ActionExtAbilityD_ConfigurationUpdate successful');
      }
    })
  }
}
...
const TAG: string = '[testTag] ActionExtAbilityC';
const DOMAIN = 0x0000;
const BUNDLE = 'ActionExtAbilityC_';

export default class ActionExtAbilityC extends ActionExtensionAbility {
  onCreate() {
    ...
    commonEventManager.publish(
      'Rely_ActionExtAbilityC_Create', commonEventPublishData, (err) => {
      if (err) {
        hilog.info(DOMAIN, TAG, BUNDLE + 'Rely_ActionExtAbilityC_Create error');
      } else {
        hilog.info(DOMAIN, TAG, BUNDLE + 'Rely_ActionExtAbilityC_Create successful');
      }
    })
  }
  ...
  onSessionCreate(want: Want, session: UIExtensionContentSession) {
    ...
    session.loadContent('pages/ActionExtAbilityPageC', storage);
  }
  ...
  onConfigurationUpdate(newConfig: Configuration): void {
    ...
    commonEventManager.publish(
      'Rely_ActionExtAbilityC_ConfigurationUpdate', commonEventPublishData, (err) => {
      if (err) {
        hilog.info(DOMAIN, TAG, BUNDLE + 'Rely_ActionExtAbilityC_ConfigurationUpdate error');
      } else {
        hilog.info(DOMAIN, TAG, BUNDLE + 'Rely_ActionExtAbilityC_ConfigurationUpdate successful');
      }
    })
  }
}
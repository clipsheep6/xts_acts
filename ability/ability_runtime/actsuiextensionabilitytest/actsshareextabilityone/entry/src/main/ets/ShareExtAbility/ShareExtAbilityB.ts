...
const TAG: string = '[testTag] ShareExtAbilityB';
const DOMAIN = 0x0000;
const BUNDLE = 'ShareExtAbilityB_';

export default class ShareExtAbilityB extends ShareExtensionAbility {
  onCreate() {
    ...
    commonEventManager.publish(
      'Rely_ShareExtAbilityB_Create', commonEventPublishData, (err) => {
      if (err) {
        hilog.info(DOMAIN, TAG, BUNDLE + 'Rely_ShareExtAbilityB_Create error');
      } else {
        hilog.info(DOMAIN, TAG, BUNDLE + 'Rely_ShareExtAbilityB_Create successful');
      }
    })
  }
  ...
  onSessionCreate(want: Want, session: UIExtensionContentSession) {
    ...
    session.loadContent('pages/ShareExtAbilityPageB', storage);
  }
  ...
  onConfigurationUpdate(newConfig: Configuration): void {
    ...
    commonEventManager.publish(
      'Rely_ShareExtAbilityB_ConfigurationUpdate', commonEventPublishData, (err) => {
      if (err) {
        hilog.info(DOMAIN, TAG, BUNDLE + 'Rely_ShareExtAbilityB_ConfigurationUpdate error');
      } else {
        hilog.info(DOMAIN, TAG, BUNDLE + 'Rely_ShareExtAbilityB_ConfigurationUpdate successful');
      }
    })
  }
}
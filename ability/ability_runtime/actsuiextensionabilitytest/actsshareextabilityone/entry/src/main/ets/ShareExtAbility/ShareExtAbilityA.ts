...
const TAG: string = '[testTag] ShareExtAbilityA';
const DOMAIN = 0x0000;
const BUNDLE = 'ShareExtAbilityA_';

export default class ShareExtAbilityA extends ShareExtensionAbility {
  onCreate() {
    ...
    commonEventManager.publish(
      'Rely_ShareExtAbilityA_Create', commonEventPublishData, (err) => {
      if (err) {
        hilog.info(DOMAIN, TAG, BUNDLE + 'Rely_ShareExtAbilityA_Create error');
      } else {
        hilog.info(DOMAIN, TAG, BUNDLE + 'Rely_ShareExtAbilityA_Create successful');
      }
    })
  }
  ...
  onSessionCreate(want: Want, session: UIExtensionContentSession) {
    ...
    session.loadContent('pages/ShareExtAbilityPageA', storage);
  }
  ...
  onConfigurationUpdate(newConfig: Configuration): void {
    ...
    commonEventManager.publish(
      'Rely_ShareExtAbilityA_ConfigurationUpdate', commonEventPublishData, (err) => {
      if (err) {
        hilog.info(DOMAIN, TAG, BUNDLE + 'Rely_ShareExtAbilityA_ConfigurationUpdate error');
      } else {
        hilog.info(DOMAIN, TAG, BUNDLE + 'Rely_ShareExtAbilityA_ConfigurationUpdate successful');
      }
    })
  }
}
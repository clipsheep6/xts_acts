...
const TAG: string = '[testTag] UIExtAbilityD';
const DOMAIN = 0x0000;
const BUNDLE = 'UIExtAbilityD_';
...
export default class UIExtAbilityB extends UIExtensionAbility {
  onCreate() {
    ...
    commonEventManager.publish(
      'Rely_UIExtAbilityD_Create', commonEventPublishData, (err) => {
      if (err) {
        hilog.info(DOMAIN, TAG, BUNDLE + 'Rely_UIExtAbilityD_Create error');
      } else {
        hilog.info(DOMAIN, TAG, BUNDLE + 'Rely_UIExtAbilityD_Create successful');
      }
    })
  }
  onForeground() {
  ...
  onSessionCreate(want: Want, session: UIExtensionContentSession) {
    ...
    session.loadContent('pages/UIExtAbilityPageD', storage);
  }
  ...
  onConfigurationUpdate(newConfig: Configuration): void {
    ...
    commonEventManager.publish(
      'Rely_UIExtAbilityD_ConfigurationUpdate', commonEventPublishData, (err) => {
      if (err) {
        hilog.info(DOMAIN, TAG, BUNDLE + 'Rely_UIExtAbilityD_ConfigurationUpdate error');
      } else {
        hilog.info(DOMAIN, TAG, BUNDLE + 'Rely_UIExtAbilityD_ConfigurationUpdate successful');
      }
    })
  }
}
...
const TAG: string = '[testTag] ShareExtAbilityD';
const DOMAIN = 0x0000;
const BUNDLE = 'ShareExtAbilityD_';

export default class ShareExtAbilityD extends ShareExtensionAbility {
  onCreate() {
    ...
    commonEventManager.publish(
      'Rely_ShareExtAbilityD_Create', commonEventPublishData, (err) => {
      if (err) {
        hilog.info(DOMAIN, TAG, BUNDLE + 'Rely_ShareExtAbilityD_Create error');
      } else {
        hilog.info(DOMAIN, TAG, BUNDLE + 'Rely_ShareExtAbilityD_Create successful');
      }
    })
  }
  ...
  onSessionCreate(want: Want, session: UIExtensionContentSession) {
    ...
    session.loadContent('pages/ShareExtAbilityPageD', storage);
  }
  ...
  onConfigurationUpdate(newConfig: Configuration): void {
    ...
    commonEventManager.publish(
      'Rely_ShareExtAbilityD_ConfigurationUpdate', commonEventPublishData, (err) => {
      if (err) {
        hilog.info(DOMAIN, TAG, BUNDLE + 'Rely_ShareExtAbilityD_ConfigurationUpdate error');
      } else {
        hilog.info(DOMAIN, TAG, BUNDLE + 'Rely_ShareExtAbilityD_ConfigurationUpdate successful');
      }
    })
  }
}
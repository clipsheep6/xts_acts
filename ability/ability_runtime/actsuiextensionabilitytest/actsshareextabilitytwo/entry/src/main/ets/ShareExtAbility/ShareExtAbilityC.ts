...
const TAG: string = '[testTag] ShareExtAbilityC';
const DOMAIN = 0x0000;
const BUNDLE = 'ShareExtAbilityC_';

export default class ShareExtAbilityC extends ShareExtensionAbility {
  onCreate() {
    ...
    commonEventManager.publish(
      'Rely_ShareExtAbilityC_Create', commonEventPublishData, (err) => {
      if (err) {
        hilog.info(DOMAIN, TAG, BUNDLE + 'Rely_ShareExtAbilityC_Create error');
      } else {
        hilog.info(DOMAIN, TAG, BUNDLE + 'Rely_ShareExtAbilityC_Create successful');
      }
    })
  }
  ...
  onSessionCreate(want: Want, session: UIExtensionContentSession) {
    ...
    session.loadContent('pages/ShareExtAbilityPageC', storage);
  }
  ...
  onConfigurationUpdate(newConfig: Configuration): void {
    ...
    commonEventManager.publish(
      'Rely_ShareExtAbilityC_ConfigurationUpdate', commonEventPublishData, (err) => {
      if (err) {
        hilog.info(DOMAIN, TAG, BUNDLE + 'Rely_ShareExtAbilityC_ConfigurationUpdate error');
      } else {
        hilog.info(DOMAIN, TAG, BUNDLE + 'Rely_ShareExtAbilityC_ConfigurationUpdate successful');
      }
    })
  }
}
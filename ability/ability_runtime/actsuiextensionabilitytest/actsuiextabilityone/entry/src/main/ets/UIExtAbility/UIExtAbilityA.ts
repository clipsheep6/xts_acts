...
const TAG: string = '[testTag] UIExtAbilityA';
const DOMAIN = 0x0000;
const BUNDLE = 'UIExtAbilityA_';
...
export default class UIExtAbilityA extends UIExtensionAbility {
  onCreate() {
    ...
    commonEventManager.publish(
      'Rely_UIExtAbilityA_Create', commonEventPublishData, (err) => {
      if (err) {
        hilog.info(DOMAIN, TAG, BUNDLE + 'Rely_UIExtAbilityA_Create error');
      } else {
        hilog.info(DOMAIN, TAG, BUNDLE + 'Rely_UIExtAbilityA_Create successful');
      }
    })
  }
  ...
  onSessionCreate(want: Want, session: UIExtensionContentSession) {
    ...
    session.loadContent('pages/UIExtAbilityPageA', storage);
  }
  ...
  onConfigurationUpdate(newConfig: Configuration): void {
    ...
    commonEventManager.publish(
      'Rely_UIExtAbilityA_ConfigurationUpdate', commonEventPublishData, (err) => {
      if (err) {
        hilog.info(DOMAIN, TAG, BUNDLE + 'Rely_UIExtAbilityA_ConfigurationUpdate error');
      } else {
        hilog.info(DOMAIN, TAG, BUNDLE + 'Rely_UIExtAbilityA_ConfigurationUpdate successful');
      }
    })
  }
}
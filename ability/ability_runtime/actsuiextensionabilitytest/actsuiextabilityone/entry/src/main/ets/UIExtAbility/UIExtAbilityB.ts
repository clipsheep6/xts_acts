...
const TAG: string = '[testTag] UIExtAbilityB';
const DOMAIN = 0x0000;
const BUNDLE = 'UIExtAbilityB_';

export default class UIExtAbilityB extends UIExtensionAbility {
  onCreate() {
    ...
    commonEventManager.publish(
      'Rely_UIExtAbilityB_Create', commonEventPublishData, (err) => {
      if (err) {
        hilog.info(DOMAIN, TAG, BUNDLE + 'Rely_UIExtAbilityB_Create error');
      } else {
        hilog.info(DOMAIN, TAG, BUNDLE + 'Rely_UIExtAbilityB_Create successful');
      }
    })
  }
  ...
  onSessionCreate(want: Want, session: UIExtensionContentSession) {
    ...
    session.loadContent('pages/UIExtAbilityPageB', storage);
  }
  ...
  onConfigurationUpdate(newConfig: Configuration): void {
    ...
    commonEventManager.publish(
      'Rely_UIExtAbilityB_ConfigurationUpdate', commonEventPublishData, (err) => {
      if (err) {
        hilog.info(DOMAIN, TAG, BUNDLE + 'Rely_UIExtAbilityB_ConfigurationUpdate error');
      } else {
        hilog.info(DOMAIN, TAG, BUNDLE + 'Rely_UIExtAbilityB_ConfigurationUpdate successful');
      }
    })
  }
}
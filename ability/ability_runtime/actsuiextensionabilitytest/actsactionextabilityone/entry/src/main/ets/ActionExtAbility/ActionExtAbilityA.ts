...
const TAG: string = '[testTag] ActionExtAbilityA';
const DOMAIN = 0x0000;
const BUNDLE = 'ActionExtAbilityA_';

export default class ActionExtAbilityA extends ActionExtensionAbility {
  onCreate() {
    ...
    commonEventManager.publish(
      'Rely_ActionExtAbilityA_Create', commonEventPublishData, (err) => {
      if (err) {
        hilog.info(DOMAIN, TAG, BUNDLE + 'Rely_ActionExtAbilityA_Create error');
      } else {
        hilog.info(DOMAIN, TAG, BUNDLE + 'Rely_ActionExtAbilityA_Create successful');
      }
    })
  }
  ...
  onSessionCreate(want: Want, session: UIExtensionContentSession) {
    ...
    session.loadContent('pages/ActionExtAbilityPageA', storage);
  }
  ...
  onConfigurationUpdate(newConfig: Configuration): void {
    ...
    commonEventManager.publish(
      'Rely_ActionExtAbilityA_ConfigurationUpdate', commonEventPublishData, (err) => {
      if (err) {
        hilog.info(DOMAIN, TAG, BUNDLE + 'Rely_ActionExtAbilityA_ConfigurationUpdate error');
      } else {
        hilog.info(DOMAIN, TAG, BUNDLE + 'Rely_ActionExtAbilityA_ConfigurationUpdate successful');
      }
    })
  }
}

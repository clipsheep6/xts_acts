...
const TAG: string = '[testTag] ActionExtAbilityB';
const DOMAIN = 0x0000;
const BUNDLE = 'ActionExtAbilityB_';

export default class ActionExtAbilityB extends ActionExtensionAbility {
  onCreate() {
    ...
    commonEventManager.publish(
      'Rely_ActionExtAbilityB_Create', commonEventPublishData, (err) => {
      if (err) {
        hilog.info(DOMAIN, TAG, BUNDLE + 'Rely_ActionExtAbilityB_Create error');
      } else {
        hilog.info(DOMAIN, TAG, BUNDLE + 'Rely_ActionExtAbilityB_Create successful');
      }
    })
  }
  ...
  onSessionCreate(want: Want, session: UIExtensionContentSession) {
    ...
    session.loadContent('pages/ActionExtAbilityPageB', storage);
  }
  ...
  onConfigurationUpdate(newConfig: Configuration): void {
    ...
    commonEventManager.publish(
      'Rely_ActionExtAbilityB_ConfigurationUpdate', commonEventPublishData, (err) => {
      if (err) {
        hilog.info(DOMAIN, TAG, BUNDLE + 'Rely_ActionExtAbilityB_ConfigurationUpdate error');
      } else {
        hilog.info(DOMAIN, TAG, BUNDLE + 'Rely_ActionExtAbilityB_ConfigurationUpdate successful');
      }
    })
  }
}
...
#ifndef OHOS_FACE_AUTH_SYSTEM_TEST_ABILITY_UTIL_H
#define OHOS_FACE_AUTH_SYSTEM_TEST_ABILITY_UTIL_H
...
class SystemTestAbilityUtil {
public:
    static std::shared_ptr<SystemTestAbilityUtil> GetInstance();
    static void DestroyInstance();
    ~SystemTestAbilityUtil() = default;
     ...
     */
    static ErrCode StartAbility(const AAFwk::Want &want, sptr<AAFwk::IAbilityManager> &abilityMs,
     ...
     */
    static AAFwk::Want MakeWant(std::string deviceId, std::string abilityName, std::string bundleName,
        MAP_STR_STR params = {});
     ...
     * @Introduction: Get Top AbilityRecord ID
     */
    static ErrCode KillApplication(const std::string &appName, sptr<AppExecFwk::IAppMgr> &appMs,
        const time_t &delay = 0);
     ...
     * @Introduction: Get Top AbilityRecord ID
     */
    static AppExecFwk::RunningProcessInfo GetAppProcessInfoByName(const std::string &processName,
        sptr<AppExecFwk::IAppMgr> &appMs, const time_t &delay = 0);
     ...
     */
    static int WaitCompleted(STtools::Event &event, const std::string &eventName, const int code,
     ...
     */
    static void DeserializationStOperatorFromVector(STtools::StOperator &ParentOperator,
     ...
     */
    static void Install(const std::string &bundleFilePath,
        const AppExecFwk::InstallFlag installFlag = AppExecFwk::InstallFlag::NORMAL);
    ...
    /**
     *
     * @Introduction: remove auth file
     */
    static void RemoveAuthFile();

    /**
     *
     * @Introduction:  recovery auth file
     */
    static void ExeShellCommand(std::string commandStr);

    /**
     *
     * @param  {int} caseNum
     * @param  {int} codeNum
     * @param  {std::map<int, ResultInfo>} resultInfos_
     * @Introduction: push StOperator Class Object In Vector
     */
    static void WriteFile(int caseNum, int codeNum, std::map<int, UserIAM::FaceAuth::ResultInfo> resultInfos_);

    void static InstallApp(const std::string &hapName);
    void static UninstallApp(const std::string &bundleName);
...
private:
    ...
    static std::shared_ptr<SystemTestAbilityUtil> instance_;
    static std::mutex mutex_;
};
} // namespace STABUtil
} // namespace OHOS
#endif // OHOS_FACE_AUTH_SYSTEM_TEST_ABILITY_UTIL_H

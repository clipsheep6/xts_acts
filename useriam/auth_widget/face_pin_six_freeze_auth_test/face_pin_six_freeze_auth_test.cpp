...
class ActsFacePinSixFreezeAuthTest : public testing::Test {
public:
...
void ActsFacePinSixFreezeAuthTest::SetUpTestCase(void)
{
}

void ActsFacePinSixFreezeAuthTest::TearDownTestCase(void)
{
}

void ActsFacePinSixFreezeAuthTest::SetUp()
{
}

void ActsFacePinSixFreezeAuthTest::TearDown()
{
}

HWTEST_F(ActsFacePinSixFreezeAuthTest, ActsFacePinSixFreezeAuthTest, TestSize.Level1)
{
    ...
    int32_t dialogId = -1;
    IAM_LOGI("ActsFacePinSixFreezeAuthTest ShowDialog start");
    int32_t width = 0, height = 0;
    GetDisplaySize(width, height);
    IAM_LOGI("ActsFacePinSixFreezeAuthTest ShowDialog width=%{public}d, height=%{public}d", width, height);
    Ace::UIServiceMgrClient::GetInstance()->ShowDialog("authentication_dialog", params,
        ...
        std::bind(&ActsFacePinSixFreezeAuthTest::FacePinAuthCallback, this, std::placeholders::_1,
            std::placeholders::_2, std::placeholders::_3),
        &dialogId);
    EXPECT_TRUE(dialogId >= 0);
    IAM_LOGI("ActsFacePinSixFreezeAuthTest ShowDialog end, dialogId:%{public}d", dialogId);
    while (!isStop) {
        sleep(1);
    }
...
void ActsFacePinSixFreezeAuthTest::FacePinAuthCallback(int32_t id, const std::string &event, const std::string &params)
{
    IAM_LOGI("ActsFacePinSixFreezeAuthTest FacePinAuthCallback start");
    ...
    switch (eventType) {
        case JsEventType::EVENT_AUTH_TYPE_READY: {
            IAM_LOGI("ActsFacePinSixFreezeAuthTest FacePinAuthCallback EVENT_AUTH_TYPE_READY");
            HandleEventAuthTypeReady(id, jsonParams);
            break;
        }
        case JsEventType::EVENT_AUTH_USER_CANCEL: {
            IAM_LOGI("ActsFacePinSixFreezeAuthTest FacePinAuthCallback EVENT_AUTH_USER_CANCEL");
            break;
        }
        case JsEventType::EVENT_AUTH_USER_CANCEL_ALL: {
            IAM_LOGI("ActsFacePinSixFreezeAuthTest FacePinAuthCallback EVENT_AUTH_USER_CANCEL_ALL");
            Ace::UIServiceMgrClient::GetInstance()->CancelDialog(id);
            isStop = true;
            break;
        }
        case JsEventType::EVENT_ANIMATION_PLAY_END: {
            IAM_LOGI("ActsFacePinSixFreezeAuthTest FacePinAuthCallback EVENT_ANIMATION_PLAY_END");
            Ace::UIServiceMgrClient::GetInstance()->CancelDialog(id);
            isStop = true;
            break;
        }
        case JsEventType::EVENT_KEY_DERIVATION_READY: {
            IAM_LOGI("ActsFacePinSixFreezeAuthTest FacePinAuthCallback EVENT_KEY_DERIVATION_READY");
...
void ActsFacePinSixFreezeAuthTest::SendFaceAuthResult(int32_t id)
{
    ...
    IAM_LOGI("ActsFacePinSixFreezeAuthTest SendFaceAuthResult %{public}s", resultOss.str().c_str());
    Ace::UIServiceMgrClient::GetInstance()->UpdateDialog(id, resultOss.str());
}

void ActsFacePinSixFreezeAuthTest::HandleEventAuthTypeReady(int32_t id, const Json::Value &jsonParams)
{
    ...
    if (strcmp(jsonParams["payload"]["type"].asCString(), "face") == 0) {
        ...
        IAM_LOGI("ActsFacePinSixFreezeAuthTest HandleEventAuthTypeReady %{public}s", oss.str().c_str());
        Ace::UIServiceMgrClient::GetInstance()->UpdateDialog(id, oss.str());
        sendResultTimer.Register(std::bind(&ActsFacePinSixFreezeAuthTest::SendFaceAuthResult, this, id), sendDelay,
            true);
...
void ActsFacePinSixFreezeAuthTest::HandleEventKeyDerivationReady(int32_t id, const Json::Value &jsonParams)
{
    if (strcmp(jsonParams["payload"]["result"].asCString(), "success") == 0) {
        ...
        Json::Value resultPayload;
        Json::Value cmdArray;
        IAM_LOGI("ActsFacePinSixFreezeAuthTest HandleEventKeyDerivationReady derivation data %{public}s",
            jsonParams["payload"]["derivation"].asCString());
        if (strcmp(jsonParams["payload"]["derivation"].asCString(), pinSixCode) == 0) {
            IAM_LOGI("ActsFacePinSixFreezeAuthTest HandleEventKeyDerivationReady pin auth success");
            ...
            timesValue["payload"] = timesPayload;
            IAM_LOGI(
                "ActsFacePinSixFreezeAuthTest HandleEventKeyDerivationReady pin auth failed, times left:%{public}d",
                pinTimesLeft);

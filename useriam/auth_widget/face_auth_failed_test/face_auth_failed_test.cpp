...
class ActsFaceAuthFailedTest : public testing::Test {
public:
...
void ActsFaceAuthFailedTest::SetUpTestCase(void)
{
}

void ActsFaceAuthFailedTest::TearDownTestCase(void)
{
}

void ActsFaceAuthFailedTest::SetUp()
{
}

void ActsFaceAuthFailedTest::TearDown()
{
}

HWTEST_F(ActsFaceAuthFailedTest, ActsFaceAuthFailedTest, TestSize.Level1)
{
    ...
    int32_t dialogId = -1;
    IAM_LOGI("ActsFaceAuthFailedTest:: ShowDialog start");
    int32_t width = 0, height = 0;
    GetDisplaySize(width, height);
    IAM_LOGI("ActsFaceAuthFailedTest ShowDialog width=%{public}d, height=%{public}d", width, height);
    Ace::UIServiceMgrClient::GetInstance()->ShowDialog("authentication_dialog", params,
        ...
        std::bind(&ActsFaceAuthFailedTest::FaceAuthCallback, this, std::placeholders::_1, std::placeholders::_2,
            std::placeholders::_3),
        &dialogId);
    EXPECT_TRUE(dialogId >= 0);
    IAM_LOGI("ActsFaceAuthFailedTest:: ShowDialog end, dialogId:%{public}d", dialogId);
    while (!isStop) {
        sleep(1);
    }
...
void ActsFaceAuthFailedTest::FaceAuthCallback(int32_t id, const std::string &event, const std::string &params)
{
    IAM_LOGI("ActsFaceAuthFailedTest FaceAuthCallback start");
    ...
    switch (eventType) {
        case JsEventType::EVENT_AUTH_TYPE_READY: {
            IAM_LOGI("ActsFaceAuthFailedTest FaceAuthCallback EVENT_AUTH_TYPE_READY");
            HandleEventAuthTypeReady(id);
            break;
        }
        case JsEventType::EVENT_AUTH_USER_CANCEL:
        case JsEventType::EVENT_AUTH_USER_CANCEL_ALL: {
            IAM_LOGI("ActsFaceAuthFailedTest FaceAuthCallback EVENT_AUTH_USER_CANCEL/EVENT_AUTH_USER_CANCEL_ALL");
            Ace::UIServiceMgrClient::GetInstance()->CancelDialog(id);
            isStop = true;
            break;
        }
        case JsEventType::EVENT_ANIMATION_PLAY_END: {
            IAM_LOGI("ActsFaceAuthFailedTest FaceAuthCallback EVENT_ANIMATION_PLAY_END");
            Ace::UIServiceMgrClient::GetInstance()->CancelDialog(id);
            isStop = true;
            break;
        }
...
void ActsFaceAuthFailedTest::SendFaceAuthResult(int32_t id)
{
    ...
    IAM_LOGI("ActsFaceAuthFailedTest SendFaceAuthResult %{public}s", resultOss.str().c_str());
    Ace::UIServiceMgrClient::GetInstance()->UpdateDialog(id, resultOss.str());
}

void ActsFaceAuthFailedTest::HandleEventAuthTypeReady(int32_t id)
{
    ...
    IAM_LOGI("ActsFaceAuthFailedTest HandleEventAuthTypeReady %{public}s", oss.str().c_str());
    Ace::UIServiceMgrClient::GetInstance()->UpdateDialog(id, oss.str());
    sendResultTimer.Register(std::bind(&ActsFaceAuthFailedTest::SendFaceAuthResult, this, id), sendDelay, true);
}
} // namespace AuthWidget
} // namespace UserIam
} // namespace OHOS
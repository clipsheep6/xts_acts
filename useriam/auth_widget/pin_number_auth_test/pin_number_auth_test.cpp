...
class ActsPinNumberAuthTest : public testing::Test {
public:
...
void ActsPinNumberAuthTest::SetUpTestCase(void)
{
}

void ActsPinNumberAuthTest::TearDownTestCase(void)
{
}

void ActsPinNumberAuthTest::SetUp()
{
}

void ActsPinNumberAuthTest::TearDown()
{
}

HWTEST_F(ActsPinNumberAuthTest, ActsPinNumberAuthTest, TestSize.Level1)
{
    ...
    int32_t dialogId = -1;
    IAM_LOGI("ActsPinNumberAuthTest ShowDialog start");
    int32_t width = 0, height = 0;
    GetDisplaySize(width, height);
    IAM_LOGI("ActsPinNumberAuthTest ShowDialog width=%{public}d, height=%{public}d", width, height);
    Ace::UIServiceMgrClient::GetInstance()->ShowDialog("authentication_dialog", params,
        ...
        std::bind(&ActsPinNumberAuthTest::PinAuthCallback, this, std::placeholders::_1, std::placeholders::_2,
            std::placeholders::_3),
        &dialogId);
...
void ActsPinNumberAuthTest::PinAuthCallback(int32_t id, const std::string &event, const std::string &params)
{
    IAM_LOGI("ActsPinNumberAuthTest PinAuthCallback start");
    ...
    switch (eventType) {
        case JsEventType::EVENT_AUTH_TYPE_READY: {
            IAM_LOGI("ActsPinNumberAuthTest PinAuthCallback EVENT_AUTH_TYPE_READY");
            HandleEventAuthTypeReady(id);
            break;
        }
        case JsEventType::EVENT_AUTH_USER_CANCEL:
        case JsEventType::EVENT_AUTH_USER_CANCEL_ALL: {
            IAM_LOGI("ActsPinNumberAuthTest PinAuthCallback EVENT_AUTH_USER_CANCEL/EVENT_AUTH_USER_CANCEL_ALL");
            Ace::UIServiceMgrClient::GetInstance()->CancelDialog(id);
            isStop = true;
            break;
        }
        case JsEventType::EVENT_ANIMATION_PLAY_END: {
            IAM_LOGI("ActsPinNumberAuthTest PinAuthCallback EVENT_ANIMATION_PLAY_END");
            Ace::UIServiceMgrClient::GetInstance()->CancelDialog(id);
            isStop = true;
            break;
        }
        case JsEventType::EVENT_KEY_DERIVATION_READY: {
            IAM_LOGI("ActsPinNumberAuthTest PinAuthCallback EVENT_KEY_DERIVATION_READY");
...
void ActsPinNumberAuthTest::HandleEventAuthTypeReady(int32_t id)
{
    ...
    IAM_LOGI("ActsPinNumberAuthTest HandleEventAuthTypeReady %{public}s", oss.str().c_str());
    Ace::UIServiceMgrClient::GetInstance()->UpdateDialog(id, oss.str());
}

void ActsPinNumberAuthTest::HandleEventKeyDerivationReady(int32_t id, const Json::Value &jsonParams)
{
    if (strcmp(jsonParams["payload"]["result"].asCString(), "success") == 0) {
        ...
        Json::Value cmdArray;
        IAM_LOGI("ActsPinNumberAuthTest HandleEventKeyDerivationReady derivation data %{public}s",
            jsonParams["payload"]["derivation"].asCString());
        if (strcmp(jsonParams["payload"]["derivation"].asCString(), pinNumberCode) == 0) {
            IAM_LOGI("ActsPinNumberAuthTest HandleEventKeyDerivationReady pin auth success");
            ...
            IAM_LOGI("ActsPinNumberAuthTest HandleEventKeyDerivationReady pin auth failed, times left:%{public}d",
                pinTimesLeft);

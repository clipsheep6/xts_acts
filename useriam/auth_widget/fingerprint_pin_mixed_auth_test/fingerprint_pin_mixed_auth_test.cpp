...
class ActsFingerprintPinMixedAuthTest : public testing::Test {
public:
...
void ActsFingerprintPinMixedAuthTest::SetUpTestCase(void)
{
}

void ActsFingerprintPinMixedAuthTest::TearDownTestCase(void)
{
}

void ActsFingerprintPinMixedAuthTest::SetUp()
{
}

void ActsFingerprintPinMixedAuthTest::TearDown()
{
}

HWTEST_F(ActsFingerprintPinMixedAuthTest, ActsFingerprintPinMixedAuthTest, TestSize.Level1)
{
    ...
    int32_t dialogId = -1;
    IAM_LOGI("ActsFingerprintPinMixedAuthTest ShowDialog start");
    int32_t width = 0, height = 0;
    GetDisplaySize(width, height);
    IAM_LOGI("ActsFingerprintPinMixedAuthTest ShowDialog width=%{public}d, height=%{public}d", width, height);
    Ace::UIServiceMgrClient::GetInstance()->ShowDialog("authentication_dialog", params,
        ...
        std::bind(&ActsFingerprintPinMixedAuthTest::FingerprintPinAuthCallback, this, std::placeholders::_1,
            std::placeholders::_2, std::placeholders::_3),
        &dialogId);
    EXPECT_TRUE(dialogId >= 0);
    IAM_LOGI("ActsFingerprintPinMixedAuthTest ShowDialog end, dialogId:%{public}d", dialogId);
    while (!isStop) {
        sleep(1);
    }
...
void ActsFingerprintPinMixedAuthTest::FingerprintPinAuthCallback(int32_t id, const std::string &event,
    const std::string &params)
{
    IAM_LOGI("ActsFingerprintPinMixedAuthTest FingerprintPinAuthCallback start");
    ...
    switch (eventType) {
        case JsEventType::EVENT_AUTH_TYPE_READY: {
            IAM_LOGI("ActsFingerprintPinMixedAuthTest FingerprintPinAuthCallback EVENT_AUTH_TYPE_READY");
            HandleEventAuthTypeReady(id, jsonParams);
            break;
        }
        case JsEventType::EVENT_AUTH_USER_CANCEL: {
            IAM_LOGI("ActsFingerprintPinMixedAuthTest FingerprintPinAuthCallback EVENT_AUTH_USER_CANCEL");
            break;
        }
        case JsEventType::EVENT_AUTH_USER_CANCEL_ALL: {
            IAM_LOGI("ActsFingerprintPinMixedAuthTest FingerprintPinAuthCallback EVENT_AUTH_USER_CANCEL_ALL");
            Ace::UIServiceMgrClient::GetInstance()->CancelDialog(id);
            isStop = true;
            break;
        }
        case JsEventType::EVENT_ANIMATION_PLAY_END: {
            IAM_LOGI("ActsFingerprintPinMixedAuthTest FingerprintPinAuthCallback EVENT_ANIMATION_PLAY_END");
            Ace::UIServiceMgrClient::GetInstance()->CancelDialog(id);
            isStop = true;
            break;
        }
        case JsEventType::EVENT_KEY_DERIVATION_READY: {
            IAM_LOGI("ActsFingerprintPinMixedAuthTest FingerprintPinAuthCallback EVENT_KEY_DERIVATION_READY");
...
void ActsFingerprintPinMixedAuthTest::SendFingerprintAuthResult(int32_t id)
{
    ...
    IAM_LOGI("ActsFingerprintPinMixedAuthTest SendFingerprintAuthResult %{public}s", resultOss.str().c_str());
    Ace::UIServiceMgrClient::GetInstance()->UpdateDialog(id, resultOss.str());
}

void ActsFingerprintPinMixedAuthTest::HandleEventAuthTypeReady(int32_t id, const Json::Value &jsonParams)
{
    ...
    if (strcmp(jsonParams["payload"]["type"].asCString(), "fingerprint") == 0) {
        ...
        IAM_LOGI("ActsFingerprintPinMixedAuthTest HandleEventAuthTypeReady %{public}s", oss.str().c_str());
        Ace::UIServiceMgrClient::GetInstance()->UpdateDialog(id, oss.str());
        sendResultTimer.Register(std::bind(&ActsFingerprintPinMixedAuthTest::SendFingerprintAuthResult, this, id),
            sendDelay, true);
...
void ActsFingerprintPinMixedAuthTest::HandleEventKeyDerivationReady(int32_t id, const Json::Value &jsonParams)
{
    if (strcmp(jsonParams["payload"]["result"].asCString(), "success") == 0) {
        ...
        Json::Value resultPayload;
        Json::Value cmdArray;
        IAM_LOGI("ActsFingerprintPinMixedAuthTest HandleEventKeyDerivationReady derivation data %{public}s",
            jsonParams["payload"]["derivation"].asCString());
        if (strcmp(jsonParams["payload"]["derivation"].asCString(), pinMixedCode) == 0) {
            IAM_LOGI("ActsFingerprintPinMixedAuthTest HandleEventKeyDerivationReady pin auth success");
            ...
            timesValue["payload"] = timesPayload;
            IAM_LOGI("ActsFingerprintPinMixedAuthTest HandleEventKeyDerivationReady pin auth failed, times "
                     "left:%{public}d",
                pinTimesLeft);

...
class ActsPinMixedAuthTest : public testing::Test {
public:
...
void ActsPinMixedAuthTest::SetUpTestCase(void)
{
}

void ActsPinMixedAuthTest::TearDownTestCase(void)
{
}

void ActsPinMixedAuthTest::SetUp()
{
}

void ActsPinMixedAuthTest::TearDown()
{
}

HWTEST_F(ActsPinMixedAuthTest, ActsPinMixedAuthTest, TestSize.Level1)
{
    ...
    int32_t dialogId = -1;
    IAM_LOGI("ActsPinMixedAuthTest ShowDialog start");
    int32_t width = 0, height = 0;
    GetDisplaySize(width, height);
    IAM_LOGI("ActsPinMixedAuthTest ShowDialog width=%{public}d, height=%{public}d", width, height);
    Ace::UIServiceMgrClient::GetInstance()->ShowDialog("authentication_dialog", params,
        ...
        std::bind(&ActsPinMixedAuthTest::PinAuthCallback, this, std::placeholders::_1, std::placeholders::_2,
            std::placeholders::_3),
        &dialogId);
...
void ActsPinMixedAuthTest::PinAuthCallback(int32_t id, const std::string &event, const std::string &params)
{
    IAM_LOGI("ActsPinMixedAuthTest PinAuthCallback start");
    ...
    switch (eventType) {
        case JsEventType::EVENT_AUTH_TYPE_READY: {
            IAM_LOGI("ActsPinMixedAuthTest PinAuthCallback EVENT_AUTH_TYPE_READY");
            HandleEventAuthTypeReady(id);
            break;
        }
        case JsEventType::EVENT_AUTH_USER_CANCEL:
        case JsEventType::EVENT_AUTH_USER_CANCEL_ALL: {
            IAM_LOGI("ActsPinMixedAuthTest PinAuthCallback EVENT_AUTH_USER_CANCEL/EVENT_AUTH_USER_CANCEL_ALL");
            Ace::UIServiceMgrClient::GetInstance()->CancelDialog(id);
            isStop = true;
            break;
        }
        case JsEventType::EVENT_ANIMATION_PLAY_END: {
            IAM_LOGI("ActsPinMixedAuthTest PinAuthCallback EVENT_ANIMATION_PLAY_END");
            Ace::UIServiceMgrClient::GetInstance()->CancelDialog(id);
            isStop = true;
            break;
        }
        case JsEventType::EVENT_KEY_DERIVATION_READY: {
            IAM_LOGI("ActsPinMixedAuthTest PinAuthCallback EVENT_KEY_DERIVATION_READY");
...
void ActsPinMixedAuthTest::HandleEventAuthTypeReady(int32_t id)
{
    ...
    IAM_LOGI("ActsPinMixedAuthTest HandleEventAuthTypeReady %{public}s", oss.str().c_str());
    Ace::UIServiceMgrClient::GetInstance()->UpdateDialog(id, oss.str());
}

void ActsPinMixedAuthTest::HandleEventKeyDerivationReady(int32_t id, const Json::Value &jsonParams)
{
    if (strcmp(jsonParams["payload"]["result"].asCString(), "success") == 0) {
        ...
        Json::Value cmdArray;
        IAM_LOGI("ActsPinMixedAuthTest HandleEventKeyDerivationReady derivation data %{public}s",
            jsonParams["payload"]["derivation"].asCString());
        if (strcmp(jsonParams["payload"]["derivation"].asCString(), pinMixedCode) == 0) {
            IAM_LOGI("ActsPinMixedAuthTest HandleEventKeyDerivationReady pin auth success");
            ...
            IAM_LOGI("ActsPinMixedAuthTest HandleEventKeyDerivationReady pin auth failed, times left:%{public}d",
                pinTimesLeft);

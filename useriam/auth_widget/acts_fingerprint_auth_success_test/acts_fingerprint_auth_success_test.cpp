...
class ActsFingerprintAuthSuccessTest : public testing::Test {
public:
...
void ActsFingerprintAuthSuccessTest::SetUpTestCase(void)
{
}

void ActsFingerprintAuthSuccessTest::TearDownTestCase(void)
{
}

void ActsFingerprintAuthSuccessTest::SetUp()
{
}

void ActsFingerprintAuthSuccessTest::TearDown()
{
}

HWTEST_F(ActsFingerprintAuthSuccessTest, ActsFingerprintAuthSuccessTest, TestSize.Level1)
{
    ...
    int32_t dialogId = -1;
    IAM_LOGI("ActsFingerprintAuthSuccessTest ShowDialog start");
    int32_t width = 0, height = 0;
    GetDisplaySize(width, height);
    IAM_LOGI("ActsFingerprintAuthSuccessTest ShowDialog width=%{public}d, height=%{public}d", width, height);
    Ace::UIServiceMgrClient::GetInstance()->ShowDialog("authentication_dialog", params,
        ...
        std::bind(&ActsFingerprintAuthSuccessTest::FingerprintAuthCallback, this, std::placeholders::_1,
            std::placeholders::_2, std::placeholders::_3),
        &dialogId);
    EXPECT_TRUE(dialogId >= 0);
    IAM_LOGI("ActsFingerprintAuthSuccessTest ShowDialog end, dialogId:%{public}d", dialogId);
    while (!isStop) {
        sleep(1);
    }
...
void ActsFingerprintAuthSuccessTest::FingerprintAuthCallback(int32_t id, const std::string &event,
    const std::string &params)
{
    IAM_LOGI("ActsFingerprintAuthSuccessTest FingerprintAuthCallback start");
    ...
    switch (eventType) {
        case JsEventType::EVENT_AUTH_TYPE_READY: {
            IAM_LOGI("ActsFingerprintAuthSuccessTest FingerprintAuthCallback EVENT_AUTH_TYPE_READY");
            HandleEventAuthTypeReady(id);
            break;
        }
        case JsEventType::EVENT_AUTH_USER_CANCEL:
        case JsEventType::EVENT_AUTH_USER_CANCEL_ALL: {
            IAM_LOGI("ActsFingerprintAuthSuccessTest FingerprintAuthCallback "
        ...
        case JsEventType::EVENT_ANIMATION_PLAY_END: {
            IAM_LOGI("ActsFingerprintAuthSuccessTest FingerprintAuthCallback EVENT_ANIMATION_PLAY_END");
            Ace::UIServiceMgrClient::GetInstance()->CancelDialog(id);
            isStop = true;
            break;
        }
...
void ActsFingerprintAuthSuccessTest::SendFingerprintAuthResult(int32_t id)
{
    ...
    IAM_LOGI("ActsFingerprintAuthSuccessTest SendFingerprintAuthResult %{public}s", resultOss.str().c_str());
    Ace::UIServiceMgrClient::GetInstance()->UpdateDialog(id, resultOss.str());
}

void ActsFingerprintAuthSuccessTest::HandleEventAuthTypeReady(int32_t id)
{
    ...
    IAM_LOGI("ActsFingerprintAuthSuccessTest HandleEventAuthTypeReady %{public}s", oss.str().c_str());
    Ace::UIServiceMgrClient::GetInstance()->UpdateDialog(id, oss.str());
    sendResultTimer.Register(std::bind(&ActsFingerprintAuthSuccessTest::SendFingerprintAuthResult, this, id), sendDelay,
        true);
}
} // namespace AuthWidget
} // namespace UserIam

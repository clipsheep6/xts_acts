...
public:
    ...
    void OnResult(const std::string nodeId, std::vector<std::string> permissions, std::vector<int32_t> grantResults)
    {
        ...
        PERMISSION_LOG_INFO(
            LABEL, "RequestPermissionsFromRemoteManualFunctionCPPTest  TestCallback :OnResult  is run!!!");
    }
    void OnCancel(const std::string nodeId, std::vector<std::string> permissions)
    {
        PERMISSION_LOG_INFO(
            LABEL, "RequestPermissionsFromRemoteManualFunctionCPPTest  TestCallback :OnCancel  is run!!!");
    }
...
void RequestPermissionsFromRemoteManualFunctionCPPTest::AppEventSubscriber::OnReceiveEvent(const CommonEventData &data)
{
    ...
    auto iter = std::find(RequestPermissionsFromRemoteManualFunctionCPPTest::eventList_.begin(),
        RequestPermissionsFromRemoteManualFunctionCPPTest::eventList_.end(),
        eventName);
 ...
 * @tc.level     Level 2
 */
HWTEST_F(RequestPermissionsFromRemoteManualFunctionCPPTest, DPMS_RequestPermissionsFromRemote_1500, TestSize.Level1)
{
    GTEST_LOG_(INFO)
 ...
 */
HWTEST_F(
    RequestPermissionsFromRemoteManualFunctionCPPTest, DPMS_GrantSensitivePermissionToRemoteApp_1000, TestSize.Level1)
{
    GTEST_LOG_(INFO)
 ...
 */
HWTEST_F(
    RequestPermissionsFromRemoteManualFunctionCPPTest, DPMS_GrantSensitivePermissionToRemoteApp_1100, TestSize.Level1)
{
    GTEST_LOG_(INFO)
    ...
    if (0 == result_verify) {
        ...
        if (result_request == 0) {
            STAbilityUtil::PublishEvent(REQ_EVENT_NAME_GROUP_THIRD_A, nomalCode, "RequestPermissionsFromRemote");
            EXPECT_EQ(
                STAbilityUtil::WaitCompleted(event_, "RequestPermissionsFromRemote", expectation, waitTime10s), 0);
        } else {
            PERMISSION_LOG_INFO(LABEL,

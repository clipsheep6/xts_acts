...
static constexpr OHOS::HiviewDFX::HiLogLabel LABEL = {
    LOG_CORE, SECURITY_DOMAIN_PERMISSION, "DistributedPermissionRemindFunctionCPPTest"};
...
static std::vector<string> bundle_name_list_ = {SYSTEM_TEST_BUNDLE_NAME_DPMS,
...
{
    FILE *ptr;
    ptr = popen("ps -ef|grep checkresult|cut -c 14-18|xargs kill -9", "r");
    pclose(ptr);
...
void DistributedPermissionRemindFunctionCPPTest::DeleteAllRemind()
{
    ...
    for (auto deviceinfo : deviceList) {
        ...
        appInfo = GetAppIdInfo(uid, deviceinfo.deviceId);
        StopPermissionRemind(appInfo);
    }
}
...
HWTEST_F(DistributedPermissionRemindFunctionCPPTest, DPMS_CheckPermissionAndStartUsing_0100, TestSize.Level4)
{
    ...
    char deviceId[device_uuid_length] = {0};
    GetDevUdid(deviceId, device_uuid_length);
    string appInfo = GetAppIdInfo(uid, deviceId);
    string permName = "";
...
/**
 * @tc.name      Disable 1 sensitive permission for local application
...
HWTEST_F(DistributedPermissionRemindFunctionCPPTest, DPMS_StopUsingPermission_1400, TestSize.Level1)
{
    ...
    STAbilityUtil::WaitCompleted(
        event_, "OnStart", OHOS::AppExecFwk::AbilityLifecycleExecutor::LifecycleState::INACTIVE, sleep_time_3000_ms);
    ...
    STAbilityUtil::WaitCompleted(
        event_, "OnStop", OHOS::AppExecFwk::AbilityLifecycleExecutor::LifecycleState::INITIAL, sleep_time_3000_ms);
    std::this_thread::sleep_for(std::chrono::milliseconds(sleep_time_3000_ms));

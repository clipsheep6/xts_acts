...
std::map<std::string, DpmsStServiceAbilityA1::func> DpmsStServiceAbilityA1::funcMap_ = {
    ...
    {"DPMS_CheckCallerPermissionAndStartUsing_0300", &DpmsStServiceAbilityA1::CheckCallerPermissionAndStartUsing_0300},
};
...
void DpmsStServiceAbilityA1::CheckCallerPermissionAndStartUsing_0300()
{
    APP_LOGI("app func CheckCallerPermissionAndStartUsing_0300 start");
    ...
    int performance_test_times = 1000;
    ...
    for (int run_times = 0; run_times < performance_test_times; run_times++) {
        ...
        DistributedPermissionKit::CheckCallerPermissionAndStartUsing(permName);
        auto endTime = std::chrono::high_resolution_clock::now();
        totalTime += endTime - startTime;
        DistributedPermissionKit::StopUsingPermission(permName, appIdInfo);
    }
    averageTime = totalTime / performance_test_times;
    APP_LOGI("CheckCallerPermissionAndStartUsing_0300  avg = %{public}f(ms)", averageTime.count());
    int32_t result = -1;
    if (averageTime.count() <= MAX_ELAPSED) {
        result = 0;
    }
    PublishEvent(APP_A1_RESP_EVENT_NAME, result, "CheckCallerPermissionAndStartUsing_0300");
    APP_LOGI("app func CheckCallerPermissionAndStartUsing_0300 end");
}

...
{
    FILE *ptr = popen("ps -ef|grep DeleteUidPermissionCommand-10001|cut -c 14-18|xargs kill -9", "r");
    pclose(ptr);
}
...
void NotifyDeletePermissionFunction1CPPTest::CheckLog(string &result_str)
{
    ...
    int endLen = 51;
    result_str = sCmdResult.substr(sCmdResult.length() - endLen, sCmdResult.length());

...
// Valid permission name,the length of permission name = 256.
string GetThresholdPermission()
{
    int permissionLength = 256;
    for (int i = 0; i < permissionLength; i++) {
        string str = "h";
        THRESHOLD_PERMISSION = THRESHOLD_PERMISSION + str;
    }
    return THRESHOLD_PERMISSION;
}
}  // namespace
namespace {
...
/**
 * @tc.name      If the length of permission name is less than 256, the permission is granted, return: 0.
...
/**
 * @tc.name      If the length of permission name is the same as 256, the permission is granted, return: 0.
 * @tc.number    DPMS_CheckCallingOrSelfPermission_0500
 * @tc.size      MEDIUM
 * @tc.type      Function
 * @tc.level     Level 1
 */
HWTEST_F(CheckDistributedPermissionFunctionCPPTest, DPMS_CheckCallingOrSelfPermission_0500, TestSize.Level1)
{
    std::string hapName = "checkSelfPermissionBundle1";
    std::string abilityName = "CheckSelfPermissionAbilityA1";
    std::string bundleName = "com.third.checkSelfPermission";
    std::string permission = GetThresholdPermission();
    STDistibutePermissionUtil::Install(hapName);
    std::vector<OHOS::Security::Permission::PermissionDef> permDefList;
    OHOS::Security::Permission::PermissionDef permissionDef_thresholdPermission = {.permissionName = permission,
        .bundleName = bundleName,
        .grantMode = 0,
        .availableScope = 1 << 0,
        .label = PERMISSION_LABEL,
        ...
        .description = DESCRIPTION,
        .descriptionId = DESCRIPTION_ID};
    permDefList.emplace_back(permissionDef_thresholdPermission);
    PermissionKit::AddDefPermissions(permDefList);
    std::vector<std::string> permList_user;
    permList_user.push_back(permission);
    PermissionKit::AddUserGrantedReqPermissions(bundleName, permList_user, 0);
    PermissionKit::GrantUserGrantedPermission(bundleName, permission, 0);
    GTEST_LOG_(INFO) << "CheckDistributedPermissionFunctionCPPTest DPMS_CheckCallingOrSelfPermission_0500 start";
    MAP_STR_STR params;
    params["permName"] = permission;
    Want want = STAbilityUtil::MakeWant("device", abilityName, bundleName, params);
    ErrCode eCode = STAbilityUtil::StartAbility(want, abilityMs_, 3000);
    EXPECT_EQ(ERR_OK, eCode);
    STAbilityUtil::PublishEvent(
        "req_com_ohos_check_self_permission_app_a1", 100, "DPMS_CheckCallingOrSelfPermission_0500");
    EXPECT_EQ(STAbilityUtil::WaitCompleted(event_, "CheckCallingOrSelfPermission0500", GRANTED, 2), 0);
    bool ret = STAbilityUtil::StopAbility("req_com_ohos_check_self_permission_app_a1", 0, "StopSelfAbility");
    EXPECT_TRUE(ret);
    GTEST_LOG_(INFO) << "CheckDistributedPermissionFunctionCPPTest DPMS_CheckCallingOrSelfPermission_0500 end";
    PermissionKit::RemoveDefPermissions(bundleName);
    STDistibutePermissionUtil::Uninstall(bundleName);
}
...
/**
 * @tc.name      If the length of permission name is less than 256, and it is restricted, return true.
...
/**
 * @tc.name      If the length of permission name is the same as 256, and it is restricted, return true.
 * @tc.number    DPMS_IsRestrictedPermission_0600
 * @tc.size      MEDIUM
 * @tc.type      Function
 * @tc.level     Level 1
 */
HWTEST_F(CheckDistributedPermissionFunctionCPPTest, DPMS_IsRestrictedPermission_0600, TestSize.Level1)
{
    STDistibutePermissionUtil::Install("dpmsThirdBundle");
    std::string bundleName = "com.third.hiworld.dpmsExample";
    std::vector<OHOS::Security::Permission::PermissionDef> permDefList;
    std::string permission = GetThresholdPermission();
    OHOS::Security::Permission::PermissionDef permissionDef_restricted = {.permissionName = permission,
        .bundleName = bundleName,
        .grantMode = 0,
        .availableScope = 1 << 2,
        .label = PERMISSION_LABEL,
    ...
    permDefList.emplace_back(permissionDef_restricted);
    PermissionKit::AddDefPermissions(permDefList);
    GTEST_LOG_(INFO) << "CheckDistributedPermissionFunctionCPPTest DPMS_IsRestrictedPermission_0600 start";
    int32_t result = 1;
    result = DistributedPermissionKit::IsRestrictedPermission(permission);
    EXPECT_TRUE(result);
    GTEST_LOG_(INFO) << permission.length()<<"permission length";
    GTEST_LOG_(INFO) << "CheckDistributedPermissionFunctionCPPTest DPMS_IsRestrictedPermission_0600 end";
    PermissionKit::RemoveDefPermissions(bundleName);
    STDistibutePermissionUtil::Uninstall(bundleName);
}
...
/**
 * @tc.name      If the permission name is valid, but it is not restricted, return false.
 * @tc.number    DPMS_IsRestrictedPermission_0700
 * @tc.size      MEDIUM
 * @tc.type      Function
 * @tc.level     Level 2
 */
HWTEST_F(CheckDistributedPermissionFunctionCPPTest, DPMS_IsRestrictedPermission_0700, TestSize.Level2)
{
    STDistibutePermissionUtil::Install("dpmsThirdBundle");
    std::string bundleName = "com.third.hiworld.dpmsExample";
    std::vector<OHOS::Security::Permission::PermissionDef> permDefList;
    ...
    permDefList.emplace_back(permissionDef_thresholdPermission);
    PermissionKit::AddDefPermissions(permDefList);
    GTEST_LOG_(INFO) << "CheckDistributedPermissionFunctionCPPTest DPMS_IsRestrictedPermission_0700 start";
    int32_t result = 1;
    result = DistributedPermissionKit::IsRestrictedPermission(permission);
    EXPECT_FALSE(result);
    GTEST_LOG_(INFO) << "CheckDistributedPermissionFunctionCPPTest DPMS_IsRestrictedPermission_0700 end";
    PermissionKit::RemoveDefPermissions(bundleName);
    STDistibutePermissionUtil::Uninstall(bundleName);
}
}  // namespace Permission

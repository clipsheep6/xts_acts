...
#include <location/oh_location.h>
#include <location/oh_location_type.h>
...
void reportlocation(Location_Info *location)
{
	return;
}
...
{
    auto requestConfig = OH_Location_CreateRequestConfig();
    OH_Location_StartLocating(requestConfig, reportlocation);
    int32_t ret = 0;
    napi_value result = nullptr;
...
{
    OH_Location_StopLocating(reportlocation);
    int32_t ret = 0;
    napi_value result = nullptr;
    napi_create_int32(env, ret, &result);
    return result;

...
describe('NetworkSearchPreferredPerformanceTest', function () {
    const SLOT_0 = 0;
    ...
    it('Telephony_NetworkSearch_getOperatorName_Promise_0300', 0, async function (done) {
        ...
        for (let index = 0; index < TEST_RUN_TIME; index++) {
            try {
                ...
                console.log(`Telephony_NetworkSearch_getOperatorName_Promise_0300 data: ${data}`);
            } catch (err) {
    ...
    it('Telephony_NetworkSearch_getPreferredNetwork_Async_0900', 0, async function (done) {
        ...
        function recursive(n) {
            if (n <= 0) {
                console.log(`Telephony_NetworkSearch_getPreferredNetwork_Async_0900 exec done useTime: ${totalTime}`);
                console.log('Telephony_NetworkSearch_getPreferredNetwork_Async_0900 finish');
                expect(totalTime).assertLess(DELAYL_LIMIT);
                done();
                return;
            }
            ...
            radio.getPreferredNetwork(SLOT_0, (err, data) => {
                ...
                totalTime += endTime - startTime;
                if (err) {
                    console.log(`Telephony_NetworkSearch_getPreferredNetwork_Async_0900 fail err: ${err}`);
                }
                console.log(`Telephony_NetworkSearch_getPreferredNetwork_Async_0900 data: ${data}`);
                recursive(n - 1);
            })
    ...
    it('Telephony_NetworkSearch_getPreferredNetwork_Promise_0900', 0, async function (done) {
        ...
        for (let index = 0; index < TEST_RUN_TIME; index++) {
            try {
                ...
                console.log(`Telephony_NetworkSearch_getPreferredNetwork_Promise_0900 data= ${data}`);
            } catch (err) {
                console.log( `Telephony_NetworkSearch_getPreferredNetwork_Promise_0900 FAIL: ${err}`);
            }
        }
        let endTime = new Date().getTime();
        console.log(`Telephony_NetworkSearch_getPreferredNetwork_Promise_0900 finish useTime= ${endTime - startTime}`);
        expect(endTime - startTime).assertLess(DELAYL_LIMIT);
        done();
    })

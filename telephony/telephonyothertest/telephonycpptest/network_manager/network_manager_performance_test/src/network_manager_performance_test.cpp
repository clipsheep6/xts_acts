...
HWTEST_F(UidPolicyTest, Telephony_NetworkManager_GetUidPolicy_0700, Function | MediumTest | Level3)
{
    ...
    for (count = 0; count < TEST_COUNTS_1000; ++count) {
        DelayedSingleton<NetPolicyClient>::GetInstance()->GetUidPolicy(5);
    }
    int32_t useTimeUs = timeHelper.GetUseTimeUs() / TEST_COUNTS_1000;
    LOG("The time of each time is : %d us", useTimeUs);
    EXPECT_LE(useTimeUs, TIME_USEC);
...
HWTEST_F(NetworkManagerTest, Telephony_NetworkManager_IsActivate_0400, Function | MediumTest | Level3)
{
    ...
    int count;
    for (count = 0; count < TEST_COUNTS_1000; ++count) {
        DelayedSingleton<EthernetClient>::GetInstance()->IsActivate(iface);
    }
    int32_t useTimeUs = timeHelper.GetUseTimeUs() / TEST_COUNTS_1000;
    LOG("The time of each time is : %d us", useTimeUs);
    EXPECT_LE(useTimeUs, TIME_USEC);
...
HWTEST_F(NetworkManagerTest, Telephony_NetworkManager_GetActivateInterfaces_0300, Function | MediumTest | Level3)
{
    ...
    int count;
    for (count = 0; count < TEST_COUNTS_1000; ++count) {
        DelayedSingleton<EthernetClient>::GetInstance()->GetActivateInterfaces();
    }
    int32_t useTimeUs = timeHelper.GetUseTimeUs() / TEST_COUNTS_1000;
    LOG("The time of each time is : %d us", useTimeUs);
    EXPECT_LE(useTimeUs, TIME_USEC);

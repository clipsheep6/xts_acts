 ...
 * @tc.desc    Reliability test
 */
HWTEST_F(NetworkManagerRegisterNetconnCallbackTest, Telephony_NetworkManager_RegisterNetConnCallback_2600,
    Function | MediumTest | Level3)
{
    ...
    for (int32_t count = 0; count < TEST_COUNTS_1000; ++count) {
        int32_t ret = DelayedSingleton<NetConnClient>::GetInstance()->RegisterNetConnCallback(netSpecifier,
            callback, timeoutMS);
        EXPECT_EQ(ret, 0);
    }
}
 ...
 * @tc.name    RegisterNetConnCallback() is called to activate the default network 1000 times,
 *             returning 0 on each success
 * @tc.desc    Reliability test
 */
HWTEST_F(NetworkManagerRegisterNetconnCallbackTest, Telephony_NetworkManager_RegisterNetConnCallback_2700,
    Function | MediumTest | Level3)
{
    sptr<INetConnCallback> callback = (std::make_unique<TestNetConnCallback>()).release();
    for (int32_t count = 0; count < TEST_COUNTS_1000; ++count) {
 ...
 * @tc.desc    Reliability test
 */
HWTEST_F(NetworkManagerRegisterNetconnCallbackTest, Telephony_NetworkManager_UnRegisterNetConnCallback_0400,
    Function | MediumTest | Level3)
{
    sptr<INetConnCallback> callback = (std::make_unique<TestNetConnCallback>()).release();
    for (int32_t count = 0; count < TEST_COUNTS_1000; ++count) {

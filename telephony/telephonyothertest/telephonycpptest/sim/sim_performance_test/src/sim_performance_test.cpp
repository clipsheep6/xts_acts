...
HWTEST_F(SimLockStateTest, Telephony_Sim_SetLockState_0800, Function | MediumTest | Level1)
{
    ...
    for (int32_t timeNumber = 0; timeNumber < SimTest::LOOP_NUMBER; ++timeNumber) {
        g_proxy.GetHandler()->SetLockState(SimTest::SLOTID_0, optionsOn, response);
    }
...
HWTEST_F(SimLockStateTest, Telephony_Sim_GetLockState_0400, Function | MediumTest | Level1)
{
    ...
    timeHelper.StartCountUs();
    for (int32_t timeNumber = 0; timeNumber < SimTest::LOOP_NUMBER; ++timeNumber) {
        g_proxy.GetHandler()->GetLockState(SimTest::SLOTID_0, OHOS::Telephony::LockType::FDN_LOCK);
    }
    int32_t useTimeUs = timeHelper.GetUseTimeUs() / SimTest::LOOP_NUMBER;
    LOG("The time of each time is : %d us", useTimeUs);
    EXPECT_LE(useTimeUs, SimTest::TIME_USEC);
...
HWTEST_F(SimStkTest, Telephony_Sim_SendTerminalResponseCmd_0500, Function | MediumTest | Level1)
{
    ...
    timeHelper.StartCountUs();
    for (int32_t timeNumber = 0; timeNumber < SimTest::LOOP_NUMBER; ++timeNumber) {
        g_proxy.GetHandler()->SendTerminalResponseCmd(SimTest::SLOTID_0, SimTest::RESPONSE_CMD);
    }
    int32_t useTimeUs = timeHelper.GetUseTimeUs() / SimTest::LOOP_NUMBER;
    LOG("The time of each time is : %d us", useTimeUs);
    EXPECT_LE(useTimeUs, SimTest::TIME_USEC);
...
HWTEST_F(SimStkTest, Telephony_Sim_SendEnvelopeCmd_0500, Function | MediumTest | Level1)
{
    ...
    timeHelper.StartCountUs();
    for (int32_t timeNumber = 0; timeNumber < SimTest::LOOP_NUMBER; ++timeNumber) {
        g_proxy.GetHandler()->SendEnvelopeCmd(SimTest::SLOTID_0, SimTest::STK_CMD);
    }
    int32_t useTimeUs = timeHelper.GetUseTimeUs() / SimTest::LOOP_NUMBER;
    LOG("The time of each time is : %d us", useTimeUs);
    EXPECT_LE(useTimeUs, SimTest::TIME_USEC);
...
HWTEST_F(SimTest, Telephony_Sim_GetCardType_0500, Function | MediumTest | Level1)
{
    ...
    timeHelper.StartCountUs();
    for (int32_t timeNumber = 0; timeNumber < SimTest::LOOP_NUMBER; ++timeNumber) {
        g_proxy.GetHandler()->GetCardType(SimTest::SLOTID_0);
    }
    int32_t useTimeUs = timeHelper.GetUseTimeUs() / SimTest::LOOP_NUMBER;
    LOG("The time of each time is : %d us", useTimeUs);
    EXPECT_LE(useTimeUs, SimTest::TIME_USEC);

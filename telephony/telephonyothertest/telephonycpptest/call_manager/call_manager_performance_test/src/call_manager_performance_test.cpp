 ...
 */
HWTEST_F(
    CallManagerPerformanceTest, Telephony_CallManager_IMS_SetImsFeatureValue_0700, Performance | MediumTest | Level3)
{
    TimeCountHelper timeHelper;
    int imsValues = 1;
    timeHelper.StartCountUs();
 ...
 * @tc.name     test UpdateCallMediaMode() 1000 times
 * @tc.desc     Performance test
 */
HWTEST_F(
    CallManagerPerformanceTest, Telephony_CallManager_IMS_UpdateCallMediaMode_0700, Performance | MediumTest | Level3)
{
    TimeCountHelper timeHelper;
    std::u16string imsValues = Str8ToStr16(IMS_VALUES_1);
    timeHelper.StartCountUs();
    for (int i = 0; i < TEST_RUN_TIME_1000; i++) {
        g_clientPtr->GetPtr()->UpdateCallMediaMode(FALSE_CALLID_999, CallMediaMode::CALL_MODE_SEND_RECEIVE);
    }
    int useTimeUs = timeHelper.GetUseTimeUs() / TEST_RUN_TIME_1000;
    LOG("useTimeUs = %d\n", useTimeUs);
    EXPECT_LE(useTimeUs, USEC_500);

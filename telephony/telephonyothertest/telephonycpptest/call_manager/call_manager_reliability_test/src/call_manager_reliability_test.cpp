    ...
    Reliability | MediumTest | Level3)
{
    ...
    LOCK_NUM_WHILE_NE(g_updateResult.GetIntValue("active"), true, SLEEP_50_MS, SLEEP_1000_MS);
}
...
/**
 * @tc.number   Telephony_CallManager_IMS_UpdateCallMediaMode_0800
 * @tc.name     test UpdateCallMediaMode() 1000 times
 * @tc.desc     Reliability test
 */
HWTEST_F(
    CallManagerReliabilityTest, Telephony_CallManager_IMS_UpdateCallMediaMode_0800, Reliability | MediumTest | Level3)
{
    for (int i = 0; i < TEST_RUN_TIME_1000; i++) {
        ASSERT_NE(g_clientPtr->GetPtr()->UpdateCallMediaMode(FALSE_CALLID_999, CallMediaMode::CALL_MODE_SEND_RECEIVE),
            SUCCESSFUL);
    }
}
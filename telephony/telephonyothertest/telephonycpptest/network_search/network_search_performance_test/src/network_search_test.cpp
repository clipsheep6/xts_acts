...
/*
 * @tc.number  Telephony_NetworkSearch_GetPreferredNetworkMode_1100
 * @tc.name    To test the GetPreferredNetworkMode() 1000 times and the preferred network mode is ORE_NETWORK_MODE_AUTO
 *             and the errcode is HEIL_ERR_SUCCESS
 * @tc.desc    Function test
 */
HWTEST_F(NetworkSearchTest, Telephony_NetworkSearch_GetPreferredNetworkMode_1100, Function | MediumTest | Level3)
{
    RecoverPreferredNetworkMode();
    for (int i = 0; i < TEST_RUN_TIME_1000; i++) {
        hasNewData[GET_PREFERRED_NETWORK_MODE] = false;
        bool getPreferredMode = GetProxy()->GetPreferredNetworkMode(SLOT_0, g_callback);
        LOCK_NUM_WHILE_NE(hasNewData[GET_PREFERRED_NETWORK_MODE], true, LOCK_WAIT_SLIP, LOCK_WAIT_TIMEOUT);
    }
}

/*
 * @tc.number  Telephony_NetworkSearch_SetPreferredNetworkMode_0600
 * @tc.name    Use SetPreferredNetworkMode() to set the preferred network mode ORE_NETWORK_MODE_AUTO 1000 times to test
 *             the callback is true and errcode is HEIL_ERR_SUCCESS the true and the errcode is HEIL_ERR_SUCCESS
 * @tc.desc    Function test
 */
HWTEST_F(NetworkSearchTest, Telephony_NetworkSearch_SetPreferredNetworkMode_0600, Function | MediumTest | Level3)
{
    for (int i = 0; i < TEST_RUN_TIME_1000; i++) {
        hasNewData[SET_PREFERRED_NETWORK_MODE] = false;
        bool setPreferredMode = GetProxy()->SetPreferredNetworkMode(SLOT_0, CORE_NETWORK_MODE_AUTO, g_callback);

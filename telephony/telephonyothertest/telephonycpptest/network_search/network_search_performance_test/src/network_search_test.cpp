 ...
 * @tc.name    Test GetNrOptionMode interface query function is executed 1000 times, the output delay is less than 500ms
...
HWTEST_F(NetworkSearchTest, Telephony_NetworkSearch_GetNrOptionMode_0600, Performance | MediumTest | Level3)
{
    ...
    for (int i = 0; i < TEST_RUN_TIME_1000; i++) {
        GetProxy()->GetNrOptionMode(SLOT_0);
    }
 ...
 * @tc.name    Test IsNrSupported interface query function is executed 1000 times, the output delay is less than 500ms
 * @tc.desc    Performance test
 */

...
HWTEST_F(NetworkSearchTest, Telephony_NetworkSearch_GetSignalInfoList_0200, Reliability | MediumTest | Level3)
{
    for (int i = 0; i < TEST_RELIABILITY_RUN_TIME_1000; i++) {
        ...
        for (const auto &singnal_sptr : signalVec) {
            ...
            ASSERT_LE(type, SignalInformation::NetworkType::NR);
    ...
    for (int i = 0; i < TEST_RELIABILITY_RUN_TIME_1000; i++) {
        ...
        for (const auto &cellList : cellInfoList) {
            ...
            if (type == CellInformation::CellType::CELL_TYPE_GSM) {
                ...
                ASSERT_EQ(gsm->GetMcc(), "460");
                ASSERT_NE(gsm->GetMnc(), "");
                ...
                ASSERT_EQ(wcdma->GetMcc(), "460");
                ASSERT_NE(wcdma->GetMnc(), "");
                ...
                ASSERT_EQ(lte->GetMcc(), "460");
                ASSERT_NE(lte->GetMnc(), "");
                ...
                ASSERT_GE(cdma->GetBaseId(), CELL_INFO_MIN);
                ASSERT_LE(cdma->GetBaseId(), CELL_INFO_LAC_TAC_MAX);
                ASSERT_GE(cdma->GetLatitude(), CELL_INFO_LATITU_MIN);
                ASSERT_LE(cdma->GetLatitude(), CELL_INFO_LATITU_MAX);
                ASSERT_GE(cdma->GetLongitude(), CELL_INFO_LONGITU_MIN);
                ASSERT_LE(cdma->GetLongitude(), CELL_INFO_LONGITU_MAX);
                ASSERT_GE(cdma->GetNid(), CELL_INFO_MIN);
                ASSERT_LE(cdma->GetNid(), CELL_INFO_NID_MAX);
                ASSERT_GE(cdma->GetSid(), CELL_INFO_MIN);
                ASSERT_LE(cdma->GetSid(), CELL_INFO_SID_MAX);
                ...
                ASSERT_GE(tdscdma->GetLac(), CELL_INFO_MIN);
                ASSERT_LE(tdscdma->GetLac(), CELL_INFO_LAC_TAC_MAX);
                ASSERT_GE(tdscdma->GetCellId(), CELL_INFO_MIN);
                ASSERT_LE(tdscdma->GetCellId(), CELL_INFO_WCDMA_CELLID_MAX);
                int uarfcn = tdscdma->GetArfcn();
                ASSERT_TRUE((uarfcn >= CELL_INFO_UARFCN_9400 && uarfcn <= CELL_INFO_UARFCN_9600) ||
                            (uarfcn >= CELL_INFO_UARFCN_10050 && uarfcn <= CELL_INFO_UARFCN_10125) ||
                            (uarfcn >= CELL_INFO_UARFCN_11500 && uarfcn <= CELL_INFO_UARFCN_12000));
                ASSERT_GE(tdscdma->GetArfcn(), CELL_INFO_MIN);
                ASSERT_LE(tdscdma->GetArfcn(), CELL_INFO_EARFCN_MAX);
                ASSERT_GE(tdscdma->GetCpid(), CELL_INFO_MIN);
                ASSERT_LE(tdscdma->GetCpid(), CELL_INFO_CPID_MAX);
                ASSERT_EQ(tdscdma->GetMcc(), "460");
                ASSERT_NE(tdscdma->GetMnc(), "");
                ...
                ASSERT_GE(nr->GetPci(), CELL_INFO_MIN);
                ASSERT_LE(nr->GetPci(), CELL_INFO_NR_PCI_MAX);
                ASSERT_GE(nr->GetArfcn(), CELL_INFO_MIN);
                ASSERT_LE(nr->GetArfcn(), CELL_INFO_NR_NRARFCN_MAX);
                ASSERT_EQ(nr->GetMcc(), "460");
                ASSERT_NE(nr->GetMnc(), "");
                ASSERT_GE(nr->GetTac(), CELL_INFO_MIN);
                ASSERT_LE(nr->GetTac(), CELL_INFO_LAC_TAC_MAX);
                ASSERT_GE(nr->GetNci(), CELL_INFO_MIN);
                ASSERT_LE(nr->GetNci(), CELL_INFO_NR_NCI_MAX);

...
{
    NetUidPolicy arr[] = { NetUidPolicy::NET_POLICY_NONE, NetUidPolicy::NET_POLICY_ALLOW_METERED_BACKGROUND,
        NetUidPolicy::NET_POLICY_TEMPORARY_ALLOW_METERED, NetUidPolicy::NET_POLICY_REJECT_METERED_BACKGROUND,
        NetUidPolicy::NET_POLICY_ALLOW_ALL, NetUidPolicy::NET_POLICY_REJECT_ALL };
    for (const NetUidPolicy &item : arr) {
        ...
        while (ptr_uid != uids.end()) {
            DelayedSingleton<NetPolicyClient>::GetInstance()->SetPolicyByUid(*ptr_uid, (NetUidPolicy)0); // reset status
            ptr_uid++;
        }
    }
}

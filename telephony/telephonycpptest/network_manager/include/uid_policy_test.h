...
void UidPolicyTest::TearDown()
{
    ...
    for (const NetUidPolicy &item : arr) {
        std::vector<uint32_t> uids = DelayedSingleton<NetPolicyManager>::GetInstance()->GetUids(item);
        std::vector<uint32_t>::iterator ptr_uid = uids.begin();
        while (ptr_uid != uids.end()) {
            DelayedSingleton<NetPolicyManager>::GetInstance()->SetUidPolicy(*ptr_uid, (NetUidPolicy)0); //reset status
            ptr_uid++;
        }
    }
}

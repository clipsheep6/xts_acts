...
/*
 * @tc.number  Telephony_NetworkManager_NetDetection_0500
 * @tc.nameThe netId is Beyond the scope of, test NetDetection()
 *             return "NET_CONN_ERR_NETID_NOT_FOUND"
 * @tc.desc    Function test
 */
HWTEST_F(DetectionTest, Telephony_NetworkManager_NetDetection_0500, Function | MediumTest | Level3)
{
    auto netConnClient = DelayedSingleton<NetConnClient>::GetInstance();
    NetHandle netHandle(NETID_OUT);
    int32_t ret = netConnClient->NetDetection(netHandle);
    std::cout << "NetDetection ret:" << ret << std::endl;
    EXPECT_EQ(ret, NET_CONN_ERR_NETID_NOT_FOUND);
}
...
/*
 * @tc.number  Telephony_NetworkManager_NetDetection_0600
 * @tc.nameThe netId is normal wifi, test NetDetection()
 *             return "NET_CONN_SUCCESS", return result the callback
 *             "NET_DETECTION_SUCCESS"
 * @tc.desc    Function test
 */
HWTEST_F(DetectionTest, Telephony_NetworkManager_NetDetection_0600, Function | MediumTest | Level3)
{
    WifiTest wifiTest;
    ...
    for (sptr<NetHandle> netHandler : netList) {
        ...
        while (count < 3)
        {
            ...
            std::cout << "m_urlRedirect: __" << TestNetDetectionCallback::m_urlRedirect << std::endl;
            if (TestNetDetectionCallback::m_detectionResult == NET_DETECTION_SUCCESS) {
                flag = true;
                break;
            }
...
/*
 * @tc.number  Telephony_NetworkManager_NetDetection_0600
 * @tc.nameThe netId is portal wifi, test NetDetection()
 *             return "NET_CONN_SUCCESS", return result the callback
 *             URL of the portal server
 * @tc.desc    Function test
 */
HWTEST_F(DetectionTest, Telephony_NetworkManager_NetDetection_0700, Function | MediumTest | Level3)
{
    WifiTest wifiTest;
    ...
    for (sptr<NetHandle> netHandler : netList) {
        ...
        while (count < 3)
        {
            ...
            sleep(3);
            std::cout << "m_urlRedirect: __" << TestNetDetectionCallback::m_urlRedirect << std::endl;
            if (TestNetDetectionCallback::m_urlRedirect.empty()) {
                result = netConnClient->NetDetection(*netHandler);
                std::cout << "NetDetection result:" << result << std::endl;
            }
            else {

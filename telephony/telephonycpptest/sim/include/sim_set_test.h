...
void SimSetTest::SetUpTestCase()
{
    ...
    if (OHOS::Str16ToStr8(g_defaultCardName) == "") {
        int32_t simId = 0;
        g_defaultCardName = g_proxy.GetHandler()->GetOperatorName(simId);
    }
...
void SimSetTest::TearDown()
{
    ...
    sleep(2);
    std::u16string retName = g_proxy.GetHandler()->GetShowName(simId);
    if (retName != g_defaultCardName) {

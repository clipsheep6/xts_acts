...
class SimContact {
public:
    void DeleteADNContact(int32_t slotId, const std::shared_ptr<OHOS::Telephony::DiallingNumbersInfo> &contactInfo);
    void DeleteFDNContact(int32_t slotId, const std::shared_ptr<OHOS::Telephony::DiallingNumbersInfo> &contactInfo);
...
void SimContact::DeleteADNContact(
    int32_t slotId, const std::shared_ptr<OHOS::Telephony::DiallingNumbersInfo> &contactInfo)
{
    int32_t typeADN = 1;
    bool isExist = false;
    ...
    if (isExist) {
        bool ret = g_proxy.GetHandler()->DelIccDiallingNumbers(slotId, typeADN, contactInfo);
        if (ret == false) {
    ...
    int32_t slotId, const std::shared_ptr<OHOS::Telephony::DiallingNumbersInfo> &contactInfo)
{
    ...
    if (isExist) {
        bool result = g_proxy.GetHandler()->DelIccDiallingNumbers(slotId, typeFDN, contactInfo);
        if (result == false) {

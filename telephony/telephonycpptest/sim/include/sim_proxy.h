...
public:
    ...
    void DeletingCardContacts(int32_t index);
    ...
    void TriggerPuk2Lock(int32_t simId);
#endif
...
void SimProxy::DeletingCardContacts(int32_t index)
{
    int32_t slotId = 0;
    int32_t typeAdn = 1;
    int32_t typeFdn = 2;
    ...
    if (notEmpty) {
        ...
        daillingAdnInfo.begin(); iter != daillingAdnInfo.end(); iter++) {
            if ((*iter)->recordNumber_ == index) {
                bool ret = GetHandler()->DelIccDiallingNumbers(slotId, typeAdn, index);
                if (!ret){
    ...
    if (notEmpty) {
        ...
        daillingAdnInfo.begin(); iter != daillingAdnInfo.end(); iter++) {
            if ((*iter)->recordNumber_ == index) {
                bool result = GetHandler()->DelIccDiallingNumbers(slotId, typeFdn, index);
                if (!result){
...
void SimProxy::MockSimStateSet(int32_t simState)
{
    ...
    sprintf_s(simStateStr, sizeof(simStateStr), "%d", rilSimState);
...
void SimProxy::TriggerPuk2Lock(int32_t simId)
{
    OHOS::Telephony::LockStatusResponse responseInfo;
    std::u16string wrongPin2 = Str8ToStr16("6789");

...
HWTEST_F(SimTest, Telephony_Sim_GetPrimarySlotId_0100, Function | MediumTest | Level2)
{
    ...
    ASSERT_EQ(defaultSlotId, SimTest::SLOTID_0);
}
...
{
    bool result = g_proxy.GetHandler()->SetPrimarySlotId(SimTest::SLOTID_1);
    ASSERT_FALSE(result);
    int32_t defaultSlotId = g_proxy.GetHandler()->GetPrimarySlotId();
    ASSERT_NE(defaultSlotId, SimTest::SLOTID_1);
}
...
{
    bool result = g_proxy.GetHandler()->SetPrimarySlotId(SimTest::SLOTID_2);
    ASSERT_FALSE(result);
    int32_t defaultSlotId = g_proxy.GetHandler()->GetPrimarySlotId();
    ASSERT_NE(defaultSlotId, SimTest::SLOTID_2);
}

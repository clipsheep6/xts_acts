...
{
    int32_t result = static_cast<int32_t>(g_proxy.GetHandler()->GetCardType(SimTest::SLOTID_1));
    ASSERT_EQ(result, (int32_t)OHOS::Telephony::CardType::UNKNOWN_CARD);
}
...
{
    int32_t result = static_cast<int32_t>(g_proxy.GetHandler()->GetCardType(SimTest::SLOTID_2));
    ASSERT_EQ(result, (int32_t)OHOS::Telephony::CardType::UNKNOWN_CARD);
}
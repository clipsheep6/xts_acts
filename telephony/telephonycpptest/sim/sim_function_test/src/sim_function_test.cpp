...
HWTEST_F(SimSetVoiceMailTest, Telephony_Sim_SetVoiceMailInfo_0500, Function | MediumTest | Level1)
{
    ...
    std::u16string mailName = Str8ToStr16("123#@\%_ABCDE");
    std::u16string mailNumber = Str8ToStr16("01234567890123456789");
    bool ret = g_proxy.GetHandler()->SetVoiceMailInfo(SimTest::SLOTID_0, mailNameOverlength, mailNumber);
    ASSERT_TRUE(ret);
    std::u16string retMailNumber = g_proxy.GetHandler()->GetVoiceMailNumber(SimTest::SLOTID_0);
    ASSERT_STREQ(Str16ToStr8(retMailNumber).c_str(), Str16ToStr8(mailNumber).c_str());
    std::u16string retMailName = g_proxy.GetHandler()->GetVoiceMailIdentifier(SimTest::SLOTID_0);
...
HWTEST_F(SimSetVoiceMailTest, Telephony_Sim_SetVoiceMailInfo_0600, Function | MediumTest | Level1)
{
    ...
    std::u16string mailNumber = Str8ToStr16("01234567890123456789");
    bool ret = g_proxy.GetHandler()->SetVoiceMailInfo(SimTest::SLOTID_0, mailName, mailNumberOverlength);
    ASSERT_TRUE(ret);
    std::u16string retMailNumber = g_proxy.GetHandler()->GetVoiceMailNumber(SimTest::SLOTID_0);
    ASSERT_STREQ(Str16ToStr8(retMailNumber).c_str(), Str16ToStr8(mailNumber).c_str());
    std::u16string retMailName = g_proxy.GetHandler()->GetVoiceMailIdentifier(SimTest::SLOTID_0);
...
HWTEST_F(SimSetTest, Telephony_Sim_IsSimActive_0300, Function | MediumTest | Level3)
{
    ...
    ASSERT_TRUE(ret);
    bool result = g_proxy.GetHandler()->IsSimActive(SimTest::SLOTID_0);
    ASSERT_FALSE(result);
    // Delay two second to obtain the refreshed card status.
    sleep(2);
    int32_t state = g_proxy.GetHandler()->GetSimState(SimTest::SLOTID_0);
...
{
    bool setCloseResult = g_proxy.GetHandler()->SetActiveSim(SimTest::SLOTID_0, SimTest::CLOSE);
    ASSERT_TRUE(setCloseResult);
    sleep(1);
    bool result = g_proxy.GetHandler()->IsSimActive(SimTest::SLOTID_0);
    ASSERT_FALSE(result);
    bool setOpenResult = g_proxy.GetHandler()->SetActiveSim(SimTest::SLOTID_0, SimTest::OPEN);
    ASSERT_TRUE(setOpenResult);
    sleep(1);
    bool ret = g_proxy.GetHandler()->IsSimActive(SimTest::SLOTID_0);
    ASSERT_TRUE(ret);
}
...
HWTEST_F(SimSetTest, Telephony_Sim_GetActiveSimAccountInfoList_0100, Function | MediumTest | Level2)
{
    ...
    bool ret = g_proxy.GetHandler()->SetActiveSim(SimTest::SLOTID_0, SimTest::OPEN);
    ASSERT_TRUE(ret);
    ret = g_proxy.GetHandler()->SetShowName(SimTest::SLOTID_0, cardName);
    ASSERT_TRUE(ret);
...
HWTEST_F(SimSetTest, Telephony_Sim_GetActiveSimAccountInfoList_0200, Function | MediumTest | Level3)
{
    ...
    ret = g_proxy.GetHandler()->SetShowNumber(SimTest::SLOTID_0, cardNumber);
    ASSERT_TRUE(ret);
    ret = g_proxy.GetHandler()->SetActiveSim(SimTest::SLOTID_0, SimTest::CLOSE);
    ASSERT_TRUE(ret);

...
{
    bool retValue = g_proxy.GetHandler()->SendTerminalResponseCmd(SimTest::SLOTID_1, SimTest::RESPONSE_CMD);
    ASSERT_FALSE(retValue);
}
...
{
    bool retValue = g_proxy.GetHandler()->SendTerminalResponseCmd(SimTest::SLOTID_2, SimTest::RESPONSE_CMD);
    ASSERT_FALSE(retValue);
}
...
{
    bool retValue = g_proxy.GetHandler()->SendTerminalResponseCmd(SimTest::SLOTID_0, SimTest::RESPONSE_CMD);
...
{
    bool retValue = g_proxy.GetHandler()->SendEnvelopeCmd(SimTest::SLOTID_1, SimTest::STK_CMD);
    ASSERT_FALSE(retValue);
}
...
{
    bool retValue = g_proxy.GetHandler()->SendEnvelopeCmd(SimTest::SLOTID_2, SimTest::STK_CMD);
    ASSERT_FALSE(retValue);
}
...
{
    bool retValue = g_proxy.GetHandler()->SendEnvelopeCmd(SimTest::SLOTID_0, SimTest::STK_CMD);
    ASSERT_TRUE(retValue);
    std::shared_ptr<SimCommonEvent> getData = std::make_shared<SimCommonEvent>();
    std::string radioField = getData->GetStkActionData();
    ASSERT_STREQ(radioField.c_str(), ACTION_SESSION_END.c_str());

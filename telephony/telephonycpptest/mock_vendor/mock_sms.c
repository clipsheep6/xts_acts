...
void ReqSendGsmSms(const ReqDataInfo *requestInfo, const char *const *data, size_t dataLen)
{
    ...
    RespSuccessWithData(HRIL_SIM_SLOT_1, requestInfo, OnSmsReport, (const uint8_t *)&g_smsMockData.hrilSmsResponse,
...
{
    TELEPHONY_LOGD("enter to [%{public}s]:%{public}d, len:%{public}d", __func__, __LINE__, dataLen);
    RespSuccessWithoutData(HRIL_SIM_SLOT_1, requestInfo, OnSmsReport);
}
...
{
    TELEPHONY_LOGD("enter to [%{public}s]:%{public}d", __func__, __LINE__);
    RespSuccessWithData(HRIL_SIM_SLOT_1, requestInfo, OnSmsReport,

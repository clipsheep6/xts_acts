...
{
    TELEPHONY_LOGD("enter to [%{public}s]:%{public}d", __func__, __LINE__);
    RESP_SUCSS_WITH_DATA(requestInfo, OnSmsReport, &g_smsMockData.hrilServiceCenterAddress, 
        sizeof(HRilServiceCenterAddress));
}

void ReqSetCellBroadcast(const ReqDataInfo *requestInfo, const HRilCellBroadcastInfo *data, size_t dataLen)
{
    TELEPHONY_LOGD("enter to [%{public}s]:%{public}d, len:%{public}d", __func__, __LINE__, dataLen);
    RESP_SUCSS_WITHOUT_DATA(requestInfo, OnSmsReport);
}

void ReqGetCellBroadcast(const ReqDataInfo *requestInfo)
{
    TELEPHONY_LOGD("enter to [%{public}s]:%{public}d", __func__, __LINE__);
}

void ReqGetCDMACellBroadcast(const ReqDataInfo *requestInfo)
{
    TELEPHONY_LOGD("enter to [%{public}s]:%{public}d", __func__, __LINE__);
}

void ReqSetCDMACellBroadcast(const ReqDataInfo *requestInfo, const HRilCDMACellBroadcastInfo *data, size_t dataLen)
{
    TELEPHONY_LOGD("enter to [%{public}s]:%{public}d, len:%{public}d", __func__, __LINE__, dataLen);
}

void ReqActiveCDMACellBroadcast(const ReqDataInfo *requestInfo)
{
    TELEPHONY_LOGD("enter to [%{public}s]:%{public}d", __func__, __LINE__);
}

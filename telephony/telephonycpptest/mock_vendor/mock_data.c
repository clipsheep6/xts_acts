...
static bool onceTimeFlag = false;
static HRilErrno mockErrno;
struct CallMockData {
...
void SetErrnoType(const char *data, int dataLen)
{
    ...
    HRilErrno err = HRIL_ERR_NULL_POINT;
    if (memcpy_s(&err, sizeof(HRilErrno), data, dataLen) != 0) {
        return;
    }
...
void PdpContextListUpdated(int cid, const ReqDataInfo *requestInfo)
{
    ...
    for (int i = 0; i < DATA_CALL_LENGTH; i++) {
        if (g_dataMockData.pDataCalls[i].cid == cid) {
            RespSuccessWithData(HRIL_SIM_SLOT_1, requestInfo, OnDataReport,
    ...
    RespSuccessWithData(HRIL_SIM_SLOT_1, requestInfo, OnDataReport, (const uint8_t *)&g_dataMockData.pDataCalls,
        DATA_CALL_LENGTH * sizeof(HRilDataCallResponse));
    NotifySuccessWithData(HRIL_SIM_SLOT_1, OnDataReport, HNOTI_DATA_PDP_CONTEXT_LIST_UPDATED,
...
void ReqActivatePdpContext(const ReqDataInfo *requestInfo, const HRilDataInfo *data)
{
    ...
    if (onceTimeFlag) {
        onceTimeFlag = false;
        sleep(2);
        RespErrnoWithoutData(HRIL_SIM_SLOT_1, requestInfo, OnDataReport, mockErrno);
        return;
    }

...
#define COMMON_RESP_WITHOUT_DATA(requestInfo, report_func, err_code, flag)                               \
    {                                                                                                    \
        do {                                                                                             \
            TELEPHONY_LOGD("RESP_FAIL_WITHOUT_DATA");                                                    \
            struct ReportInfo reportInfo = CreateReportInfo(requestInfo, err_code, HRIL_RESPONSE, flag); \
...
#define NOTIFY_SUCSS_DATA(report_func, notify_id, data, dataLen)                                         \
    {                                                                                             \
        do {                                                                                      \
            struct ReportInfo reportInfo;                                                         \
            (void)memset_s(&reportInfo, sizeof(struct ReportInfo), 0, sizeof(struct ReportInfo)); \
            ...
            reportInfo.type = HRIL_NOTIFICATION;                                                  \
            reportInfo.error = HRIL_ERR_SUCCESS;                                                  \
            report_func(reportInfo, data, dataLen);                                                     \
            TELEPHONY_LOGD("NOTIFY_SUCSS_DATA id：%{public}d", notify_id);                \
        } while (0);                                                                              \
    }

...
{
    if (newState == HRIL_RADIO_POWER_STATE_ON_MINI) {
        CleanOperatorInfo();
        ModifyRegStatusInfo();
    } else if (newState == HRIL_RADIO_POWER_STATE_ON && g_radioState == HRIL_RADIO_POWER_STATE_ON_MINI) {
        InitNetworkMockData();
    } else {
        TELEPHONY_LOGD("> newState :%{public}d", newState);
    }
    g_radioState = newState;
    TELEPHONY_LOGE("g_radioState :%{public}d", g_radioState);
    NotifySuccessWithData(HRIL_SIM_SLOT_1, OnModemReport, HNOTI_MODEM_RADIO_STATE_UPDATED,

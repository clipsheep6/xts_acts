...
{
    MockCellInfo ci = {type, lac, cellId, arfcn, bsic, mcc, mnc};
    memset_s(nbuff_, BUFSIZ, 0, BUFSIZ);
    if (memcpy_s(nbuff_, sizeof(nbuff_), &ci, sizeof(MockCellInfo)) != 0) {
        return;
    }
    MockVendorSet(NETWORK_SEARCH_SET_CELL_INFO, nbuff_, sizeof(MockCellInfo));
}
...
int MockVendor::MockVendorSet(int id, const char *data, int dataLen) const
{
    ...
    if (dataLen > 0) {
        ...
        LOGE("> dataLen is invalid! id:%d", id);
        return -1;
    }
    SetFull(); // 已满

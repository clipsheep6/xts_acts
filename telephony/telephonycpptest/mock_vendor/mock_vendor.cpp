...
void MockVendor::MockVendorSetReg(int domain, int reg)
{
    ...
    num[0] = reg;
    num[1] = domain;
    int ret = memcpy_s(nbuff_, sizeof(nbuff_), num, sizeof(int) * numLen);
    if (ret != 0) {
        return;
    }
...
void MockVendor::MockVendorSetPlmnState(int state)
{
    ...
    int ret = memcpy_s(nbuff_, sizeof(nbuff_), &state, sizeof(int));
    if (ret != 0) {
        return;
    }
...
{
    memset_s(nbuff_, BUFSIZ, 0, BUFSIZ);
    int ret = memcpy_s(nbuff_, sizeof(nbuff_), &rat, sizeof(int));
    if (ret != 0) {
        return;
    }
...
int MockVendor::MockVendorSet(int id, const char *data, int dataLen) const
{
    ...
    if (dataLen > 0) {
        int ret = memcpy_s(sendSt_->data, BUFSIZ, data, dataLen);
        if (ret != 0) {
            return -1;
        }
    }

...
{
    // 获得共享内存
    shmid_ = shmget(static_cast<key_t>(shmAdress_), buffSize_, 0666);
    if (shmid_ == -1) {
        LOG("> shmget errno = %d\n", errno);
        return -1;
    }
...
void MockVendor::MockVendorSetReg(int domain, int reg)
{
    ...
    num[0] = reg;
    num[1] = domain;
    memcpy_s(nbuff_, sizeof(nbuff_), num, sizeof(int) * numLen);
...
int MockVendor::MockVendorSet(int id, const void* data, int dataLen) const
{
    // 自己设计的锁
    ...
    if (dataLen > 0) {
        memcpy_s(sendSt_->data, BUFSIZ, data, dataLen);
    }
    SetFull(); // 已满
...
int MockVendor::MockVendorSetWithDelay(int id, const void* data, int dataLen) const
{
    // 自己设计的锁
    for (int i = 0; i < maxLoop1000_; i++) {
        if (!IsFull()) {

...
static const int cellInfoLen = 7;
int MockVendor::Init()
{
    mode_t mode = 0666;
    // 获得共享内存
...
void MockVendor::MockVendorSetCellInfo(int type, int lac, int cellId, int arfcn, int bsic, int mcc, int mnc)
{
    int cellInfo[cellInfoLen] = {type, lac, cellId, arfcn, bsic, mcc, mnc};
    if (memcpy_s(nbuff_, sizeof(nbuff_), cellInfo, sizeof(cellInfo)) != 0) {
        return;
    }
    MockVendorSet(NETWORK_SEARCH_SET_CELL_INFO, nbuff_, sizeof(cellInfo));
}
...
int MockVendor::MockVendorSet(int id, const char *data, int dataLen) const
{
    ...
    // 赋值
    sendSt_->id = id;
    memset_s(sendSt_->data, BUFSIZ, 0, BUFSIZ);
    sendSt_->dataLen = dataLen;
    if (dataLen > 0) {
        if (memcpy_s(sendSt_->data, BUFSIZ, data, dataLen) != 0) {
            return -1;
        }
    }
    SetFull(); // 已满

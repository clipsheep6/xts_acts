...
void SetSimPin2(void* data, int dataLen)
{
...
void SetSimPuk2(void* data, int dataLen)
{
    CLEAR_AND_COPY_FROM_DIRTY_BUFFER(g_simMockData.puk2, data, dataLen);
}
...
void ReqUnlockSimPin2(const ReqDataInfo *requestInfo, const char* puk2, const char* pin2)
{
    ...
    VerifyPukFail2(ret);
    if (ret == 0) {
        ret = VerifyPin2(pin2, HRIL_SIM_READY);
        if ((ret) >= 0) { // not format problem

...
void ReqGetSimStatus(const ReqDataInfo *requestInfo)
{
    ...
    RESP_SUCSS_WITH_DATA(HRIL_SIM_SLOT_1, requestInfo, OnSimReport, &g_simMockData.cardState, sizeof(HRilCardState));
}
void ReqGetSimIO(const ReqDataInfo *requestInfo, const HRilSimIO *data, size_t dataLen)
{
    ...
    if (pSim->command == READ_RECORD && pSim->fileid == ELEMENTARY_FILE_SMS) {
        ...
        if (pSim->command == UPDATE_RECORD) {
            ...
            if (pSim->p1 <= 10 && g_simMockData.memFdnFull[pSim->p1] == true) {
                response = g_simMockData.memFdn[pSim->p1];
            } else if (pSim->p1 <= 10 && g_simMockData.memFdnFull[pSim->p1] == false) {
        ...
        if (pSim->command == UPDATE_RECORD) {
            ...
            if (pSim->p1 <= 10 && g_simMockData.memAdnFull[pSim->p1] == true) {
                response = g_simMockData.memFdn[pSim->p1];
            } else if (pSim->p1 <= 250 && g_simMockData.memAdnFull[pSim->p1] == false) {
                response = "FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF";
            } else {
                SimResponse.sw1 = 106;
                SimResponse.sw2 = 131;
        ...
        response = "FFFFFFFFFFFFFFFF";
        SimResponse.response = response;
        RESP_SUCSS_WITH_DATA(HRIL_SIM_SLOT_1, requestInfo, OnSimReport, &SimResponse, sizeof(SimResponse));
        free(cmd);
        return;
    }
    RESP_SUCSS_WITH_DATA(HRIL_SIM_SLOT_1, requestInfo, OnSimReport, &SimResponse, sizeof(SimResponse));
...
{
    TELEPHONY_LOGD("enter to [%{public}s]:%{public}d", __func__, __LINE__);
    RESP_SUCSS_WITH_DATA(HRIL_SIM_SLOT_1, requestInfo, OnSimReport, &g_simMockData.imsi, sizeof(g_simMockData.imsi));
}
void ReqGetSimIccID(const ReqDataInfo *requestInfo)
{
    TELEPHONY_LOGD("enter to [%{public}s]:%{public}d", __func__, __LINE__);
    RESP_SUCSS_WITH_DATA(HRIL_SIM_SLOT_1, requestInfo, OnSimReport, &g_simMockData.iccid, sizeof(g_simMockData.iccid));
}
void ReqGetSimLockStatus(const ReqDataInfo *requestInfo, const HRilSimClock *data, size_t dataLen)
{
    ...
    if (index == -1) {
        RESP_FAIL_WITHOUT_DATA(HRIL_SIM_SLOT_1, requestInfo, OnSimReport);
        return;
    }
    RESP_SUCSS_WITH_DATA(HRIL_SIM_SLOT_1, requestInfo, OnSimReport, &g_simMockData.stateList[index].state, sizeof(int));
}
void ReqSetSimLock(const ReqDataInfo *requestInfo, const HRilSimClock *data, size_t dataLen)
{
    ...
    RESP_CODE_WITHOUT_DATA(HRIL_SIM_SLOT_1, requestInfo, OnSimReport, ret);
}
...
void ReqGetSimPinInputTimes(const ReqDataInfo *requestInfo)
{
    ...
    RESP_SUCSS_WITH_DATA(HRIL_SIM_SLOT_1, requestInfo, OnSimReport, &g_simMockData.pinInputTimes, sizeof(HRilPinInputTimes));
}
...
void ReqSetActiveSim(const ReqDataInfo *requestInfo, int index, int enable)
{
    ...
    if (enable == 0) {
        ...
        RESP_CODE_WITHOUT_DATA(HRIL_SIM_SLOT_1, requestInfo, OnSimReport, HRIL_ERR_INVALID_PARAMETER);
        return;
    }
    NotifySuccessWithoutData(HRIL_SIM_SLOT_1, OnSimReport, HNOTI_SIM_STATUS_CHANGED);
    RESP_CODE_WITHOUT_DATA(HRIL_SIM_SLOT_1, requestInfo, OnSimReport, HRIL_ERR_SUCCESS);
}

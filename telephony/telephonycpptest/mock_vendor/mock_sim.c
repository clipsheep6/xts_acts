...
{
    char buff[10];
    strncpy_s(buff, sizeof(buff), data, dataLen);
    int int_state = 0;
...
void InitSimMockData(void)
{
    ...
    strncpy_s(g_simMockData.puk, sizeof(g_simMockData.puk2), tmpPuk, strlen(tmpPuk));
...
void GetSimIoFdn(HRilSimIO *pSim, HRilSimIOResponse *SimResponse)
{
    char *response = NULL;
...
void GetSimIoAdn(HRilSimIO *pSim, HRilSimIOResponse *SimResponse)
{
    char *response = NULL;
    if (pSim->command == UPDATE_RECORD) {
...
void ReqGetSimIO(const ReqDataInfo *requestInfo, const HRilSimIO *data, size_t dataLen)
{
    ...
    if (pSim->command == READ_RECORD && pSim->fileid == ELEMENTARY_FILE_SMS) {
        ...
        GetSimIoFdn(pSim, &SimResponse);
    } else if (pSim->fileid == ELEMENTARY_FILE_ADN) {
        GetSimIoAdn(pSim, &SimResponse);

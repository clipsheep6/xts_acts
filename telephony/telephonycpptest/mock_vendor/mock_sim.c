...
{
    TELEPHONY_LOGD("enter to [%{public}s]:%{public}d sim status: %{public}d", __func__, __LINE__, g_simMockData.cardState.simState);
...
{
    TELEPHONY_LOGD("enter to [%{public}s]:%{public}d", __func__, __LINE__);
    RESP_SUCSS_WITH_DATA(requestInfo, OnSimReport, &g_simMockData.imsi, sizeof(void *));
}

void ReqGetSimIccID(const ReqDataInfo *requestInfo)
{
    TELEPHONY_LOGD("enter to [%{public}s]:%{public}d", __func__, __LINE__);
    RESP_SUCSS_WITH_DATA(requestInfo, OnSimReport, &g_simMockData.iccid, sizeof(void *));
}
...
void ReqGetSimLockStatus(const ReqDataInfo *requestInfo, const HRilSimClock *data, size_t dataLen)
{
    ...
    RESP_SUCSS_WITH_DATA(requestInfo, OnSimReport, &g_simMockData.stateList[index].state, sizeof(void *));
}
...
{
    TELEPHONY_LOGD("enter to [%{public}s]:%{public}d, puk2:%{public}s, pin2:%{public}s", __func__, __LINE__, puk2, pin2);

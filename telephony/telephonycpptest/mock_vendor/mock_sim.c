...
const int ICC_TELECOM_INTERNAL_ROAM_TYPE = 7;
const int ICC_UNICOM_DUAL_MODE_TYPE = 8;
const int ICC_TELECOM_LTE_TYPE = 9;
...
void ViewMem(int id)
{
    if (id == ELEMENTARY_FILE_ADN) {
        for (int i = 0; i < FDN_SIZE; i++) {
            TELEPHONY_LOGD("CRSM:mem_%{public}d[%{public}d]:%{public}s", id, i, g_simMockData.memAdn[i]);
        }
    } else if (id == ELEMENTARY_FILE_FDN) {
        for (int i = 0; i < FDN_SIZE; i++) {
            TELEPHONY_LOGD("CRSM:mem_%{public}d[%{public}d]:%{public}s", id, i, g_simMockData.memFdn[i]);
        }
    } else {
        TELEPHONY_LOGD("enter to [%{public}s]:%{public}d no this mem to view", __func__, __LINE__);
        return;
    }
}
...
enum CardType ConvertCardType(int simType)
{
    ...
    switch (simType) {
        ...
        case CT_NATIONAL_ROAMING_CARD:
            ret = ICC_TELECOM_INTERNAL_ROAM_TYPE;
            break;
        case CU_DUAL_MODE_CARD:
            ret = ICC_UNICOM_DUAL_MODE_TYPE;
            break;
        case DUAL_MODE_TELECOM_LTE_CARD:
            ret = ICC_TELECOM_LTE_TYPE;
            break;
...
void SetSimCardType(const char *data, int dataLen)
{
    ...
    TELEPHONY_LOGD("simType:%{public}d", g_simMockData.cardState.simType);
    NotifySuccessWithData(HRIL_SIM_SLOT_1, OnSimReport, HNOTI_SIM_STATUS_CHANGED,
...
void InitSimMockData(void)
{
    TELEPHONY_LOGD("enter to [%{public}s]:%{public}d", __func__, __LINE__);
    InitLockState();
    g_simMockData.cardState.simType = HRIL_USIM;
...
{
    TELEPHONY_LOGD("reset Pin Input Times");
...
{
    TELEPHONY_LOGD("reset Pin2 Input Times");
...
bool IsPinFormatCorrect(const char *pin)
{
    ...
    if (pinLen < PIN_MIN || pinLen > PIN_MAX) {
        TELEPHONY_LOGD("Verify Pin len: %{public}d", pinLen);
        return false;
    }
    for (int i = 0; i < pinLen; i++) {
        if (pin[i] < '0' || pin[i] > '9') {
            TELEPHONY_LOGD("Verify Pin char %{public}c", pin[i]);
            return false;
        }
    }
    return true;
...
bool IsPukFormatCorrect(const char *puk)
{
    ...
    for (int i = 0; i < PUK_LEN; i++) {
        if (puk[i] < '0' || puk[i] > '9') {
            TELEPHONY_LOGD("Verify Puk char %{public}c", puk[i]);
            return false;
        }
    }
    return true;
...
int VerifyPin(const char *pin, int state)
{
    if (!IsPinFormatCorrect(pin)) {
        TELEPHONY_LOGD("Verify Pin format error");
        return -1;
    }
    ...
    if (strcmp(pin, g_simMockData.stateList[GetLockStatusIndex("SC")].passwd) != 0) {
        TELEPHONY_LOGD("Verify passwd error");
...
int VerifyPin2(const char *pin2, int state)
{
    ...
    if (g_simMockData.stateList[GetLockStatusIndex("P2")].state == 0) {
        TELEPHONY_LOGD("Verify Pin2 lock state: Lock");
        return -1;
    }

    if (g_simMockData.pinInputTimes.pin2Times <= 0) {
        TELEPHONY_LOGD("Get pin2 remain times are 0. %{public}d", state);
        return -1;
    }
    if (strcmp(pin2, g_simMockData.stateList[GetLockStatusIndex("P2")].passwd) != 0) {
        return UNLOCK_PIN_PUK_INCORRECT;
    }
    TELEPHONY_LOGD("Verify Pin2 succuss");
    return 0;
}
...
{
    if (errCode == 0) {
        TELEPHONY_LOGD("reset puk inputtimes");
        ResetPukInputTimes();
        return;
    }
...
{
    if (errCode == 0) {
        TELEPHONY_LOGD("reset puk2 inputtimes");
        ResetPuk2InputTimes();
        return;
    }
    if (errCode != UNLOCK_PIN_PUK_INCORRECT) {
...
int VerifyPuk(const char *puk, int state)
{
    ...
    if (!IsPukFormatCorrect(puk)) {
        TELEPHONY_LOGD("puk format error");
        return -1;
    }
    if (g_simMockData.pinInputTimes.pinTimes != 0) {
        TELEPHONY_LOGD("puk lock is not triggered, %{public}d", state);
        return -1;
    }
    if (g_simMockData.pinInputTimes.puk2Times == 0) {
        TELEPHONY_LOGD("Get puk remain times are 0.");
        return -1;
    }
...
void ReqGetSimStatus(const ReqDataInfo *requestInfo)
{
    ...
    RespSuccessWithData(
        HRIL_SIM_SLOT_1, requestInfo, OnSimReport, (const uint8_t *)&g_simMockData.cardState, sizeof(HRilCardState));
}
...
void ReqGetSimIO(const ReqDataInfo *requestInfo, const HRilSimIO *data, size_t dataLen)
{
    ...
    TELEPHONY_LOGD("CRSM:req > %{public}s", cmd);
    if (pSim->command == READ_RECORD && pSim->fileid == ELEMENTARY_FILE_SMS) {
        ...
        if (pSim->command == UPDATE_RECORD) {
            ...
            response = "621E82054221001CFA83026F3AA503";
        } else if (pSim->command == READ_RECORD) {
        ...
        SimResponse.response = response;
        RespSuccessWithData(
            HRIL_SIM_SLOT_1, requestInfo, OnSimReport, (const uint8_t *)&SimResponse, sizeof(SimResponse));
        free(cmd);
        return;
    }
    RespSuccessWithData(HRIL_SIM_SLOT_1, requestInfo, OnSimReport, (const uint8_t *)&SimResponse, sizeof(SimResponse));
    free(cmd);
}
...
void ReqGetSimImsi(const ReqDataInfo *requestInfo)
{
    TELEPHONY_LOGD("enter to [%{public}s]:%{public}d", __func__, __LINE__);
    RespSuccessWithData(
        HRIL_SIM_SLOT_1, requestInfo, OnSimReport, (const uint8_t *)&g_simMockData.imsi, sizeof(g_simMockData.imsi));
}
void ReqGetSimIccID(const ReqDataInfo *requestInfo)
{
    TELEPHONY_LOGD("enter to [%{public}s]:%{public}d", __func__, __LINE__);
    RespSuccessWithData(
        HRIL_SIM_SLOT_1, requestInfo, OnSimReport, (const uint8_t *)&g_simMockData.iccid, sizeof(g_simMockData.iccid));
}
void ReqGetSimLockStatus(const ReqDataInfo *requestInfo, const HRilSimClock *data, size_t dataLen)
{
    ...
    if (index == -1) {
        RespErrnoWithoutData(HRIL_SIM_SLOT_1, requestInfo, OnSimReport, HRIL_ERR_GENERIC_FAILURE);
        return;
    }
    RespSuccessWithData(
        HRIL_SIM_SLOT_1, requestInfo, OnSimReport, (const uint8_t *)&g_simMockData.stateList[index].state, sizeof(int));
}
void ReqSetSimLock(const ReqDataInfo *requestInfo, const HRilSimClock *data, size_t dataLen)
{
    ...
    RespErrnoWithoutData(HRIL_SIM_SLOT_1, requestInfo, OnSimReport, ret);
}
...
{
    TELEPHONY_LOGD("enter to [%{public}s]:%{public}d, pin:%{public}s", __func__, __LINE__, pin);
    int ret = -1;
...
{
    TELEPHONY_LOGD("enter to [%{public}s]:%{public}d, pin2:%{public}s", __func__, __LINE__, pin2);
    int ret = -1;
...
void ReqGetSimPinInputTimes(const ReqDataInfo *requestInfo)
{
    ...
    RespSuccessWithData(HRIL_SIM_SLOT_1, requestInfo, OnSimReport, (const uint8_t *)&g_simMockData.pinInputTimes,
...
void ReqSetActiveSim(const ReqDataInfo *requestInfo, int index, int enable)
{
    ...
    if (enable == 0) {
        ...
        RespErrnoWithoutData(HRIL_SIM_SLOT_1, requestInfo, OnSimReport, HRIL_ERR_INVALID_PARAMETER);
        return;
    }
    NotifySuccessWithoutData(HRIL_SIM_SLOT_1, OnSimReport, HNOTI_SIM_STATUS_CHANGED);
    RespErrnoWithoutData(HRIL_SIM_SLOT_1, requestInfo, OnSimReport, HRIL_ERR_SUCCESS);
}

void ReqSendTerminalResponseCmd(const ReqDataInfo *requestInfo, const char *strCmd)
{
    TELEPHONY_LOGD("enter to [%{public}s]:%{public}d", __func__, __LINE__);
    RespSuccessWithoutData(HRIL_SIM_SLOT_1, requestInfo, OnSimReport);
}

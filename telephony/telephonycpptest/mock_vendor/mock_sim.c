...
void ReqGetSimStatus(const ReqDataInfo *requestInfo)
{
    ...
    RespSuccessWithData(HRIL_SIM_SLOT_1, requestInfo, OnSimReport, (const uint8_t *)&g_simMockData.cardState,
        sizeof(HRilCardState));
}
...
void ReqGetSimIO(const ReqDataInfo *requestInfo, const HRilSimIO *data, size_t dataLen)
{
    ...
    if (pSim->command == READ_RECORD && pSim->fileid == ELEMENTARY_FILE_SMS) {
        ...
        response = "FFFFFFFFFFFFFFFF";
        SimResponse.response = response;
        RespSuccessWithData(HRIL_SIM_SLOT_1, requestInfo, OnSimReport, (const uint8_t *)&SimResponse,
            sizeof(SimResponse));
        free(cmd);
        return;
    }
...
{
    TELEPHONY_LOGD("enter to [%{public}s]:%{public}d", __func__, __LINE__);
    RespSuccessWithData(HRIL_SIM_SLOT_1, requestInfo, OnSimReport, (const uint8_t *)&g_simMockData.imsi,
        sizeof(g_simMockData.imsi));
}
void ReqGetSimIccID(const ReqDataInfo *requestInfo)
{
    TELEPHONY_LOGD("enter to [%{public}s]:%{public}d", __func__, __LINE__);
    RespSuccessWithData(HRIL_SIM_SLOT_1, requestInfo, OnSimReport, (const uint8_t *)&g_simMockData.iccid,
        sizeof(g_simMockData.iccid));
}
void ReqGetSimLockStatus(const ReqDataInfo *requestInfo, const HRilSimClock *data, size_t dataLen)
{
    ...
    RespSuccessWithData(HRIL_SIM_SLOT_1, requestInfo, OnSimReport,
        (const uint8_t *)&g_simMockData.stateList[index].state, sizeof(int));
}
...
void ReqGetSimPinInputTimes(const ReqDataInfo *requestInfo)
{
    ...
    RespSuccessWithData(HRIL_SIM_SLOT_1, requestInfo, OnSimReport,
        (const uint8_t *)&g_simMockData.pinInputTimes, sizeof(HRilPinInputTimes));
}

...
void ReqGetSimIO(const ReqDataInfo *requestInfo, const HRilSimIO *data, size_t dataLen)
{
    ...
    RespSuccessWithData(HRIL_SIM_SLOT_0, requestInfo, OnSimReport, (const uint8_t *)&SimResponse, sizeof(SimResponse));
    free(cmd);
}
...
void ReqSetRadioProtocol(const ReqDataInfo *requestInfo, const HRilSimProtocolRequest *data, size_t dataLen)
{
    ...
    if (data == NULL || dataLen == 0) {
        RespErrnoWithoutData(HRIL_SIM_SLOT_0, requestInfo, OnSimReport, HRIL_ERR_INVALID_RESPONSE);
        return;
    }
    if (simResponse.phase == 0) {
        simResponse.phase = data->phase;
        simResponse.result = 0;
        simResponse.slotId = data->slotId;
    } else {
        simResponse.phase = data->phase;
...
void ReqTransmitApduSimIO(const ReqDataInfo *requestInfo, HRilApduSimIO *data, size_t dataLen)
{
    ...
    HRilApduSimIO *pApduSimIO = data;
    if (pApduSimIO == NULL) {
        RespErrnoWithoutData(HRIL_SIM_SLOT_0, requestInfo, OnSimReport, HRIL_ERR_INVALID_RESPONSE);
        return;
    }
    apduSimResponse.sw1 = 0;
    apduSimResponse.sw2 = 0;
    apduSimResponse.response =
        ("FF403CE23AE12CC114ABCD92CBB156B280FA4E1429A6ECEEB6E5C1BFE4CA14636F6D2E6F686F732E6F70656E686172"
         "6D6F6E79E30ADB080000000000000001");
    RespSuccessWithData(
        HRIL_SIM_SLOT_0, requestInfo, OnSimReport, (const uint8_t *)&apduSimResponse, sizeof(HRilSimIOResponse));
}

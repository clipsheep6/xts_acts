...
static const int NUM_F = 145;
static const int NUM_S = 129;
...
{
    TELEPHONY_LOGD("enter to [%{public}s]:%{public}d", __func__, __LINE__);
    RESP_SUCSS_WITH_DATA(requestInfo, OnCallReport, g_callMockData.pp_calls, 
                            sizeof(void*) * g_callMockData.call_index);
    HandleCallState();
}
...
void ReqDial(ReqDataInfo *requestInfo, const void *data, size_t dataLen)
{
    ...
    TELEPHONY_LOGD("enter to [%{public}s]:%{public}d, address:%{public}s, clir:%{public}d, dataLen:%{public}d", 
    __func__, __LINE__, p_dial->address, p_dial->clir, dataLen);
...
void SetCallForwardingInfo(const int reason, const int mode, char *number, const int numType, const int classx)
{
    ...
    if (mode == QUERY) { // query
        ...
        CLEAR_AND_COPY_FROM_CLEAN_BUFFER(g_callMockData.cf_info[reason].number, defaultNumber);

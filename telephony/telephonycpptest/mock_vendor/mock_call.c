...
struct CallMockData {
    ...
    int UssdCusd;
    int mute;
    struct HRilCallStatusInfo callStatusInfo;
    struct HRilCallEndInfo callEndInfo;
...
void UploadCallEndInfo(int callId)
{
    ...
    NotifySuccessWithData(HRIL_SIM_SLOT_0, OnCallReport, HNOTI_CALL_BASE,
        (const uint8_t *)&g_callMockData.callEndInfo, sizeof(struct HRilCallEndInfo));
}
...
void UploadCallStatusInfo(int callId, int status, int voiceDomain)
{
    ...
    TELEPHONY_LOGI("> call id:%{public}d, status:%{public}d, voiceDomain:%{public}d", callId + CALLID_OFFSET_VALUE,
        status, voiceDomain);
    NotifySuccessWithData(HRIL_SIM_SLOT_0, OnCallReport, HNOTI_CALL_STATE_UPDATED,
        (const uint8_t *)&g_callMockData.callStatusInfo, sizeof(struct HRilCallStatusInfo));
    ChangeCallState();
}

...
void AddBaseCallInfo(char* number, HRilCallState state)
{
    ...
    // p_calls->numberRemind = 0;
    // p_calls->remoteName = NULL;
    // p_calls->nameRemind = 0;
...
{
    for (int i = 0; i < g_callMockData.call_index; i++) {
        if (strcmp(g_callMockData.pp_calls[i]->number, number) == 0 && (g_callMockData.pp_calls[i]->state == HRIL_CALL_ALERTING || g_callMockData.pp_calls[i]->state == HRIL_CALL_DIALING)) {
...
void AnswerCall()
{
    ...
    for (int i = 0; i < g_callMockData.call_index; i++) {
        ...
        if (g_callMockData.pp_calls[i]->state == HRIL_CALL_INCOMING || g_callMockData.pp_calls[i]->state == HRIL_CALL_WAITING) {
            g_callMockData.pp_calls[i]->state = HRIL_CALL_ACTIVATE;
        }
    }
    NOTIFY_SUCSS_WITHOUT_DATA(OnCallReport, HNOTI_CALL_STATE_UPDATED);
...
void InitCallMockData()
{
    ...
    g_callMockData.pp_calls = (HRilCallInfo**)malloc(sizeof(HRilCallInfo*) * PP_CALL_MAX_LEN);
    g_callMockData.call_index = 0;
    g_callMockData.call_id = 0;
...
static void HangUpAndActiveCall()
{
    TELEPHONY_LOGD("enter to [%{public}s]:%{public}d", __func__, __LINE__);
    int holdId = -1;
    int hangId = -1;
...
static void HangUpAllHoldWaitCall()
{
    TELEPHONY_LOGD("enter to [%{public}s]:%{public}d", __func__, __LINE__);
    for (int i = 0; i < g_callMockData.call_index; i++) {
        if (g_callMockData.pp_calls[i]->state == HRIL_CALL_HOLDING || g_callMockData.pp_calls[i]->state == HRIL_CALL_WAITING) {
...
void ReqHold(const ReqDataInfo *requestInfo)
{
    TELEPHONY_LOGD("enter to [%{public}s]:%{public}d", __func__, __LINE__);
    HoldAndActiveAtSend(requestInfo);
}

void ReqActive(const ReqDataInfo* requestInfo)
{
    TELEPHONY_LOGD("enter to [%{public}s]:%{public}d", __func__, __LINE__);
    HoldAndActiveAtSend(requestInfo);
}
void ReqSwap(const ReqDataInfo* requestInfo)
{
    TELEPHONY_LOGD("enter to [%{public}s]:%{public}d", __func__, __LINE__);
    HoldAndActiveAtSend(requestInfo);
}
...
void ReqJoin(const ReqDataInfo* requestInfo, int callType)
{
    TELEPHONY_LOGD("enter to [%{public}s]:%{public}d", __func__, __LINE__);
    RESP_SUCSS_WITHOUT_DATA(OnCallReport);
}
void ReqSplit(const ReqDataInfo* requestInfo, int nThCall, int callType)
{
    TELEPHONY_LOGD("enter to [%{public}s]:%{public}d", __func__, __LINE__);
    SplitCall(nThCall, callType);
    RESP_SUCSS_WITHOUT_DATA(OnCallReport);
}
void ReqGetCallWait(const ReqDataInfo* requestInfo)
{
    TELEPHONY_LOGD("enter to [%{public}s]:%{public}d", __func__, __LINE__);
    RESP_SUCSS_WITH_DATA(OnCallReport, &g_callMockData.hrilCallWaitResult, sizeof(HRilCallWaitResult));
}
void ReqSetCallWait(const ReqDataInfo* requestInfo, int active)
{
    TELEPHONY_LOGD("enter to [%{public}s]:%{public}d", __func__, __LINE__);
...
void SetCallForwardingInfo(int reason, int mode, char* number, int numType, int classx)
{
    ...
    static const int RGIESTER = 3;
    static const int UNRGIESTER = 4;
    char* defaultNumber = "";
    if (number == NULL) {
    ...
    if (mode == QUERY) { // query
        ...
        CLEAR_AND_COPY_FROM_CLEAN_BUFFER(g_callMockData.cf_info[reason].number, number);
...
void ReqSetCallForwarding(const ReqDataInfo* requestInfo, HRilCFInfo info)
{
    TELEPHONY_LOGD("enter to [%{public}s]:%{public}d", __func__, __LINE__);
    int numType;
...
void ReqGetCallForwarding(const ReqDataInfo* requestInfo, int reason)
{
    TELEPHONY_LOGD("enter to [%{public}s]:%{public}d", __func__, __LINE__);

...
void ReqSetRadioState(const ReqDataInfo *requestInfo, int function, int reset)
{
    ...
    if (GetRadioState() == function) {
        RespErrnoFixDataLenEqualOne(HRIL_SIM_SLOT_1, requestInfo, OnModemReport, HRIL_ERR_REPEAT_STATUS);
        return;
    }
    SetRadioState(function, reset);
    RespSuccessWithoutData(HRIL_SIM_SLOT_1, requestInfo, OnModemReport);
    return;
}
...
void ReqGetRadioState(const ReqDataInfo *requestInfo)
{
    ...
    RespSuccessWithData(HRIL_SIM_SLOT_1, requestInfo, OnModemReport, (const uint8_t *)&state, sizeof(HRilRadioState));
}

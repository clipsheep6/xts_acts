...
static CellInfo cellInfo[4] = {
    { RAT_TYPE_GSM, { {1, 49, 51, 3674, 10138, -88} }},
    { RAT_TYPE_GSM, { {2, 625, 20, 0, 10138, -90} }},
    { RAT_TYPE_GSM, { {2, 617, 55, 4452, 10138, -94} }},
    { RAT_TYPE_GSM, { {2, 622, 50, 0, 10138, -95} }}
};
static CurrentCellInfoVendor cciv = { RAT_TYPE_GSM, 460, 0, { {1, 48, 54, 5141, 10138, -82, 0, 12} }};
...
void SetCurrentCellInfo(const char *buff)
{
    TELEPHONY_LOGD("enter to [%{public}s]:%{public}d", __func__, __LINE__);
    int i = 0;
    int type = *((int *)buff + i++);
    int lac = *((int *)buff + i++);
    int cellId = *((int *)buff + i++);
    int arfcn = *((int *)buff + i++);
    int bsic = *((int *)buff + i++);
    int mcc = *((int *)buff + i++);
    int mnc = *((int *)buff + i++);
    TELEPHONY_LOGD("type:%{public}d lac:%{public}d cellId:%{public}d arfcn:%{public}d bsic:%{public}d mcc:%{public}d "
        "mnc:%{public}d", type, lac, cellId, arfcn, bsic, mcc, mnc);
    cciv.ratType = type;
    cciv.mcc = mcc;
    cciv.mnc = mnc;

    if (type == RAT_TYPE_GSM)
    {
        cciv.ServiceCellParas.gsm.arfcn = arfcn;
        cciv.ServiceCellParas.gsm.cellId = cellId;
        cciv.ServiceCellParas.gsm.bsic = bsic;
        cciv.ServiceCellParas.gsm.lac = lac;
    } else if (type == RAT_TYPE_LTE)
    {
        cciv.ServiceCellParas.lte.arfcn = arfcn;
        cciv.ServiceCellParas.lte.cellId = cellId;
        cciv.ServiceCellParas.lte.pci = bsic;
        cciv.ServiceCellParas.lte.tac = lac;
    } else if (type == RAT_TYPE_WCDMA)
    {
        cciv.ServiceCellParas.wcdma.arfcn = arfcn;
        cciv.ServiceCellParas.wcdma.cellId = cellId;
        cciv.ServiceCellParas.wcdma.psc = bsic;
        cciv.ServiceCellParas.wcdma.lac = lac;
    } else
    {
...
void ReqGetNetworkSearchInformation(const ReqDataInfo *requestInfo)
{
    ...
    RESP_SUCSS_WITH_DATA(HRIL_SIM_SLOT_1, requestInfo,
        OnNetworkReport, (const uint8_t *)g_networkMockData.pp_OperInfo, g_operCount * sizeof(AvailableOperInfo *));
}

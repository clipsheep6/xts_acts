...
void InitNetworkMockData(void)
{
    ...
    g_networkMockData.pp_OperInfo = (AvailableOperInfo**)malloc(g_operCount * sizeof(void *));
...
{
    TELEPHONY_LOGD("enter to [%{public}s]:%{public}d", __func__, __LINE__);
    RESP_SUCSS_WITH_DATA(requestInfo, OnNetworkReport, &g_networkMockData.operator_response, 3 * sizeof(void *));
}
...
void RequestSetPreferredNetworkPara(const void *requestInfo, const void *data, size_t dataLen)
{
    TELEPHONY_LOGD("enter to [%{public}s]:%{public}d", __func__, __LINE__);

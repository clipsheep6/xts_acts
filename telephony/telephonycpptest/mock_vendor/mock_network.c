...
void ReqGetOperatorInfo(const ReqDataInfo *requestInfo)
{
    ...
    RESP_SUCSS_WITH_DATA(
        HRIL_SIM_SLOT_1, requestInfo, OnNetworkReport, &g_networkMockData.operator_response, OPERATOR_INFO_ARR_LENGTH);
}
...
void ReqGetNetworkSearchInformation(const ReqDataInfo *requestInfo)
{
    TELEPHONY_LOGD("enter to [%{public}s]:%{public}d", __func__, __LINE__);
    ShowCanUseOperInfo();
    RESP_SUCSS_WITH_DATA(HRIL_SIM_SLOT_1, requestInfo, OnNetworkReport, (const uint8_t *)g_networkMockData.pp_OperInfo,
...
void ReqGetImei(const ReqDataInfo *requestInfo)
{
    TELEPHONY_LOGD("enter to [%{public}s]:%{public}d", __func__, __LINE__);
    RESP_SUCSS_WITH_DATA(
        HRIL_SIM_SLOT_1, requestInfo, OnNetworkReport, (const uint8_t *)g_networkMockData.imei, sizeof(char *));
}
...
void ReqGetCellInfoList(const ReqDataInfo *requestInfo)
{
    TELEPHONY_LOGD("enter to [%{public}s]:%{public}d", __func__, __LINE__);
    RESP_SUCSS_WITH_DATA(HRIL_SIM_SLOT_1, requestInfo, OnNetworkReport,
        (const uint8_t *)&g_networkMockData.cellInfoList, sizeof(CellInfoList));
}

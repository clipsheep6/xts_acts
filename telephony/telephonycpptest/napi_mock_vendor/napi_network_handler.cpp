...
NetworkStatus *ParseNetworkStats(napi_env env, napi_callback_info info)
{
    ...
    HILOG_DEBUG(LOG_APP, "parse : src data: %{public}s %{public}s %{public}s ", networkStats->longOptName,
        networkStats->shortOptName, networkStats->plmn);
    HILOG_DEBUG(LOG_APP, "parse : src data: %{public}d %{public}d ", networkStats->regStatus, networkStats->nsaState);
    HILOG_DEBUG(LOG_APP, "parse : src data: %{public}d %{public}d %{public}d ", networkStats->isRoaming,
        networkStats->isCaActive, networkStats->isEmergency);
    return networkStats;
}
...
SignalInfo *ParseSignal(napi_env env, napi_callback_info info)
{
    ...
    HILOG_DEBUG(
        LOG_APP, "ParseSignal : type: %{public}d  level:: %{public}d", signalInfo->signalType, signalInfo->signalLevel);
    return signalInfo;
}
} // namespace NetworkHandler
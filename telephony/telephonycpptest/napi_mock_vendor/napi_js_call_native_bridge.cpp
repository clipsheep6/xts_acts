...
napi_value MockVendorSetInt(napi_env env, napi_callback_info info)
{
    ...
    size_t bufsize = 1;
    int src[1] = {value};
    int ret = memcpy_s(buffer, sizeof(buffer), src, sizeof(src));
    napi_value result = nullptr;
    if (ret != 0) {
...
napi_value ChangeNetworkStats(napi_env env, napi_callback_info info) {
    napi_value ret = nullptr;
    NetworkStatus *data  = parse(env, info);
    if (!data) {
        napi_create_int32(env, -1, &ret);
        return ret;
    }
    
    int mvSetRet = mMockVendor.MockVendorSet(NETWORK_SEARCH_CHANGE_STATE, (const char*)data, sizeof(struct NetworkStatus));

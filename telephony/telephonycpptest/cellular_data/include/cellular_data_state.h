...
int32_t CallManager::CallAbilityCallbackBasic::OnSupplementResult(
    const CallResultReportId id, AppExecFwk::PacMap &resultInfo)
{
    return SUCCESSFUL;
}
...
void CelluarDataStateTest::RecoverState()
{
    ...
    LOCK_NUM_WHILE_NE(static_cast<RadioTech>(GetProxy_()->GetNetworkState(SLOT_ID)->GetPsRadioTech()) ==
        RadioTech::RADIO_TECHNOLOGY_LTE || static_cast<RadioTech>(GetProxy_()->GetNetworkState(SLOT_ID)
        ->GetCsRadioTech()) == RadioTech::RADIO_TECHNOLOGY_LTE, true, LOCK_WAIT_SLIP, LOCK_WAIT_TIMEOUT);
    sleep(WAIT_TIME);
}
...
void CelluarDataStateTest::TearDownTestCase()
{
    ...
    hasNewData[SET_RADIO_STATUS] = false;
    hasNewData[SET_PREFERRED_NETWORK_MODE] = false;
    ASSERT_TRUE(GetProxy_()->SetPreferredNetwork(SLOT_ID, static_cast<int32_t>
        (PreferredNetwork::CORE_NETWORK_MODE_AUTO), g_callback));
    LOCK_NUM_WHILE_NE(hasNewData[SET_PREFERRED_NETWORK_MODE], true, LOCK_WAIT_SLIP, LOCK_WAIT_TIMEOUT);
#endif
#ifdef CELLULAR_DATA_MOCK_TEST

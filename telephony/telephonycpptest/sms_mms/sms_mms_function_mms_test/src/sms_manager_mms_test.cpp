...
HWTEST_F(MmsManagerTest, Telephony_SmsMms_EncodeMsg_0100, Function | MediumTest | Level1)
{
    ...
    EXPECT_TRUE(attachments[0].GetContentDisposition() == "from-data");
    ...
    EXPECT_TRUE(attachments[1].GetContentDisposition() == "attachment");
    ...
    EXPECT_TRUE(attachments[2].GetContentDisposition() == "inline");
}
 ...
 * @tc.name     If fieldId is set to MMS_CONTENT_CLASS, Value is the normal value MMS_STORE_STATUS_ERROR_TRANSIENT_FAILURE. View the result
 * @tc.desc     Function test
 */
...
{
    MmsMsg encodeMsg;
    EXPECT_FALSE(encodeMsg.SetHeaderOctetValue(MMS_STORE_STATUS,
        static_cast<uint8_t>(MmsStoreStatus::MMS_STORE_STATUS_SUCCESS) - 1));
}
...
HWTEST_F(MmsManagerTest, Telephony_SmsMms_SetHeaderOctetValue_3000, Function | MediumTest | Level1)
{
    MmsMsg encodeMsg;
    EXPECT_FALSE(encodeMsg.SetHeaderOctetValue(MMS_STORE_STATUS,
        static_cast<uint8_t>(MmsStoreStatus::MMS_STORE_STATUS_ERROR_PERMANENT_MMBOX_FULL) + 1));
}

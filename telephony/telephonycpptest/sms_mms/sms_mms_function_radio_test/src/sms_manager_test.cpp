...
HWTEST_F(SmsManagerTest, Telephony_SmsMms_SendMessageText_0100, Function | MediumTest | Level1)
{
    ...
    LockNumWHhileNe(getRadioStatusCallback, false, LOCK_WAIT_SLIP, LOCK_WAIT_TIMEOUT);
    ...
    LockNumWHhileNe((sendCallBackPtr->GetSendCallbackResult() || deliveryCallBackPtr->GetDeliverybackResult()), true,
        LOCK_WAIT_SLIP, LOCK_WAIT_TIMEOUT * 2);
    ...
    LockNumWHhileNe(hasNewData[SET_RADIO_STATUS], true, LOCK_WAIT_SLIP, LOCK_WAIT_TIMEOUT);
    ASSERT_TRUE(GetProxy_()->GetRadioState(slotId, g_callback));
    LockNumWHhileNe(getRadioStatusCallback, true, LOCK_WAIT_SLIP, LOCK_WAIT_TIMEOUT);
}
} // namespace Telephony
} // namespace OHOS
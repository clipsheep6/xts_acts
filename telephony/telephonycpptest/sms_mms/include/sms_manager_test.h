...
bool SmsManagerTest::getRadioStatusCallback = false;
...
public:
    ...
    static sptr<ICoreService> GetProxy_();
    static void RecoverState();
    static bool PduDataMatchCdma(OHOS::Telephony::ShortMessage SmsRecord);
    // PDU code data that meets specifications for testing --CDMA
    std::u16string CDMA_SMS_PDU =
        u"0000021002040602448D159E240601FC081800032000100106102E8CBB366F0306220025162448080100";
#endif
...
// Matching PDU Data --CDMA_SMS_PDU
bool SimSmsManagerTest::PduDataMatchCdma(OHOS::Telephony::ShortMessage smsRecord)
{
    return (smsRecord.GetVisibleMessageBody() == u"hello" && smsRecord.GetVisibleRawAddress() == u"123456789" &&
            smsRecord.GetMessageClass() == (int32_t)OHOS::Telephony::ShortMessage::SmsMessageClass::SMS_CLASS_UNKNOWN &&
            smsRecord.GetProtocolId() == 0 && smsRecord.GetScTimestamp() >= 0 &&
            smsRecord.IsReplaceMessage() == false && smsRecord.HasReplyPath() == false &&
            smsRecord.GetPdu().size() > 0 && smsRecord.GetStatus() == 0 &&
...
{
    return (smsRecord.GetVisibleMessageBody() == u"hello" && smsRecord.GetVisibleRawAddress().empty() &&
            smsRecord.GetMessageClass() == (int32_t)OHOS::Telephony::ShortMessage::SmsMessageClass::SMS_CLASS_UNKNOWN &&
            smsRecord.GetProtocolId() == 0 && smsRecord.GetScAddress().size() == ADDRESS_LENGTH &&
            smsRecord.GetScTimestamp() >= 0 && smsRecord.IsReplaceMessage() == false &&
            smsRecord.HasReplyPath() == false && smsRecord.GetPdu().size() > 0 && smsRecord.GetStatus() == 0 &&
            smsRecord.IsSmsStatusReportMessage() == false);
}
...
{
    return (smsRecord.GetVisibleMessageBody() == u"99" && smsRecord.GetVisibleRawAddress().size() == ADDRESS_LENGTH &&
            smsRecord.GetMessageClass() == (int32_t)OHOS::Telephony::ShortMessage::SmsMessageClass::SMS_CLASS_UNKNOWN &&
            smsRecord.GetProtocolId() == 0 && smsRecord.GetScAddress().size() == ADDRESS_LENGTH &&
            smsRecord.GetScTimestamp() >= 0 && smsRecord.IsReplaceMessage() == false &&
            smsRecord.HasReplyPath() == false && smsRecord.GetPdu().size() > 0 && smsRecord.GetStatus() == 0 &&

...
{
    bool result = g_smsClientPtr->GetPtr()->DelSimMessage(ERROR_VALUES, 0);
    ASSERT_FALSE(result);
}
 ...
 * @tc.desc     Function test
 */
HWTEST_F(SmsManagerTest, Telephony_SmsMms_DelSimMessage_0400, Function | MediumTest | Level3)
{
    std::vector<OHOS::Telephony::ShortMessage> beforeSmsRecord = g_smsClientPtr->GetPtr()->GetAllSimMessages(SLOT_ID_0);
    int addIndex = beforeSmsRecord.size() + 1;
    bool result = g_smsClientPtr->GetPtr()->DelSimMessage(SLOT_ID_0, addIndex);
    ASSERT_FALSE(result);
}
...
HWTEST_F(SmsManagerTest, Telephony_SmsMms_GetAllSimMessages_0100, Function | MediumTest | Level3)
{
    ...
    OHOS::Telephony::ShortMessage addMessages = addSmsRecord[addIndex];
    ...
    bool isAdd =
        (initPdu == pdu && addMessages.GetIccMessageStatus() ==
                               OHOS::Telephony::ShortMessage::SmsSimMessageStatus::SMS_SIM_MESSAGE_STATUS_SENT);
    ASSERT_TRUE(isAdd);
    bool isDelete = g_smsClientPtr->GetPtr()->DelSimMessage(SLOT_ID_0, addIndex);
    ...
    OHOS::Telephony::ShortMessage delMessages = delSmsRecord[addIndex];
    bool isDel = (StringUtils::StringToHex(delMessages.GetPdu()).empty());
    ASSERT_TRUE(isDel);
}
...
{
    std::vector<OHOS::Telephony::ShortMessage> curSmsRecord = g_smsClientPtr->GetPtr()->GetAllSimMessages(ERROR_VALUES);
...
HWTEST_F(SmsManagerTest, Telephony_SmsMms_SetSmscAddr_0200, Function | MediumTest | Level3)
{
    ...
    bool result = g_smsClientPtr->GetPtr()->SetSmscAddr(ERROR_VALUES, newSmsc);
    ASSERT_FALSE(result);
}
...
HWTEST_F(SmsManagerTest, Telephony_SmsMms_SetSmscAddr_0400, Function | MediumTest | Level3)
{
    ...
    ASSERT_TRUE(result);
    std::u16string curSmsc = g_smsClientPtr->GetPtr()->GetSmscAddr(SLOT_ID_0);
    bool isSetScAddr = (newSmsc == curSmsc);
    ASSERT_TRUE(isSetScAddr);
}
...
{
    std::u16string curSmsc = g_smsClientPtr->GetPtr()->GetSmscAddr(ERROR_VALUES);
...
HWTEST_F(SmsManagerTest, Telephony_SmsMms_SetCBConfig_0600, Function | MediumTest | Level3)
{
    ...
    uint32_t toMsgId = 0xFFFF;
    bool enable = true;
    bool result = g_smsClientPtr->GetPtr()->SetCBConfig(ERROR_VALUES, enable, fromMsgId, toMsgId, netType);
    ASSERT_FALSE(result);
}
...
{
    int curSlotId = g_smsClientPtr->GetPtr()->GetDefaultSmsSlotId();
    LOG("> ---------- Telephony_SmsMms_GetDefaultSmsSlotId_0100  curSlotId = %d", curSlotId);

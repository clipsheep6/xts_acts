...
static void PrintMenu()
{
    ...
    cout << "17.set current cell info." << endl;
    cout << "18.set radio tech." << endl;
...
int main()
{
    ...
    while (input != 0 && loop < maxLoop) {
        ...
        case 16:
            ...
            break;
        case 17:
            int type, lac, cellId, arfcn, bsic, mcc, mnc;
            printf("type, lac, cellId, arfcn, bsic, mcc, mnc\n");
            scanf("%d%d%d%d%d%d%d", &type, &lac, &cellId, &arfcn, &bsic, &mcc, &mnc);
            mockVendor.MockVendorSetCellInfo(type, lac, cellId, arfcn, bsic, mcc, mnc);
            break;
        case 18:
            int domain, tech;
            scanf("%d%d", &domain, &tech);
            mockVendor.MockVendorSetTech(domain, tech);
            break;
        default:

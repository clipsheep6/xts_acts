...
{
    std::vector<std::u16string> targetCallNumberList{
        Str8ToStr16(TARGET_PHONY_NUMBER), Str8ToStr16(OBJECT_PHONY_NUMBER)};
...
{
    std::vector<std::u16string> targetCallNumberList{Str8ToStr16(OBJECT_PHONY_NUMBER), Str8ToStr16(DEST_PHONY_NUMBER)};
    int ret = Dial(Str8ToStr16(TARGET_PHONY_NUMBER));
    ASSERT_EQ(ret, SUCCESSFUL);
 ...
 * @tc.name     testing enable EnableLteEnhanceMode, test the callback return value
 * @tc.desc     Function test
 */
 ...
 * @tc.name     testing enable DisableLteEnhanceMode, test the callback return value
 * @tc.desc     Function test
 */
...
HWTEST_F(CallManagerFunctionTest, Telephony_CallManager_IMS_GetImsConfig_0100, Function | MediumTest | Level3)
{
    ...
    LOCK_NUM_WHILE_NE(g_updateResult.GetIntValue("result"), SUCCESSFUL, SLEEP_50_MS, SLEEP_1000_MS);
    LOCK_NUM_WHILE_NE(g_updateResult.GetIntValue("value"), OPEN, SLEEP_50_MS, SLEEP_1000_MS);
}
...
HWTEST_F(CallManagerFunctionTest, Telephony_CallManager_IMS_SetImsConfig_0300, Function | MediumTest | Level3)
{
    ...
    EXPECT_EQ(
        g_clientPtr->GetPtr()->SetImsConfig(SLOT_ID, ImsConfigItem::ITEM_IMS_SWITCH_STATUS, imsValues), SUCCESSFUL);
    LOCK_NUM_WHILE_NE(g_updateReportId, static_cast<CallResultReportId>(18), SLEEP_50_MS, SLEEP_12000_MS);
 ...
 * @tc.name     testing UpdateCallMediaMode,add a active call,and call UpdateCallMediaMode input CALL_MODE_AUDIO_ONLY
 * @tc.desc     Function test
 */

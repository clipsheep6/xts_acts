 ...
 * @tc.name     Import cameraId and callingPackage, dial call test ControlCamera(),return zero,and close camera
 * @tc.desc     Function test
 */
HWTEST_F(CallManagerIMSTest, Telephony_CallManager_ControlCamera_0100, Function | MediumTest | Level3)
{
    ...
    EXPECT_EQ(ret, SUCCESSFUL);
    ret = g_clientPtr->GetPtr()->ControlCamera(Str8ToStr16(CLOSE_CAMERA), Str8ToStr16(callingPackage));
    EXPECT_EQ(ret, SUCCESSFUL);
}

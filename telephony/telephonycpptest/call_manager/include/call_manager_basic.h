...
private:
    ...
    int32_t UpdateSwitchResultRequestBasic(MessageParcel &data, MessageParcel &reply);
...
public:
    ...
    int32_t OnRttResult(CallResultReportId reportId, AppExecFwk::PacMap &resultInfo);
...
int32_t CallAbilityCallbackBasicStub::UpdateSwitchResultRequestBasic(MessageParcel &data, MessageParcel &reply)
{
    int32_t result = TELEPHONY_SUCCESS;
    AppExecFwk::PacMap resultInfo;
    CallResultReportId reportId = static_cast<CallResultReportId>(data.ReadInt32());
    ...
    result = OnVolteResult(reportId, resultInfo);
    if (!reply.WriteInt32(result)) {
        TELEPHONY_LOGE("writing parcel failed");
        return TELEPHONY_ERR_FAIL;
    }
...
CallAbilityCallbackBasicStub::CallAbilityCallbackBasicStub()
{
    ...
    memberFuncMap_[UPDATE_CALL_SWITCH_REQUEST] = &CallAbilityCallbackBasicStub::UpdateSwitchResultRequestBasic;
}

...
{
    CallTransfer.settingType = ERASURE;
    ASSERT_EQ(strcpy_s(CallTransfer.transferNum, BUFSIZ, VALID_TRANSFER_NUMBER.c_str()), SUCCESSFUL);
...
void CallManagerFunctionTest::RecoveryCallRestriction(CallRestrictionInfo CallRestriction)
{
    ...
    ASSERT_EQ(strcpy_s(CallRestriction.password, BUFSIZ, VALID_CALL_PASSWORD.c_str()), SUCCESSFUL);
...
void CallManagerReliabilityTest::TearDownTestCase()
{
    ...
    CallTransfer.settingType = ERASURE;
    CallTransfer.type = UNCONDITIONAL;

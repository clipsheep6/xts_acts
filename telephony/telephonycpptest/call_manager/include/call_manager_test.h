...
public:
    ...
    const int INVALID_TRANSFER_TYPE = 5;
    ...
    const int FIRST_WAITING_TIME = 25;
...
int CallManagerMockTest::CleanCallList()
{
    if (g_clientPtr == nullptr) {
        ...
        LockNumWHhileNe(g_clientPtr->GetPtr()->GetCallState(), static_cast<int>(CallStateToApp::CALL_STATE_IDLE),
...
void CallManagerFunctionTest::TestRestrictionCallBack(int HasRestriction)
{
    ...
    LockNumWHhileNe(g_updateResult.GetIntValue("status"), HasRestriction, SLEEP_50_MS, SLEEP_1000_MS);
}
...
void CallManagerFunctionTest::TestTransferCallBack(int HasTransfer)
{
    ...
    LockNumWHhileNe(g_updateResult.GetIntValue("result"), SUCCESSFUL, SLEEP_50_MS, SLEEP_1000_MS);
    LockNumWHhileNe(g_updateResult.GetIntValue("status"), HasTransfer, SLEEP_50_MS, SLEEP_1000_MS);
    LockStringWhiteNe(
...
void CallManagerDialTest::TestRegisterCallBack(char phoneNumber[BUFSIZ])
{
    ...
    LockNumWHhileNe(g_updateSpeaker, true, CallTestEnum::SLEEP_50_MS, CallTestEnum::SLEEP_12000_MS);
    LockNumWHhileNe(
    ...
    LockNumWHhileNe(g_updateIsEcc, false, CallTestEnum::SLEEP_50_MS, CallTestEnum::SLEEP_12000_MS);
    LockNumWHhileNe(g_updateCallType, CallType::TYPE_CS, CallTestEnum::SLEEP_50_MS, CallTestEnum::SLEEP_12000_MS);
    LockNumWHhileNe(g_updateConference, TelConferenceState::TEL_CONFERENCE_IDLE, CallTestEnum::SLEEP_50_MS,
...
int32_t CallAbilityCallbackBasic::OnReportMmiCodeResult(const MmiCodeInfo &info)
{
...
void CallManagerMockTest::SetUpTestCase()
{
    ...
    ASSERT_EQ(ret, 0);
    LockNumWHhileNe(g_updateResult.GetIntValue("result"), 0, CallTestEnum::SLEEP_50_MS, CallTestEnum::SLEEP_1000_MS);
...
void CallManagerFunctionTest::SetUpTestCase()
{
    ...
    sleep(CallTestEnum::FIRST_WAITING_TIME);
    LOG("connect server success!!!");

...
{
    int getPsRadioTech = static_cast<int>(GetNetworkStateCache()->GetPsRadioTech());
    int getCsRadioTech = static_cast<int>(GetNetworkStateCache()->GetCsRadioTech());
    auto signalVec = GetProxy()->GetSignalInfoList(SLOT_0);
    int type;
    bool result = false;
    ASSERT_FALSE(signalVec.empty());
...
/*
 * @tc.number  Telephony_NetworkSearch_GetNetworkSearchResult_0900
 * @tc.name    Set the slot id is -1 to test the GetNetworkSearchResult and the result is false
 * @tc.desc    Function test
 */
HWTEST_F(NetworkSearchTest, Telephony_NetworkSearch_GetNetworkSearchResult_0900, Function | MediumTest | Level3)
{
    int slotId = -1;
    bool networkSearchResult = GetProxy()->GetNetworkSearchResult(slotId, g_callback);
...
HWTEST_F(NetworkSearchTest, Telephony_NetworkSearch_GetIsoCountryCodeForNetwork_0100, Function | MediumTest | Level3)
{
    ...
    // Set Radio turn on or turn off need to wait 2 seconds to get the data

...
HWTEST_F(NetworkSearchTest, Telephony_NetworkSearch_GetNetworkSearchInformation_0500, Function | MediumTest | Level3)
{
    ...
    g_mockVender.MockVendorSetPlmnState(NETWORK_PLMN_STATE_AVAILABLE);
    g_mockVender.MockVendorSetRat(NETWORK_LTE);
    ...
    LOCK_NUM_WHILE_NE(networkPlmnState.at(0), NETWORK_PLMN_STATE_AVAILABLE, LOCK_WAIT_SLIP, LOCK_WAIT_TIMEOUT);
    LOCK_NUM_WHILE_NE(networkRat.at(0), NETWORK_LTE, LOCK_WAIT_SLIP, LOCK_WAIT_TIMEOUT);
}
...
HWTEST_F(NetworkSearchTest, Telephony_NetworkSearch_GetNetworkSearchInformation_0600, Function | MediumTest | Level3)
{
    ...
    g_mockVender.MockVendorSetPlmnState(NETWORK_PLMN_STATE_REGISTERED);
    g_mockVender.MockVendorSetRat(NETWORK_GSM_OR_GPRS);
    bool networkSearchResult = GetProxy()->GetNetworkSearchInformation(SLOT_0, g_callback);
    ASSERT_TRUE(networkSearchResult);
    LOCK_NUM_WHILE_EQ((int)operatorLongName.size(), NETWORK_WAIT_LOCK_SIZE, LOCK_WAIT_SLIP, LOCK_WAIT_TIMEOUT);
    ...
    LOCK_NUM_WHILE_NE(networkPlmnState.at(0), NETWORK_PLMN_STATE_REGISTERED, LOCK_WAIT_SLIP, LOCK_WAIT_TIMEOUT);
    LOCK_NUM_WHILE_NE(networkRat.at(0), NETWORK_GSM_OR_GPRS, LOCK_WAIT_SLIP, LOCK_WAIT_TIMEOUT);
}
...
HWTEST_F(NetworkSearchTest, Telephony_NetworkSearch_GetNetworkSearchInformation_0700, Function | MediumTest | Level3)
{
    ...
    g_mockVender.MockVendorSetPlmnState(NETWORK_PLMN_STATE_FORBIDDEN);
    g_mockVender.MockVendorSetRat(NETWORK_WCDMA);
    bool networkSearchResult = GetProxy()->GetNetworkSearchInformation(SLOT_0, g_callback);
    ASSERT_TRUE(networkSearchResult);
    LOCK_NUM_WHILE_EQ((int)operatorLongName.size(), NETWORK_WAIT_LOCK_SIZE, LOCK_WAIT_SLIP, LOCK_WAIT_TIMEOUT);
    ...
    LOCK_NUM_WHILE_NE(networkPlmnState.at(0), NETWORK_PLMN_STATE_FORBIDDEN, LOCK_WAIT_SLIP, LOCK_WAIT_TIMEOUT);
    LOCK_NUM_WHILE_NE(networkRat.at(0), NETWORK_WCDMA, LOCK_WAIT_SLIP, LOCK_WAIT_TIMEOUT);
}
...
{
    g_mockVender.MockVendorSetReg(DOMAIN_TYPE_PS, REG_STATE_UNKNOWN);
    g_mockVender.MockVendorSetTech(DOMAIN_TYPE_PS, RADIO_TECHNOLOGY_GSM);
    g_mockVender.MockVendorSetReg(DOMAIN_TYPE_CS, REG_STATE_UNKNOWN);
    g_mockVender.MockVendorSetTech(DOMAIN_TYPE_CS, RADIO_TECHNOLOGY_GSM);
    sleep(1);
    int regState = GetNetworkState()->GetRegStatus();
...
{
    g_mockVender.MockVendorSetReg(DOMAIN_TYPE_PS, REG_STATE_NO_SERVICE);
    g_mockVender.MockVendorSetTech(DOMAIN_TYPE_PS, RADIO_TECHNOLOGY_GSM);
    g_mockVender.MockVendorSetReg(DOMAIN_TYPE_CS, REG_STATE_NO_SERVICE);
    g_mockVender.MockVendorSetTech(DOMAIN_TYPE_CS, RADIO_TECHNOLOGY_GSM);
    sleep(1);
    int regState = GetNetworkState()->GetRegStatus();
...
{
    g_mockVender.MockVendorSetReg(DOMAIN_TYPE_PS, REG_STATE_SEARCH);
    g_mockVender.MockVendorSetTech(DOMAIN_TYPE_PS, RADIO_TECHNOLOGY_GSM);
    g_mockVender.MockVendorSetReg(DOMAIN_TYPE_CS, REG_STATE_SEARCH);
    g_mockVender.MockVendorSetTech(DOMAIN_TYPE_CS, RADIO_TECHNOLOGY_GSM);
    sleep(1);
    int regState = GetNetworkState()->GetRegStatus();
...
HWTEST_F(NetworkSearchTest, Telephony_NetworkSearch_SetPreferredNetwork_1100, Function | MediumTest | Level3)
{
    ...
    g_mockVender.MockVendorSetTech(DOMAIN_TYPE_PS, RADIO_TECHNOLOGY_LTE);
    g_mockVender.MockVendorSetTech(DOMAIN_TYPE_CS, RADIO_TECHNOLOGY_LTE);
    sleep(1);
    RadioTech getPsRadioTech = static_cast<RadioTech>(GetNetworkState()->GetPsRadioTech());
    EXPECT_EQ(getPsRadioTech, RADIO_TECHNOLOGY_LTE);
    RadioTech getCsRadioTech = static_cast<RadioTech>(GetNetworkState()->GetCsRadioTech());
    EXPECT_EQ(getCsRadioTech, RADIO_TECHNOLOGY_LTE);

    g_mockVender.MockVendorSetTech(DOMAIN_TYPE_PS, RADIO_TECHNOLOGY_WCDMA);
    g_mockVender.MockVendorSetTech(DOMAIN_TYPE_CS, RADIO_TECHNOLOGY_WCDMA);
    sleep(1);
    getPsRadioTech = static_cast<RadioTech>(GetNetworkState()->GetPsRadioTech());
    EXPECT_EQ(getPsRadioTech, RADIO_TECHNOLOGY_WCDMA);
    getCsRadioTech = static_cast<RadioTech>(GetNetworkState()->GetCsRadioTech());
    EXPECT_EQ(getCsRadioTech, RADIO_TECHNOLOGY_WCDMA);

    g_mockVender.MockVendorSetTech(DOMAIN_TYPE_PS, RADIO_TECHNOLOGY_GSM);
    g_mockVender.MockVendorSetTech(DOMAIN_TYPE_CS, RADIO_TECHNOLOGY_GSM);
    sleep(1);
    getPsRadioTech = static_cast<RadioTech>(GetNetworkState()->GetPsRadioTech());
    EXPECT_EQ(getPsRadioTech, RADIO_TECHNOLOGY_GSM);
    getCsRadioTech = static_cast<RadioTech>(GetNetworkState()->GetCsRadioTech());
    EXPECT_EQ(getCsRadioTech, RADIO_TECHNOLOGY_GSM);
}
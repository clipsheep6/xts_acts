...
public:
    ...
    void CheckGsmCellInformation(sptr<CellInformation> cellList);
    void CheckWcdmaCellInformation(sptr<CellInformation> cellList);
    void CheckLteCellInformation(sptr<CellInformation> cellList);
    void CheckCdmaCellInformation(sptr<CellInformation> cellList);
    void CheckTdscdmaCellInformation(sptr<CellInformation> cellList);
    void CheckNrCellInformation(sptr<CellInformation> cellList);
#ifdef NETWORSEARCH_MOCK_TEST
...
void NetworkSearchTest::CheckGsmCellInformation(sptr<CellInformation> cellList)
{
...
void NetworkSearchTest::CheckWcdmaCellInformation(sptr<CellInformation> cellList)
{
...
void NetworkSearchTest::CheckLteCellInformation(sptr<CellInformation> cellList)
{
...
void NetworkSearchTest::CheckCdmaCellInformation(sptr<CellInformation> cellList)
{
...
void NetworkSearchTest::CheckTdscdmaCellInformation(sptr<CellInformation> cellList)
{
...
void NetworkSearchTest::CheckNrCellInformation(sptr<CellInformation> cellList)
{
...
{
    bool ret = GetProxy()->SetNetworkSelectionMode(SLOT_ID, static_cast<int32_t>(SelectionMode::MODE_TYPE_AUTO),
    ...
    if (ret == false) {
        ...
        LockNumWHhileNe(hasNewData[SET_NETWORK_MODEL], true, LOCK_WAIT_SLIP, LOCK_WAIT_TIMEOUT);
...
{
    bool ret = GetProxy()->SetRadioState(SLOT_ID, true, g_callback);
    if (ret == false) {
        LOG("RecoverRadioState is faild");
    } else {
        LockNumWHhileNe(hasNewData[SET_RADIO_STATUS], true, LOCK_WAIT_SLIP, LOCK_WAIT_TIMEOUT);
...
void NetworkSearchTest::RecoverPreferredNetwork()
{
    bool ret = GetProxy()->SetPreferredNetwork(
        SLOT_ID, static_cast<int32_t>(PreferredNetworkMode::CORE_NETWORK_MODE_AUTO), g_callback);
    if (ret == false) {
...
{
    bool ret = GetProxy()->SetPreferredNetwork(SLOT_ID, static_cast<int32_t>(cdmaType), g_callback);
    if (ret == false) {
...
{
    return proxy_.GetHandler()->GetNetworkState(SLOT_ID);
}
    ...
    const std::string longname, const std::string shortname, const std::string numeric)
{
    int sleepTime = 2;
    int err = 0;
    (void)memset_s(g_buff, sizeof(g_buff), '\0', sizeof(g_buff));

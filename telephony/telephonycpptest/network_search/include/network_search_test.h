...
public:
    ...
    static const int WAIT_TELEPHONY_RUN = 60;
...
{
    int ret = GetProxy()->SetRadioState(true, g_callback);
    if (ret == false) {
...
{
    int ret = GetProxy()->SetPreferredNetwork(SLOT_0, CORE_NETWORK_MODE_AUTO, g_callback);
    if (ret == false) {

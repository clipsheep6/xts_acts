            ...
            function excuteCallback () {
                args[args.length - 1] = function (error, data) {
                    if (count === 0) {
                        ...
                        console.log(`${caseName} excuteCallbackPromise callback,` +
                            `count error:${toString(error)},data:${toString(data)}`);
                        count--;
                        excuteCallback();
                    }
                };
    ...
    if (typeof args[args.length - 1] === 'function') {
        ...
        for (;count > 0;count--) {
            ...
            try {
                ...
                flag && compareFunc(error, data);
            } catch (err) {
                error = err;
                flag && compareFunc(error, data);
            }
            console.log(`${caseName} promise count ${count} flag:${flag}`);
        }
    }
    let endTime = new Date().getTime();
    const cost = endTime - startTime;
    console.log(`${caseName} ${func} result flag:${toString(flag)} cost ${cost}`);

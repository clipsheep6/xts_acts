  ...
  return new Promise((resolve, reject) => {
    ...
    call.on('callDetailsChange', (err, callStateInfo) => {
      ...
      if (callStateInfo.callState === checkState) {
        ...
        console.log(`${caseName} scenceInCalling get the right state ${checkState}`);
        resolve({isHanup:checkState === CALL_STATUS_DISCONNECTED, ...callStateInfo});
        return;
      }
      if (callStateInfo.callState === CALL_STATUS_DISCONNECTED) {
        ...
        // reject(`${caseName} hungup from otherperson`)
        hangupCall(caseName, callStateInfo.callId)
          .then(data => {
            console.log(`${caseName} hangupCall success,data:${data}`);
  ...
  return new Promise(async (resolve, reject) => {
    ...
    call.on('callDetailsChange', (err, callStateInfo) => {
      ...
      if (callStateInfo.callState === CALL_STATUS_IDLE) { // this maybe the last state
        console.log(`${caseName} scenceInCalling get the state CALL_STATUS_IDLE`);
        reject(`${caseName} get the state CALL_STATUS_IDLE`);
        return;
      }
    });
...
export function hangupCall2 (caseName, done, callId) {
  ...
  if (callId) {
    call.hangup(callId)
      .then((data) => {
        console.log(`${caseName} hangup success,data:${data}`);
        done();
      })
      .catch(error => {
...
export function hangupCall (caseName, callId) {
  return new Promise((resolve, reject) => {
    call.hangup(callId)
      .then((data) => {
        console.log(`${caseName} func hangup success,${data}`);
        resolve(true);
      })
      .catch(error => {
        console.log(`${caseName} func hangup error ${error.message},error:${toString(error)}`);
        reject(false);
      });
  });
}
  ...
  return new Promise((resolve, reject) => {
    ...
    scenceInCalling({...options})
      .then(data => {
        ...
        if (data.isHanup) {
          console.log(`${caseName}  scenceInCalling data isHanup`);

...
describe('CallManageImsCall', function () {
     ...
     *             {accountId:DEFAULT_SLOT_ID,videoState:MEDIA_TYPE_VOICE,dialScene:DIAL_SCENCE_CALL_PRIVILEGED,
     *             dialType:DIAL_CARRIER_TYPE} by callback,the function return true,
     *             and the callEventChange event can get  EVENT_SEND_USSD_FAILED
     * @tc.desc    Function test
     */
    it('Telephony_CallManager_IMS_USSD_dial_Async_0600', 0, function (done) {
        ...
        let flag = false;
        reachCallEventState(EVENT_SEND_USSD_FAILED).then(data => {
            flag = flag && ((data && data.eventId) === EVENT_SEND_USSD_FAILED);
            expect(flag).assertTrue();
            console.log(`${caseName} reachCallEventState success,case ${flag ? 'success' : 'failed'},data:${
                toString(data)}`);
            done();
        }).catch(error => {
            expect().assertFail();

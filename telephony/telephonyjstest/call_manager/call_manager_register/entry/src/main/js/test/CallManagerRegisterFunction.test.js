...
describe('CallManagementRegister', function () {
    ...
    it('Telephony_Statemonitoring_on_Async_0100', 0, async function (done) {
        ...
        try {
            ...
            console.log(`${caseName} data,case ${data.state === CALL_STATE_OFFHOOK &&
              data.number === PHONE_NUMBER_LENGTH_11 ? 'success' : 'error'},data:${toString(data)}`);
            expect(data.state === CALL_STATE_OFFHOOK).assertTrue();
            expect(data.number === PHONE_NUMBER_LENGTH_11).assertTrue();
            done();
        } catch (error) {
     ...
     * @tc.name    Dial a call,when the call state is CALL_STATUS_ACTIVE,
     *             get the state by events callStateChange  by on and compare the state
     * @tc.desc    Function test
     */
     ...
     *             dial a call by number AUTO_ACCEPT_NUMBER and after  being accepted,then cat the data to
     *             judge the data.
     * @tc.desc    Function test
     */
    ...
    it('Telephony_Statemonitoring_off_Async_0100', 0, function (done) {
        ...
        let callId = null;
        let timerHanup = null;
        observer.on('callStateChange', {slotId: DEFAULT_SLOT_ID}, function (callStateInfo) {
            ...
            if (callStateInfo.state === CALL_STATE_OFFHOOK) {
                ...
                observer.on('callStateChange', {slotId: DEFAULT_SLOT_ID}, function (callStateInfo2) {
        ...
        scenceInCalling({
            ...
            .then((data) => {
                ...
                scenceInHangup({caseName:caseName, callId:data.callId, checkState:CALL_STATUS_DISCONNECTING})
                    .then(data2 => {
                        let flag = data2.accountNumber === AUTO_ACCEPT_NUMBER && data2.accountId === DEFAULT_SLOT_ID &&
              data2.videoState === VIDEO_STATE_AUDIO && data2.callState === CALL_STATUS_DISCONNECTING &&
              data2.conferenceState === TEL_CONFERENCE_IDLE;
                        console.log(`${caseName} success,case ${flag ? 'success' : 'failed'},data:${toString(data2)}`);
                        expect(data2.accountNumber === AUTO_ACCEPT_NUMBER).assertTrue();
                        expect(data2.accountId === DEFAULT_SLOT_ID).assertTrue();

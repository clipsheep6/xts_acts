...
export function reachState (caseName, checkState, tag, isHunup, timeOutLenth) {
  ...
  timeOutLenth = timeOutLenth ? timeOutLenth * TIMEOUT_LENTH : REACH_TIMES * TIMEOUT_LENTH;
...
export function scenceInCalling (options) {
  ...
  return new Promise((resolve, reject) => {
    ...
    try {
      ...
      // console.log(`${caseName} dial options:${toString(options)},arg:${toString(obj)}`);
  ...
  return new Promise((resolve, reject) => {
    ...
    try {
      call.dial(NULL_PHONE_NUMBER, (err, data) => {
        ...
        const message = `${caseName} should be CALL_MANAGER_PHONE_NUMBER_NULL,` +
...
export function scenceInUnHolding (options) {
  ...
  return new Promise(async (resolve, reject) => {
    let {caseName, callId} = options;
    if (callId) {
      reachState(caseName, TATUS_ACTIVE, 'scenceInUnHolding', false)
        .then(data => {
...
export function scenceInCallingForRegister (options) {
  ...
  return new Promise((resolve, reject) => {
    ...
    func('callStateChange', {slotId: REGISTER_SLOT_ID}, (data) => {
...
export function scenceInCallingHoldingForRegister (options) {
  ...
  return new Promise((resolve, reject) => {
    ...
    if (typeof func !== 'function') {
      reject({message:`${caseName} observer.once or observer.on is not exit,options:${toString(options)}`});
      return;
    }
    func('callStateChange', {slotId: REGISTER_SLOT_ID}, (data) => {
      console.log(`${caseName}  callStateChange,data:${toString(data)}`);

...
export function reachCallEventState (eventId, off) {
...
export function reachCallDisconnectedCause (eventId, off) {
    off = off === false ? false : true;
    return new Promise(function (resolve, reject) {
        let time = null;
        call.on('callDisconnectedCause', (err, value) => {
            console.log(`Telephony_CallManager reachCallDisconnectedCause error ${toString(err)},callStateInfo ${
                toString(value)}`);
            if (err) {
                let message = `Telephony_CallManager reachCallDisconnectedCause err message = ${toString(err)}`;
                console.log(message);
                clearTimeout(time);
                if (off) {
            ...
            if (eventId === value.disconnectedCause) {
                let message = `Telephony_CallManager reachCallDisconnectedCause get the right eventId ${
                    toString(value)}`;
                console.log(message);
                clearTimeout(time);
                if (off) {
                ...
                let message = `Telephony_CallManager reachCallDisconnectedCause get the error eventId ${
                    toString(value)}`;
                console.log(message);
                clearTimeout(time);
                if (off) {
        ...
        time = setTimeout(function () {
            let message = 'Telephony_CallManager reachCallDisconnectedCause time out';
            console.log(message);
            if (off) {
                call.off('callDisconnectedCause');
            }

...
describe('SimReliabilityTest', function () {
  ...
  const activateSim = async function () {
    try {
      ...
      console.log(`activeSim sim card fail, error:${error.message}`);
    }
  };
  ...
  it('Telephony_Sim_hasSimCard_Async_0400', 0, async function (done) {
    const recursive = function (n) {
      ...
      console.log(`Telephony_Sim_hasSimCard_Async_0400 run ${env.GENERAL_RUN_TIMES - n + 1} times`);
  ...
  it('Telephony_Sim_hasSimCard_Promise_0400', 0, async function (done) {
    for (let index = 0; index < env.GENERAL_RUN_TIMES; index++) {
      console.log(`Telephony_Sim_hasSimCard_Promise_0400 run ${index + 1} times`);
      try {
  ...
  it('Telephony_Sim_isSimActive_Async_0500', 0, async function (done) {
    const recursive = function (n) {
      ...
      console.log(`Telephony_Sim_isSimActive_Async_0500 run ${env.GENERAL_RUN_TIMES - n + 1} times`);
      sim.isSimActive(env.SLOTID0, (err, isActive) => {
        if (err || !isActive) {
          console.log(
            `Telephony_Sim_isSimActive_Async_0500  fail, err : ${err && err.message}, sim state: ${isActive}`
          );
          expect().assertFail();
          done();
          return;
  ...
  it('Telephony_Sim_isSimActive_Promise_0500', 0, async function (done) {
    for (let index = 0; index < env.GENERAL_RUN_TIMES; index++) {
      console.log(`Telephony_Sim_isSimActive_Promise_0500 run ${index + 1} times`);
      try {
        let data = await sim.isSimActive(env.SLOTID0);
        if (!data) {
          throw new Error(`isSimActive state: ${data}}`);
        }
      } catch (err) {
  ...
  it('Telephony_Sim_activateSim_Async_0400', 0, async function (done) {
    const recursive = function (n) {
      ...
      console.log(`Telephony_Sim_activateSim_Async_0400 run ${env.GENERAL_RUN_TIMES - n + 1} times`);
      sim.activateSim(env.SLOTID0, (err, value) => {
        ...
        sim.isSimActive(env.SLOTID0, (err, isActiveState) => {
          if (err) {
          ...
          if (!isActiveState) {
            expect(isActiveState).assertTrue();
            done();
          } else {
            recursive(n - 1);
          }
  ...
  it('Telephony_Sim_activateSim_Promise_0400', 0, async function (done) {
    for (let index = 0; index < env.GENERAL_RUN_TIMES; index++) {
      console.log(`Telephony_Sim_activateSim_Promise_0400 run ${index + 1} times`);
      try {
        await sim.activateSim(env.SLOTID0);
        try {
  ...
  it('Telephony_Sim_deactivateSim_Async_0400', 0, async function (done) {
    const recursive = async function (n) {
      if (n <= 0) {
        ...
        await activateSim(env.SLOTID0);
        done();
        return;
      }
      console.log(`Telephony_Sim_deactivateSim_Async_0400 run ${env.GENERAL_RUN_TIMES - n + 1} times`);
      sim.deactivateSim(env.SLOTID0, async (err) => {
        if (err) {
        ...
        sim.isSimActive(env.SLOTID0, async (err, isActiveState) => {
          if (err) {
          ...
          if (!isActiveState) {
            expect(isActiveState).assertFalse();
            await activateSim(env.SLOTID0);
            done();
          } else {
            recursive(n - 1);
  ...
  it('Telephony_Sim_deactivateSim_Promise_0400', 0, async function (done) {
    for (let index = 0; index < env.GENERAL_RUN_TIMES; index++) {
      console.log(`Telephony_Sim_deactivateSim_Promise_0400 run ${index + 1} times`);
      try {
        await sim.deactivateSim(env.SLOTID0);
        let ret = await sim.isSimActive(env.SLOTID0);
        if (ret) {
          throw new Error(`after deactivateSim, sim state: ${ret}`);
        }
      } catch (err) {
  ...
  it('Telephony_Sim_getActiveSimAccountInfoList_Async_0400', 0, async function (done) {
    const recursive = async function (n) {
      ...
      console.log(`Telephony_Sim_getActiveSimAccountInfoList_Async_0400 run ${env.GENERAL_RUN_TIMES - n + 1} times`);
      sim.activateSim(env.SLOTID0, async (err) => {
        if (err) {
        ...
        sim.getActiveSimAccountInfoList((error, info) => {
          if (error) {
            console.log(`Telephony_Sim_getActiveSimAccountInfoList_Async_0400 error: ${error.message}`);
            expect().assertFail();
            done();
            return;
          }
          let isExist = info.some(item => item.slotIndex === env.SLOTID0
            && item.simId === env.SLOTID0
            && item.isEsim === false
            && item.isActive === true
            && item.iccId === result
            && item.showName === env.CARD_NAME
            && item.showNumber === env.CARD_NUMBER);
          if (isExist) {
            recursive(n - 1);
          } else {
            console.log(
              `Telephony_Sim_getActiveSimAccountInfoList_Async_0400 result: ${JSON.stringify(info)}`
            );
  ...
  it('Telephony_Sim_getActiveSimAccountInfoList_Promise_0400', 0, async function (done) {
    for (let index = 0; index < env.GENERAL_RUN_TIMES; index++) {
      console.log(`Telephony_Sim_getActiveSimAccountInfoList_Promise_0400 run ${index + 1} times`);
      try {
        await sim.activateSim(env.SLOTID0);
      ...
      try {
        ...
        let isExist = info.some(item => item.slotIndex === env.SLOTID0
          ...
          && item.showNumber === env.CARD_NUMBER);
        if (!isExist) {
          console.log(`Telephony_Sim_getActiveSimAccountInfoList_Promise_0400 info data: ${JSON.stringify(info)}`);
          expect(isExist).assertTrue();
          break;
        }
      } catch (err) {
  ...
  it('Telephony_Sim_setShowName_Async_0500', 0, async function (done) {
    const recursive = function (n) {
      ...
      console.log(`Telephony_Sim_setShowName_Async_0500 run ${env.GENERAL_RUN_TIMES - n + 1} times`);
      sim.setShowName(env.SLOTID0, env.CARD_NAME, (err) => {
        if (err) {
        ...
        sim.getShowName(env.SLOTID0, (err, value) => {
          if (err || value !== env.CARD_NAME) {
            console.log(
              `Telephony_Sim_setShowName_Async_0500 getShowName , err : ${err && err.message}, name: ${value}`
            );
            expect().assertFail();
            done();
            return;
  ...
  it('Telephony_Sim_setShowName_Promise_0500', 0, async function (done) {
    for (let index = 0; index < env.GENERAL_RUN_TIMES; index++) {
      console.log(`Telephony_Sim_setShowName_Promise_0500 run ${index + 1} times`);
      try {
        await sim.setShowName(env.SLOTID0, env.CARD_NAME);
        try {
  ...
  it('Telephony_Sim_getShowName_Async_0500', 0, async function (done) {
    const recursive = function (n) {
      ...
      console.log(`Telephony_Sim_getShowName_Async_0500 run ${env.GENERAL_RUN_TIMES - n + 1} times`);
  ...
  it('Telephony_Sim_getShowName_Promise_0500', 0, async function (done) {
    for (let index = 0; index < env.GENERAL_RUN_TIMES; index++) {
      console.log(`Telephony_Sim_getShowName_Promise_0500 run ${index + 1} times`);
      try {
        await sim.setShowName(env.SLOTID0, env.CARD_NAME);
        let value;
        try {
  ...
  it('Telephony_Sim_setShowNumber_Async_0500', 0, async function (done) {
    const recursive = function (n) {
      ...
      console.log(`Telephony_Sim_setShowNumber_Async_0500 run ${env.GENERAL_RUN_TIMES - n + 1} times`);
  ...
  it('Telephony_Sim_setShowNumber_Promise_0500', 0, async function (done) {
    for (let index = 0; index < env.GENERAL_RUN_TIMES; index++) {
      console.log(`Telephony_Sim_setShowNumber_Promise_0500 run ${index + 1} times`);
      try {
        await sim.setShowNumber(env.SLOTID0, env.CARD_NUMBER);
        try {
  ...
  it('Telephony_Sim_getShowNumber_Async_0400', 0, async function (done) {
    sim.setShowNumber(env.SLOTID0, env.CARD_NAME, (err, data) => {
      ...
      const recursive = function (n) {
        ...
        console.log(`Telephony_Sim_getShowNumber_Async_0400 run ${env.GENERAL_RUN_TIMES - n + 1} times`);
        sim.getShowNumber(env.SLOTID0, (err, value) => {
          if (err) {
  ...
  it('Telephony_Sim_getShowNumber_Promise_0400', 0, async function (done) {
    ...
    for (let index = 0; index < env.GENERAL_RUN_TIMES; index++) {
      console.log(`Telephony_Sim_getShowNumber_Promise_0400 run ${index + 1} times`);
      try {
        let value = await sim.getShowNumber(env.SLOTID0);
        expect(env.CARD_NAME === value).assertTrue();
      } catch (err) {
  ...
  it('Telephony_Sim_getOperatorConfigs_Async_0400', 0, async function (done) {
    const recursive = function (n) {
      ...
      console.log(`Telephony_Sim_getOperatorConfigs_Async_0400 run ${env.GENERAL_RUN_TIMES - n + 1} times`);
  ...
  it('Telephony_Sim_getOperatorConfigs_Promise_0400', 0, async function (done) {
    for (let index = 0; index < env.GENERAL_RUN_TIMES; index++) {
      console.log(`Telephony_Sim_getOperatorConfigs_Promise_0400 run ${index + 1} times`);
      try {
        let data = await sim.getOperatorConfigs(env.SLOTID0);
        expect(data.length !== 0).assertTrue();
      } catch (err) {
        console.log(`Telephony_Sim_getOperatorConfigs_Promise_0400 fail, error: ${err.message}`);
        done();
        return;
      }
    }
  ...
  it('Telephony_Sim_getSimTelephoneNumber_Async_0400', 0, async function (done) {
    const recursive = function (n) {
      ...
      console.log(`Telephony_Sim_getSimTelephoneNumber_Async_0400 run ${env.GENERAL_RUN_TIMES - n + 1} times`);
  ...
  it('Telephony_Sim_getSimTelephoneNumber_Promise_0400', 0, async function (done) {
    for (let index = 0; index < env.GENERAL_RUN_TIMES; index++) {
      console.log(`Telephony_Sim_getSimTelephoneNumber_Promise_0400 run ${index + 1} times`);
      try {
        let data = await sim.getSimTelephoneNumber(env.SLOTID0);
        expect(data.length !== 0 && data.length >= 11).assertTrue();
      } catch (err) {
  ...
  it('Telephony_Sim_getVoiceMailIdentifier_Async_0400', 0, async function (done) {
    const recursive = function (n) {
      ...
      console.log(`Telephony_Sim_getVoiceMailIdentifier_Async_0400 run ${env.GENERAL_RUN_TIMES - n + 1} times`);
      sim.setVoiceMailInfo(env.SLOTID0, env.MAIL_NAME_LEN12, env.MAIL_NUMBER_LEN20, err => {
        if (err) {
          console.log(`Telephony_Sim_getVoiceMailIdentifier_Async_0400 setVoiceMailInfo err: ${err.message}`);
          done();
          return;
        }
        sim.getVoiceMailIdentifier(env.SLOTID0, (err, data) => {
          if (err || data !== env.MAIL_NAME_LEN12) {
            console.log(
              `Telephony_Sim_getVoiceMailIdentifier_Async_0400 test err : ${err && err.message}, identifier: ${data}`
            );
            expect().assertFail();
            done();
            return;
  ...
  it('Telephony_Sim_getVoiceMailIdentifier_Promise_0400', 0, async function (done) {
    try {
      for (let i = 0; i < env.GENERAL_RUN_TIMES; i++) {
        console.log(`Telephony_Sim_getVoiceMailIdentifier_Promise_0400 run ${i + 1} times`);
        try {
          await sim.setVoiceMailInfo(env.SLOTID0, env.MAIL_NAME_LEN12, env.MAIL_NUMBER_LEN20);
        } catch (error) {
          console.log(`Telephony_Sim_getVoiceMailIdentifier_Promise_0400 setVoiceMailInfo error: ${error.message}`);
          done();
          return;
        }
        const indentifier = await sim.getVoiceMailIdentifier(env.SLOTID0);
        if (indentifier !== env.MAIL_NAME_LEN12) {
          console.log(`Telephony_Sim_getVoiceMailIdentifier_Promise_0400 test fail, indentifier: ${indentifier}`);
      ...
      console.log(`Telephony_Sim_getVoiceMailIdentifier_Promise_0400 error: ${error.message}`);
      expect().assertFail();
    }
    done();
  });
  ...
  it('Telephony_Sim_getVoiceMailNumber_Async_0400', 0, async function (done) {
    const recursive = function (n) {
      ...
      console.log(`Telephony_Sim_getVoiceMailNumber_Async_0400 run ${env.GENERAL_RUN_TIMES - n + 1} times`);
      sim.setVoiceMailInfo(env.SLOTID0, env.MAIL_NAME_LEN12, env.MAIL_NUMBER_LEN20, err => {
        if (err) {
          console.log(`Telephony_Sim_getVoiceMailNumber_Async_0400 setVoiceMailInfo err: ${err.message}`);
          done();
          return;
        }
        sim.getVoiceMailNumber(env.SLOTID0, (err, data) => {
          if (err || data !== env.MAIL_NUMBER_LEN20) {
            console.log(
              `Telephony_Sim_getVoiceMailNumber_Async_0400 test err : ${err && err.message}, identifier: ${data}`
            );
            expect().assertFail();
            done();
            return;
  ...
  it('Telephony_Sim_getVoiceMailNumber_Promise_0400', 0, async function (done) {
    try {
      for (let i = 0; i < env.GENERAL_RUN_TIMES; i++) {
        console.log(`Telephony_Sim_getVoiceMailNumber_Promise_0400 run ${i + 1} times`);
        try {
          await sim.setVoiceMailInfo(env.SLOTID0, env.MAIL_NAME_LEN12, env.MAIL_NUMBER_LEN20);
        } catch (error) {
          console.log(`Telephony_Sim_getVoiceMailNumber_Promise_0400 setVoiceMailInfo error: ${error.message}`);
          done();
          return;
        }
        const number = await sim.getVoiceMailNumber(env.SLOTID0);
        if (number !== env.MAIL_NUMBER_LEN20) {
          console.log(`Telephony_Sim_getVoiceMailNumber_Promise_0400 test fail, number: ${number}`);
      ...
      console.log(`Telephony_Sim_getVoiceMailNumber_Promise_0400 error: ${error.message}`);
      expect().assertFail();
    }
    done();
  });
  ...
  it('Telephony_Sim_setVoiceMailInfo_Async_0900', 0, function (done) {
    const recursive = function (n) {
      ...
      console.log(`Telephony_Sim_setVoiceMailInfo_Async_0900 run ${env.GENERAL_RUN_TIMES - n + 1} times`);
        ...
        sim.getVoiceMailNumber(env.SLOTID0, (error, value) => {
          ...
          sim.getVoiceMailIdentifier(env.SLOTID0, (error, value) => {
            if (error || value !== env.MAIL_NAME_LEN12) {
              console.log(`Telephony_Sim_setVoiceMailInfo_Async_0900 error: ${error.message}, identifier: ${value}`);
              expect().assertFail();
              done();
              return;
            }
  ...
  it('Telephony_Sim_setVoiceMailInfo_Promise_0900', 0, async function (done) {
    try {
      for (let i = 0; i < env.GENERAL_RUN_TIMES; i++) {
        console.log(`Telephony_Sim_getShowNumber_Promise_0400 run ${i + 1} times`);
        try {
          await sim.setVoiceMailInfo(env.SLOTID0, env.MAIL_NAME_LEN12, env.MAIL_NUMBER_LEN20);
        } catch (error) {
          console.log(`Telephony_Sim_setVoiceMailInfo_Promise_0900 setVoiceMailInfo error: ${error.message}`);
          done();
          return;
        }

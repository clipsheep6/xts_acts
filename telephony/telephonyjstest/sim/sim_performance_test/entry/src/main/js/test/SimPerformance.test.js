...
describe('SimPerformance', function () {
  ...
  it('Telephony_Sim_setDefaultVoiceSlotId_Async_0200', 0, async function (done) {
    ...
    sim.getDefaultVoiceSlotId((err, result) => {
      if (err) {
        console.log(`Telephony_Sim_setDefaultVoiceSlotId_Async_0200 getDefaultVoiceSLOT_ID_fail, err: ${err.message}`);
        expect().assertFail();
        done();
        return;
      }
    ...
    function recursive(n) {
      if (n <= 0) {
        ...
        sim.setDefaultVoiceSlotId(defaultValue, (err, ret) => {
          if (err) {
            console.log(`Telephony_Sim_setDefaultVoiceSlotId_Async_0200 setDefaultVoiceSLOT_ID_err: ${err.message}`);
            expect().assertFail();
            done();
            return;
          }

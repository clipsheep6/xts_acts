  ...
  it('Telephony_Sim_getSimAccountInfo_Async_0100', 0, async function (done) {
    ...
    sim.getSimAccountInfo(env.SLOTID0, (err, info) => {
      ...
      sim.activateSim(env.SLOTID0, (err) => {
        if (err) {
          console.log(`${CASE_NAME} activateSim fail, err : ${err.message}`);
          expect().assertFail();
          done();
          return;
        }

...
describe('SimGetSimAccountInfoFunctionTest', function () {
  ...
  const setShowNameWriteToPromise = function () {
    return new Promise((resolve, reject) => {
      ...
      sim.setShowName(env.DEFAULT_SLOTID, env.SIM_SLOT_NAME, (err) => {
        if (err) {
  ...
  const setShowNumberWriteToPromise = function () {
    return new Promise((resolve, reject) => {
      ...
      sim.setShowNumber(env.DEFAULT_SLOTID, env.SIM_SLOT_NUMBER, (err) => {
        if (err) {
  ...
  /**
    ...
    * @tc.name    Test getSimAccountInfo async callback interface slotid exception,
    *             enter parameter 3, and check the callback value
    * @tc.desc    Function test
    */
  it('Telephony_Sim_getSimAccountInfo_Async_0500', 0, async function (done) {
    ...
    sim.getSimAccountInfo(env.SLOTID3, (err) => {
      if (err) {
        console.log(`${CASE_NAME} expect error: ${err.message}`);
        console.log(`${CASE_NAME} finish`);
        done();
  ...
  it('Telephony_Sim_getSimAccountInfo_Promise_0100', 0, async function (done) {
    ...
    try {
      await sim.setShowName(env.DEFAULT_SLOTID, env.SIM_SLOT_NAME);
      await sim.setShowNumber(env.DEFAULT_SLOTID, env.SIM_SLOT_NUMBER);
      var iccIdValue = await sim.getSimIccId(env.DEFAULT_SLOTID);
      await sim.activateSim(env.DEFAULT_SLOTID);
    } catch (err) {
      console.log(`${CASE_NAME} setOrGet fail, err : ${err.message}`);
      expect().assertFail();
    ...
    try {
      let info = await sim.getSimAccountInfo(env.DEFAULT_SLOTID);
  ...
  it('Telephony_Sim_getSimAccountInfo_Promise_0500', 0, async function (done) {
    ...
    try {
      await sim.getSimAccountInfo(env.SLOTID3);
      expect().assertFail();
      console.log(`${CASE_NAME} fail`);
    } catch (err) {
      console.log(`${CASE_NAME} expect error: ${err.message}`);

...
describe("ActsCellularDataPreferredCMCCTest", function () {
    const SLOT_0 = 0;
     ...
     * @tc.name     When calling under the GSM network, call getCellularDataFlowType() to get the data flow type and
     *              verify that the callback value is the sleep type
        ...
        cellular.enableCellularData(async function (err) {
            ...
            radio.setPreferredNetwork(SLOT_0, radio.PREFERRED_NETWORK_MODE_GSM, async function (err) {
                ...
                call.dial(CMCC_NUMBER, async function (err) {
                    ...
                    cellular.getCellularDataFlowType(async (err, data) => {
                        if (err) {
                            expect().assertFail();
                        ...
                        expect(data === cellular.DATA_FLOW_TYPE_DORMANT).assertTrue();

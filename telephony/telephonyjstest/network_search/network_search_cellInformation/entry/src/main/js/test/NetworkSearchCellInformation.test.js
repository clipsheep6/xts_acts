...
describe('NetworkSearchCellInformationTest', function () {
    ...
    const TIME_RADIO_TURNON = 6000;
    function turnOnRadio() {
        radio.isRadioOn().then(isRadioOn => {
            if (!isRadioOn) {
                radio.turnOnRadio(err => {
                    if (err) {
                        console.log(`Telephony_NetworkSearch_CellInformation turnOnRadio err: ${err}`)
                        expect().assertFail();
                    }
                });
            }
        }).catch(err => {
            console.log(`Telephony_NetworkSearch_CellInformation isRadioOn err:${err}`)
            expect().assertFail();
        })
    }

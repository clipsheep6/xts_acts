...
describe('ActsNetworkSearchStubRadioTechTest', function () {
    const SLOT_0 = 0;
    ...
    const CELL_NR_PCI_MAX = 1007;
    const CELL_NR_NRARFCN_MAX = 3279165;
    const CELL_NR_TAC_MAX = 65535;
    const CELL_NR_NCI_MAX = 68719476735;
    ...
    function assertCellInformation(data) {
        ...
        for (let i = 0; i < data.length; i++) {
            ...
            if (data[0].networkType === radio.NETWORK_TYPE_LTE) {
                ...
                expect(data[i].data.latitude >= CELL_CDMA_LATITUDE_MIN && data[i].data.latitude <=
                    CELL_CDMA_LATITUDE_MAX).assertTrue();
                expect(data[i].data.baseId >= CELL_INFO_MIN && data[i].data.baseId <= CELL_CDMA_BASEID_MAX).assertTrue()
                expect(data[i].data.longitude >= CELL_CDMA_LONGITUDE_MIN && data[i].data.longitude <=
                    CELL_CDMA_LONGITUDE_MAX).assertTrue();
                ...
                expect(data[i].data.lac >= CELL_INFO_MIN && data[i].data.lac <= CELL_LAC_TAC_MAX).assertTrue();
                expect(data[i].data.cellId >= CELL_INFO_MIN && data[i].data.cellId <= CELL_CELLID_MAX).assertTrue();
                expect((data[i].data.uarfcn >= CELL_TDSCDMA_UARFCN1_MIN && data[i].data.uarfcn <=
                    CELL_TDSCDMA_UARFCN1_MAX) || (data[i].data.uarfcn >= CELL_TDSCDMA_UARFCN2_MIN && data[i].data.uarfcn <= CELL_TDSCDMA_UARFCN2_MAX) || (data[i].data.uarfcn >= CELL_TDSCDMA_UARFCN3_MIN && data[i].data.uarfcn <= CELL_TDSCDMA_UARFCN3_MAX)).assertTrue();
                ...
                expect(data[i].data.pci >= CELL_INFO_MIN && data[i].data.pci <= CELL_NR_PCI_MAX).assertTrue();
                expect(data[i].data.nrarfcn >= CELL_INFO_MIN &&
                    data[i].data.nrarfcn <= CELL_NR_NRARFCN_MAX).assertTrue();
                expect(data[i].data.mcc).assertEqual(CELL_MCC);
                expect(data[i].data.mnc >= CELL_INFO_MIN && data[i].data.mnc <= CELL_MNC_MAX).assertTrue();
                expect(data[i].data.tac >= CELL_INFO_MIN && data[i].data.tac <= CELL_NR_TAC_MAX).assertTrue();
                expect(data[i].data.nci >= CELL_INFO_MIN && data[i].data.nci <= CELL_NR_NCI_MAX).assertTrue();
            }else {
                expect().assertFail();
            }
        }
    ...
    /**
     * @tc.number  Telephony_NetworkSearch_getRadioTech_Async_1100
     * @tc.name    Test getRadioTech(0) to check the callback result when PreferredNetwork
     *             is PREFERRED_NETWORK_MODE_LTE_EVDO_CDMA
     * @tc.desc    Function test
     */
    it('Telephony_NetworkSearch_getRadioTech_Async_1100', 0, async function (done) {
        radio.setPreferredNetwork(SLOT_0, radio.PREFERRED_NETWORK_MODE_LTE_EVDO_CDMA, (err) => {
            if (err) {
                console.log(`Telephony_NetworkSearch_getRadioTech_Async_1100 set fail err: ${err}`);
                expect().assertFail();
                done();
                return;
            }
            radio.getRadioTech(SLOT_0, (err, data) => {
                if (err) {
                    console.log(`Telephony_NetworkSearch_getRadioTech_Async_1100 get fail err: ${err}`);
                    expect().assertFail();
                    done();
                    return;
                }
                console.log(`Telephony_NetworkSearch_getRadioTech_Async_1100 end data: ${JSON.stringify(data)}`);
                expect(data.csRadioTech).assertEqual(radio.RADIO_TECHNOLOGY_LTE_CA);
                expect(data.psRadioTech).assertEqual(radio.RADIO_TECHNOLOGY_LTE_CA);
                done();
            });
        })
    })
    ...
    /**
     * @tc.number  Telephony_NetworkSearch_getRadioTech_Async_1200
     * @tc.name    Test getRadioTech(0) to check the callback result when PreferredNetwork
     *             is PREFERRED_NETWORK_MODE_NR
     * @tc.desc    Function test
     */
    it('Telephony_NetworkSearch_getRadioTech_Async_1200', 0, async function (done) {
        radio.setPreferredNetwork(SLOT_0, radio.PREFERRED_NETWORK_MODE_NR, (err) => {
            if (err) {
                console.log(`Telephony_NetworkSearch_getRadioTech_Async_1200 set fail err: ${err}`);
                expect().assertFail();
                done();
                return;
            }
            radio.getRadioTech(SLOT_0, (err, data) => {
                if (err) {
                    console.log(`Telephony_NetworkSearch_getRadioTech_Async_1200 get fail err: ${err}`);
                    expect().assertFail();
                    done();
                    return;
                }
                console.log(`Telephony_NetworkSearch_getRadioTech_Async_1200 end data: ${JSON.stringify(data)}`);
                expect(data.csRadioTech).assertEqual(radio.RADIO_TECHNOLOGY_NR);
                expect(data.psRadioTech).assertEqual(radio.RADIO_TECHNOLOGY_NR);
                done();
            });
        })
    })
    ...
    /**
     * @tc.number  Telephony_NetworkSearch_getRadioTech_Promise_1100
     * @tc.name    Test getRadioTech(0) to check the callback result when PreferredNetwork
     *             is PREFERRED_NETWORK_MODE_LTE_EVDO_CDMA
     * @tc.desc    Function test
     */
    it('Telephony_NetworkSearch_getRadioTech_Promise_1100', 0, async function (done) {
        radio.setPreferredNetwork(SLOT_0, radio.PREFERRED_NETWORK_MODE_LTE_EVDO_CDMA).then(() => {
            radio.getRadioTech(SLOT_0).then(data => {
                console.log(`Telephony_NetworkSearch_getRadioTech_Promise_1100 end data: ${JSON.stringify(data)}`);
                expect(data.csRadioTech).assertEqual(radio.RADIO_TECHNOLOGY_LTE_CA);
                expect(data.psRadioTech).assertEqual(radio.RADIO_TECHNOLOGY_LTE_CA);
                done();
            }).catch(err => {
                console.log(`Telephony_NetworkSearch_getRadioTech_Promise_1100 fail: ${err}`);
                expect().assertFail();
                done();
            })
        }).catch(err => {
            console.log(`Telephony_NetworkSearch_getRadioTech_Promise_1100 set fail err: ${err}`);
            expect().assertFail();
            done();
        })
    });
    ...
    /**
     * @tc.number  Telephony_NetworkSearch_getRadioTech_Promise_1200
     * @tc.name    Test getRadioTech(0) to check the callback result when PreferredNetwork
     *             is PREFERRED_NETWORK_MODE_NR
     * @tc.desc    Function test
     */
    it('Telephony_NetworkSearch_getRadioTech_Promise_1200', 0, async function (done) {
        radio.setPreferredNetwork(SLOT_0, radio.PREFERRED_NETWORK_MODE_NR).then(() => {
            radio.getRadioTech(SLOT_0).then(data => {
                console.log(`Telephony_NetworkSearch_getRadioTech_Promise_1200 end data: ${JSON.stringify(data)}`);
                expect(data.csRadioTech).assertEqual(radio.RADIO_TECHNOLOGY_NR);
                expect(data.psRadioTech).assertEqual(radio.RADIO_TECHNOLOGY_NR);
                done();
            }).catch(err => {
                console.log(`Telephony_NetworkSearch_getRadioTech_Promise_1200 fail: ${err}`);
                expect().assertFail();
                done();
            })
        }).catch(err => {
            console.log(`Telephony_NetworkSearch_getRadioTech_Promise_1200 set fail err: ${err}`);
            expect().assertFail();
            done();
        })
    });
    ...
    it('Telephony_NetworkSearch_getNetworkState_Async_0100', 0, async function (done) {
        radio.getNetworkState((err, data) => {
            if (err) {
                console.log(`Telephony_NetworkSearch_getNetworkState_Async_0100 get fail err: ${err}`);
                expect().assertFail();
                done();
                return;
            }
            console.log(`Telephony_NetworkSearch_getNetworkState_Async_0100 end data: ${JSON.stringify(data)}`);
            ...
            expect(data.regStatus).assertEqual(radio.REG_STATE_IN_SERVICE);
     ...
     * @tc.name    Test getNetworkState(0) to check the callback result
     * @tc.desc    Function test
     */
    it('Telephony_NetworkSearch_getNetworkState_Async_0200', 0, async function (done) {
        radio.getNetworkState(SLOT_0, (err, data) => {
            if (err) {
                console.log(`Telephony_NetworkSearch_getNetworkState_Async_0200 get fail err: ${err}`);
                expect().assertFail();
                done();
                return;
            }
            console.log(`Telephony_NetworkSearch_getNetworkState_Async_0200 end data: ${JSON.stringify(data)}`);
            expect(data.longOperatorName.length).assertLarger(0);
            expect(data.shortOperatorName.length).assertLarger(0);
            expect(data.plmnNumeric.length).assertLarger(0);
            expect(data.regStatus).assertEqual(radio.REG_STATE_IN_SERVICE);
    ...
    it('Telephony_NetworkSearch_getNetworkState_Promise_0100', 0, async function (done) {
        radio.getNetworkState().then(data => {
            console.log(`Telephony_NetworkSearch_getNetworkState_Promise_0100 end data: ${JSON.stringify(data)}`);
            expect(data.longOperatorName.length).assertLarger(0);
            expect(data.shortOperatorName.length).assertLarger(0);
            expect(data.plmnNumeric.length).assertLarger(0);
            expect(data.regStatus).assertEqual(radio.REG_STATE_IN_SERVICE);
            ...
            console.log(`Telephony_NetworkSearch_getNetworkState_Promise_0100 fail: ${err}`);
            expect().assertFail();
            done();
        })
    })
    ...
    it('Telephony_NetworkSearch_getNetworkState_Promise_0200', 0, async function (done) {
        radio.getNetworkState(SLOT_0).then(data => {
            console.log(`Telephony_NetworkSearch_getNetworkState_Promise_0200 end data: ${JSON.stringify(data)}`);
            expect(data.longOperatorName.length).assertLarger(0);
            expect(data.shortOperatorName.length).assertLarger(0);
            expect(data.plmnNumeric.length).assertLarger(0);
            expect(data.regStatus).assertEqual(radio.REG_STATE_IN_SERVICE);
            ...
            console.log(`Telephony_NetworkSearch_getNetworkState_Promise_0200 fail: ${err}`);
            expect().assertFail();
            done();
        })
    })
    ...
    /**
     * @tc.number  Telephony_NetworkSearch_getSignalInformation_Async_0700
     * @tc.name    Test getSignalInformation(0) to check the callback result when PreferredNetwork
     *             is PREFERRED_NETWORK_MODE_NR
     * @tc.desc    Function test
     */
     it('Telephony_NetworkSearch_getSignalInformation_Async_0700', 0, async function (done) {
        radio.setPreferredNetwork(SLOT_0, radio.PREFERRED_NETWORK_MODE_NR, (err) => {
            if (err) {
                console.log(`Telephony_NetworkSearch_getSignalInformation_Async_0700 set fail err: ${err}`);
                expect().assertFail();
                done();
                return;
            }
            radio.getSignalInformation(SLOT_0, (err, data) => {
                if (err) {
                    console.log(`Telephony_NetworkSearch_getSignalInformation_Async_0700 fail err: ${err}`);
                    expect().assertFail();
                    done();
                    return;
                }
                console.log(`Telephony_NetworkSearch_getSignalInformation_Async_0700 end data: ${JSON.stringify(data)}`)
                expect(data.length).assertLarger(0);
                expect(data[0].signalType).assertEqual(radio.NETWORK_TYPE_NR);
                expect(NETWORK_SIGNAL).assertContain(data[0].signalLevel);
                done();
            });
        })
    ...
    /**
     * @tc.number  Telephony_NetworkSearch_getSignalInformation_Promise_0700
     * @tc.name    Test getSignalInformation(0) to check the callback result when PreferredNetwork
     *             is PREFERRED_NETWORK_MODE_NR
     * @tc.desc    Function test
     */
     it('Telephony_NetworkSearch_getSignalInformation_Promise_0700', 0, async function (done) {
        radio.setPreferredNetwork(SLOT_0, radio.PREFERRED_NETWORK_MODE_NR).then(() => {
            radio.getSignalInformation(SLOT_0).then(data => {
                console.log(`Telephony_NetworkSearch_getSignalInformation_Promise_0700 data: ${JSON.stringify(data)}`);
                expect(data.length).assertLarger(0);
                expect(data[0].signalType).assertEqual(radio.NETWORK_TYPE_NR);
                expect(NETWORK_SIGNAL).assertContain(data[0].signalLevel);
                done();
            }).catch(err => {
                console.log(`Telephony_NetworkSearch_getSignalInformation_Promise_0700 fail: ${err}`);
                expect().assertFail();
                done();
            })
        }).catch(err => {
            console.log(`Telephony_NetworkSearch_getSignalInformation_Promise_0700 set fail err: ${err}`);
            expect().assertFail();
            done();
        })
    });
    ...
    /**
     * @tc.number  Telephony_NetworkSearch_getCellInformation_Async_1200
     * @tc.name    Test getCellInformation(0) to check the callback result when PreferredNetwork
     *             is PREFERRED_NETWORK_MODE_NR
     * @tc.desc    Function test
     */
     it('Telephony_NetworkSearch_getCellInformation_Async_1200', 0, async function (done) {
        radio.setPreferredNetwork(SLOT_0, radio.PREFERRED_NETWORK_MODE_NR, (err) => {
            if (err) {
                console.log(`Telephony_NetworkSearch_getCellInformation_Async_1200 set fail err: ${err}`);
                expect().assertFail();
                done();
                return;
            }
            radio.getCellInformation(SLOT_0, (err, data) => {
                if (err) {
                    console.log(`Telephony_NetworkSearch_getCellInformation_Async_1200 fail: ${err}`);
                    expect().assertFail();
                    done();
                    return;
                }
                console.log(`Telephony_NetworkSearch_getCellInformation_Async_1200 end data: ${JSON.stringify(data)}`);
                expect(data[0].networkType).assertEqual(radio.NETWORK_TYPE_NR);
                assertCellInformation(data);
                done();
            });
        })
    ...
    /**
     * @tc.number  Telephony_NetworkSearch_getCellInformation_Promise_1200
     * @tc.name    Test getCellInformation(0) to check the callback result when PreferredNetwork
     *             is PREFERRED_NETWORK_MODE_NR
     * @tc.desc    Function test
     */
     it('Telephony_NetworkSearch_getCellInformation_Promise_1200', 0, async function (done) {
        radio.setPreferredNetwork(SLOT_0, radio.PREFERRED_NETWORK_MODE_NR).then(() => {
            radio.getCellInformation(SLOT_0).then(data => {
                console.log(`Telephony_NetworkSearch_getCellInformation_Promise_1200 end data: ${JSON.stringify(data)}`)
                expect(data[0].networkType).assertEqual(radio.NETWORK_TYPE_NR);
                assertCellInformation(data);
                done();
            }).catch(err => {
                console.log(`Telephony_NetworkSearch_getCellInformation_Promise_1200 fail: ${err}`);
                expect().assertFail();
                done();
            })
        }).catch(err => {
            console.log(`Telephony_NetworkSearch_getCellInformation_Promise_1200 set fail err: ${err}`);
            expect().assertFail();
            done();
        })
    });

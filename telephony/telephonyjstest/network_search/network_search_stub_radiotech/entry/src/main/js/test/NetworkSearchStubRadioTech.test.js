...
describe('NetworkSearchStubRadioTechTest', function () {
    const SLOT_0 = 0;
    const MOKE_CDMA = 200;
    const MOKE_TD_SCDMA = 207;
    const MOKE_EVDO = 203;
    const MOKE_EHRPD = 206;
    const MOKE_LIMIT = 205;
    ...
    function assertCellInformation(data) {
        ...
        for (let i = 0; i < data.length; i++) {
            ...
            if (data[0].networkType === radio.NETWORK_TYPE_LTE) {
                ...
                expect(data[i].data.latitude >= CELL_CDMA_LATITUDE_MIN && data[i].data.latitude <= CELL_CDMA_LATITUDE_MAX).assertTrue();
                expect(data[i].data.baseId >= CELL_INFO_MIN && data[i].data.baseId <= CELL_CDMA_BASEID_MAX).assertTrue()
                expect(data[i].data.longitude >= CELL_CDMA_LONGITUDE_MIN && data[i].data.longitude <= CELL_CDMA_LONGITUDE_MAX).assertTrue();
                ...
                expect(data[i].data.lac >= CELL_INFO_MIN && data[i].data.lac <= CELL_LAC_TAC_MAX).assertTrue();
                expect(data[i].data.cellId >= CELL_INFO_MIN && data[i].data.cellId <= CELL_CELLID_MAX).assertTrue();
                expect((data[i].data.uarfcn >= CELL_TDSCDMA_UARFCN1_MIN && data[i].data.uarfcn <= CELL_TDSCDMA_UARFCN1_MAX) || (data[i].data.uarfcn >= CELL_TDSCDMA_UARFCN2_MIN && data[i].data.uarfcn <= CELL_TDSCDMA_UARFCN2_MAX) || (data[i].data.uarfcn >= CELL_TDSCDMA_UARFCN3_MIN && data[i].data.uarfcn <= CELL_TDSCDMA_UARFCN3_MAX)).assertTrue();
    ...
    it('Telephony_NetworkSearch_getRadioTech_Async_0600', 0, async function (done) {
        await radio.setPreferredNetwork(SLOT_0, MOKE_TD_SCDMA);
        await sleep(WAIT_TIME);
        radio.getRadioTech(SLOT_0, (err, data) => {
            if (err) {
    ...
    it('Telephony_NetworkSearch_getRadioTech_Async_0700', 0, async function (done) {
        await radio.setPreferredNetwork(SLOT_0, MOKE_EVDO);
        await sleep(WAIT_TIME);
        radio.getRadioTech(SLOT_0, (err, data) => {
            if (err) {
    ...
    it('Telephony_NetworkSearch_getRadioTech_Async_0800', 0, async function (done) {
        await radio.setPreferredNetwork(SLOT_0, MOKE_EHRPD);
        await sleep(WAIT_TIME);
        radio.getRadioTech(SLOT_0, (err, data) => {
            if (err) {
    ...
    it('Telephony_NetworkSearch_getNetworkState_Async_0800', 0, async function (done) {
        await radio.setPreferredNetwork(SLOT_0, MOKE_LIMIT);
        await sleep(WAIT_TIME);
        radio.getNetworkState(SLOT_0, (err, data) => {
            ...
            expect(data.regStatus).assertEqual(radio.REG_STATE_EMERGENCY_CALL_ONLY);
    ...
    it('Telephony_NetworkSearch_getSignalInformation_Promise_0500', 0, async function (done) {
        ...
        await sleep(WAIT_TIME);
        radio.getSignalInformation(SLOT_0).then(data => {
            console.log(`Telephony_NetworkSearch_getSignalInformation_Promise_0500 end data: ${JSON.stringify(data)}`);
            expect(data.length).assertLarger(0);
            expect(data[0].signalType).assertEqual(radio.NETWORK_TYPE_CDMA);
            expect(data[0].signalLevel).assertEqual(5);
            done();
    ...
    it('Telephony_NetworkSearch_getSignalInformation_Promise_0600', 0, async function (done) {
        ...
        await sleep(WAIT_TIME);
        radio.getSignalInformation(SLOT_0).then(data => {
            console.log(`Telephony_NetworkSearch_getSignalInformation_Promise_0600 end data: ${JSON.stringify(data)}`);
            expect(data.length).assertLarger(0);
            expect(data[0].signalType).assertEqual(radio.NETWORK_TYPE_TDSCDMA);
            expect(data[0].signalLevel).assertEqual(5);
            done();

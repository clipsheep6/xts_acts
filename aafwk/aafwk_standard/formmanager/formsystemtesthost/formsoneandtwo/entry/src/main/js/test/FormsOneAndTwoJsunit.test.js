...
describe('ActsGetAllFormsInfoTest', function () {
    ...
    it('FMS_getFormsInfo_0600_callback', 0, async function (done) {
        ...
        var ret = formManager.getAllFormsInfo(
            (result, data) => {
                ...
                console.log("FMS_getFormsInfo_0600_callback getAllFormsInfo async::sucess, data json:"
                    + JSON.stringify(data));
                ...
                for(var i = 0; i < dataA.length; i++) {
                    expect(dataA[i].description).assertEqual("form_description");
                    expect(dataA[i].type).assertEqual(1);
                    expect(dataA[i].colorMode).assertEqual(-1);
                    expect(dataA[i].jsComponentName).assertEqual("card");
                    expect(dataA[i].isDefault).assertEqual(1);
                    expect(dataA[i].updateEnabled).assertEqual(1);
                    expect(dataA[i].updateDuration).assertEqual(1);
                    expect(dataA[i].scheduledUpdateTime).assertEqual("10:30");
                    expect(dataA[i].formVisibleNotify).assertEqual(1);
                    expect(dataA[i].defaultDimension).assertEqual(1);
                    expect(dataA[i].supportDimensions[0]).assertEqual(1);
                    expect(dataA[i].supportDimensions[1]).assertEqual(2);
                    expect(dataA[i].metaData.customizeData[0].name).assertEqual("originWidgetName");
                    expect(dataA[i].metaData.customizeData[0].value).assertEqual("myTest");
                }
    ...
    it('FMS_getFormsInfo_0600_promise', 0, async function (done) {
        ...
        promise.then((data) => {
            ...
            console.log("FMS_getFormsInfo_0600_promise async::sucess, data json:" + JSON.stringify(data));
            var dataA = data.filter((p) => {
                return p.bundleName == "com.form.formsystemtestservicea";
            });
            var dataB = data.filter((p) => {

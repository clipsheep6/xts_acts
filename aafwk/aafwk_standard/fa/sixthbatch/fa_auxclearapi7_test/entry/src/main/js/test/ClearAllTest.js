    ...
    it("SUB_AA_OpenHarmony_Clearall", 0, async function (done) {
        ...
        setTimeout(() => {
            missionManager.getMissionInfos("", 15, (err, data) => {
                console.log('SUB_AA_OpenHarmony_Clearall getMissionInfos errCode : ' + JSON.stringify(err) + " data: " + JSON.stringify(data));
                for (var i = 0;i < data.length; i++) {
                    console.log("SUB_AA_OpenHarmony_Clearall bundleName222222：" + data[i].want["abilityName"] + "data[i]['lockedState']::::" + data[i]['lockedState']);

...
describe('clearTests1800', function () {
    ...
    it("SUB_AA_OpenHarmony_Clear_1800", 0, async function (done) {
        ...
        setTimeout(() => {
            missionManager.getMissionInfos("", 10, (err, data) => {
                for (var i = 0;i < data.length; i++) {
                        console.log('SUB_AA_OpenHarmony_Clear_1800 data[i].want["abilityName"]2222' + data[i].want["abilityName"] + '::' + data[i]["missionId"]);
        ...
        setTimeout(() => {
            console.log("SUB_AA_OpenHarmony_Clear_1800 lockMission1 missionIds[1]+++++123456::::" + mainAbility2missionId)
            for (var i = 0;i < mainAbility2missionId.length; i++) {
                missionManager.lockMission(parseInt(mainAbility2missionId[i]), (err, data) => {
                    console.log('SUB_AA_OpenHarmony_Clear_1800 lockMission1 errCode : ' + JSON.stringify(err) + " data: " + JSON.stringify(data));
                });
            };
        }, 2000);
        ...
        setTimeout(() => {
            missionManager.getMissionInfos("", 10, (err, data) => {
                console.log('SUB_AA_OpenHarmony_Clear_1800 getMissionInfo33 errCode : ' + JSON.stringify(err) + " data: " + JSON.stringify(data));
                for (var i = 0;i < data.length; i++) {
                    console.log("SUB_AA_OpenHarmony_Clear_1800 bundleName22222233333：" + data[i].want["abilityName"]);
                    console.log("SUB_AA_OpenHarmony_Clear_1800 data[i]['lockedState']2222223333::::" + data[i]['lockedState']);
                }
            });
        }, 2500)

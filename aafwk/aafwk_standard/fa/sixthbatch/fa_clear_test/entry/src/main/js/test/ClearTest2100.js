...
describe('clearTests2100', function () {
    ...
    it("SUB_AA_OpenHarmony_Clear_2100", 0, async function (done) {
        ...
        setTimeout(() => {
            console.log("SUB_AA_OpenHarmony_Clear_2100 lockMission1 missionIds[1]+++++123456::::"
            + mainAbility2missionId)
            for (var i = 0;i < mainAbility2missionId.length; i++) {
                missionManager.lockMission(parseInt(mainAbility2missionId[i]), (err, data) => {
                    console.log('SUB_AA_OpenHarmony_Clear_2100 lockMission1 errCode : ' + JSON.stringify(err)
        ...
        setTimeout(() => {
            missionManager.getMissionInfos("", 10, (err, data) => {
                console.log('SUB_AA_OpenHarmony_Clear_2100 getMissionInfo33 errCode : ' + JSON.stringify(err)
                + " data: " + JSON.stringify(data));
                for (var i = 0;i < data.length; i++) {

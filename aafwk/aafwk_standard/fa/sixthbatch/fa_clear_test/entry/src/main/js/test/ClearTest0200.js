...
function fns() {
    ...
    setTimeout(() => {
        missionManager.getMissionInfos("", 20, (err, data) => {
            ...
            console.log("SUB_AA_OpenHarmony_Clear_0200 bundleName：" + data[i].want["abilityName"] + ":::id " + data[i].missionId)
    ...
    setTimeout(() => {
        for (var i = 0;i < missionIds.length; i++) {
            missionManager.lockMission(parseInt(missionIds[i]), (err, data) => {
                console.log('SUB_AA_OpenHarmony_Clear_0200 lockMission1 errCode : ' + JSON.stringify(err) + " data: " + JSON.stringify(data));
            });
        }
        ;
    }, 2000);

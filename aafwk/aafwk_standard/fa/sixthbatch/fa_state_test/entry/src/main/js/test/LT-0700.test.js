...
describe("appManagerTest", function () {
    ...
    it("SUB_AA_OpenHarmony_StateListener_0700", 0, async function (done) {
        ...
        appManager.killProcessWithAccount('com.example.fzstatelistener', 100).then((data) => {
            console.log('SUB_AA_OpenHarmony_StateListener_1000 successful. data: ' + JSON.stringify(data));
        }).catch((error) => {
            console.error('SUB_AA_OpenHarmony_StateListener_1000 failed. error: ' + JSON.stringify(error));
            expect().assertFail();
        })
    })
    function sleep(time) {

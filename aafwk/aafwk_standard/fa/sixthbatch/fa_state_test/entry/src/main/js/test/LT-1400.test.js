...
var AbilityStateChanged1_1400 = []
var ForegroundApplication1_1400 = []
var testData_1400
var assistData_1400
var assistData1_1400
var assistData2_1400
var onProcessCreated1_1400 = []
describe("appManagerTest", function () {
    ...
    it("SUB_AA_OpenHarmony_StateListener_1400", 0, async function (done) {
        ...
        featureAbility.startAbility({
            ...
            console.log(TAG + '==error start=>: ' + JSON.stringify(error) + " ==>data===>>: " + JSON.stringify(data));
        ...
        setTimeout(() => {
            console.log(TAG + '==ForegroundApplication1==>>>' + JSON.stringify(ForegroundApplication1_1400))
            console.log(TAG + '==AbilityStateChanged1==>>>' + JSON.stringify(AbilityStateChanged1_1400))
            console.log(TAG + '==onProcessCreated1==>>>' + JSON.stringify(onProcessCreated1_1400))
            checkEx(ForegroundApplication1_1400, AbilityStateChanged1_1400, onProcessCreated1_1400)
            done();
        }, 3900)
        ...
        console.log(TAG + '======end=======>')
        chenK(AbilityStateChanged1_1400)
    })
    ...
    function checkEx(ForegroundApplication1, AbilityStateChanged1, onProcessCreated1) {
        for (let i = 0;i < ForegroundApplication1.length; i++) {
            ...
            if (ForegroundApplication1[i].bundleName == 'com.example.fzstatelistener') {
                assistData_1400 = ForegroundApplication1[i]
                console.log(TAG + '==>>>Data==> :' + JSON.stringify(assistData_1400))
            }
        }
        checkExt(assistData_1400)
        for (let i = 0;i < AbilityStateChanged1.length; i++) {
            ...
            if (AbilityStateChanged1[i].state == '0') {
                assistData1_1400 = AbilityStateChanged1[i]
                console.log(TAG + '==check ass==> :' + JSON.stringify(assistData1_1400))
            }
            if (AbilityStateChanged1[i].state == '2' && AbilityStateChanged1[i].abilityType == '2') {
                assistData2_1400 = AbilityStateChanged1[i]
                console.log(TAG + '==check ass2==> :' + JSON.stringify(assistData2_1400))
            }
        }
        cheExt1(assistData1_1400)
        cheExt2(assistData2_1400)
        for (let i = 0;i < onProcessCreated1.length; i++) {
            ...
            if (onProcessCreated1[i].bundleName == 'com.example.fzstatelistener') {
                testData_1400 = onProcessCreated1[i]
                console.log(TAG + '====> :' + JSON.stringify(testData_1400))
            }
        }
        chekT(testData_1400)
    }

    ...
    beforeAll(async function(){
        ...
        abilitymanager.queryRecentAbilityMissionInfos(maxnum, flag, data => {
            ...
            for (var i = 0; i < data.length; i++) {
                if(data[i].baseAbility.bundleName != 'com.example.actsamstestfirstscene'&& data[i].topAbility.bundleName != 'com.example.actsamstestfirstscene.MainAbility'){
 ...
 */
    it('Acts_Ams_test_0300', 0, async function (done) {
        ...
        for (var i = 0; i < data.length; i++) {
            ...
            //expect(data[i].missionDescription.label.length).assertEqual(0);
            expect(typeof (data[i].missionDescription.iconPath)).assertEqual("string");
            //expect(data[i].missionDescription.iconPath.length).assertEqual(0);
        }
        done();
    })
    /*

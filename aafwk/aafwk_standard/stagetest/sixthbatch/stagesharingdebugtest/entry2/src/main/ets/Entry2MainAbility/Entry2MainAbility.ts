...
export default class Entry2MainAbility extends Ability {
  ...
  onCreate(want, launchParam) {
    ...
    console.info(JSON.stringify(want),'wantttttt')
    commonEvent.publish("entry2MainAbility_onCreate", () => {
      console.log("MainAbility Publish CallBack entry2MainAbility_onCreate")
    });
    if(want.action == "action.system.home2" && want.parameters.key == "123"){
      commonEvent.publish("entry2MainAbility_parameter",() => {
        console.log("MainAbility Publish CallBack entry2MainAbility_parameter");
      })
    }
  }

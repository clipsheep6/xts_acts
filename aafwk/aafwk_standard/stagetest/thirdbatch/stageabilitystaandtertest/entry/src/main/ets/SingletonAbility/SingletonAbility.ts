...
export default class SingletonAbility extends Ability {
    ...
    onCreate(want, launchParam) {
        ...
        this.context.startAbility({
            abilityName: "com.example.entry2.Entry2SingleMainAbility",
            bundleName: "com.example.etsmyapplication",
        }, (error, data) => {
            console.log("Entry2SingleMainAbility success, err: " +
            JSON.stringify(error) + ",data: " + JSON.stringify(data));
            commonEvent.publish("Entry2SingleMainAbility_Event", () => {
                console.log("Entry2SingleMainAbility_Event publish success!")
            });
        });
    }
    ...
    onWindowStageCreate(windowStage) {
        ...
        setTimeout(()=>{
            this.context.terminateSelf().then((data) => {
                console.log("SingletonAbilityEventTest terminateSelf windowStage data: " +
                JSON.stringify(data))
            }).catch((error) => {
                console.log("SingletonAbilityEventTest terminateSelf windowStage error: " +
    ...
    onForeground() {
        ...
        setTimeout(()=>{
            this.context.terminateSelf().then((data) => {
                console.log("SingletonAbilityEventTest terminateSelf onForeground data: " +
                JSON.stringify(data))
            }).catch((error) => {
                console.log("SingletonAbilityEventTest terminateSelf onForeground error: " +
                JSON.stringify(error))
            })
        }, 2000)
    }

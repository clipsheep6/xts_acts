...
describe("appManagerTest", function () {
     ...
     * @tc.name: Find Application Validation for all receptions
     * @tc.desc: Function test
     * @tc.level   0
     */
    it("SUB_AA_OpenHarmony_StateListener_0300", 0, async function (done) {
        await  appManager.getForegroundApplications((error, data) => {
            console.log("---appM- starting---");
            for (var i = 0; i < data.length; i++) {
                if (data[i].bundleName == "com.example.sum_statelistener") {
                    appBefore = data[i]
                }
            }
        ...
        await  appManager.getForegroundApplications().then((data) => {
            for (var i = 0; i < data.length; i++) {
                if (data[i].bundleName == "com.example.fzstatelistener") {
                    appAfter = data[i]
                }
            }
    ...
    function checkAppAfter(app) {
        ...
        expect(app.bundleName).assertEqual('com.example.fzstatelistener');
    }

    function checkAppBefore(app) {
        expect(app.state == 2).assertTrue();
        expect(app.uid > 0).assertTrue();
        expect(app.bundleName).assertEqual('com.example.sum_statelistener');
    }

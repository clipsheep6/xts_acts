...
export class KeyboardController {
  ...
  public onDestroy(): void {
    ...
    globalThis.textInputClient.getTextIndexAtCursor().then((index: number) => {
  ...
  private initWindow(): void {
    display.getDefaultDisplay().then(dis => {
      ...
      let config: windowManager.Configuration = {
  ...
  private Sub_Misc_inputMethod_Panel_offShow_0190(): void {
    ...
    try {
      if (this.softKeyboardPanel !== null) {
        this.softKeyboardPanel.on('show', async () => {
          console.info(TAG + '====>receive Sub_Misc_inputMethod_Panel_offShow_0190 offShow if success');
          this.softKeyboardPanel.off('show');
          count += 1;
  ...
  private Sub_Misc_inputMethod_Panel_offHide_0200(): void {
    ...
    try {
      if (this.softKeyboardPanel !== null) {
        this.softKeyboardPanel.on('hide', async () => {
          console.info(TAG + '====>receive Sub_Misc_inputMethod_Panel_offHide_0200 offHide if success');
          this.softKeyboardPanel.off('hide');
          count += 1;

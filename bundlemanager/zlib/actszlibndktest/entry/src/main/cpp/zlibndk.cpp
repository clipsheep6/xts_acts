...
static napi_value Crc32(napi_env env, napi_callback_info info) {
    ...
    NAPI_ASSERT(env, err != Z_ERRNO, "crc32 error");
...
static napi_value Crc32_z(napi_env env, napi_callback_info info) {
    ...
    NAPI_ASSERT(env, err != Z_ERRNO, "crc32_z error");
...
static napi_value Crc32Combine(napi_env env, napi_callback_info info) {
    ...
    NAPI_ASSERT(env, err != Z_ERRNO, "crc32_combine error");
...
static napi_value Adler32(napi_env env, napi_callback_info info) {
    ...
    NAPI_ASSERT(env, err != Z_ERRNO, "adler32 error");
...
static napi_value Adler32_z(napi_env env, napi_callback_info info) {
    ...
    NAPI_ASSERT(env, err != Z_ERRNO, "adler32_z error");
...
static napi_value Adler32Combine(napi_env env, napi_callback_info info) {
    ...
    NAPI_ASSERT(env, err != Z_ERRNO, "adler32_combine error");
...
static napi_value Deflate(napi_env env, napi_callback_info info) {
    ...
    NAPI_ASSERT(env, comp != NULL, "comp is NULL");
...
static napi_value DeflateFullFlush(napi_env env, napi_callback_info info) {
    ...
    comp[3]++; /* force an error in first compressed block */
...
static napi_value Inflate(napi_env env, napi_callback_info info) {
    ...
    NAPI_ASSERT(env, zone != NULL, "zone is NULL");
    zone->first = nullptr;
    zone->total = 0;
    zone->highwater = 0;
    zone->limit = 0;
    ...
    NAPI_ASSERT(env, err == Z_OK, "inflate error");
    err = inflatePrime(&stream, 5, 31);
    NAPI_ASSERT(env, err == Z_OK, "inflate error");
    err = inflatePrime(&stream, -1, 0);
    NAPI_ASSERT(env, err == Z_OK, "inflate error");
    err = inflateSetDictionary(&stream, nullptr, 0);
    NAPI_ASSERT(env, err == Z_STREAM_ERROR, "test inflateSetDictionary nullptr failed");
    err = inflateEnd(&stream);
    NAPI_ASSERT(env, err == Z_OK, "inflate error");
    stream.avail_in = 0;
    stream.next_in = nullptr;
    err = inflateInit_(&stream, ZLIB_VERSION - 1, static_cast<int>(sizeof(z_stream)));
    NAPI_ASSERT(env, err == Z_VERSION_ERROR, "test inflateInit_ version error failed");

    err = inflateInit2_(&stream, windowBits, ZLIB_VERSION - 1, static_cast<int>(sizeof(z_stream)));
    NAPI_ASSERT(env, err == Z_VERSION_ERROR, "test inflateInit2_ version error failed");
    free(zone);

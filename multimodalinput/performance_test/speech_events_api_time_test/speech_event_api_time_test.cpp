...
namespace {
class SpeechEventApiTimeTest : public testing::Test {
public:
    static void SetUpTestCase(void) {}
    static void TearDownTestCase(void) {}
};

HWTEST_F(SpeechEventApiTimeTest, Test_GetAction, TestSize.Level1)
{
    bool isTimeOk = false;
...
HWTEST_F(SpeechEventApiTimeTest, Test_GetScene, TestSize.Level1)
{
    bool isTimeOk = false;
    OHOS::Statistics items;
    SpeechEvent speechEventTest;
...
HWTEST_F(SpeechEventApiTimeTest, Test_GetActionProperty, TestSize.Level1)
{
    bool isTimeOk = false;
    OHOS::Statistics items;    
    SpeechEvent speechEventTest;
    items.time_start = items.GetCurTime(); 
    for (auto i = 0; i < CALLAPINUM; i++) {
        speechEventTest.GetActionProperty();
    }
    items.time_end = items.GetCurTime();
    auto retCallApiTime = items.GetCallApiTime(items.time_start, items.time_end);
    if (retCallApiTime < CALLAPITIME) {
...
HWTEST_F(SpeechEventApiTimeTest, Test_GetMatchMode, TestSize.Level1)
{
    bool isTimeOk = false;
    OHOS::Statistics items;    
    SpeechEvent speechEventTest;

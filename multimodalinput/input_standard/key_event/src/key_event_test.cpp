...
public:
    ...
    void TearDown(void);
protected:
	void KeyClickMonitorCallBack(StCheckFuncList funcList, std::shared_ptr<OHOS::MMI::KeyEvent> keyEvent);
	void WaitEnd();
...
protected:
	...
	std::list<std::function<int32_t (std::shared_ptr<OHOS::MMI::KeyEvent>)>> funcList_;
	std::vector<bool> results_;
};

void KeyEventTest::SetUp(void)
{
	monitorId_ = InputManager::GetInstance()->AddMonitor(std::bind(KeyEventTest::KeyCombinationMonitorCallBack, this, funcList, std::placeholders::_1));
...
void KeyEventTest::KeyCombinationMonitorCallBack(StCheckFuncList funcList, std::shared_ptr<OHOS::MMI::KeyEvent> keyEvent)
{
	const int currentCallFuncIndex = results_.size();
	ASSERT_LT(currentCallFuncIndex, funcList.size());

	auto callback = funcList[currentCallFuncIndex];
	if (callback != nullptr) {
		callback(keyEvent);
	}

	// 检查是否全部处理完成
	if (results_.size() == funcList.size()) {
		sem_post(&semId_);
		return;
	}
}
...
HWTEST_F(KeyEventTest, MultiModalInputTest_KeyEvent_005, TestSize.Level1)
{
	...
	funcList_.push_back(std::bind(KeyEventTest::HandleInjectResult1, this, std::placeholders::_1));
	funcList_.push_back(std::bind(KeyEventTest::HandleInjectResult2, this, std::placeholders::_1));
	funcList_.push_back(std::bind(KeyEventTest::HandleInjectResult1, this, std::placeholders::_1));

	ExecInjection("keyboard_KEY_CTRL_LEFT_C.json"); //crtl + c

...
{
    static const char *test_charsets[] = {
        "iso-8859-1",
        "iso-8859-2",
        "iso-8859-4",
        "iso-8859-5",
    ...
    for (i = 0; test_charsets[i]; i++) {
        ...
        if (there == reinterpret_cast<iconv_t>(-1)) {
            continue;
        }
        iconv_t andback = iconv_open(test_charsets[i], ("UTF-8"));
        if (andback == reinterpret_cast<iconv_t>(-1)) {
            iconv_close(there);
            continue;
        }
        char u8buf[1024];
        ...
        int r1 = static_cast<size_t>(iconv(there, &tmp1, &tmp2, &tmp3, &u8rem));
        size_t u8len = sizeof u8buf - u8rem;
        tmp1 = (char*) {u8buf};
        tmp2 = (size_t) {u8len};
        tmp3 = (char*) {buf};
        size_t tmp4 = (size_t) {sizeof buf};
        int r2 = static_cast<int>(iconv(andback, &tmp1, &tmp2, &tmp3, &tmp4));
        ...
        EXPECT_FALSE(memcmp(all_codepoints, buf, sizeof buf)) <<
            "round trip corrupted " << test_charsets[i] << " characters" << endl;

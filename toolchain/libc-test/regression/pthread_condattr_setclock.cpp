...
HWTEST_F(PthreadCondattrSetclockSuite, PthreadCondattrSetclockTest, Function | MediumTest | Level2)
{
    ...
    EXPECT_EQ(r, EINVAL) << "pthread_condattr_setclock CLOCK_PROCESS_CPUTIME_ID should fail with EINVAL, got" << strerror(r) << endl;
                         
    r = pthread_condattr_setclock(&a, CLOCK_THREAD_CPUTIME_ID);
    EXPECT_EQ(r, EINVAL) << "pthread_condattr_setclock CLOCK_THREAD_CPUTIME_ID should fail with EINVAL, got " << strerror(r) << endl;

    T(r, pthread_condattr_getclock(&a, &clk));
    EXPECT_EQ(clk, CLOCK_REALTIME) << "condattr default clock is " << (int)clk << ", wanted CLOCK_REALTIME (" << (int)CLOCK_REALTIME << (int)CLOCK_REALTIME << endl;

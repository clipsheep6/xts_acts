...
HWTEST_F(Env, EnvTest, Function | MediumTest | Level2)
{
    ...
    EXPECT_FALSE(putenv((char *)"TEST=1")) << "putenv: "
                                           << "strerror(errno)" << endl;
    EXPECT_FALSE(strcmp(environ[0], "TEST=1") != 0)
        << "putenv failed: environ[0]: " << environ[0] << ", wanted \"TEST=1\"" << endl;
    ...
    EXPECT_EQ(0, strcmp(s = getenv("TEST"), "2")) << "getenv(\"TEST\"): \""
                                                  << "\", wanted \"2\"" << endl;
    EXPECT_EQ(0, strcmp(environ[0], "TEST=2")) <<
        "setenv failed: environ[0]: " << environ[0] << ", wanted \"TEST=2\"" << endl;
    errno = 0;

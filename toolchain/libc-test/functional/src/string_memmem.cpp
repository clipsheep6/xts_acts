...
/**  
* @brief  register a test suit named "StringMemmemSuite"  
* @param  test subsystem name is libc 
* @param  example module name is  Functional
* @param  test suit name is  StringMemmemSuite
*/
...
/**
 * @tc.name      : StringMemmemSuite001
 * @tc.desc      : Test StringMemmemSuite001 3 items whth async flush
 * @tc.level     : Level 1
 */
#define N(s, tail, sub) { \
	char *p = s tail; \
	char *q = memmem(p, strlen((const char *)s), sub, strlen((const char *)sub)); \
	if (q) \
		t_error("memmem("#s" "#tail", %d, "#sub", %d) returned str+%d, wanted 0\n",\
			strlen((const char *)s), strlen((const char *)sub), q-p); \
}

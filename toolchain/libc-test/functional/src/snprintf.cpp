...
/**  
* @brief  register a test suit named "SnprintfSuite"  
* @param  test subsystem name is libc 
* @param  example module name is  Functional
* @param  test suit name is  SnprintfSuite
*/
...
/**
 * @tc.name      : SnprintfSuite001
 * @tc.desc      : Test SnprintfSuite001 3 items whth async flush
 * @tc.level     : Level 1
 */
...
HWTEST_F(SnprintfSuite, SnprintfTest, Function | MediumTest | Level2)
{
	...
	errno = 0;
	TESTT(i, snprintf(NULL, 0, "%.*u", 2147483647, 0), 2147483647, "cannot print max length %d");
	TESTT(i, snprintf(NULL, 0, "%.*u ", 2147483647, 0), -1, "integer overflow %d");
	...
	for (j=0; fp_tests[j].fmt; j++) {
		...
		EXPECT_EQ(0 , strcmp(b, fp_tests[j].expect)) << "bad floating-point conversion: got \"" 
			<< b << "\", want \"" << fp_tests[j].expect << "\"\"" << endl;

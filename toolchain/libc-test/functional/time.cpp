...
static int tm_cmp(struct tm tm1, struct tm tm2)
{
...
static char *tm_str(struct tm tm)
{
    static int i, n = 4;
    static char b[4][64];
    i = (i + 1) % n;
...
static void sec2tm(time_t t, string m)
{
    struct tm *tm;
    time_t r;
...
static void tm2sec(struct tm *tm, int big, string m)
{
    ...
    EXPECT_FALSE(!overflow && t == -1) << m <<
        ": mktime(" << tm_str(*tm) << ") expected success, got (time_t)-1" << endl;
    EXPECT_FALSE(!overflow && errno) << m <<
        ": mktime(" << tm_str(*tm) << ") expected no error, got (" << strerror(errno) << ")" << endl;
    r = gmtime(&t);
...
HWTEST_F(Time, TimeTest, Function | MediumTest | Level2)
{
    ...
    tm2sec(&tmmy, 0, string("gmtime(0)"));
    TM(7, 14, 3, 19, 0, 138, 2, 18, 0);
    tm2sec(&tmmy, 0, string("2038-1s"));
    TM(8, 14, 3, 19, 0, 138, 2, 18, 0);
    tm2sec(&tmmy, 1, string("2038"));

    sec2tm(0, string("EPOCH"));
    for (t = 1; t < 1000; t++) {
        sec2tm(t * 100003, string("EPOCH+eps"));
    }
}

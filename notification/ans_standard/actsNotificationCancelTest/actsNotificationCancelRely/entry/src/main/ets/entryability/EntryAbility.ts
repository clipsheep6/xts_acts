...
export default class EntryAbility extends UIAbility {
  ...
  onForeground() {
    ...
    console.info('ActsNotificationCancelRely is onForeground');
    let representativeBundle: notificationManager.BundleOption = {
      bundle: "com.example.actsnotificationcancelasbundletest"
    };
    let errorCode;
    try {
      notificationManager.cancel(representativeBundle, 202).then(() => {
    ...
    setTimeout(() => {
        console.info("Start terminateSelfWithResult");
        console.info('ErrorCode is  :' + errorCode);

...
static void* CommTcpClientTask(void *param)
{
    ...
    *ret = 0;
    ...
    for (int i = 0; i < 4; i++) {
        sleep(sleepSecond);
        mem_ret = memset_s(dataBuf, sizeof(dataBuf), 0, sizeof(dataBuf));
        EXPECT_EQ(0, mem_ret);
...
static void* SampleTcpServerTask(void *p)
{
    ...
    mem_ret = memset_s(buf, BUF_SIZE, 0, BUF_SIZE);
    EXPECT_EQ(0, mem_ret);
    ...
    mem_ret = memset_s(bufrec, sizeof(bufrec), 0, BUF_SIZE);
    EXPECT_EQ(0, mem_ret);
    mem_ret = memset_s(&msg, sizeof(msg), 0, sizeof(msg));
    EXPECT_EQ(0, mem_ret);
    msg.msg_name = &clnAddr;
    msg.msg_namelen = sizeof(clnAddr);
    msg.msg_iov = iov;
...
HWTEST_F(ActsNetTest, testIoctlIfconf, Function | MediumTest | Level2)
{
    ...
    struct ifreq ifr[5];
    struct ifconf ifc = {0};
    mem_ret = memset_s(&ifc, sizeof(struct ifconf), 0, sizeof(struct ifconf));
    EXPECT_EQ(0, mem_ret);
...
HWTEST_F(ActsNetTest, testIoctlIfnetAddrOthers, Function | MediumTest | Level2)
{
    ...
    for (int i = 0; i < ifrCount; i++) {
        ...
        if (strcmp("lo", ifr[i].ifr_name) != 0) {
            // get inet addr
            mem_ret = memset_s(&ifrTmp, sizeof(struct ifreq), 0, sizeof(struct ifreq));
            EXPECT_EQ(0, mem_ret);
...
HWTEST_F(ActsNetTest, testSocketOpt, Function | MediumTest | Level2)
{
    ...
    mem_ret = memset_s(&timeout, len, 0, len);
    EXPECT_EQ(0, mem_ret);
    ret = getsockopt(fd, SOL_SOCKET, SO_RCVTIMEO, &timeout, &len);
    EXPECT_EQ(0, ret);

...
describe('UsbCoreJsTest_A', function () {

  beforeAll(function () {
    console.log('*************Usb Unit UsbCoreJsTest_A Begin*************');
    var Version = usb.getVersion()
    console.info('begin test getversion :' + Version)
    // version > 17  host currentMode = 2 device currentMode = 1
    var usbPortList = usb.getPorts()
    if (usbPortList.length > 0) {
      usb.setPortRoles(usbPortList[0].id, 2, 2)
    }
  })
  afterEach(function () {
  ...
  afterAll(function () {
    console.log('*************Usb Unit UsbCoreJsTest_A End*************');
  })
...
gDeviceList = usb.getDevices();
    ...
    for (var i = 0; i < gDeviceList.length; i++) {
      usb.requestRight(gDeviceList[i].name);
    }
expect(true).assertTrue();

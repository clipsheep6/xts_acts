...
static napi_value ffrt_loop_normal_0003(napi_env env, napi_callback_info info)
{
    ...
    auto loop = ffrt_loop_create(queue_handle);
    int result1 = 0;
    std::function<void()>&& basicFunc1 = [&result1]() { int testNumber = 10; result1 += testNumber; };
    ffrt_task_handle_t task1 = ffrt_queue_submit_h(queue_handle,
        create_function_wrapper(basicFunc1, ffrt_function_kind_queue), nullptr);
    pthread_t thread;
    pthread_create(&thread, 0, ThreadFunc, loop);
    ...
    int testNumber4 = 2;
    if (staticReslult != testNumber4) {
        result += 1;
    }
    int result2 = 0;
    std::function<void()>&& basicFunc2 = [&result2]() { int testNumber2 = 20; result2 += testNumber2; };
    ffrt_task_handle_t task2 = ffrt_queue_submit_h(queue_handle,
        create_function_wrapper(basicFunc2, ffrt_function_kind_queue), nullptr);
    ffrt_queue_wait(task1);
    ffrt_queue_wait(task2);
    int testNumber1 = 10;
    int testNumber3 = 20;
    if (result2 != testNumber3 or result1 != testNumber1) {
        result += 1;
    }
    ffrt_loop_stop(loop);
    pthread_join(thread, nullptr);
...
static napi_value ffrt_get_main_queue_0001(napi_env env, napi_callback_info info)
{
    ...
    int result1 = 0;
    std::function<void()>&& basicFunc = [&result1]() {
        OnePlusForTest(static_cast<void*>(&result1));
        OnePlusForTest(static_cast<void*>(&result1));
        int timeSleepSeconds = 3000;
        usleep(timeSleepSeconds);
    };
    ffrt_task_handle_t task1 = ffrt_queue_submit_h(currentQueue,
        create_function_wrapper(basicFunc, ffrt_function_kind_queue) , &attr);
    ffrt_queue_wait(task1);
    napi_value flag = nullptr;
    napi_create_double(env, result, &flag);
    if (result1 != 0) {
...
static napi_value Init(napi_env env, napi_value exports)
{
    napi_property_descriptor desc[] = {
        ...
        { "queue_parallel_cancel_0002", nullptr, queue_parallel_cancel_0002, nullptr, nullptr,
            nullptr, napi_default, nullptr },
        { "queue_parallel_0001", nullptr, queue_parallel_0001, nullptr, nullptr,
            nullptr, napi_default, nullptr },

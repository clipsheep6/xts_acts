...
HWTEST_F(HdiNNCoreCompilation, SUB_AI_NNRt_Core_Func_North_Create_Compilation_Options_0200, Function | MediumTest | Level1)
{
    ...
    ASSERT_EQ(nullptr, OH_NNCore_CreateOptions(backendName));
}
...
/**
 * @tc.name: SUB_AI_NNRt_Core_Func_North_Build_Compilation_1200
 * @tc.desc: modelbuffer，传入modelData为合法离线模型buffer，modelSize正确，编译成功
 * @tc.type: FUNC
 */
HWTEST_F(HdiNNCoreCompilation, SUB_AI_NNRt_Core_Func_North_Build_Compilation_1200, Function | MediumTest | Level1)
{
    int modelSize = 0;
    char* buffer = nullptr;
    GetBuffer(SUPPORTMODELPATH.c_str(), &buffer, modelSize);
...
/**
 * @tc.name: SUB_AI_NNRt_Core_Func_North_Destroy_Compilation_0200

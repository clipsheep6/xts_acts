...
static napi_value GetPluralStringByName(napi_env env, napi_callback_info info){
    ...
    char *result;
    uint32_t num = 1;
    napi_get_value_uint32(env, argv[2],&num);
...
static napi_value GetConfiguration(napi_env env, napi_callback_info info){
    ...
    bool flag = (code==0 && config.direction==0 && config.deviceType==0 && config.screenDensity==3 && config.colorMode==1 && config.mcc==0 && config.mnc==0 && strcmp(config.locale,"zh_Hans_CN")==0);
    napi_value value = nullptr;
    napi_get_boolean(env, flag, &value);
    return value;
}
...
static napi_value AddResource(napi_env env, napi_callback_info info){
    ...
    napi_value jsValue = nullptr;
    napi_create_int32(env, resultValue, &jsValue);
    return jsValue;
}
...
static napi_value RemoveResource(napi_env env, napi_callback_info info){
    ...
    OH_ResourceManager_AddResource(mNativeResMgr,path);
    OH_ResourceManager_GetIntByName(mNativeResMgr,"integer_1",&resultValue);
    ResourceManager_ErrorCode code=OH_ResourceManager_RemoveResource(mNativeResMgr,path);
    OH_ResourceManager_GetIntByName(mNativeResMgr,"integer_1",&resultValue2);
    
    bool flag = (code == 0 && resultValue==123 && resultValue==100);
    napi_value value = nullptr;
    napi_get_boolean(env, flag, &value);
    return value;
}

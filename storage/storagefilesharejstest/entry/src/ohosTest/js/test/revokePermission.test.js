    ...
    describe('RevokePermission_test', function () {
      ...
      */
      it('FileShare_revokePermission_001', 0, async function (done) {
          try {
            let fpath = await featureAbility.getContext().getFilesDir() + '/FileShare_revokePermission_001.txt';
      ...
      */
      it('FileShare_revokePermission_002', 0, async function (done) {
          try {
          let fpath = await featureAbility.getContext().getFilesDir() + '/FileShare_revokePermission_002.txt';
          let uri = fileUri.getUriFromPath(fpath);
          let fd = await fs.open(uri,fs.OpenMode.CREATE);
          await fs.close(fd);
          let policyInfo = {
      ...
      */
      it('FileShare_revokePermission_003', 3, async function (done) {
          try {
          let fpath = await featureAbility.getContext().getFilesDir() + '/FileShare_revokePermission_003.txt';
          let uri1 = fileUri.getUriFromPath(fpath);
          let policyInfo1 = {
              uri: uri1, 
              operationMode: fileShare.OperationMode.READ_MODE,
          };
          let fpath2 = await featureAbility.getContext().getFilesDir() + '/FileShare_revokePermission_00301.txt';

...
describe('fileIO_randomAccessFile_read', function () {
    ...
    it('fileIO_randomaccessfile_read_async_000', 0, async function (done) {
        ...
        try {
            ...
            console.info('fileIO_randomaccessfile_read_async_000 num =  ' + num);
            expect(num == length).assertTrue();
            randomaccessfile.setFilePointerSync(0);
            randomaccessfile.read(new ArrayBuffer(length), function (err, readOut) {
                console.info('fileIO_randomaccessfile_read_async_000 readOut.bytesRead =  ' + readOut.bytesRead);
                console.info('fileIO_randomaccessfile_read_async_000 length =  ' + length);
    ...
    it('fileIO_randomaccessfile_read_async_001', 0, async function (done) {
        ...
        try {
            fileIO.createRandomAccessFile(fpath, 0, 0o102, async function (err, randomaccessfile) {
                let length = 20;
                let num = await randomaccessfile.write(new ArrayBuffer(length));
                console.info('fileIO_randomaccessfile_read_async_001 num =  ' + num);
                expect(num == length).assertTrue();
                randomaccessfile.setFilePointerSync(0);
                let readOut = await randomaccessfile.read(new ArrayBuffer(length), { offset: 1 });
                console.info('fileIO_randomaccessfile_read_async_001 readOut.bytesRead =  ' + readOut.bytesRead);
    ...
    it('fileIO_randomaccessfile_read_async_002', 0, async function (done) {
        ...
        try {
            let file = fileIO.openSync(fpath, fileIO.OpenMode.CREATE | fileIO.OpenMode.READ_WRITE);
            let randomaccessfile = await fileIO.createRandomAccessFile(file.fd, 0);
            let length = 20;
            let num = await randomaccessfile.write(new ArrayBuffer(length));
            console.info('fileIO_randomaccessfile_read_async_002 num =  ' + num);
            expect(num == length).assertTrue();
            randomaccessfile.setFilePointerSync(0);
            let readOut = await randomaccessfile.read(new ArrayBuffer(length), { offset: 1 });
            console.info('fileIO_randomaccessfile_read_async_002 readOut.bytesRead =  ' + readOut.bytesRead);
            console.info('fileIO_randomaccessfile_read_async_002 readOut.offset =  ' + readOut.offset);
            expect(readOut.bytesRead == length - 1).assertTrue();
    ...
    it('fileIO_randomaccessfile_read_async_003', 0, async function (done) {
        ...
        try {
            ...
            randomaccessfile.read(new ArrayBuffer(length), { offset: 1, length: 5 }, function (err, readOut) {
                console.info('fileIO_randomaccessfile_read_async_003 readOut.bytesRead =  ' + readOut.bytesRead);
                console.info('fileIO_randomaccessfile_read_async_003 readOut.offset =  ' + readOut.offset);
    ...
    it('fileIO_randomaccessfile_read_async_004', 0, async function (done) {
        ...
        try {
            ...
            console.info('fileIO_randomaccessfile_read_async_004 readOut.bytesRead =  ' + readOut.bytesRead);
            console.info('fileIO_randomaccessfile_read_async_004 readOut.offset =  ' + readOut.offset);
    ...
    it('fileIO_randomaccessfile_read_async_005', 0, async function (done) {
        ...
        try {
            let randomaccessfile = await fileIO.createRandomAccessFile(fpath, 0, 0o102);
            let length = 20;
            let readOut = await randomaccessfile.read(new ArrayBuffer(length), { offset: 6});
            console.info('fileIO_randomaccessfile_read_async_005 readOut.bytesRead =  ' + readOut.bytesRead);
            console.info('fileIO_randomaccessfile_read_async_005 readOut.offset =  ' + readOut.offset);
            expect(readOut.bytesRead == FILE_CONTENT.length - 6).assertTrue();
            expect(readOut.offset == 6).assertTrue();
            // let start = readOut.offset;
            // let end = readOut.offset + readOut.bytesRead;
            let start = 0
            let end = readOut.bytesRead;
            let result = String.fromCharCode.apply(null, new Uint8Array(readOut.buffer.slice(start, end)));
            console.info('fileIO_randomaccessfile_read_async_005 result =  ' + result);

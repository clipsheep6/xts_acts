...
import {
  ...
  it('fileIO_test_moveFile_sync_002', 0, async function () {
    ...
    let fpath = dpath + '/dir_002';
    let ddpath = dpath + '/dir_002_1';

    try {
      fileIO.moveFileSync(fpath, ddpath);
      expect(false).assertTrue();
    } catch (e) {
  ...
  it('fileIO_test_moveFile_sync_004', 0, async function () {
    ...
    let dddpath = ddpath + '/dir_003_1';
    fileIO.mkdirSync(dpath);
    fileIO.mkdirSync(ddpath);

    try {
      fileIO.moveFileSync(dpath, dddpath);
      expect(false).assertTrue();
    } catch (e) {
      fileIO.rmdirSync(dpath);
...
/**
   ...
   * @tc.desc Test moveFileSync() interface. Promise.
   * A file with the same name already exists under the destination path.
   * @tc.size MEDIUM
   * @tc.type Function
   * @tc.level Level 0
...
/**
   ...
   * @tc.desc Test moveFileSync() interface. Callback.
   * A file with the same name already exists under the destination path.
   * @tc.size MEDIUM
   * @tc.type Function
   * @tc.level Level 0
  ...
  it('fileIO_test_moveFile_async_004', 0, async function (done) {
    ...
    let fpath = dpath + '/dir_009';
    let ddpath = dpath + '/dir_009_1';

    try {
      await fileIO.moveFile(fpath, ddpath);
      expect(false).assertTrue();
    } catch (e) {
  ...
  it('fileIO_test_moveFile_async_005', 0, async function (done) {
    ...
    let fpath = dpath + '/dir_010';
    let ddpath = dpath + '/dir_010_1';

    try {
      fileIO.moveFile(fpath, ddpath, (err) => {
        if (err) {
  ...
  it('fileIO_test_moveFile_async_006', 0, async function (done) {
    ...
    let dddpath = ddpath + '/dir_011_1';
    fileIO.mkdirSync(dpath);
    fileIO.mkdirSync(ddpath);

    try {
      await fileIO.moveFile(dpath, dddpath);
      expect(false).assertTrue();
    } catch (e) {
      fileIO.rmdirSync(dpath);
  ...
  it('fileIO_test_moveFile_async_007', 0, async function (done) {
    ...
    let dddpath = ddpath + '/dir_012_1';
    fileIO.mkdirSync(dpath);
    fileIO.mkdirSync(ddpath);

    try {
      fileIO.moveFile(dpath, dddpath, () => {
        expect(false).assertTrue();
      });
    } catch (e) {
      fileIO.rmdirSync(dpath);

...
export default function fileIOCopyDir() {
  ...
  it('fileIO_test_copyDir_async_000', 3, async function (done) {
    ...
    let ffpath = ddpath + '/old.txt';
    let ffpath2 = ddpath + '/move.doc';
    let dddpath = ddpath + '/dir_thir';
    let fffpath = dddpath + '/third.txt';
    let ddpath2 = dpath + '/dir2_000';
    ...
    try {
      ...
      let stat1 = fileIO.statSync(ddpath2 + '/dir_000/move.doc');
      expect(stat1.size == FILE_CONTENT.length).assertTrue();
      let stat2 = fileIO.statSync(ddpath2 + '/dir_000/old.txt');
      expect(stat2.size == FILE_CONTENT.length).assertTrue();
      expect(fileIO.accessSync(ddpath)).assertTrue();
      expect(fileIO.accessSync(ddpath2 + '/dir_000/dir_thir')).assertTrue();
      let stat3 = fileIO.statSync(ddpath2 + '/dir_000/dir_thir/third.txt');
   ...
   * @tc.desc Test copyDir() interface.Promise.
   * There is a target folder(src) under path dest but it is empty.Mode is DIRMODE_FILE_COPY_REPLACE.
   * @tc.size MEDIUM
   * @tc.type Function
   * @tc.level Level 3
   * @tc.require
  ...
  it('fileIO_test_copyDir_async_002', 3, async function (done) {
    ...
    let dddpath = ddpath + '/dir_thir';
    let fffpath = dddpath + '/third.txt';
    let ddpath2 = dpath + '/dir2_001';
    let ffpath3 = ddpath2 + '/new.txt';
    let dddpath2= ddpath2 + '/dir_001';
    let fffpath2 = dddpath2 + '/has.doc';
    fileIO.mkdirSync(dpath);
    fileIO.mkdirSync(ddpath);
    fileIO.mkdirSync(ddpath2);
    fileIO.mkdirSync(dddpath);
    ...
    try {
      await fileIO.copyDir(ddpath, ddpath2, DIRMODE_FILE_COPY_REPLACE);
      let stat1 = fileIO.statSync(ddpath2 + '/dir_001/move.doc');
      expect(stat1.size == FILE_CONTENT.length).assertTrue();
      let stat2 = fileIO.statSync(ddpath2 + '/dir_001/old.txt');
      expect(stat2.size == FILE_CONTENT.length).assertTrue();
      expect(fileIO.accessSync(ddpath)).assertTrue();
      expect(fileIO.accessSync(ddpath2 + '/dir_001/dir_thir')).assertTrue();
      let stat3 = fileIO.statSync(ddpath2 + '/dir_001/dir_thir/third.txt');
      expect(stat3.size == FILE_CONTENT.length).assertTrue();
      let stat4 = fileIO.statSync(ddpath2 + '/dir_001/has.doc');
   ...
   * @tc.desc Test copyDir() interface.Promise.
   * There is a target folder(src) under path dest and it is not empty.Mode is DIRMODE_FILE_COPY_THROW_ERR.
   * @tc.size MEDIUM
   * @tc.type Function
   * @tc.level Level 0
   * @tc.require
  ...
  it('fileIO_test_copyDir_async_014', 3, async function (done) {
    ...
    let dddpath = ddpath + '/dir_thir';
    let qpath = dddpath + '/dir666';
    let apath = qpath + '/dirabc';
    let afile = apath + '/file.png';
    fileIO.mkdirSync(dpath);
    fileIO.mkdirSync(ddpath);
    fileIO.mkdirSync(dddpath);
    fileIO.mkdirSync(qpath);
    fileIO.mkdirSync(apath);
    expect(prepareFile(afile, FILE_CONTENT)).assertTrue();

    try {
      await fileIO.copyDir(ddpath, apath, DIRMODE_FILE_COPY_REPLACE);
      expect(false).assertTrue();
    } catch (err) {
  ...
  it('fileIO_test_copyDir_async_015', 3, async function (done) {
    ...
    try {
      await fileIO.copyDir(ddpath, apath, DIRMODE_FILE_COPY_REPLACE, (err) => {
        if (err) {

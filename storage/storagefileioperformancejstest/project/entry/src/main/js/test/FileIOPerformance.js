...
describe('fileTest', function () {
  ...
  it('fileioPerformance_prop', 0, function () {
    ...
    let stat = fileio.Stat.statSync(dpath + "999/f0999")
      ...
      for (let i = 0; i < 1000; i++) {
        let copyFileSync;
        let start7 = new Date().getTime();
        copyFileSync = fileio.copyFileSync(dpath + "999/f0" + i,
            dpath + "998/f0" + i);
        let end7 = new Date().getTime();
        let time7 = end7 - start7;
      ...
      for (let i = 0; i < 1000; i++) {
        ...
        renameSync = fileio.renameSync(dpath + "998/f0" + i,
            dpath + "998/f0" + i + "a");
  ...
  /* 1. Preset 1m file to be written
   * 2. ROM remaining less than 10% of the total*/
  ...
  /*0.5G Synchronous opening performance test of test files
  （The path needs to be manually placed in advance"/data/accounts/account_0/appdata/ohos.acts.distributeddatamgr.distributedfile/cache/p1"的0.5g文件）*/

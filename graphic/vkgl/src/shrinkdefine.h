...
#define SHRINK_HWTEST_F1(TestCase, VkglCase1, VkglCase2)   do {\
        GTEST_LOG_(INFO) << #TestCase << "start";                     \
        int argc = 3;                                                 \
        const char *argv[3] = {".", "--deqp-case="                      \
            VkglCase1VkglCase2,                                       \
            "--deqp-archive-dir=/data/local/tmp/"                     \
        };                                                            \
        FuncRunResult result = RunTestKHRGLES(argc, argv);            \
        } while (0)
#define SHRINK_HWTEST_F2(TestSuite)   do {\
...
#define SHRINK_HWTEST_F3(TestCase)   do {\

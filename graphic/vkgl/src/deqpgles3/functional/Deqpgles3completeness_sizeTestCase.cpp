...
static HWTEST_F(ActsDeqpgles30032TestSuite, TestCase_031403, Function | MediumTest | Level2)
{
    GTEST_LOG_(INFO) << "TestCase_031403 start";
    int argc = 3;
    const char *argv[3] = {
        ".",
        "--deqp-case="
    ...
    if (result.numNotSupported == 1) {
        GTEST_LOG_(INFO) << "TestCase_031403 notsupport!";
    } else if (result.isComplete) {
        EXPECT_TRUE(result.isComplete);
        EXPECT_TRUE(result.numPassed == 1);
    };
    GTEST_LOG_(INFO) << "TestCase_031403 end";
}

static HWTEST_F(ActsDeqpgles30032TestSuite, TestCase_031404, Function | MediumTest | Level2)
{
    GTEST_LOG_(INFO) << "TestCase_031404 start";
    int argc = 3;
    const char *argv[3] = {
        ".",
        "--deqp-case="
    ...
    ActsDeqpgles30032TestSuite::runResult.numWaived += result.numWaived;
    if (result.numNotSupported == 1) {
        GTEST_LOG_(INFO) << "TestCase_031404 notsupport!";
    } else if (result.isComplete) {
        EXPECT_TRUE(result.isComplete);
        EXPECT_TRUE(result.numPassed == 1);
    };
    GTEST_LOG_(INFO) << "TestCase_031404 end";
}

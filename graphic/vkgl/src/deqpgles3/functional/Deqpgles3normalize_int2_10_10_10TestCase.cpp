...
static HWTEST_F(ActsDeqpgles30034TestSuite, TestCase_033071, Function | MediumTest | Level2)
{
    GTEST_LOG_(INFO) << "TestCase_033071 start";
    int argc = 3;
    const char *argv[3] = {
        ".",
        "--deqp-case="
    ...
    if (result.numNotSupported == 1) {
        GTEST_LOG_(INFO) << "TestCase_033071 notsupport!";
    } else if (result.isComplete) {
        EXPECT_TRUE(result.isComplete);
        EXPECT_TRUE(result.numPassed == 1);
    };
    GTEST_LOG_(INFO) << "TestCase_033071 end";
}

static HWTEST_F(ActsDeqpgles30034TestSuite, TestCase_033072, Function | MediumTest | Level2)
{
    GTEST_LOG_(INFO) << "TestCase_033072 start";
    int argc = 3;
    const char *argv[3] = {
        ".",
        "--deqp-case="
    ...
    ActsDeqpgles30034TestSuite::runResult.numWaived += result.numWaived;
    if (result.numNotSupported == 1) {
        GTEST_LOG_(INFO) << "TestCase_033072 notsupport!";
    } else if (result.isComplete) {
        EXPECT_TRUE(result.isComplete);
        EXPECT_TRUE(result.numPassed == 1);
    };
    GTEST_LOG_(INFO) << "TestCase_033072 end";
}

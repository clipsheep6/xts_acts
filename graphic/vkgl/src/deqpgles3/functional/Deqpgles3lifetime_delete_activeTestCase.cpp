...
static HWTEST_F(ActsDeqpgles30045TestSuite, TestCase_044388, Function | MediumTest | Level2)
{
    GTEST_LOG_(INFO) << "TestCase_044388 start";
    int argc = 3;
    const char *argv[3] = {
        ".",
        "--deqp-case="
    ...
    if (result.numNotSupported == 1) {
        GTEST_LOG_(INFO) << "TestCase_044388 notsupport!";
    } else if (result.isComplete) {
        EXPECT_TRUE(result.isComplete);
        EXPECT_TRUE(result.numPassed == 1);
    };
    GTEST_LOG_(INFO) << "TestCase_044388 end";
}

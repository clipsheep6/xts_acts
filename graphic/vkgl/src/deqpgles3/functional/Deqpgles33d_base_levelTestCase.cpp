...
static HWTEST_F(ActsDeqpgles30026TestSuite, TestCase_025189, Function | MediumTest | Level2)
{
...
static HWTEST_F(ActsDeqpgles30026TestSuite, TestCase_025190, Function | MediumTest | Level2)
{
    GTEST_LOG_(INFO) << "TestCase_025190 start";
    int argc = 3;
    const char *argv[3] = {
        ".",
        "--deqp-case="
    ...
    ActsDeqpgles30026TestSuite::runResult.numWaived += result.numWaived;
    if (result.numNotSupported == 1) {
        GTEST_LOG_(INFO) << "TestCase_025190 notsupport!";
    } else if (result.isComplete) {
        EXPECT_TRUE(result.isComplete);
        EXPECT_TRUE(result.numPassed == 1);
    };
    GTEST_LOG_(INFO) << "TestCase_025190 end";
}

static HWTEST_F(ActsDeqpgles30026TestSuite, TestCase_025191, Function | MediumTest | Level2)
{
    GTEST_LOG_(INFO) << "TestCase_025191 start";
    int argc = 3;
    const char *argv[3] = {
        ".",
        "--deqp-case="
    ...
    ActsDeqpgles30026TestSuite::runResult.numWaived += result.numWaived;
    if (result.numNotSupported == 1) {
        GTEST_LOG_(INFO) << "TestCase_025191 notsupport!";
    } else if (result.isComplete) {
        EXPECT_TRUE(result.isComplete);
        EXPECT_TRUE(result.numPassed == 1);
    };
    GTEST_LOG_(INFO) << "TestCase_025191 end";
}

static HWTEST_F(ActsDeqpgles30026TestSuite, TestCase_025192, Function | MediumTest | Level2)
{

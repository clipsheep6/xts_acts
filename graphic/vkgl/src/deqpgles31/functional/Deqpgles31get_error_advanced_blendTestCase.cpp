...
static HWTEST_F(ActsDeqpgles310020TestSuite, TestCase_019809, Function | MediumTest | Level2)
{
    GTEST_LOG_(INFO) << "TestCase_019809 start";
    int argc = 3;
    const char *argv[3] = {
        ".",
        "--deqp-case="
    ...
    if (result.numNotSupported == 1) {
        GTEST_LOG_(INFO) << "TestCase_019809 notsupport!";
    } else if (result.isComplete) {
        EXPECT_TRUE(result.isComplete);
        EXPECT_TRUE(result.numPassed == 1);
    };
    GTEST_LOG_(INFO) << "TestCase_019809 end";
}

static HWTEST_F(ActsDeqpgles310020TestSuite, TestCase_019810, Function | MediumTest | Level2)
{
    GTEST_LOG_(INFO) << "TestCase_019810 start";
    int argc = 3;
    const char *argv[3] = {
        ".",
        "--deqp-case="
    ...
    ActsDeqpgles310020TestSuite::runResult.numWaived += result.numWaived;
    if (result.numNotSupported == 1) {
        GTEST_LOG_(INFO) << "TestCase_019810 notsupport!";
    } else if (result.isComplete) {
        EXPECT_TRUE(result.isComplete);
        EXPECT_TRUE(result.numPassed == 1);
    };
    GTEST_LOG_(INFO) << "TestCase_019810 end";
}

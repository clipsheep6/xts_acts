...
static HWTEST_F(ActsDeqpgles310021TestSuite, TestCase_020048, Function | MediumTest | Level2)
{
    GTEST_LOG_(INFO) << "TestCase_020048 start";
    int argc = 3;
    const char *argv[3] = {
        ".",
        "--deqp-case="
    ...
    if (result.numNotSupported == 1) {
        GTEST_LOG_(INFO) << "TestCase_020048 notsupport!";
    } else if (result.isComplete) {
        EXPECT_TRUE(result.isComplete);
        EXPECT_TRUE(result.numPassed == 1);
    };
    GTEST_LOG_(INFO) << "TestCase_020048 end";
}

static HWTEST_F(ActsDeqpgles310021TestSuite, TestCase_020049, Function | MediumTest | Level2)
{
    GTEST_LOG_(INFO) << "TestCase_020049 start";
    int argc = 3;
    const char *argv[3] = {
        ".",
        "--deqp-case="
    ...
    ActsDeqpgles310021TestSuite::runResult.numWaived += result.numWaived;
    if (result.numNotSupported == 1) {
        GTEST_LOG_(INFO) << "TestCase_020049 notsupport!";
    } else if (result.isComplete) {
        EXPECT_TRUE(result.isComplete);
        EXPECT_TRUE(result.numPassed == 1);
    };
    GTEST_LOG_(INFO) << "TestCase_020049 end";
}

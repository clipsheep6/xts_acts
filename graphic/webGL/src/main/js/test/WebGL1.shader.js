...
export default function webgl1_shader() {
	...
	describe('webgl1_shader', function () {
        ...
        it('webgl_test_createShader', 0, async function (done) {
            ...
            gl.deleteShader(vShader);//delete shader
            expect(checkError(gl)).assertEqual(gl.NO_ERROR);
            let nShader = gl.createShader(1);//invalid shader type return null
        ...
        it('webgl_test_createShader_1', 0, async function (done) {
            ...
            if (vShader) gl.deleteShader(vShader);//delete shader
            expect(checkError(gl)).assertEqual(gl.INVALID_ENUM);
            checkError(gl);
            done();
        })
        ...
        // WebGL API的方法将GLSL着色器编译成二进制数据，以便WebGL程序可以使用它。
        ...
        // 返回指定WebGLShader对象的信息日志。它包含警告、调试和编译信息。
        ...
        it('webgl_test_getShaderInfoLog', 0, async function (done) {
            //返回指定 WebGLShader 对象的信息日志。 它包含警告，调试和编译信息。
        ...
        // 如果传入的WebGLShader有效，WebGL API的方法返回true，否则返回false。
        ...
        it('webgl_test_isShader', 0, async function (done) {
            //方法如果传递的 WebGLShader 有效，则返回 true，否则返回 false。
        ...
        // 方法将给定的WebGLShader对象标记为删除。当着色器不再使用时，它将被删除。如果着色器已经被删除，这个方法没有作用，当WebGLShader被垃圾收集器销毁时，它被自动标记为删除。
        ...
        //方法将片段或顶点WebGLShader附加到WebGLProgram。
        ...
        // 方法返回附加到WebGLProgram的WebGLShader对象列表。
        ...
        // 方法从WebGLProgram中分离先前附加的WebGLShader。

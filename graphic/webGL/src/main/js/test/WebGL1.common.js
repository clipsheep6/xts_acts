...
export default function webgl1_common() {
	...
	describe('webgl1_common', function () {
        ...
        it('webgl_test_getError', 0, async function (done) {
            //WebGL API 的 WebGLRenderingContext.getError（） 方法返回错误信息。
        ...
        // 返回一个布尔值，指示 WebGL 上下文是否已丢失并且必须在渲染恢复之前重新建立。
        ...
        it('webgl_test_isEnabled', 0, async function (done) {
            ...
            expect(checkError(gl)).assertEqual(gl.NO_ERROR);
            //RASTERIZER_DISCARD in WebGL2.0
        ...
        //方法返回一个包含实际上下文参数的WebGLContextAttributes对象。如果上下文丢失，可能返回null。
        ...
        it('webgl_test_getContextAttributes', 0, async function (done) {
            ...
            if (value) {
                ...
                expect(typeof (value["powerPreference"])).assertEqual("string");//|"low-power"| "high-performance"
                expect(value["powerPreference"]).assertEqual("default" || "low-power" || "high-performance");//"default"|"low-power"| "high-performance"
            } else {
        ...
        // 方法返回一个布尔值，该值指示WebGL上下文是否已经丢失，并且必须在恢复呈现之前重新建立。
        ...
        // WebGL API的方法指定深度缓冲区的clear值。这指定了调用clear()方法时要使用的深度值。该值固定在0到1之间。
        ...
        it('webgl_test_clearDepth', 0, async function (done) {
            //用于设置深度缓冲区的深度清除值。这个深度清除值的设定，是为了调用clear() 的时候使用，这个值的范围是 0 到 1。
        ...
        // 将缓冲区清除为预设值。clearColor()可以通过、clearDepth()或 来 设置预设值clearStencil()。
        ...
        // 方法从当前颜色帧缓冲区的指定矩形中读取像素块到typearray或DataView对象中。

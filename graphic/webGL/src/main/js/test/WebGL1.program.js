...
export default function webgl1_program() {
	...
	describe('webgl1_program', function () {
        ...
        it('webgl_test_createProgram', 0, async function (done) {
            //WebGL API 的 WebGLRenderingContext.createProgram（） 方法创建并初始化 WebGLProgram 对象。
        ...
        // 方法删除给定的WebGLProgram对象。如果程序已被删除，此方法无效。
        ...
        // 如果传入的WebGLProgram有效，则返回true，否则返回false。
        ...
        it('webgl_test_isProgram', 0, async function (done) {
            //如果传递的 WebGLProgram 有效，WebGL API 的 WebGLRenderingContext.isProgram（） 方法返回 true，否则返回 false。
        ...
        //方法链接一个给定的WebGLProgram，完成为程序的片段和顶点着色器准备GPU代码的过程。
        ...
        // 方法验证WebGL程序。它检查它是否被成功链接，以及它是否可以在当前WebGL状态下使用。
        ...
        it('webgl_test_validateProgram', 0, async function (done) {
            //主要是用来验证 WebGLProgram。它在检查 WebGLProgram 程序是否链接成功的同时还会检查其是否能在当前的 WebGL 中使用。
        ...
        // 方法将指定的WebGLProgram设置为当前呈现状态的一部分。
        ...
        // 返回指定WebGLProgram对象的信息日志。它包含在失败链接或验证WebGLProgram对象期间发生的错误。
        ...
        it('webgl_test_getProgramInfoLog', 0, async function (done) {
            //返回一个字符串，包含了最近一次调用gl.linkProgram()时的链接信息日志。

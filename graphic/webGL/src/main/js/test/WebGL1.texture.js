...
export default function webgl1_texture() {
	...
	describe('webgl1_texture', function () {
         ...
         * @tc.name webgl_test_texImage2D_1
         * @tc.desc Test texImage2D.
         */
        it('webgl_test_texImage2D_1', 0, async function (done) {
            console.info("webgltest [webgl_test_texImage2D_1] texImage2D");
            let {program, vertexShader, fragmentShader} = createProgram(gl, `
                attribute vec4 a_Position;
                attribute vec2 a_TexCoord;
                varying vec2 v_TexCoord;
        ...
        it('webgl_test_texImage2D_Error', 0, async function (done) {
            ...
            let data = new Uint8Array(width * height * 4); // 4个字节表示一个像素的 RGBA 值
            for (let i = 0; i < data.length; i += 4) {
                data[i] = 0; // 红色通道
                data[i + 1] = 0; // 绿色通道
                data[i + 2] = 255; // 蓝色通道
        ...
        it('webgl_test_compressedTexImage2D_Error', 0, async function (done) {
            ...
            const ext =
                ...
                gl.getExtension("MOZ_WEBGL_compressed_texture_s3tc") ||
                gl.getExtension("WEBKIT_WEBGL_compressed_texture_s3tc") || {};
            console.info("webgltest [webgl_test_compressedTexImage2D] compressedTexImage2D no target");
            ...
            console.info("webgltest [webgl_test_compressedTexImage2D] compressedTexImage2D no level");
            ...
            console.info("webgltest [webgl_test_compressedTexImage2D] compressedTexImage2D no internalformat");
            ...
            console.info("webgltest [webgl_test_compressedTexImage2D] compressedTexImage2D no width");
            ...
            console.info("webgltest [webgl_test_compressedTexImage2D] compressedTexImage2D no height");
            ...
            console.info("webgltest [webgl_test_compressedTexImage2D] compressedTexImage2D no border");
            ...
            console.info("webgltest [webgl_test_compressedTexImage2D] compressedTexImage2D no pixels");
            gl.compressedTexImage2D(gl.TEXTURE_2D, 0, ext.COMPRESSED_RGBA_S3TC_DXT1_EXT, width, height, 0, new Uint8Array([]));
            if (ext.COMPRESSED_RGBA_S3TC_DXT1_EXT) {
                expect(checkError(gl)).assertEqual(gl.INVALID_OPERATION);
            } else {

...
describe("SensorJsTest_sensor_41", function () {
    ...
    it("newAmbient_Light_SensorJsTest004", TestType.FUNCTION | Size.MEDIUMTEST | Level.LEVEL3, function (done) {
        ...
        try {
            ...
            console.info("newAmbient_Light_SensorJsTest004 error:" +error);
            expect(false).assertTrue();
            done();
        }
        setTimeout(()=>{
    ...
    it("newAmbient_Light_SensorJsTest008", TestType.FUNCTION | Size.MEDIUMTEST | Level.LEVEL3, async function (done) {
        try {
            ...
            console.info('newAmbient_Light_SensorJsTest008 error:' +error);
            expect(error.code).assertEqual(PARAMETER_ERROR_CODE)
            expect(error.message).assertEqual(PARAMETER_ERROR_MSG)
            done();
        }
	...
	/*
     * @tc.number:SUB_SensorsSystem_AMBIENT_LIGHT_JSTest_0010
     * @tc.name: newnewAmbient_Light_SensorJsTest010
     * @tc.desc:SensorId1000000 of incoming exception
     */
    it("newnewAmbient_Light_SensorJsTest010", TestType.FUNCTION | Size.MEDIUMTEST | Level.LEVEL3, async function (done) {
        function onSensorCallback(data) {
            console.info('newnewAmbient_Light_SensorJsTest010 on error');
            expect(false).assertTrue();
        }
        try {
            sensor.off(1000000, onSensorCallback);
        } catch (error) {
            console.info("newnewAmbient_Light_SensorJsTest010 error:" +error);
            expect(error.code).assertEqual(PARAMETER_ERROR_CODE)
            expect(error.message).assertEqual(PARAMETER_ERROR_MSG)
            done();
        }
     ...
     * @tc.desc:Verification results of the incorrect parameters of the test interface
     */
    it("newAmbient_Light_SensorJsTest012", TestType.FUNCTION | Size.MEDIUMTEST | Level.LEVEL3, function (done) {
        try {
            sensor.off(sensor.SensorId.AMBIENT_LIGHT, 5);
        } catch (error) {
            console.info('newAmbient_Light_SensorJsTest012 error:'+error);
            expect(error.code).assertEqual(PARAMETER_ERROR_CODE)
            expect(error.message).assertEqual(PARAMETER_ERROR_MSG)
            done();
        }

...
describe('ChildProcessTest', function () {
    ...
    it('test_getErrorOutput_001', 0, async function () {
        ...
        child.wait()
        var array = new Uint8Array([115,104,58,32,109,97,107,100,105,114,58,32,105,110,97,99,99,101,115,115,105,98,
    ...
    it('test_getErrorOutput_003', 0, async function () {
        ...
        child.wait()
        var array = new Uint8Array([115,104,58,32,49,58,32,105,110,97,99,99,101,
            115,115,105,98,108,101,32,111,114,32,110,111,116,32,102,111,117,110,100,10,0])
        child.getErrorOutput().then(val=>{
            for (var i = 0; i < array.length; i++) {
                expect(val[i]).assertEqual(array[i])
            }
    ...
    it('kill_test_001', 0, function () {
        ...
        var result = process.kill(child.pid, 9)
        expect(result).assertEqual(true)
    })

    it('kill_test_002', 0, function () {
        var child = process.runCmd("sleep 5; ls")
        var result = process.kill(child.pid, 28)
        expect(result).assertEqual(true)
    })
    ...
    it('kill_test_003', 0, function () {
        var child = process.runCmd("sleep 5; ls")
        var result = process.kill(child.pid, 17)
        expect(result).assertEqual(true)
    })

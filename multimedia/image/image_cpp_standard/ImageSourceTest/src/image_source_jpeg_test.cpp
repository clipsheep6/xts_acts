...
static const std::string IMAGE_INPUT_EXIF_JPEG_PATH = "data/local/tmp/image/image/test_exif.jpg";
...
static OHOS::BufferRequestConfig requestConfig = {
    .width = 0x100,
    .height = 0x100,
    .strideAlignment = 0x8,
...
static OHOS::BufferFlushConfig flushConfig = {
    .damage = {
        .w = 0x100,
        .h = 0x100,
    },
...
HWTEST_F(ImageSourceJpegTest, TC029, TestSize.Level3)
{
    ...
    std::unique_ptr<ImageSource> imageSource = ImageSource::CreateImageSource(
        IMAGE_INPUT_HW_JPEG_PATH, opts, errorCode);
    ASSERT_EQ(errorCode, SUCCESS);
    ASSERT_NE(imageSource.get(), nullptr);
}
/**
...
HWTEST_F(ImageSourceJpegTest, TC035, TestSize.Level3)
{
    ...
    std::unique_ptr<PixelMap> pixelMap = imageSource->CreatePixelMap(index, opts_, errorCode_);
    ASSERT_EQ(errorCode_, SUCCESS);
...
HWTEST_F(ImageSourceJpegTest, TC059, TestSize.Level3)
{
    ...
    uint32_t index = 0;
    DecodeOptions opts_;
    std::unique_ptr<PixelMap> pixelMap = imageSource->CreatePixelMap(index, opts_, errorCode);
    ASSERT_EQ(errorCode, SUCCESS);
    ImagePacker imagePacker ;
...
/**
 * @tc.name: TC060 Private
 * @tc.desc: StartPackingImpl(option)
 * @tc.type: FUNC
 */
/**
 * @tc.name: TC061
...
HWTEST_F(ImageSourceJpegTest, JpegImageDecode011, TestSize.Level3)
{
    ...
    ASSERT_NE(imageSource.get(), nullptr);
    /**
     * @tc.steps: step2. Obtains the integer value of a specified property key for an image at the given index.
    ...
    ASSERT_EQ(errorCode, SUCCESS);
    /**
     * @tc.steps: step3. Obtains the integer value of a specified property key for an image at the given index.
...
/**
 * @tc.name: JpegImageReceiver001
 * @tc.desc: ImageReceiver unit test.
 * @tc.type: FUNC
 */
HWTEST_F(ImageSourceJpegTest, JpegImageReceiver001, TestSize.Level3)
{
    ...
    opts.editable = true;
    imageReceiver = ImageReceiver::CreateImageReceiver(8 * 1024, 8);
    OHOS::sptr<OHOS::Surface> receiverSurface = imageReceiver->GetReceiverSurface();
    ...
    int32_t errorcode = imageReceiver->SaveBufferAsImage("/data/local/tmp/Receiver_buffer1.jpg", buffer, opts);
    errorcode = imageReceiver->SaveBufferAsImage("/data/local/tmp/Receiver_buffer2.jpg", opts);
    std::unique_ptr<PixelMap> pixelMap = imageReceiver->getSurfacePixelMap(opts);
    int64_t packSize = PackImage("/data/local/tmp/Receiver_buffer3.jpg", std::move(pixelMap));
    imageReceiver->ReleaseReceiver();
    EXPECT_NE((packSize < 0), true);

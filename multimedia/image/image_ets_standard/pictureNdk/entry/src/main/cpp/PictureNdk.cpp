...
static napi_value GetAuxiliaryPicture(napi_env env, napi_callback_info info)
{
    ...
    IMG_NAPI_CHECK_NULL_PTR(ctx.picture, result);
    if (OH_PictureNative_GetAuxiliaryPicture(ctx.picture, auxType, &ctx.auxPicture) != IMAGE_SUCCESS) {
...
static napi_value AuxiliaryPictureInfoSetTypeErrorCode(napi_env env, napi_callback_info info)
{
    ...
    IMG_NAPI_CHECK_NULL_PTR(ctx.auxInfo, result);
    if (OH_AuxiliaryPictureInfo_SetType(ctx.auxInfo, auxType) != IMAGE_BAD_PARAMETER) {
        return result;
    }
...
static napi_value AuxiliaryPictureInfoSetSizeErrorCode(napi_env env, napi_callback_info info)
{
    napi_value result = nullptr;
    napi_value argValue[NUM_1] = {0};
    size_t argCount = NUM_1;
    ...
    napi_get_undefined(env, &result);
    if (!IMG_IS_OK(napi_get_cb_info(env, info, &argCount, argValue, nullptr, nullptr)) || argCount < NUM_1) {
        LOGE("AuxiliaryPictureInfoSetSizeErrorCode: failed to get input arguments!");
        return result;
    }
    ...
    IMG_NAPI_CHECK_NULL_PTR(ctx.auxInfo, result);
    if (OH_AuxiliaryPictureInfo_SetSize(ctx.auxInfo, &size) != IMAGE_BAD_PARAMETER) {
        napi_throw_error(env, nullptr, "OH_AuxiliaryPictureInfo_SetSize failed");
        return result;
    }
...
static napi_value AuxiliaryPictureInfoSetRowStrideErrorCode(napi_env env, napi_callback_info info)
{
    napi_value result = nullptr;
    napi_value argValue[NUM_1] = {0};
    size_t argCount = NUM_1;
    ...
    napi_get_undefined(env, &result);
    if (!IMG_IS_OK(napi_get_cb_info(env, info, &argCount, argValue, nullptr, nullptr)) || argCount < NUM_1) {
        LOGE("AuxiliaryPictureInfoSetRowStrideErrorCode: failed to get input arguments!");
        return result;
    }
    ...
    IMG_NAPI_CHECK_NULL_PTR(ctx.auxInfo, result);
    if (OH_AuxiliaryPictureInfo_SetRowStride(ctx.auxInfo, rowStride) != IMAGE_BAD_PARAMETER) {
        napi_throw_error(env, nullptr, "OH_AuxiliaryPictureInfo_SetRowStride failed");
        return result;
    }
...
static napi_value AuxiliaryPictureInfoSetPixelFormatErrorCode(napi_env env, napi_callback_info info)
{
    napi_value result = nullptr;
    napi_value argValue[NUM_1] = {0};
    size_t argCount = NUM_1;
    ...
    napi_get_undefined(env, &result);
    if (!IMG_IS_OK(napi_get_cb_info(env, info, &argCount, argValue, nullptr, nullptr)) || argCount < NUM_1) {
        LOGE("AuxiliaryPictureInfoSetPixelFormatErrorCode: failed to get input arguments!");
        return result;
    }
    ...
    IMG_NAPI_CHECK_NULL_PTR(ctx.auxInfo, result);
    if (OH_AuxiliaryPictureInfo_SetPixelFormat(ctx.auxInfo, pixelFormat) != IMAGE_BAD_PARAMETER) {
        napi_throw_error(env, nullptr, "OH_AuxiliaryPictureInfo_SetPixelFormat failed");
        return result;
    }
...
static napi_value MetadataGetPropertyErrorCode(napi_env env, napi_callback_info info)
{
    ...
    Image_String imageKey = {key, keySize};
    Image_String imageValue = {nullptr, 0};
    errCode = OH_PictureMetadata_GetProperty(ctx.metadata, &imageKey, &imageValue);
...
static napi_value GetSingleMainPixelmapInfo(napi_env env, napi_callback_info info)
{
    napi_value result = nullptr;
    napi_get_undefined(env, &result);
    IMG_NAPI_CHECK_NULL_PTR(ctx.singlePicture, result);

    OH_PictureNative_GetMainPixelmap(ctx.singlePicture, &ctx.mainPixelmap);
    OH_Pixelmap_ImageInfo *imageInfo = nullptr;
    OH_PixelmapImageInfo_Create(&imageInfo); 
    Image_ErrorCode ret = OH_PixelmapNative_GetImageInfo(ctx.mainPixelmap, imageInfo);
...
static napi_value CreateSinglePictureByImageSource(napi_env env, napi_callback_info info)
{
    ...
    if (ctx.isImageSourceUpdated) {
        IMG_NAPI_RELEASE_PTR(ctx.singlePicture, OH_PictureNative_Release(ctx.singlePicture));
    } else {
        IMG_NAPI_NOT_NULL_PTR_RETURN(ctx.singlePicture, result);
    }

    IMG_NAPI_CHECK_NULL_PTR(ctx.imageSource, result);
    IMG_NAPI_CHECK_NULL_PTR(ctx.decodingOptions, result);
    if (OH_ImageSourceNative_CreatePicture(ctx.imageSource, ctx.decodingOptions, &ctx.singlePicture) != IMAGE_SUCCESS) {
        napi_throw_error(env, nullptr, "OH_ImageSourceNative_CreatePicture failed");
        return result;
    }
    ctx.isImageSourceUpdated = false;
...
static napi_value GetHdrPixelmapInfo(napi_env env, napi_callback_info info)
{
    ...
    IMG_NAPI_CHECK_NULL_PTR(ctx.singlePicture, result);
    OH_PixelmapNative *hdrPixelmap = nullptr;
    OH_PictureNative_GetHdrComposedPixelmap(ctx.singlePicture, &hdrPixelmap);
    OH_Pixelmap_ImageInfo *imageInfo = nullptr;
    OH_PixelmapImageInfo_Create(&imageInfo); 
...
static napi_value SetAuxiliarySinglePicture(napi_env env, napi_callback_info info)
{
    ...
    IMG_NAPI_CHECK_NULL_PTR(ctx.singlePicture, result);
    if (OH_PictureNative_SetAuxiliaryPicture(ctx.singlePicture, auxType, auxPicture) != IMAGE_SUCCESS) {
        napi_throw_error(env, nullptr, "OH_PictureNative_SetAuxiliaryPicture failed");
        return result;
    }
...
static napi_value GetGainmapPixelmapInfo(napi_env env, napi_callback_info info)
{
    ...
    Image_ErrorCode errorCode = OH_PictureNative_GetGainmapPixelmap(ctx.picture, &ctx.gainmapPixelmap);
    ...
    errorCode = OH_PixelmapNative_GetImageInfo(ctx.gainmapPixelmap, imageInfo);
    if(errorCode != IMAGE_SUCCESS){
...
static napi_value GetMainPixelmapInfo(napi_env env, napi_callback_info info)
{
    ...
    OH_PictureNative_GetMainPixelmap(ctx.picture, &ctx.mainPixelmap);
    OH_Pixelmap_ImageInfo *imageInfo = nullptr;
    OH_PixelmapImageInfo_Create(&imageInfo); 
    Image_ErrorCode ret = OH_PixelmapNative_GetImageInfo(ctx.mainPixelmap, imageInfo);
...
static napi_value CreateAuxiliaryPictureErrorCode(napi_env env, napi_callback_info info)
{
    ...
    if (ctx.auxPicture == nullptr) {
        errCode = OH_AuxiliaryPictureNative_Create(data, length, &imageSize, auxType, &ctx.auxPicture);
        if (errCode != IMAGE_SUCCESS) {
            napi_create_int32(env, errCode, &result);
            return result;
        }
...
static napi_value AuxiliaryPictureGetMetadataErrorCode(napi_env env, napi_callback_info info)
{
    ...
    IMG_NAPI_CHECK_NULL_PTR(ctx.auxPicture, result);
    OH_PictureMetadata *metadata = nullptr;
    Image_ErrorCode err = OH_AuxiliaryPictureNative_GetMetadata(ctx.auxPicture, metadataType, &metadata);
...
static napi_value PackToDataFromPictureErrorCode(napi_env env, napi_callback_info info)
{
    ...
    Image_ErrorCode errorcode =
        OH_ImagePackerNative_PackToDataFromPicture(ctx.imagePacker, ctx.packingOptions, ctx.picture, outData, &outSize);
...
static napi_value AuxiliaryPictureReadPixelsErrorCode(napi_env env, napi_callback_info info)
{
    ...
    IMG_NAPI_CHECK_NULL_PTR(ctx.auxPicture, result);
    Image_ErrorCode errorcode = OH_AuxiliaryPictureNative_ReadPixels(ctx.auxPicture, data, &length);
    napi_create_int32(env, errorcode, &result);
    return result;
}
...
static napi_value SetAuxiliaryPictureErrorCode(napi_env env, napi_callback_info info)
{
    ...
    IMG_NAPI_CHECK_NULL_PTR(ctx.picture, result);
    if (OH_PictureNative_SetAuxiliaryPicture(ctx.picture, auxType, auxPicture) != IMAGE_BAD_PARAMETER) {
        napi_throw_error(env, nullptr, "OH_PictureNative_SetAuxiliaryPicture failed");
        return result;
    }
...
static napi_value MetadataSetPropertyErrorCode(napi_env env, napi_callback_info info)
{
    ...
    Image_ErrorCode errCode = IMAGE_SUCCESS;
    errCode = OH_PictureMetadata_SetProperty(ctx.metadata, &imageKey, &imageValue);
    napi_create_int32(env, errCode, &result);
    return result;
}
...
static napi_value PictureSetMetadataErrorCode(napi_env env, napi_callback_info info)
{
    ...
    IMG_NAPI_CHECK_NULL_PTR(ctx.picture, result);
    Image_ErrorCode errCode = IMAGE_SUCCESS;
    errCode = OH_PictureNative_SetMetadata(ctx.picture, metadataType, metadata);
    napi_create_int32(env, errCode, &result);
    return result;
}
...
static napi_value PictureGetMetadataErrorCode(napi_env env, napi_callback_info info)
{
    ...
    OH_PictureMetadata *metadata = nullptr;
    Image_ErrorCode errCode = IMAGE_SUCCESS;
    errCode = OH_PictureNative_GetMetadata(ctx.picture, metadataType, &metadata);
    napi_create_int32(env, errCode, &result);
    return result;
}
...
static napi_value GetAuxiliaryPictureErrorCode(napi_env env, napi_callback_info info)
{
    ...
    IMG_NAPI_CHECK_NULL_PTR(ctx.picture, result);
    OH_AuxiliaryPictureNative *auxPicture = nullptr;
    if (OH_PictureNative_GetAuxiliaryPicture(ctx.picture, auxType, &auxPicture) != IMAGE_BAD_PARAMETER) {
        napi_throw_error(env, nullptr, "OH_PictureNative_GetAuxiliaryPicture failed");
        return result;
    }
...
static napi_value AuxiliaryPictureWritePixelsErrorCode(napi_env env, napi_callback_info info)
{
    ...
    IMG_NAPI_CHECK_NULL_PTR(ctx.auxPicture, result);
    if (OH_AuxiliaryPictureNative_WritePixels(ctx.auxPicture, data, length) != IMAGE_BAD_PARAMETER) {
        napi_throw_error(env, nullptr, "OH_AuxiliaryPictureNative_WritePixels failed");
        return result;
    }
...
static napi_value GetPixelmapBuffer(napi_env env, napi_callback_info info)
{
    ...
    Image_ErrorCode errorCode = OH_PixelmapNative_ReadPixels(ctx.pixelMap, data, &bufferSize);
    if(errorCode != IMAGE_SUCCESS)
    {
...
static napi_value AuxiliaryPictureSetInfoToCtx(napi_env env, napi_callback_info info)
{
    ...
    if (OH_AuxiliaryPictureNative_SetInfo(ctx.auxPicture, ctx.auxInfo) != IMAGE_SUCCESS) {
        napi_throw_error(env, nullptr, "OH_AuxiliaryPictureNative_SetInfo failed");
        return result;
    }
...
static napi_value SetNeedsPackPropertiesToTestPackingOptions(napi_env env, napi_callback_info info)
{
    ...
    IMG_NAPI_CHECK_NULL_PTR(ctx.packingOptions, result);
    if (OH_PackingOptions_SetNeedsPackProperties(ctx.packingOptions, needsPackProperties) != IMAGE_SUCCESS) {
        napi_throw_error(env, nullptr, "OH_PackingOptions_SetDesiredDynamicRange failed");
        return result;
    }
...
static napi_value GetImageSourceInfoSize(napi_env env, napi_callback_info info)
{
    ...
    int32_t index = 0;
    if (OH_ImageSourceNative_GetImageInfo(ctx.imageSource, index, imageInfo) != IMAGE_SUCCESS) {
        napi_throw_error(env, nullptr, "OH_PixelmapNative_GetImageInfo failed");
        return result;
    }
    uint32_t width = 0;
...
static napi_value Init(napi_env env, napi_value exports)
{
    napi_property_descriptor desc[] = {
        ...
        {"GetAuxiliaryPictureErrorCode", nullptr, GetAuxiliaryPictureErrorCode, nullptr, nullptr, nullptr, napi_default, nullptr},
        ...
        {"AuxiliaryPictureInfoSetTypeErrorCode", nullptr, AuxiliaryPictureInfoSetTypeErrorCode, nullptr, nullptr,
            nullptr, napi_default, nullptr},
        {"AuxiliaryPictureInfoSetSizeErrorCode", nullptr, AuxiliaryPictureInfoSetSizeErrorCode, nullptr, nullptr,
            nullptr, napi_default, nullptr},
        {"AuxiliaryPictureInfoSetRowStrideErrorCode", nullptr, AuxiliaryPictureInfoSetRowStrideErrorCode, nullptr,
            nullptr, nullptr, napi_default, nullptr},
        {"AuxiliaryPictureInfoSetPixelFormatErrorCode", nullptr, AuxiliaryPictureInfoSetPixelFormatErrorCode, nullptr,
            nullptr, nullptr, napi_default, nullptr},
        ...
        {"AuxiliaryPictureSetInfoToCtx", nullptr, AuxiliaryPictureSetInfoToCtx, nullptr, nullptr, nullptr, napi_default,
            nullptr},
        {"setNeedsPackPropertiesToTestPackingOptions", nullptr, SetNeedsPackPropertiesToTestPackingOptions, nullptr, nullptr,
            nullptr, napi_default, nullptr},

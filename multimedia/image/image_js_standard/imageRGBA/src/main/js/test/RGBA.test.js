...
describe('Image', function () {
    const { RGBA_F16, ALPHA_8, RGB_565, ARGB_8888, BGRA_8888, UNKNOWN, RGB_888 } = image.PixelMapFormat;
    beforeAll(function () {
    ...
    */
     ...
     * @tc.level     : Level 0
     */
    it('RGBA_017', 0, async function (done) {
        const Color = new ArrayBuffer(96);
        let opts = { editable: true, pixelFormat: RGB_888, size: { height: 4, width: 6 } }
        image.createPixelMap(Color, opts)
            .then(pixelmap => {
                console.log('RGBA_017 pixelFormat: 5');
                expect(pixelmap != undefined).assertTrue();
                expect(pixelmap.isEditable == opts.editable).assertTrue();
                done();
            })
            .catch(error => {
                console.log('RGBA_017 err: ' + error);
                expect(false).assertTrue();
                done();
            })
    })
     ...
     * @tc.level     : Level 0
    */
    it('RGBA_018', 0, async function (done) {
        const Color = new ArrayBuffer(96);
        let opts = { editable: true, pixelFormat: RGB_888, size: { height: 4, width: 6 } }
        image.createPixelMap(Color, opts, (err, pixelmap) => {
            if (err != undefined) {
                console.log('RGBA_018 err: ' + err);
                expect(false).assertTrue();
                done();
            }
            console.log('RGBA_018 pixelFormat: 5');
            expect(pixelmap != undefined).assertTrue();
            expect(pixelmap.isEditable == opts.editable).assertTrue();
            done();
        })

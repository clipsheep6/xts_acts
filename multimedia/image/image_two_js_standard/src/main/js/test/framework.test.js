...
import {testPng, testJpg, base64Image, scale2x1, Translate3x1, Rotate90, FlipH,testBmp, testGif, Crop3x3, scale1x4, SetAlpha0_8, Translate1x3} from './testImg'
     ...
     it('frmwk_018', 0, async function (done) {
        ...
        try {
            ...
            if (pixelMap != undefined) {
                ...
                var bufferArr2 = new Uint8Array(readBuffer);
                for (var i = 0; i < bufferArr2; i++ ) {
                    if(bufferArr2[i] != SetAlpha0_8[i]) {
                        logger.log(`pixel[${i}] current[${bufferArr2[i]}] target[${SetAlpha0_8[i]}]`);
                        expect(false).assertTrue();
                        done();
                    }
                }

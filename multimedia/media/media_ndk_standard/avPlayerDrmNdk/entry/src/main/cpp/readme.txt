...
#define LICENSE_SERVICE_IP "http://license.dev.trustdta.com:8080/drmproxy/v3/getLicense"
...
int getHttp(std::string url, unsigned char *request, uint32_t requestLen, unsigned char *response,
    int32_t *responseLen, uint32_t timeout)
{
    int ret = -1;
    CURL *curl = curl_easy_init();
    if (curl == nullptr) {
        OH_LOG_DEBUG(LOG_APP,"testCL_curl_easy_init failed\n");
        return -1;
    }
    // url
    ...
    if (res != CURLE_OK) {
        OH_LOG_ERROR(LOG_APP,"[testCL_error] curl_easy_perform failed, ret = %{public}d",res);
        ret = -1;
    }
    long resCode;
    curl_easy_getinfo(curl, CURLINFO_RESPONSE_CODE, &resCode);
    if (resCode != CURLEASYGETINFO) {
        OH_LOG_ERROR(LOG_APP,"[testCL_error] curl_easy_getinfo failed ret = %{public}ld",resCode);
        ret = -1;
    }
    memcpy(response, tempStr.c_str(), tempStr.size());
...
static int GenDeviceCertificate(MediaKeySystem *drmKeySystem) {
    ...
    Drm_ErrCode ret1 =
        OH_MediaKeySystem_GenerateKeySystemRequest(drmKeySystem, request, &requestLen, defaultUrl, defaultUrlLen);
    unsigned char response[DATA_BUFF_LEN] = {
        0x31, 0x64, 0x6E, 0x5A, 0x32, 0x4E, 0x57, 0x74, 0x76, 0x4D, 0x47, 0x34, 0x34, 0x4E, 0x6A, 0x42, 0x30,
        0x4D, 0x32, 0x77, 0x33, 0x4E, 0x67, 0x3D, 0x3D, 0x3A, 0x59, 0x7A, 0x56, 0x78, 0x63, 0x48, 0x64, 0x70,
        0x61, 0x6D, 0x30, 0x34, 0x59, 0x57, 0x45, 0x34, 0x5A, 0x48, 0x6B, 0x79, 0x4D, 0x67, 0x3D, 0x3D};
    int32_t responseLen = DATA_BUFF_LEN;
    if (strcmp(judge_uuid(), YELLOW_AREA)) {
//         int ret = getHttp(PROVISION_SERVICE_IP, PROVISION_SERVICE_PORT, "/getProvision",
//                           reinterpret_cast<const char *>(request), requestLen, response, (unsigned int *)&responseLen);
        int ret = getHttp(defaultUrl, request, (uint32_t)requestLen, response, &responseLen, HTTP_TIMEOUT);
        OH_LOG_INFO(LOG_APP, "xx ret http is %{public}d",ret);
        OH_LOG_INFO(LOG_APP, "xx ret defaultUrl is %{public}s",defaultUrl);
        if (ret != 0) {
            return -1;
        }
    } else {
...
//         int ret3 = getHttp(LICENSE_SERVICE_IP,reinterpret_cast<const char *>(requestData.data), 
//         requestData.dataLen, response,(unsigned int *)&responseLen);
        int ret3 = getHttp(LICENSE_SERVICE_IP,requestData.data, (uint32_t)requestData.dataLen, 
        response, &responseLen,HTTP_TIMEOUT);
        CHECK_RETURN((ret3 != 0), -1);
    }
    Drm_ErrCode ret2 =
        OH_MediaKeySession_ProcessMediaKeyResponse(mediaKeySession, response, responseLen, keyId, &keyIdLen);
...
void testMediaKeySystemInfo(OH_AVPlayer *play, DRM_MediaKeySystemInfo *mediaKeySystemInfo) {
    OH_LOG_INFO(LOG_APP, "xx ret testMediaKeySystemInfo start");
    ...
    OH_LOG_INFO(LOG_APP, "xx ret testMediaKeySystemInfo end");
    IsCallBack = 0x55;
}
static napi_value Test_AVPlayer_SetMediaKeySystemInfoCallback(napi_env env, napi_callback_info info) {
    ...
    OH_LOG_INFO(LOG_APP, "xx ret fileDescribe is %{public}d",fileDescribe);
    
    int64_t fileSize = GetFileSize(PATH);
    OH_LOG_INFO(LOG_APP, "xx ret filesize is %{public}d",fileSize);
...
static napi_value Test_AVPlayer_GetMediaKeySystemInfo(napi_env env, napi_callback_info info) {
    ...
    OH_LOG_INFO(LOG_APP, "xx ret IsCallBack is %{public}d",IsCallBack);
    if (IsCallBack == 0x55) {
        IsCallBack = 0;
    ...
    OH_LOG_INFO(LOG_APP, "xx ret6 is %{public}d",ret6);
...
static napi_value Test_AVPlayer_SetDecryptionConfig(napi_env env, napi_callback_info info) {
    ...
    OH_LOG_INFO(LOG_APP, "xx ret7 is %{public}d",ret7);
    OH_AVErrCode ret8 = OH_AVPlayer_Prepare(playerHandle);
    OH_LOG_INFO(LOG_APP, "xx ret8 is %{public}d",ret8);
    OH_AVErrCode ret9 = OH_AVPlayer_Play(playerHandle);
    OH_LOG_INFO(LOG_APP, "xx ret9 is %{public}d",ret9);
    OH_AVErrCode ret10 = OH_AVPlayer_Release(playerHandle);
    OH_LOG_INFO(LOG_APP, "xx ret10 is %{public}d",ret10);

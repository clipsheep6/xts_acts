...
constexpr uint32_t LONGTIME = 8*3600;
...
/**
 * @tc.number    : OH_EFFECTMANAGER_SR2_STABLITY_TEST_001
 * @tc.name      : GetEffectInfoArray LongTime Test
 * @tc.desc      : Stablity test
 */
HWTEST_F(EffectManagerLongTimeTest, OH_EFFECTMANAGER_SR2_STABLITY_TEST_001, TestSize.Level2)
{
	...
	while (difftime(curTime, startTime) < LONGTIME)
	{
		...
		cout << "SR2_STABLITY_TEST_001 is run:" <<difftime(curTime, startTime) << "second"<<endl;
	}
 ...
 * @tc.desc      : Stablity test
 */
HWTEST_F(EffectManagerLongTimeTest, OH_EFFECTMANAGER_SR2_STABLITY_TEST_002, TestSize.Level2)
{
	ContentType contentType = ContentType::CONTENT_TYPE_MUSIC;
	StreamUsage streamUsage = StreamUsage::STREAM_USAGE_MEDIA;
	...
	while (difftime(curTime, startTime) < LONGTIME)
	{
		...
		curTime = time(NULL);
		cout << "SR2_STABLITY_TEST_002 is run:" <<difftime(curTime, startTime) << "second" << endl;
	}
...
/**
 * @tc.number    : OH_EFFECTMANAGER_SR2_STABLITY_TEST_003
 * @tc.name      : Get AudioEffectInfoArray LongTime Test
 * @tc.desc      : Stablity test
 */
HWTEST_F(EffectManagerLongTimeTest, OH_EFFECTMANAGER_SR2_STABLITY_TEST_003, TestSize.Level2)
{	
	time_t startTime = time(NULL);
    time_t curTime = time(NULL);
	ContentType contentType = ContentType::CONTENT_TYPE_MUSIC;
	...
	while (difftime(curTime, startTime) < LONGTIME)
	{
		...
		curTime = time(NULL);
		cout << "SR2_STABLITY_TEST_003 is run:" <<difftime(curTime, startTime) << "second"<<endl;

...
describe('VideoRecorderSpliterPromiseStabilityTest', function () {
    let fdPath;
    let fileAsset;
    let fdNumber = null;
    ...
    async function nextStep(done) {
        ...
        switch (mySteps[0]) {
            ...
            case GET_NEW_PATH_STATE:
                await toIniFdData(VIDEO_SUB_STR + recodeCount + '.mp4');
                checkNextStep(done);
                break;
    ...
    /* *
       ...
       * @tc.desc      : set sequentfile when max size is approaching
       * @tc.size      : MediumTest
       * @tc.type      : Stability test
       * @tc.level     : Level0
       */

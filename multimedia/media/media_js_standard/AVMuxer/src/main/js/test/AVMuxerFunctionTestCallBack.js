...
describe('AVMuxerFunctionTestCallBack', function () {
    ...
    let concurrentWriteTrackSample_Event = 'concurrentWriteTrackSample';
    let stop_Event = 'stop';
    ...
    function toNextStep(testAVMuxer, steps, done) {
        if (steps.length == 0) {
            ...
            if (steps[FIRST_INDEX] == createAVMuxer_Event) {
                eventEmitter.emit(steps[FIRST_INDEX], steps, done);
            } else if (steps[FIRST_INDEX] == audioPlay_Event || steps[FIRST_INDEX] == videoPlay_Event) {
    ...
    eventEmitter.on(concurrentWriteTrackSample_Event, (testAVMuxer, steps, done) => {
        ...
        && videoFrameCount <= parameters.videoFrameSize.length) {
            testAVMuxer.writeTrackSample(data, info, (err, data) => {
                ...
                if (err != undefined) {
                    ...
                    eventEmitter.emit(concurrentWriteTrackSample_Event, testAVMuxer, steps, done);
                }
            })
        } else if (audioFrameCount >= parameters.audioFrameSize.length
        && videoFrameCount < parameters.videoFrameSize.length) {
    ...
    */
    it('SUB_MEDIA_AVMUXER_FUNCTION_CALLBACK_1600', 0, async function (done) {
        ...
        let steps = [createAVMuxer_Event, setOutput_Event, audioAddTrack_Event, videoAddTrack_Event, start_Event,
        concurrentWriteTrackSample_Event,'audio', stop_Event, release_Event, videoPlay_Event];
    ...
    */
    it('SUB_MEDIA_AVMUXER_FUNCTION_CALLBACK_1700', 0, async function (done) {
        ...
        let steps = [createAVMuxer_Event, setOutput_Event, audioAddTrack_Event, videoAddTrack_Event, start_Event,
        concurrentWriteTrackSample_Event, 'video',stop_Event, release_Event, videoPlay_Event];

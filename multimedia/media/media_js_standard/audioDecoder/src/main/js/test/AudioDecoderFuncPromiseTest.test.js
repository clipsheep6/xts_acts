...
describe('AudioDecoderFuncPromiseTest', function () {
    ...
    */ 
    ...
    it('SUB_MEDIA_AUDIO_DECODER_FUNCTION_PROMISE_00_0100', 0, async function (done) {
        console.log("set EOS after last frame and reset");
    ...
    /* *
        * @tc.number    : SUB_MEDIA_AUDIO_DECODER_FUNCTION_PROMISE_00_0100
        * @tc.name      : 001.set EOS manually before last frame and reset
        * @tc.desc      : basic decode function
        * @tc.size      : MediumTest
        * @tc.type      : Function test
        * @tc.level     : Level1
    */ 
    ...
    it('SUB_MEDIA_AUDIO_DECODER_FUNCTION_PROMISE_01_0100', 0, async function (done) {
        console.log("set EOS manually before last frame and reset");
        let mediaDescription = {
                    "channel_count": 2, 
                    "sample_rate": 44100,
                    "audio_raw_format":4,
    ...
    */ 
    ...
    it('SUB_MEDIA_AUDIO_DECODER_FUNCTION_PROMISE_01_0200', 0, async function (done) {
        ...
        await sleep(5000).then(() => {
            audioDecodeProcessor.flush().then(() => {console.log("flush after 5s")}, failCallback).catch(failCatch);
        });
    })
    ...
    */ 
    ...
    it('SUB_MEDIA_AUDIO_DECODER_FUNCTION_PROMISE_01_0400', 0, async function (done) {
        ...
        await sleep(5000).then(() => {audioDecodeProcessor.start().then(() => {console.log("stop after 5s success")}, failCallback).catch(failCatch);});
    ...
    */ 
    ...
    it('SUB_MEDIA_AUDIO_DECODER_FUNCTION_PROMISE_01_0500', 0, async function (done) {
        ...
        await sleep(5000).then(() => {
            console.log("stop decoding after 5s");
        });     
        await audioDecodeProcessor.stop().then(() => {console.log("stop after 5s success")}, failCallback).catch(failCatch);

        await sleep(2000).then(() => {
            console.log("restart decoding after 2s");
            resetParam();
            readFile(AUDIOPATH);
        });   
    ...
    */ 
    ...
    it('SUB_MEDIA_AUDIO_DECODER_FUNCTION_PROMISE_01_0600', 0, async function (done) {
        ...
        await audioDecodeProcessor.configure(mediaDescription2).
        then(() => {console.log("configure 2 success"); resetParam(); readFile(AUDIOPATH2)}, failCallback).catch(failCatch);

        SAVEPATH = '/data/media/promise0601.txt'
        workdoneAtEOS = true;
        ES = [0, 287, 223, 191, 205, 203, 217, 213, 211, 197, 207, 213, 197, 201, 212, 213, 207, 261, 211, 200, 200, 194, 199, 213, 221, 237, 205, 242, 195, 207, 181, 200, 195, 217, 202, 212, 202, 182, 212, 214, 217, 183, 200, 251, 208, 214, 218, 209, 214, 200, 186, 200, 196, 241, 189, 204, 206, 200, 201, 207, 212, 204, 205, 201, 214, 216, 200, 192, 215, 213, 219, 213, 215, 216, 205, 194, 216, 204, 224, 202, 210, 211, 211, 196, 193, 223, 209, 207, 208, 218, 226, 198, 183, 200, 217, 199, 203, 204, 212, 221, 197, 199];
        ES_LENGTH = 100;

        setCallback(audioDecodeProcessor, done);

        console.log("start prepare");
        await audioDecodeProcessor.prepare().then(() => {console.log("prepare2 success")}, failCallback).catch(failCatch);

        console.log("start decoding");
        await audioDecodeProcessor.start().then(() => {console.log("start2 success")}, failCallback).catch(failCatch);
    })
    ...
    */ 
    ...
    it('SUB_MEDIA_AUDIO_DECODER_FUNCTION_PROMISE_01_0700', 0, async function (done) {
        console.log("case test reconfigure codec for new file with different formats");
        let mediaDescription = {
                    "channel_count": 2, 
                    "sample_rate": 44100,
                    "audio_raw_format":4,
        ...
        await sleep(10000).then(() => {
            console.log("start createaudiodecoder 2");
        });       
        
        await media.createAudioDecoderByMime('audio/flac')
        .then((processor) => {
            console.log("create createAudioDecoder flac success"); 
            audioDecodeProcessor = processor;
        }, failCallback).catch(failCatch); 
        ...
        workdoneAtEOS = true;
        ES = [0, 2116, 2093, 2886, 2859, 2798, 2778, 2752, 2752, 2754, 2720, 2898, 2829, 2806, 2796, 2786, 2774, 2758, 2741, 3489, 3342, 3272, 3167, 3048, 3060, 2919, 2839, 2794, 2770, 2763, 2756, 2722, 2950, 2876, 2842, 2827, 2792, 3121, 3075, 2969, 3240, 3199, 3082, 2975, 2895, 3135, 3024, 2970, 2902, 2860, 2802, 2778, 2779, 2963, 2893, 2870, 2831, 2800, 2762, 2752, 2760, 2759, 2726, 2889, 2960, 2876, 3223, 3040, 3006, 3042, 2980, 3097, 3069, 3215, 3365, 3296, 3653, 3480, 3300, 3140, 3014, 2975, 2952, 2896, 2897, 2842, 2929, 3567, 3929, 3485, 3082, 3625, 4953, 5239, 5094, 4993, 4821, 4431, 5195, 5542, 5557, 4894]
        ES_LENGTH = 100;

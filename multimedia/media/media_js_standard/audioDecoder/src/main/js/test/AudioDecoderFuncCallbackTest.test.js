...
describe('AudioDecoderFuncCallbackTest', function () {
    ...
    async function dequeueAllOutputs(audioDecodeProcessor, queue, done){
        while (queue.length > 0 && !sawOutputEOS){
            ...
            if (outputobject.flags == 1){
                sawOutputEOS = true;
                if (stopAtEOS){
                    audioDecodeProcessor.stop((err) => {expect(err).assertUndefined();console.log("stop success");})}
                if (resetAtEOS){
    ...
    */ 
    ...
    it('SUB_MEDIA_AUDIO_DECODER_FUNCTION_CALLBACK_01_0500', 0, async function (done) {
        console.log("test stop and restart");
        let events = require('events');
        let eventEmitter = new events.EventEmitter();
        let mediaDescription = {
            "channel_count": 2, 
    ...
    */ 
    ...
    it('SUB_MEDIA_AUDIO_DECODER_FUNCTION_CALLBACK_01_0600', 0, async function (done) {
        console.log("case test reconfigure for new file with different formats");
        let events = require('events');
        let eventEmitter = new events.EventEmitter();
        let mediaDescription = {
            "channel_count": 2, 

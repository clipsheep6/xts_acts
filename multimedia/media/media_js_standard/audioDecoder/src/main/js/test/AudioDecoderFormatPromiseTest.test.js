...
describe('AudioDecoderFormatTest', function () {
    ...
    async function enqueueAllInputs(audioDecodeProcessor, queue, done){
        ...
        while (queue.length > 0 && !sawInputEOS){
            ...
            dequeInputCnt += 1;
            if (i == ES_LENGTH + 1){
                console.info("input last frame, flag=1")
                inputobject.flags = 1;  
                ...
                if (i==1 && isVorbis){
                    ...
                    console.info("write mp3 frame to buffer success");
                    inputobject.flags = 0;
                }
                else{
                    inputobject.length = ES[i] - FREAM_HEAD_BYTE;  
                    getContent(inputobject.data, ES[i]);
                    console.info("write frame to buffer success");
                    inputobject.flags = 0;
                }
            }
    ...
    async function dequeueAllOutputs(audioDecodeProcessor, queue, done){
        while (queue.length > 0 && !sawOutputEOS){
            ...
            if (outputobject.flags == 1){
                sawOutputEOS = true;
                if (workdoneAtEOS) {
                    await audioDecodeProcessor.stop().then(() => {}, failCallback).catch(failCatch);
    ...
    */ 
    ...
    it('SUB_MEDIA_AUDIO_DECODER_FORMAT_PROMISE_01_0200', 0, async function (done) {
        ...
        ES = [0, 2116, 2093, 2886, 2859, 2798, 2778, 2752, 2752, 2754, 2720, 2898, 2829, 2806, 2796, 
            ...
            3082, 3625, 4953, 5239, 5094, 4993, 4821, 4431, 5195, 5542, 5557, 4894, 4414, 5085, 5496, 
            5089, 4459, 4532, 4778, 5460, 5372, 5224, 4395, 4701, 5512, 5304, 4798, 7128, 6534, 5704, 
            5562, 5539, 5675, 5780, 6058, 6194, 6076, 5971, 5758, 5202, 4438, 3821, 4267, 5550, 4896, 
            3841, 5166, 4904, 4504, 5530, 5354, 5047, 5695, 5422, 5168, 5338, 5154, 5385, 5322, 4934, 
            4512, 4001, 3793, 3624, 3589, 3562, 4346, 5407, 5582, 5645, 5581, 5605, 5520, 5399, 5164, 
            4991, 4556, 4058, 3662, 3757, 4627, 5183, 5061, 4114, 5840, 5051, 5126, 4573, 5269, 4437, 
            5021, 5508, 5289, 5171, 5153, 5118, 5721, 5375, 4863, 6353, 5477, 5608, 5614, 5737, 5178, 
            4614, 4671, 5204, 5027, 4977, 3922, 5904]
        ES_LENGTH = 200;
    ...
    */ 
    ...
    it('SUB_MEDIA_AUDIO_DECODER_FORMAT_PROMISE_01_0400', 0, async function (done) {
        ...
        needgetOutputMediaDescription = true;
        samplerate = 48;
        SAVEPATH = SAVEPATH + 'vorbisPromise0100.txt';
        isVorbis = true;
        isMp3 = false;

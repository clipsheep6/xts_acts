...
describe('videoEncoderFuncParameterPromise', function () {
    ...
    */
    it('SUB_MEDIA_VIDEO_SOFTWARE_ENCODER_FUNCTION_PAR_PROMISE_00_0100', 0, async function (done) {
        ...
        await toPrepare();
        await toSetParameter(parameterDescription);
        toStartStream();
        await toStart();
    })
    ...
    */
    it('SUB_MEDIA_VIDEO_SOFTWARE_ENCODER_FUNCTION_PAR_PROMISE_01_0700', 0, async function (done) {
        ...
        let parameterDescriptionTwo = {
            "codec_profile": 33,
            "video_encode_birate_mode": '',
        }
        let savePath2 = BASIC_PATH + '0601.h264';
        toSetStreamParam(width, height, frameRate, frameTotal);
        await toConfigure(parameterDescription, savePath2);
        setCallback(done);
        await toGetInputSurface();
        await toPrepare();
        await toSetParameter(parameterDescriptionTwo);
        toStartStream();
        await toStart();
    })

...
describe('VideoRecorderSpliterCallbackTest', function () {
    ...
    const FILE_SUFFIX = 'fc.mp4';
    const VIDEO_PER_STR = 'VideoSpliterFuncCallback';
    ...
    const SET_SOURCE_ONLY_VIDEO_STATE = 18;
    ...
    function setSourceToAudioVideo() {
        console.info('setSourceToAudioVideo is called');
    ...
    function setSourceToOnlyVideo() {
        console.info('setSourceToOnlyVideo is called');
        videoProfile = MEDIA_PROFILE.onlyVideoProfile;
        videoConfig = MEDIA_CONFIG.onlyVideoConfig;
        videoConfig.maxSize = 0;
        videoConfig.maxDuration = 0;
        trackType = 'only_video';
    }
    ...
    function checkNextStep(done) {
        console.info('yf---222;mySteps: ' + JSON.stringify(mySteps));
        mySteps.shift();
        if (mySteps.length == 0) {
    ...
    async function nextStep(done) {
        ...
        switch (mySteps[0]) {
            ...
            case SET_NEW_STREAM_STATE:
                ...
                checkNextStep(done);
                break;
            case SET_SOURCE_ONLY_VIDEO_STATE:
                setSourceToOnlyVideo();
                checkNextStep(done);
                break;
            default:
            ...
            if (!!err) {
                ...
                videoRecorder.getInputSurface((err, outPutSurface) => {
                    if (!!err) {
                        ...
                        toStartStream();
                        console.info('yf---isStreamRunning: ' + isStreamRunning);
                        checkNextStep(done);
                        console.info('yf---11111');
                    }
                });
            }
        });
    ...
    async function toCheckVideo(done) {
        await spliterTestBase.toCheckVideo(done, videoArr, videoProfile, playerSurfaceId, trackType);
        checkNextStep(done);
    }
    ...
    */
    ...
    /* *
        ...
        * @tc.desc      : start->stop->reset->prepare-start,then setsequentfile when max size is approaching
        * @tc.size      : MediumTest
        * @tc.type      : Function
        * @tc.level     : Level0
    */
    ...
    */
    ...
    /* *
        ...
        * @tc.desc      : start，then stop when max size is approaching
        * @tc.size      : MediumTest
        * @tc.type      : Function
        * @tc.level     : Level0
    */
    ...
    */
    it('SUB_MEDIA_VIDEO_RECORDER_SPLITER_CALLBACK_TEST_1600', 0, async function (done) {
        ...
        mySteps = [
            ...
            CHECK_PRE_FILE_STATE, APPROACHING_SIZE_STATE, SET_SUB_STATE, WAIT_STATE,
            CHECK_PRE_FILE_STATE, REACHED_SIZE_STATE, CHECK_FILE_STATE, IGNORE_SIZE_STATE,
            STOP_STATE, RELEASE_STATE, CHECK_VIDEO_STATE, SET_SOURCE_ONLY_VIDEO_STATE, END_STATE
        ];

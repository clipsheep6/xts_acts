...
#define FAKE_POINTER(addr) (reinterpret_cast<uintptr_t>(POINTER_MASK & reinterpret_cast<uintptr_t>(addr)))
...
void NdkAVPlayerUser::OnSpeedDoneCb(OH_AVPlayer *player, OH_AVFormat *infoBody)
{
    ...
    this->speed = static_cast<AVPlaybackSpeed>(value);
    LOG("AV_INFO_TYPE_SPEEDDONE value: %d", value);
}
...
void NdkAVPlayerUser::OnBitRateDoneCb(OH_AVPlayer *player, OH_AVFormat *infoBody)
{
    ...
    this->bitRateDone = static_cast<uint32_t>(value);
    LOG("AV_INFO_TYPE_BITRATEDONE value: %d", value);
}
...
void NdkAVPlayerUser::OnBufferingUpdateCb(OH_AVPlayer *player, OH_AVFormat *infoBody)
{
    ...
    this->bufferType = static_cast<AVPlayerBufferingType>(value);
...
void NdkAVPlayerUser::OnBitRateCollectedCb(OH_AVPlayer *player, OH_AVFormat *infoBody)
{
    ...
    if (this->bitRatesSize <= 0 || bitRates == nullptr) {
    ...
    for (size_t i = 0; i < this->bitRatesSize; i++) {
        this->bitRates[i] = reinterpret_cast<uint32_t *>(bitRates)[i];
    ...
    void *userData)
{
    ...
    if (user->onInfoFuncs_.count(type) > 0) {
        ...
        LOGD("0x%06x" " OnInfo: no member func, type %d", FAKE_POINTER(user), type);
    }
    return;
}

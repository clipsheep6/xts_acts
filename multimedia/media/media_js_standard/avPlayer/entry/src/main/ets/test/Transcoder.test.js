...
export default function AVTransCoderTest() {
  describe('AVTransCoderTest', function () {
    ...
    afterEach(async function () {
      ...
      await mediaTestBase.closeFd(inputFd?.fdNumber);
      await mediaTestBase.closeFd(outputFd?.fdNumber);
    })
    ...
    eventEmitter.on(SET_SRC_AND_DST, (avTranscoder, config, pauseTime, steps, done) => {
        steps.shift();
        try{
            setfdScrAndfdDst(avTranscoder, config, pauseTime, steps, done);
    ...
    async function setfdScrAndfdDst(avTranscoder, config, pauseTime, steps, done) {
        inputFdObject = await mediaTestBase.getFileDescriptor(inputName);
        outputFdObject = await mediaTestBase.getFd(outputName);
        avTranscoder.fdSrc = inputFdObject;
    ...
    function setCallback(avTranscoder, config, pauseTime, steps, done) {
      ...
      avTranscoder.on('complete', async () => {
        ...
        expect(steps[0]).assertEqual(RELEASEAVTRANSCODER_PROMISE_EVENT);
        toNextStep(avTranscoder, config, pauseTime, steps, done);
      });
    }
    ...
    */
    it('SUB_MULTIMEDIA_AVTANSCODER_0100', 0, async function (done) {
      ...
      eventEmitter.emit(mySteps[0], avTranscoder, avTranscoderConfig, recorderTime, mySteps, done);

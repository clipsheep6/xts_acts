...
describe('VideoEncoderFuncParameterCallback', function () {
    ...
    eventEmitter.on('setParameter', (mediaDescription, done) => {
        console.info('setParameter parameter: ' + JSON.stringify(parameter));
        videoEncodeProcessor.setParameter(parameter, (err) => {
            ...
            setTimeout(() => {
                eventEmitter.emit('start', mediaDescription, done);
            }, delayTime);
        });
    });
    ...
    eventEmitter.on('reset_for_CALLBACK_0700', async (done) => {
        ...
        parameter.codec_profile = media.AVCProfile.AVC_PROFILE_HIGH;
        parameter.video_encode_bitrate_mode = media.VideoEncodeBitrateMode.CBR;
    ...
    */
    it('SUB_MEDIA_VIDEO_SOFTWARE_ENCODER_FUNCTION_PAR_CALLBACK_0100', 0, async function (done) {
        ...
        await getFdWrite(decPath);
        parameter.codec_profile = media.AVCProfile.AVC_PROFILE_HIGH;
        parameter.video_encode_bitrate_mode = media.VideoEncodeBitrateMode.VBR;
    ...
    */
    it('SUB_MEDIA_VIDEO_SOFTWARE_ENCODER_FUNCTION_PAR_CALLBACK_0200', 0, async function (done) {
        ...
        let action= 'stop';
        await getFdWrite(decPath);
        parameter.codec_profile = media.AVCProfile.AVC_PROFILE_BASELINE;
        parameter.video_encode_bitrate_mode = media.VideoEncodeBitrateMode.VBR;
        finalFrameId = 50;
    ...
    */
    it('SUB_MEDIA_VIDEO_SOFTWARE_ENCODER_FUNCTION_PAR_CALLBACK_0400', 0, async function (done) {
        ...
        let action = 'stop';
        await getFdWrite(decPath);
        parameter.codec_profile = media.AVCProfile.AVC_PROFILE_MAIN;
        parameter.video_encode_bitrate_mode = media.VideoEncodeBitrateMode.CBR;
        finalFrameId = 50;
        reStart = true;
        toCreateByName(name, action, mediaDescription, done);

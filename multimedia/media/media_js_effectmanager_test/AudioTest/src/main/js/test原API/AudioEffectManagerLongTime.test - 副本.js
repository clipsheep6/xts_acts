...
export default function effectManagerLongtime() {
    ...
    describe('effectManagerLongtime', function () {
		...
		/**
		 *@tc.number	: OH_EFFECTMANAGER_SR2_STABLITY_TEST_002
		 *@tc.name		: EFFECTMANAGER_SR2_STABLITY_TEST
		 ...
		 *@tc.level 	: Level 1
		 */
		it('OH_EFFECTMANAGER_SR2_STABLITY_TEST_002', 1, function (done) {
			console.info(TAG + 'OH_EFFECTMANAGER_SR2_STABLITY_TEST_002')
			let i = 0;
			let timer = setInterval(async function() {
				if(i < LONGTIME){
					await audioSteamManager.getAudioEffectInfoArray(contentType,streamUsage,async (err,AudioSceneEffectInfoArray) =>{
						if(err){
							clearInterval(timer);
							console.log(TAG +(err));
							console.log(TAG +"OH_EFFECTMANAGER_SR2_STABLITY_TEST_002 async failed");
							console.log(TAG+(i));
				        	expect().assertFail();
							done();
						}
						console.log(TAG +(AudioSceneEffectInfoArray))
						if(AudioSceneEffectInfoArray[0] != audio.AudioEffectMode.EFFECT_NONE && AudioSceneEffectInfoArray[1] != audio.AudioEffectMode.EFFECT_DEFAULT)
	                    {
							clearInterval(timer);
	                      	console.log(TAG+(i));
		 ...
		 *@tc.level 	: Level 1
		 */
		it('OH_EFFECTMANAGER_EFFECTINFO_FUZZ_TEST_002', 1, function (done) {
		...
		/**
		 *@tc.number	: OH_EFFECTMANAGER_EFFECTINFO_FUZZ_TEST_003
		 *@tc.name		: EFFECTMANAGER_SR2_STABLITY_TEST
		 *@tc.desc		: getAudioEffcetInfoArray callback longtime test
		 *@tc.size		: MEDIUM
		 *@tc.type		: Function
		 *@tc.level 	: Level 1
		 */
		it('OH_EFFECTMANAGER_EFFECTINFO_FUZZ_TEST_003', 1, function (done) {
			console.info(TAG + 'OH_EFFECTMANAGER_EFFECTINFO_FUZZ_TEST_003')
			let random1 = Math.floor(Math.random()*65535);
			let random2 = Math.floor(Math.random()*65535);
			let Type = generateMixed(random1);
			let Usage = generateMixed(random2);			
			...
			let timer = setInterval(async function() {
				if(i < LONGTIME){
					await audioSteamManager.getAudioEffectInfoArray(Type,Usage,async (err,AudioSceneEffectInfoArray) =>{
						if(err){
							//console.log(TAG +"OH_EFFECTMANAGER_EFFECTINFO_FUZZ_TEST_003 async failed");
							//console.log(TAG+(i));
							clearInterval(timer);
							console.log(TAG +(err));
							console.log(TAG +"OH_EFFECTMANAGER_EFFECTINFO_FUZZ_TEST_003 failed");
							//expect().assertFail();
							//done();
						}
					});
					i = i+IntervalTime;
					console.log(TAG +"OH_EFFECTMANAGER_EFFECTINFO_FUZZ_TEST_003: " + i);
				} else {
					clearInterval(timer);
					expect(true).assertTrue();
					done();	
		 ...
		 *@tc.level 	: Level 1
		 */
		it('OH_EFFECTMANAGER_API_STABLITY_TEST_003', 1, function (done) {
		...
		/**
		 *@tc.number	: OH_EFFECTMANAGER_SR2_STABLITY_TEST_006
		 *@tc.name		: EFFECTMANAGER_SR2_STABLITY_TEST
		 *@tc.desc		: getAudioEffcetInfoArray callback longtime test
		 *@tc.size		: MEDIUM
		 *@tc.type		: Function
		 *@tc.level 	: Level 1
		 */
		it('OH_EFFECTMANAGER_SR2_STABLITY_TEST_006', 1, function (done) {
			console.info(TAG + 'OH_EFFECTMANAGER_SR2_STABLITY_TEST_006')
			let random1 = Math.floor(Math.random()*65535);
			let random2 = Math.floor(Math.random()*65535);
			let Type = generateMixed(random1);
			let Usage = generateMixed(random2);			
			...
			let timer = setInterval(async function() {
				if(i < LONGTIME){
					await audioRenderer.getAudioEffectMode(async (err, audioEffectMode) => { //
	                  if (err) {
						clearInterval(timer);
					  	console.log(TAG +(err));
					...
					i = i+IntervalTime;
					console.log(TAG +"OH_EFFECTMANAGER_API_STABLITY_TEST_006: " + i);
				} else {
					clearInterval(timer);
					expect(true).assertTrue();
					done();	
		 ...
		 *@tc.level 	: Level 1
		 */
		it('OH_EFFECTMANAGER_API_STABLITY_TEST_004', 1, function (done) {
		 ...
		 *@tc.level 	: Level 1
		 */
		it('OH_EFFECTMANAGER_API_STABLITY_TEST_005', 1, function (done) {
			...
			let timer = setInterval(async function() {
				if(i < LONGTIME){
					await audioRenderer.setAudioEffectMode(AudioEffectModeTest, (err) => {

...
Camera_ErrorCode NDKCamera::CaptureSessionRegisterCallbackOn(int useCaseCode)
{
    ...
    if (ret_ == CAMERA_OK) {
        LOG("注册拍照会话事件回调监听成功.%d", ret_);
    } else {
        LOG("注册拍照会话事件回调监听失败.%d", ret_);
    }
    return ret_;
}
Camera_ErrorCode NDKCamera::CaptureSessionUnregisterCallbackOff(int useCaseCode)
{
    ...
    if (ret_ == CAMERA_OK) {
        LOG("注销拍照会话事件回调监听成功.%d", ret_);
    } else {
        LOG("注销拍照会话事件回调监听失败.%d", ret_);
    }
    return ret_;
}
    ...
    if (cameras != nullptr) {
        ...
        if (cameraIndex < this->size_) {
            *camera = &cameras[cameraIndex];
            ...
            LOG("读取失败，超出相机设备列表范围. %d", ret_);
        }
    } else {
        ret_ = CAMERA_INVALID_ARGUMENT;
        LOG("读取失败，相机设备列表为空. %d", ret_);
    }
    return ret_;
}
...
Camera_ErrorCode NDKCamera::DeleteSceneModes(int useCaseCode)
{
    ...
    if (sceneModes_ != nullptr || ret_ != CAMERA_OK) {
        LOG("删除相机设备支持的模式失败. %d", ret_);
    } else {
        LOG("删除相机设备支持的模式成功. %d", ret_);
    }
    return ret_;
}
...
Camera_ErrorCode NDKCamera::GetSupportedCameraOutputCapabilityWithSceneMode(int useCaseCode)
{
    ...
    if (cameraOutputCapability_ == nullptr || ret_ != CAMERA_OK) {
        LOG("获取特定相机设备在特定相机模式下的输出功能失败. %d", ret_);
    } else {
        LOG("获取特定相机设备在特定相机模式下的输出功能成功. %d", ret_);
    }
    return ret_;
}
...
Camera_ErrorCode NDKCamera::CanAddInput(int useCaseCode)
{
    ...
    if (ret_ != CAMERA_OK) {
        LOG("判断能否向会话添加输入失败. %d", ret_);
    } else {
        LOG("判断能否向会话添加输入成功. %d", ret_);
        if (isAddInput_ == true) {
            LOG("能向会话添加输入");
        } else {
            LOG("不能向会话添加输入");
...
Camera_ErrorCode NDKCamera::CanAddPreviewOutput(int useCaseCode)
{
    ...
    if (ret_ != CAMERA_OK) {
        LOG("判断能否向会话添加预览输出失败. %d", ret_);
    } else {
        LOG("判断能否向会话添加预览输出成功. %d", ret_);
        if (isAddInput_ == true) {
            LOG("能向会话添加预览输出");
        } else {
            LOG("不能向会话添加预览输出");
...
Camera_ErrorCode NDKCamera::CanAddPhotoOutput(int useCaseCode)
{
    ...
    if (ret_ != CAMERA_OK) {
        LOG("判断能否向会话添加拍照输出失败. %d", ret_);
    } else {
        LOG("判断能否向会话添加拍照输出成功. %d", ret_);
        if (isAddInput_ == true) {
            LOG("能向会话添加拍照输出");
        } else {
            LOG("不能向会话添加拍照输出");
...
Camera_ErrorCode NDKCamera::CanAddVideoOutput(int useCaseCode)
{
    ...
    if (ret_ != CAMERA_OK) {
        LOG("判断能否向会话添加录像输出失败. %d", ret_);
    } else {
        LOG("判断能否向会话添加录像输出成功. %d", ret_);
        if (isAddInput_ == true) {
            LOG("能向会话添加录像输出");
        } else {
            LOG("不能向会话添加录像输出");
...
Camera_ErrorCode NDKCamera::AddSecureOutput(int useCaseCode)
{
    ...
    if (ret_ == CAMERA_OK) {
        LOG("将一个预览输出添加为安全输出成功. %d", ret_);
    } else {
        LOG("将一个预览输出添加为安全输出失败. %d", ret_);
    }
    return ret_;
}

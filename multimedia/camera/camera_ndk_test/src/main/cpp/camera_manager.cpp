...
NDKCamera::NDKCamera(char* str)
    : cameras_(nullptr), cameraOutputCapability_(nullptr),
      ...
      torchMode_(ON), frameRatesSize_(0), videoFrameRatesSize_(0),
...
{
    if (useCaseCode == PARAMETER_OK) {
        if(isTorchSupported_ == true){
            ret_ = OH_CameraManager_IsTorchSupportedByTorchMode(cameraManager_, torchMode_, &isTorchSupported_);
        }
    } else if (useCaseCode == PARAMETER1_ERROR) {
        ret_ = OH_CameraManager_IsTorchSupportedByTorchMode(nullptr, torchMode_, &isTorchSupported_);
    } else if (useCaseCode == PARAMETER2_ERROR) {
...
Camera_ErrorCode NDKCamera::SetTorchMode(int useCaseCode)
{
    if (useCaseCode == PARAMETER_OK) {
        if(isTorchSupported_ == true){
            ...
            LOG("Not Supported SetTorchMode.");
            ret_ = CAMERA_OK;
        }
    } else if (useCaseCode == PARAMETER1_ERROR) {
...
Camera_ErrorCode NDKCamera::SetSmoothZoom(int useCaseCode)
{
    ...
    if (useCaseCode == PARAMETER_OK) {
        ...
        ret_ = OH_CaptureSession_SetSmoothZoom(captureSession_, -1, smoothZoomMode);
    } else if (useCaseCode == PARAMETER3_ERROR) {
        ret_ = OH_CaptureSession_SetSmoothZoom(captureSession_, targetZoom,
            static_cast<Camera_SmoothZoomMode>(-1));
    }
...
Camera_ErrorCode NDKCamera::SetFrameRate(int useCaseCode)
{
    ...
    if (useCaseCode == PARAMETER_OK) {
        ...
        ret_ = OH_PreviewOutput_SetFrameRate(previewOutput_, -1, maxFps);
    } else if (useCaseCode == PARAMETER2_ERROR) {
        ret_ = OH_PreviewOutput_SetFrameRate(previewOutput_, minFps, -1);
    }
    if (ret_ != CAMERA_OK) {
        LOG("OH_PreviewOutput_DeleteFrameRates failed.");
    }
...
Camera_ErrorCode NDKCamera::VideoOutputGetSupportedFrameRates(int useCaseCode)
{
    ...
    if (frameRateRange_ == nullptr || videoFrameRatesSize_ == 0 || ret_ != CAMERA_OK) {
...
{
    Camera_FrameRateRange videoActiveFrameRateRange = {0, 0};
    ...
    if (useCaseCode == PARAMETER_OK) {
        if (videoFrameRateRange_ != nullptr) {
            ...
            ret_ = OH_VideoOutput_GetActiveFrameRate(videoOutput_, &videoActiveFrameRateRange);
        } else {
            LOG("Not Supported FrameRate.");
            ret_ = CAMERA_OK;
        }
        ...
        ret_ = OH_VideoOutput_SetFrameRate(videoOutput_, -1, maxFps);
    }
    else if (useCaseCode == PARAMETER2_ERROR) {
        ret_ = OH_VideoOutput_SetFrameRate(videoOutput_, minFps, -1);
    }

    if (ret_ != CAMERA_OK && videoActiveFrameRateRange.min == 0 && videoActiveFrameRateRange.max == 0) {
...
{
    ret_ = OH_CaptureSession_UnregisterSmoothZoomInfoCallback(captureSession_, CaptureSessionSmoothZoomInfoCallback);
    if (ret_ != CAMERA_OK) {
...
{
    ret_ = OH_PhotoOutput_RegisterCaptureStartWithInfoCallback(photoOutput_, PhotoOutputCaptureStartWithInfoCallback);
    if (ret_ != CAMERA_OK) {

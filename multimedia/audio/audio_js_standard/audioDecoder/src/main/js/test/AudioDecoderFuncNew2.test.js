...
describe('AudioDecoderFunc', function () {
    ...
    async function enqueueAllInputs(audioDecodeProcessor, queue, inputbuffer){
        ...
        while (queue.length > 0 && sawInputEOS == false){
            ...
            if (enqueueEOS){
                ...
                console.log("i:" + i);                 
                console.log("inputbuffer.byteLength: " + inputbuffer.byteLength);               
                inputobject.timeMs = timestamp;
                inputobject.length = ES[i] - FREAM_HEAD_BYTE;   
                console.log("inputobject.data.byteLength2" + inputobject.data.byteLength);   
    ...
    function setCallback(audioDecodeProcessor, inputbuffer){
        ...
        audioDecodeProcessor.on('inputBufferAvailable', async(inBuffer) => {
            ...
            console.info("inBuffer.data.byteLength: "+ inBuffer.data.byteLength);
            inputQueue.push(inBuffer);
            await enqueueAllInputs(audioDecodeProcessor, inputQueue, inputbuffer);
        });
    ...
    function readFile(path){
        try{
            ...
            console.info("error happens, read file fail");
            return ''
        }           
    }

...
describe('AudioDecoderFunc', function () {
    ...
    function bufferDeepcopy(frombuffer, tobuffer, startpoint, length){
        return tobuffer
    }
    ...
    async function enqueueAllInputs(audioDecodeProcessor, queue){
        ...
        while (queue.length > 0 && sawInputEOS == false){
            let inputobject = queue.shift();
            dequecnt += 1;
            console.log('dequecnt: ' + dequecnt);    
    ...
    async function dequeueAllOutputs(audioDecodeProcessor, queue){
        console.log('queue.length:' + queue.length);
        while (queue.length > 0 && sawOutputEOS == false){
            let outputobject = queue.shift();
            console.log('outputobject.index: ' + outputobject.index);
            if (outputobject.flags == 1){
                sawOutputEOS == True
            }
            let data = outputobject.data;
            let outputbuffer = data.slice(outputobject.offset, outputobject.offset + outputobject.length);
    ...
    function setCallback(audioDecodeProcessor){
        ...
        audioDecodeProcessor.on('outputBufferAvailable', async(outBuffer) => {
            ...
            await dequeueAllOutputs(audioDecodeProcessor, outBuffer);
        });
...
// })
    ...
    it('SUB_MEDIA_Audio_Decoder_Function_01_0100', 0, async function (done) {
        ...
        console.log("start decoding");
        await audioDecodeProcessor.start().then(() => {console.log("start success");console.log(Date.now())}, failCallback).catch(failCatch);

        console.log("get Output MediaDescription");
        await audioDecodeProcessor.getOutputMediaDescription().then(() => {console.log("get Outpu MediaDescription");}, failCallback).catch(failCatch);


        console.log("start flush");
        await audioDecodeProcessor.flush().then(() => {console.log("flush success");console.log(Date.now())}, failCallback).catch(failCatch);
                ...
                audioDecodeProcessor.prepare((err) => {
                    ...
                    audioDecodeProcessor.start((err) => {
                        ...
                        audioDecodeProcessor.getOutputMediaDescription((err,MediaDescription) => {
                            asserTrue(typeof(err) == undefined);
                            console.log("get Outpu MediaDescription: " + MediaDescription.toString());
                            console.log("start flush");

...
describe('AudioDecoderFunc', function () {
    ...
    async function enqueueAllInputs(audioDecodeProcessor, queue){
        ...
        while (queue.length > 0 && sawInputEOS == false){
            ...
            await audioDecodeProcessor.queueInput(inputobject) 
            .then(() => {
    ...
    async function dequeueAllOutputs(audioDecodeProcessor, queue){
        ...
        while (queue.length > 0 ){
            ...
            await audioDecodeProcessor.releaseOutput(outputobject) 
            .then(() => {
                console.info('queueInput success');
            }, failCallback).catch(failCatch);
        }
    ...
    function setCallback(audioDecodeProcessor, inputbuffer){
        ...
        audioDecodeProcessor.on('inputBufferAvailable', async(inBuffer) => {
            ...
            timestamp += ES[i]/44.1;
            i += 1;
            console.log('inputQueue.length:' + inputQueue.length);

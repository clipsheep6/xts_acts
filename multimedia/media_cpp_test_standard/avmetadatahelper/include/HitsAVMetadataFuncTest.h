...
namespace OHOS {
namespace Media {
class HitsAVMetadataFuncTest : public testing::Test {
public:
    // SetUpTestCase: before all testcases
    static void SetUpTestCase(void)
    {
        MEDIA_DEBUG_LOG("HitsAVMetadataHelperFuncTest::SetUpTestCase");
        TestAVMetadataBasic::TestParamsConfig::GetInstance().InitMountPath();
    };
    // TearDownTestCase: after all testcase
    static void TearDownTestCase(void)
    {
        MEDIA_DEBUG_LOG("HitsAVMetadataHelperFuncTest::TearDownTestCase");
    };
    // SetUp
    void SetUp(void)
    {
        testInfo_ = ::testing::UnitTest::GetInstance()->current_test_info();
        MEDIA_DEBUG_LOG("HitsPlayerFuncTest::SetUp:%s", testInfo_->name());
    };
    // TearDown
    void TearDown(void)
    {
        MEDIA_DEBUG_LOG("HitsAVMetadataHelperFuncTest::TearDown");
    };
    void CheckMeta(std::string uri, std::unordered_map<int32_t, std::string> expectMeta);
    void GetThumbnail(const std::string uri);
    void GetAbnormalThumbnail(const std::string uri);

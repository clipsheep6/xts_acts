...
const presetsCount = {
    ActsUserFileMgrAlbum: { albumsCount: 7, assetsCount: 19 },
    ActsUserFileMgrBase: { albumsCount: 11, assetsCount: 11 },
    ActsUserFileMgrFavorite: { albumsCount: 6, assetsCount: 32 },
    ActsUserFileMgrFile: { albumsCount: 6, assetsCount: 21 },
    ActsUserFileMgrFileAsset: { albumsCount: 27, assetsCount: 72 },
    ActsUserFileMgrFileKey: { albumsCount: 2, assetsCount: 2 },
    ActsUserFileMgrFileResult: { albumsCount: 4, assetsCount: 13 },
    ActsUserFileMgrGetThumbnailTest: { albumsCount: 3, assetsCount: 3 },
    ActsUserFileMgrMediafetchoptions: { albumsCount: 4, assetsCount: 8 },
}
...
const fetchOps = function (testNum, path, type) {
    let ops = {
...
const uriFetchOps = function (testNum, albumUri) {
    let ops = {
        selections: FILEKEY.URI + '= ?',
        selectionArgs: [albumUri + ''],
    };
    console.info(`${testNum}: fetchOps${JSON.stringify(ops)}`)
    return ops
}
...
const checkPresetsAssets = async function (usrfilemgr, hapName) {
...
const getPermission = async function (name = 'ohos.acts.multimedia.mediaLibrary') {
    ...
    if (!(result1 == 0 && result2 == 0 && result3 == 0) || !(isGranted1 == 0 && isGranted2 == 0 && isGranted3 == 0) ||
        !(resultReadImageVideo == 0 && resultReadAudio == 0 && resultReadDocument == 0) ||

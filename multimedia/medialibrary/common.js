...
const presetsCount = {
    ActsMediaLibraryAlbum: { assetsCount: 34, albumsCount: 10 },
    ActsMediaLibraryFavorite: { assetsCount: 32, albumsCount: 6 },
    ActsMediaLibraryFileResult: { assetsCount: 112, albumsCount: 4 },
}
...
const sleep = async function sleep(times = 200) {
    ...
    try {
        let ops = {
            selections: '(' + RELATIVE_PATH + '= ? or ' +
                ...
                MEDIA_TYPE + '= ? or ' +
                MEDIA_TYPE + '= ?)',
            selectionArgs: [paths[0], paths[1], paths[2], albumName, types[0].toString(), types[1].toString(), types[2].toString()],
            ...others
        };
        return ops
    } catch (error) {
...
const checkAssetsCount = async function (done, testNum, fetchFileResult, expectCount) {
    ...
    if (count != expectCount) {
        ...
        console.info(`${testNum} failed:: assets length 0`);

...
namespace OHOS {
namespace MTUtil {
std::mutex ModuleTestDumpUtil::mutex_;
std::shared_ptr<ModuleTestDumpUtil> ModuleTestDumpUtil::instance_ = nullptr;
std::shared_ptr<ModuleTestDumpUtil> ModuleTestDumpUtil::GetInstance()
{
    if (instance_ == nullptr) {
        std::lock_guard<std::mutex> lock_l(mutex_);
        if (instance_ == nullptr) {
            instance_ = std::make_shared<ModuleTestDumpUtil>();
        }
    }
    return instance_;
}

ModuleTestDumpUtil::ModuleTestDumpUtil()
{
...
bool ModuleTestDumpUtil::CompStrVec(const str_vec &strVec_1, const str_vec &strVec_2)
{
...
str_iter ModuleTestDumpUtil::GetSpecific(const string &matchStr, const str_vec &dumpInfo, const str_iter &begin)
{
...
bool ModuleTestDumpUtil::MatchRegex(const regex &rgx, const string &text, string &result)
{
    std::smatch baseMatch;
    int32_t num = 2;
    if (std::regex_match(text, baseMatch, rgx)) {
        if (baseMatch.size() == num) {
...
size_t ModuleTestDumpUtil::GetAll(const string &args, const str_vec &dumpInfo, str_vec &results)
{
    results.clear();
    // args not exist
...
str_iter ModuleTestDumpUtil::GetFirst(
    const string &args, const str_vec &dumpInfo, const str_iter &begin, string &result)
{
    result.clear();
    str_iter end;
    std::advance(end, dumpInfo.size());
...
string ModuleTestDumpUtil::GetBy(const string &key, const string &value, const string &args, const str_vec &dumpInfo)
{
    str_vec items;
...
size_t ModuleTestDumpUtil::GetBindingsByUri(const string &uri, const str_vec &dumpInfo, str_vec &result)
{
    result.clear();

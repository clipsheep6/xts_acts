...
import AccessibilityExtensionAbility, { GesturePath, GesturePoint}  from '@ohos.application.AccessibilityExtensionAbility'
                ...
                .then((elementArray) => {
                    if (focus) {
                        elementArray[0].performAction(action)
                            .then((res) => {
                                console.info(logTag + ' action: ' + action + ' res:' + JSON.stringify(res))
                            })
                            .catch((error) => {
...
function getGesturePath(durationTime: number, array: Array<string>): GesturePath {
    let path: Array<GesturePoint> = new Array<GesturePoint>()
    for (let item of array) {
        let pos = item.split(',')
        let point: GesturePoint = {
            positionX: Number(pos[0]), positionY: Number(pos[1])
        }
        path.push(point)
    }
...
let click1: Array<string> = ["207,57"]
let click3: Array<string> = ["207,57", "207,57"]
...
let click2: Array<string> = ["260,508"]
//click

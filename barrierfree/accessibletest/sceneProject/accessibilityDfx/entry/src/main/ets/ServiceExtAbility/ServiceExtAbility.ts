        ...
        if (res) {
            res.findElement('content', searchText).then((AccessibilityElementArray) => {
                ...
                let id = setInterval(() => {
                    AccessibilityElementArray[0].performAction(action, args).then((performActionBack) => {
                        if (performActionBack) {
                            console.info(logTag + times + " accessibilityFocus  performActionBack Promise success=" + JSON.stringify(performActionBack));
                            setTimeout(()=>{
                                AccessibilityElementArray[0].performAction('clearAccessibilityFocus', args).then((accessibilityFocusClearBack) => {
                                    if (accessibilityFocusClearBack) {
                                        console.info(logTag + times + " clearAccessibilityFocus performActionBack Promise success=" + JSON.stringify(performActionBack));
                                        success++;
                                        console.info(logTag + " success: " + success);
                                    } else {
                                        console.info(logTag + times + " clearAccessibilityFocus  performActionBack Promise err=");
                                    }
                                }).catch((actionNamesErr) => {

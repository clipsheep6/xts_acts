...
const log = "[xtsLogC] "
class ServiceExtAbilityAllAbility2 extends AccessibilityExtensionAbility {
    onConnect() {
        console.info(log + "ServiceExtAbilityAllAbility2 onConnect");
        ...
        commonEvent.createSubscriber(subscriberInfo).then(async (data) => {
            console.info(log + "createSubscriber data:" + JSON.stringify(data));
            Subscriber = data;
...
const setEventTypeFilter = (context, eventTypeArr, description) => {
    ...
    context.setEventTypeFilter(eventTypeArr, (err, result) => {
        if (err?.code) {
            console.info(logTag + "callback err=" + JSON.stringify(err));
        } else {
            console.info(logTag + "callback res=" + result);
        }
    });
}
...
const setEventTypeFilterPromise = (context, eventTypeArr, description) => {
    ...
    context.setEventTypeFilter(eventTypeArr).then((res) => {
        console.info(logTag + "execute res:" + JSON.stringify(res));
    }).catch((err) => {
...
const setEventTypeFilterCallBack = (eventType, description) => {
    ...
    let commonEventPublishData = {
        data: "on_assist1_change_extra_success"
    }
...
export default ServiceExtAbilityAllAbility2
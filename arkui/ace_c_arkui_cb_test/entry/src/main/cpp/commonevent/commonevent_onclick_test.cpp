...
static void OnEventReceive(ArkUI_NodeEvent *event) {
    ...
    if (event == nullptr) {
        OH_LOG_Print(
            LOG_APP, LOG_ERROR, LOG_PRINT_DOMAIN, "CommonEventOnClickTest", "OnEventReceive: event is null");
        return;
    }
...
napi_value CommonEventOnClickTest::CreateNativeNode(napi_env env, napi_callback_info info) {
    ...
    if (OH_NativeXComponent_AttachNativeRootNode(PluginManager::GetInstance()->GetNativeXComponent(id), column) == INVALID_PARAM) {
        OH_LOG_Print(LOG_APP, LOG_ERROR, LOG_PRINT_DOMAIN, "CommonEventOnClickTest", "OH_NativeXComponent_AttachNativeRootNode failed");
    }

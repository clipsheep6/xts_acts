...
/*
 * @tc.number: SUB_ARKUI_CAPI_SCROLL_SCALE_0400
 * @tc.name: testScrollScale004
 * @tc.desc: testing scale for scroll with abnormal value
 * @tc.size: MediumTest
 * @tc.type: Function
 * @tc.level: Level 1
 */
HWTEST_F(ScrollCapiTest, testScrollScale004, Function | MediumTest | Level1)
{
    ArkUI_NumberValue value[] = {{.f32 = ABNORMAL_VALUE}, {.f32 = ABNORMAL_VALUE}};
    ArkUI_AttributeItem value_item = {value, sizeof(value) / sizeof(ArkUI_NumberValue)};
    auto ret = nodeAPI->setAttribute(scroll, NODE_SCALE, &value_item);
    ASSERT_EQ(ret, INVALID_PARAM);
    ASSERT_NE(nodeAPI->getAttribute(scroll, NODE_SCALE)->value[PARAM_0].f32, ABNORMAL_VALUE);
    ASSERT_NE(nodeAPI->getAttribute(scroll, NODE_SCALE)->value[PARAM_1].f32, ABNORMAL_VALUE);
}

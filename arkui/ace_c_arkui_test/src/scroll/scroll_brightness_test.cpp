...
/*
 * @tc.number: SUB_ARKUI_CAPI_SCROLL_BRIGHTNESS_0400
 * @tc.name: testScrollBrightness004
 * @tc.desc: testing brightness for scroll with abnormal value
 * @tc.size: MediumTest
 * @tc.type: Function
 * @tc.level: Level 1
 */
HWTEST_F(ScrollCapiTest, testScrollBrightness004, Function | MediumTest | Level1)
{
    ArkUI_NumberValue value[] = {{.f32 = ABNORMAL_PARAM}};
    ArkUI_AttributeItem value_item = {value, sizeof(value) / sizeof(ArkUI_NumberValue)};
    auto ret = nodeAPI->setAttribute(scroll, NODE_BRIGHTNESS, &value_item);
    ASSERT_EQ(ret, INVALID_PARAM);
    ASSERT_NE(nodeAPI->getAttribute(scroll, NODE_BRIGHTNESS)->value->f32, ABNORMAL_PARAM);
}

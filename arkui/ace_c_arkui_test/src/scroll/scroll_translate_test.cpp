...
{
    int32_t mode = NORMAL_VALUE;
    ArkUI_NumberValue value[] = {{.f32 = mode}};
    ArkUI_AttributeItem value_item = {value, sizeof(value) / sizeof(ArkUI_NumberValue)};
    auto ret = nodeAPI->setAttribute(scroll, NODE_TRANSLATE, &value_item);
    ASSERT_EQ(ret, INVALID_PARAM);
    ASSERT_NE(nodeAPI->getAttribute(scroll, NODE_TRANSLATE)->value[PARAM_0].f32, NORMAL_VALUE);
    ASSERT_NE(nodeAPI->getAttribute(scroll, NODE_TRANSLATE)->value[PARAM_1].f32, NORMAL_VALUE);
    ASSERT_NE(nodeAPI->getAttribute(scroll, NODE_TRANSLATE)->value[PARAM_2].f32, NORMAL_VALUE);
}

...
/*
 * @tc.number: SUB_ARKUI_CAPI_SCROLL_BLUR_0400
 * @tc.name: testScrollBlur004
 * @tc.desc: testing blur for scroll with abnormal value
 * @tc.size: MediumTest
 * @tc.type: Function
 * @tc.level: Level 1
 */
HWTEST_F(ScrollCapiTest, testScrollBlur004, Function | MediumTest | Level1)
{
    ArkUI_NumberValue value[] = {{.f32 = ABNORMAL_PARAM}};
    ArkUI_AttributeItem value_item = {value, sizeof(value) / sizeof(ArkUI_NumberValue)};
    auto ret = nodeAPI->setAttribute(scroll, NODE_BLUR, &value_item);
    ASSERT_EQ(ret, INVALID_PARAM);
    ASSERT_NE(nodeAPI->getAttribute(scroll, NODE_BLUR)->value->f32, ABNORMAL_PARAM);
}

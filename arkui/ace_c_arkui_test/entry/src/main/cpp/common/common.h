...
#define ASSERT_EQ(result, expect)                                                                                      \
    do {                                                                                                               \
        if ((result) != (expect)) {                                                                                    \
            std::string assertStr = "assert equal failed, expect is " + std::to_string(result) + " and result is " +   \
...
#define ASSERT_NE(result, expect)                                                                                      \
    do {                                                                                                               \
        if ((result) == (expect)) {                                                                                    \
            std::string assertStr = "assert not equal failed, expect is " + std::to_string(result) +                   \
                                    " and result is " + std::to_string(expect);                                        \
            char assertChars[assertStr.size() + PARAM_1];                                                              \
            strcpy(assertChars, assertStr.c_str());                                                                    \
            napi_throw_error((env), nullptr, assertChars);                                                             \
        }                                                                                                              \

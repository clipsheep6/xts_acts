...
napi_value static TestListNodeAdapter003(napi_env env, napi_callback_info info)
{
    ...
    // 注册事件回调函数
    int32_t ret = OH_ArkUI_NodeAdapter_RegisterEventReceiver(adapter, nullptr, [](ArkUI_NodeAdapterEvent *event) {
        OH_LOG_Print(LOG_APP, LOG_INFO, LOG_PRINT_DOMAIN, "TestListNodeAdapter", "nodeadapter event callback func");
        ...
        //         ret = OH_ArkUI_NodeAdapter_InsertItem(adapter, startPosition, itemCount);
        //         ASSERT_EQ(ret, ARKUI_ERROR_CODE_NO_ERROR);
        //         break;
        ...
        //         uint32_t from = 1;
        //         uint32_t to = 2;
        //         ret = OH_ArkUI_NodeAdapter_MoveItem(adapter, from, to);
        //         ASSERT_EQ(ret, ARKUI_ERROR_CODE_NO_ERROR);
        //         break;
        //     case NODE_REFRESH_ON_REFRESH:
        //         // 通知Adapter进行全量元素变化
        //         ret = OH_ArkUI_NodeAdapter_ReloadAllItems(adapter);
        //         ASSERT_EQ(ret, ARKUI_ERROR_CODE_NO_ERROR);
        //         break;
        //     case NODE_REFRESH_ON_REFRESH:
        //         // 通知Adapter进行局部元素变化
        //         startPosition = 1;
        //         itemCount = 2;
        //         ret = OH_ArkUI_NodeAdapter_ReloadItem(adapter, startPosition, itemCount);
        //         ASSERT_EQ(ret, ARKUI_ERROR_CODE_NO_ERROR);
        //         break;
        ...
        //         startPosition = 1;
        //         itemCount = 2;
        //         ret = OH_ArkUI_NodeAdapter_RemoveItem(adapter, startPosition, itemCount);
        //         ASSERT_EQ(ret, ARKUI_ERROR_CODE_NO_ERROR);
        //         break;
        //     default:
        //         // 获取存储在Adapter中的所有元素
        ...
        //         uint32_t ret_size;
        //         ret = OH_ArkUI_NodeAdapter_GetAllItems(adapter, items, &ret_size);
        //         ASSERT_EQ(ret, ARKUI_ERROR_CODE_NO_ERROR);
        // }
    });
    // 断言是否注册成功

...
static void PrintException(JSVM_Env env, JSVM_Value exception, const char *call) {
  ...
  CHECK_FATAL(isObject, "[bug] exception returned from OH_JSVM_GetAndClearLastException is not a JS object [%s]", call);
...
static void CheckErrorAndException(JSVM_Env env, JSVM_Status returnStatus, const char *call) {
  ...
  CHECK_FATAL(returnStatus == errorInfo->errorCode,
              "[bug] return status (%d) is not equal to errorInfo status (%d) [%s]", returnStatus, errorInfo->errorCode,
              call);
  ...
  if (hasException) {
    ...
    CHECK_FATAL(returnStatus != JSVM_OK, "[bug] There is exception caught, bug return status is JSVM_OK [%s]", call);
...
// global functions
static inline JSVM_Value Str(const char *s) {
  JSVM_Value str;
  JSVMTEST_CALL(OH_JSVM_CreateStringUtf8(jsvm_env, s, JSVM_AUTO_LENGTH, &str));
  return str;
}

static inline JSVM_Value Str(const std::string &stdString) {
  return Str(stdString.c_str());
}

static inline JSVM_Script Compile(JSVM_Value js_str, const uint8_t *cache = nullptr, size_t size = JSVM_AUTO_LENGTH) {
  JSVM_Script script;
  JSVMTEST_CALL(OH_JSVM_CompileScript(jsvm_env, js_str, cache, size, false, nullptr, &script));
  return script;
}

static inline JSVM_Script CompileWithName(JSVM_Value js_str, const char *name) {
  JSVM_Script script;
  JSVM_ScriptOrigin origin = {
    .sourceMapUrl = nullptr,
    .resourceName = name,
  };
  JSVMTEST_CALL(OH_JSVM_CompileScriptWithOrigin(jsvm_env, js_str, NULL, JSVM_AUTO_LENGTH, false, NULL, &origin, &script));
  return script;
}
 
static inline JSVM_Script Compile(const char *s, const uint8_t *cache = nullptr, size_t size = JSVM_AUTO_LENGTH) {
  return Compile(Str(s), cache, size);
}
 
static inline JSVM_Script CompileWithName(const char *s, const char *name) {
  return CompileWithName(Str(s), name);
}

static inline JSVM_Value Run(JSVM_Script script) {
  JSVM_Value result;
  JSVMTEST_CALL(OH_JSVM_RunScript(jsvm_env, script, &result));
  return result;
}

static inline JSVM_Value Run(const char *s) {
  return Run(Compile(s));
}

static inline bool ToBoolean(JSVM_Value val) {
...
static inline double ToNumber(JSVM_Value val) {
...
static inline std::string ToString(JSVM_Value val) {
...
// object property
static inline JSVM_Value GetProperty(JSVM_Value object, JSVM_Value key) {
  JSVM_Value result;
  JSVMTEST_CALL(OH_JSVM_GetProperty(jsvm_env, object, key, &result));
  return result;
}

static inline JSVM_Value GetProperty(JSVM_Value object, const char *name) {
  return GetProperty(object, Str(name));
}

static inline void SetProperty(JSVM_Value object, JSVM_Value key, JSVM_Value value) {
  JSVMTEST_CALL(OH_JSVM_SetProperty(jsvm_env, object, key, value));
}

static inline void SetProperty(JSVM_Value object, const char *name, JSVM_Value value) {
  JSVMTEST_CALL(OH_JSVM_SetProperty(jsvm_env, object, Str(name), value));
}

// Get named property with name `name` from globalThis
static inline JSVM_Value Global(const char *name) {
  return GetProperty(Global(), Str(name));
}

// Get property with key `key` from globalThis
static inline JSVM_Value Global(JSVM_Value key) {
  return GetProperty(Global(), key);
}

// Call function with normal function with empty argument list
static JSVM_Value Call(JSVM_Value func) {
  JSVM_Value result;
...
static JSVM_Value Call(JSVM_Value func, JSVM_Value thisArg, std::initializer_list<JSVM_Value> args) {
  JSVM_Value result;
  int argc = args.size();
...
static JSVM_Value New(JSVM_Value constructor) {
  JSVM_Value result;
...
static inline bool IsNullOrUndefined(JSVM_Value value) {
  bool result;
  JSVMTEST_CALL(OH_JSVM_IsNullOrUndefined(jsvm_env, value, &result));
  return result;
}

static inline bool IsBoolean(JSVM_Value value) {
  bool result;
  JSVMTEST_CALL(OH_JSVM_IsBoolean(jsvm_env, value, &result));
  return result;
}

static inline bool IsTrue(JSVM_Value value) {
  return IsBoolean(value) && ToBoolean(value) == true;
}

static inline bool IsFalse(JSVM_Value value) {
...
static inline bool IsBigInt(JSVM_Value value) {
  bool result;
  JSVMTEST_CALL(OH_JSVM_IsBigInt(jsvm_env, value, &result));
  return result;
}

static inline bool IsSymbol(JSVM_Value value) {
  bool result;
...
static inline bool IsPromise(JSVM_Value value) {
  bool result;
  JSVMTEST_CALL(OH_JSVM_IsPromise(jsvm_env, value, &result));
  return result;
}

static inline size_t StringLength(JSVM_Value str) {
  CHECK(IsString(str));
  size_t length;
...
static inline uint32_t ArrayLength(JSVM_Value array) {
  CHECK(IsArray(array));
  uint32_t length;
...
static inline JSVM_Value ArrayAt(JSVM_Value array, uint32_t index) {
  JSVM_Value element;
  JSVMTEST_CALL(OH_JSVM_GetElement(jsvm_env, array, index, &element));
  return element;
}

static inline JSVM_Value JsonParse(JSVM_Value str) {
  JSVM_Value obj;
  JSVMTEST_CALL(OH_JSVM_JsonParse(jsvm_env, str, &obj));
  return obj;
}

static inline JSVM_Value JsonStringify(JSVM_Value obj) {
  JSVM_Value str;
  JSVMTEST_CALL(OH_JSVM_JsonStringify(jsvm_env, obj, &str));
  return str;
}

static inline bool Equals(JSVM_Value lhs, JSVM_Value rhs) {
  bool result;
  JSVMTEST_CALL(OH_JSVM_Equals(jsvm_env, lhs, rhs, &result));
  return result;
}

static inline bool StrictEquals(JSVM_Value lhs, JSVM_Value rhs) {
  bool result;
  JSVMTEST_CALL(OH_JSVM_StrictEquals(jsvm_env, lhs, rhs, &result));
  return result;
}

static inline bool InstanceOf(JSVM_Value value, JSVM_Value constructor) {
  bool result;
...
static void InstallMyConsoleLog(JSVM_Env env) {

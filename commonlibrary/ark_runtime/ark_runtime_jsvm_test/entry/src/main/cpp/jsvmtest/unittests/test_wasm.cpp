...
// #ifdef OHOS_JSVM_XTS
#include "add.wasm.h"
#include "ammo.wasm.wasm.h"
#include "bfs.wasm.h"
#include "cJSON.wasm.h"
#include "global.wasm.h"
#include "illegal2.wasm.h"
#include "illegal.wasm.h"
#include "imports.wasm.h"
#include "laya.physics3D.wasm.wasm.h"
#include "math.wasm.h"
#include "memory.wasm.h"
#include "newton.wasm.h"
#include "reloc_info.wasm.h"
#include "simple.wasm.h"
#include "table2.wasm.h"
#include "table.wasm.h"
#include <unordered_map>
// #endif

#if defined(USE_HEADER_WASM)
static const std::string wasmPath = "";
static std::unordered_map<std::string, std::pair<unsigned char*, unsigned int>> wasmMap;
void InitWasmMap() {
  if (wasmMap.size() == 0) {
    ...
    wasmMap["laya.physics3D.wasm.wasm"] = std::pair<unsigned char*, unsigned int>(laya_physics3D_wasm_wasm, laya_physics3D_wasm_wasm_len);
...
static bool IsWebAssemblyMemory(JSVM_Value value) {
  auto Memory = jsvm::GetProperty(jsvm::Global("WebAssembly"), "Memory");
  return jsvm::InstanceOf(value, Memory);
}

static bool IsWebAssemblyTable(JSVM_Value value) {
...
static void ReadBinaryFile(const char *path, std::vector<uint8_t> &buffer) {
  ...
  // std::cout << ", " << (void*)p.first << ", " << p.second << std::endl;
  // std::cout << "123" << std::endl;
  // memcpy(buffer.data(), p.first, p.second);
#endif
}
...
static void ReadTextFile(const std::string &path, std::string &content) {
...
TEST(Wasm_IsWasmModuleObject) {
  ...
  CHECK(!IsWasmModuleObject(number));
  CHECK(!IsWasmModuleObject(jsvm::Undefined()));
  CHECK(!IsWasmModuleObject(jsvm::Null()));
  CHECK(!IsWasmModuleObject(jsvm::Str("hello")));
}
...
void RandomDeserialize(JSVM_Env env, const std::string &filename) {
  static int cnt = 0;
  ...
  if (!infile.good()) {
    ...
    JSVM_Value wasmModule;
    bool cacheRejected;
    JSVMTEST_CALL(OH_JSVM_CompileWasmModule(env, buffer.data(), buffer.size(), cache.data(), cache.size(), &cacheRejected, &wasmModule));
    CHECK(cacheRejected == false);
    CHECK(IsWasmModuleObject(wasmModule));
    // 运行一定次数后，删除缓存
    if (++cnt % 3 == 0) {
...
TEST(Wasm_CompileIllegalWasmModule) {
  ...
  for (const std::string &path : pathVec) {
    ...
    CHECK(buffer.size() > 0);
    JSVM_Value wasmModule;
    JSVM_Status status = OH_JSVM_CompileWasmModule(jsvm_env, buffer.data(), buffer.size(), NULL, 0, NULL, &wasmModule);
...
TEST(Wasm_MultipleInstances) {
  auto wasmModule = CompileWasmModule(wasmPath + "add.wasm");
  {
    ...
    CHECK(IsWebAssemblyInstance(instance));
  }
  {
...
TEST(Wasm_MDN_table) {
  ...
  CHECK(IsWebAssemblyTable(table));

 ...
 * @testcase.expect    : return TEEC_ERROR_BAD_PARAMETERS
 */
TEE_TEST(TeeBasicTestFramework, Opensession_ContextIsNotInit, Function | MediumTest | Level0)
{
    TEEC_Result ret;
    uint32_t origin;
...
TEE_TEST(TeeBasicTestFramWithInitContext, Opensession_WithOperationIsValue, Function | MediumTest | Level0)
{
    ...
    ASSERT_EQ(operation.params[NUM2].value.a, 0x555);
    ASSERT_EQ(operation.params[NUM2].value.b, 0x666);
    ASSERT_EQ(operation.params[NUM3].value.a, 0x777);
    ASSERT_EQ(operation.params[NUM3].value.b, 0x888);
}
...
TEE_TEST(TeeBasicTestFramWithInitContext, Opensession_WithOperationIsTempMem, Function | MediumTest | Level0)
{
    ...
    operation.params[NUM2].tmpref.buffer = testData2;
    operation.params[NUM2].tmpref.size = len2;
    operation.params[NUM3].tmpref.buffer = testData3;
    operation.params[NUM3].tmpref.size = len3;
    ...
    ASSERT_STREQ(reinterpret_cast<char *>(operation.params[NUM2].tmpref.buffer), testData2);
    ASSERT_EQ(operation.params[NUM2].tmpref.size, len2);
    ASSERT_STREQ(reinterpret_cast<char *>(operation.params[NUM3].tmpref.buffer), testData3);
    ASSERT_EQ(operation.params[NUM3].tmpref.size, len3);
}
...
TEE_TEST(TeeBasicTestFramWithInitContext, Opensession_WithOperationIsPartialMem, Function | MediumTest | Level0)
{
    ...
    operation.params[NUM2].memref.parent = &sharedMem;
    operation.params[NUM2].memref.offset = 0;
    operation.params[NUM2].memref.size = sharedMem.size;
    operation.params[NUM3].memref.parent = &sharedMem;
    operation.params[NUM3].memref.offset = 0;
    operation.params[NUM3].memref.size = sharedMem.size;
    ...
    ASSERT_EQ(operation.params[NUM2].memref.size, sharedMem.size);
    ASSERT_EQ(operation.params[NUM3].memref.size, sharedMem.size);
}
 ...
 */
TEE_TEST(TeeBasicTestFramework, RequestCancellationTest, Function | MediumTest | Level0)
{
    TEEC_Result ret;
    TEEC_UUID testId = CLIENTAPI_UUID_1;

...
struct unIntMapping g_unIntMap[] = {
    // bool
    { (char*)GPD_TA_INSTANCEKEEPALIVE, CMD_TEE_GET_PROPERTY_AS_BOOL, TEE_PROPSET_CURRENT_TA,
      (char*)VALUE_PREDEFINED_FALSE, sizeof(VALUE_PREDEFINED_FALSE) },
    { (char*)GPD_TA_MULTISESSION, CMD_TEE_GET_PROPERTY_AS_BOOL, TEE_PROPSET_CURRENT_TA,
      (char*)VALUE_PREDEFINED_BOOLEAN, sizeof(VALUE_PREDEFINED_BOOLEAN) },
    { (char*)GPD_TA_SINGLEINSTANCE, CMD_TEE_GET_PROPERTY_AS_BOOL, TEE_PROPSET_CURRENT_TA,
      (char*)VALUE_PREDEFINED_BOOLEAN, sizeof(VALUE_PREDEFINED_BOOLEAN) },
    { (char*)GPD_TEE_CRYPTOGRAPHY_ECC, CMD_TEE_GET_PROPERTY_AS_BOOL, TEE_PROPSET_IMPLEMENTATION,
      (char*)VALUE_PREDEFINED_FALSE, sizeof(VALUE_PREDEFINED_FALSE) },
    { (char*)GPD_TEE_CRYPTOGRAPHY_NIST, CMD_TEE_GET_PROPERTY_AS_BOOL, TEE_PROPSET_IMPLEMENTATION,
      (char*)VALUE_PREDEFINED_FALSE, sizeof(VALUE_PREDEFINED_FALSE) },
    { (char*)GPD_TEE_CRYPTOGRAPHY_BSI_R, CMD_TEE_GET_PROPERTY_AS_BOOL, TEE_PROPSET_IMPLEMENTATION,
      (char*)VALUE_PREDEFINED_FALSE, sizeof(VALUE_PREDEFINED_FALSE) },
    { (char*)GPD_TEE_CRYPTOGRAPHY_BSI_T, CMD_TEE_GET_PROPERTY_AS_BOOL, TEE_PROPSET_IMPLEMENTATION,
      (char*)VALUE_PREDEFINED_FALSE, sizeof(VALUE_PREDEFINED_FALSE) },
    { (char*)GPD_TEE_CRYPTOGRAPHY_IETF, CMD_TEE_GET_PROPERTY_AS_BOOL, TEE_PROPSET_IMPLEMENTATION,
      (char*)VALUE_PREDEFINED_FALSE, sizeof(VALUE_PREDEFINED_FALSE) },
    { (char*)GPD_TEE_CRYPTOGRAPHY_OCTA, CMD_TEE_GET_PROPERTY_AS_BOOL, TEE_PROPSET_IMPLEMENTATION,
      (char*)VALUE_PREDEFINED_FALSE, sizeof(VALUE_PREDEFINED_FALSE) },

    // binaryblock
    { (char*)SMC_TA_TESTBINARYBLOCK, CMD_TEE_GET_PROPERTY_AS_BINARY_BLOCK, TEE_PROPSET_CURRENT_TA,
      (char*)VALUE_PREDEFINED_BINARY_BLOCK, sizeof(VALUE_PREDEFINED_BINARY_BLOCK) },

    // uuid
    { (char*)GPD_TA_APPID, CMD_TEE_GET_PROPERTY_AS_UUID, TEE_PROPSET_CURRENT_TA, NULL, 0 },
    // do not test GPD_TEE_DEVICEID
    { (char*)GPD_TEE_DEVICEID, CMD_TEE_GET_PROPERTY_AS_UUID, TEE_PROPSET_IMPLEMENTATION, NULL, 0 },

    // string
    // this only test get uuid as string
    { (char*)GPD_TA_APPID, CMD_TEE_GET_PROPERTY_AS_STRING,
      TEE_PROPSET_CURRENT_TA, (char*)VALUE_PREDEFINED_UUID, BIG_SIZE },
    { (char*)GPD_CLIENT_IDENTITY, CMD_TEE_GET_PROPERTY_AS_STRING,
      TEE_PROPSET_CURRENT_CLIENT, (char*)VALUE_PREDEFINED_CLIENT_IDENTITY, BIG_SIZE },
    { (char*)GPD_TA_VERSION, CMD_TEE_GET_PROPERTY_AS_STRING,
      TEE_PROPSET_CURRENT_TA, (char*)VALUE_PREDEFINED_TA_VERSION, BIG_SIZE },
    { (char*)GPD_TA_DESCRIPTION, CMD_TEE_GET_PROPERTY_AS_STRING,
      TEE_PROPSET_CURRENT_TA, (char*)VALUE_PREDEFINED_TA_DESCRIPTION, BIG_SIZE },
    { (char*)GPD_TEE_APIVERSION, CMD_TEE_GET_PROPERTY_AS_STRING,
      TEE_PROPSET_IMPLEMENTATION, (char*)TEE_API_VERSION, BIG_SIZE },
    { (char*)GPD_TEE_DESCRIPTION, CMD_TEE_GET_PROPERTY_AS_STRING,
      TEE_PROPSET_IMPLEMENTATION, (char*)TEE_BUILD_VER, BIG_SIZE },
    { (char*)GPD_TEE_TRUSTEDOS_IMP_VERSION, CMD_TEE_GET_PROPERTY_AS_STRING,
      TEE_PROPSET_IMPLEMENTATION, (char*)TEE_IMP_VERSION, BIG_SIZE },
    { (char*)GPD_TEE_TRUSTEDOS_IMP_BINARYVERSION, CMD_TEE_GET_PROPERTY_AS_STRING,
      TEE_PROPSET_IMPLEMENTATION, (char*)TEE_IMP_VERSION, BIG_SIZE },
    { (char*)GPD_TEE_FIRMWARE_IMP_VERSION, CMD_TEE_GET_PROPERTY_AS_STRING,
      TEE_PROPSET_IMPLEMENTATION, (char*)TEE_FIRMWARE_IMP_VERSION, BIG_SIZE },
    { (char*)GPD_TEE_FIRMWARE_IMP_BINARYVERSION, CMD_TEE_GET_PROPERTY_AS_STRING,
      TEE_PROPSET_IMPLEMENTATION, (char*)TEE_FIRMWARE_IMP_VERSION, BIG_SIZE },
    { (char*)GPD_TEE_TRUSTEDOS_MANUFACTURER, CMD_TEE_GET_PROPERTY_AS_STRING,
      TEE_PROPSET_IMPLEMENTATION, (char*)TEE_MANUFACTURER, BIG_SIZE },
    { (char*)GPD_TEE_FIRMWARE_MANUFACTURER, CMD_TEE_GET_PROPERTY_AS_STRING,
      TEE_PROPSET_IMPLEMENTATION, (char*)TEE_FIRMWARE_MANUFACTURER, BIG_SIZE },
    { (char*)GPD_TEE_DEVICEID, CMD_TEE_GET_PROPERTY_AS_STRING,
      TEE_PROPSET_IMPLEMENTATION, (char*)TEE_FIRMWARE_MANUFACTURER, BIG_SIZE },
};

struct intMapping g_intMap[] = {
    // int32
    { (char*)GPD_TA_DATASIZE, CMD_TEE_GET_PROPERTY_AS_U32,
      TEE_PROPSET_CURRENT_TA, VALUE_PREDEFINED_DATASIZE },
    { (char*)GPD_TA_STACKSIZE, CMD_TEE_GET_PROPERTY_AS_U32,
      TEE_PROPSET_CURRENT_TA, VALUE_PREDEFINED_STACKSIZE },
    { (char*)GPD_TA_ENDIAN, CMD_TEE_GET_PROPERTY_AS_U32,
      TEE_PROPSET_CURRENT_TA, 0 },
    { (char*)GPD_TA_API_LEVEL, CMD_TEE_GET_PROPERTY_AS_U32,
      TEE_PROPSET_CURRENT_TA, API_LEVEL1_2 },
    { (char*)GPD_CLIENT_ENDIAN, CMD_TEE_GET_PROPERTY_AS_U32,
      TEE_PROPSET_CURRENT_CLIENT, VALUE_PREDEFINED_CLIENT_ENDIAN },
    { (char*)GPD_TEE_INTERNALCORE_VERSION, CMD_TEE_GET_PROPERTY_AS_U32,
      TEE_PROPSET_IMPLEMENTATION, TEE_INTERNAL_CORE_VERSION },
    { (char*)GPD_TEE_SYSTEM_TIME_PROTECTIONLEVEL, CMD_TEE_GET_PROPERTY_AS_U32,
      TEE_PROPSET_IMPLEMENTATION, TEE_TIME_PROTECT_LEVEL },
    { (char*)GPD_TEE_TA_PERSISTENT_TIME_PROTECTIONLEVEL, CMD_TEE_GET_PROPERTY_AS_U32,
      TEE_PROPSET_IMPLEMENTATION, TA_TIME_PROTECT_LEVEL },
    { (char*)GPD_TEE_ARITH_MAXBIGINTSIZE, CMD_TEE_GET_PROPERTY_AS_U32,
      TEE_PROPSET_IMPLEMENTATION, MAX_BIG_INT_SIZE },
    { (char*)GPD_TEE_ANTIROLLBACK_PROTECTIONLEVEL, CMD_TEE_GET_PROPERTY_AS_U32,
      TEE_PROPSET_IMPLEMENTATION, 0 },
    { (char*)GPD_TEE_ROLLBACKDETECT_PROTECTIONLEVEL, CMD_TEE_GET_PROPERTY_AS_U32,
      TEE_PROPSET_IMPLEMENTATION, 0 },
    { (char*)GPD_TEE_EVENT_MAXSOURCES, CMD_TEE_GET_PROPERTY_AS_U32,
      TEE_PROPSET_IMPLEMENTATION, 0 },
    { (char*)GPD_TEE_API_LEVEL, CMD_TEE_GET_PROPERTY_AS_U32,
      TEE_PROPSET_IMPLEMENTATION, TEE_MAX_API_LEVEL_CONFIG },
    // int64
    { (char*)SMC_TA_TESTU64, CMD_TEE_GET_PROPERTY_AS_U64,
      TEE_PROPSET_CURRENT_TA, VALUE_PREDEFINED_U64 },
};

static TEEC_Result TestGetPropertyFromUnIntMap(TEEC_Context *context, TEEC_Session *session, TestData *val, int no)
{
    TEEC_Result result = Invoke_GetPropertyAsX(context, session, val);
    ...
    if (val->cmd != CMD_TEE_GET_PROPERTY_AS_UUID) {
        if (val->outBufferLen != g_unIntMap[no].expectLen ||
            (strncmp(val->outBuffer, g_unIntMap[no].expectResult, g_unIntMap[no].expectLen) != 0)) {
            TEST_PRINT_ERROR("getProperty from Enumerator with %s is fail! outlen=0x%x, expect outlen=0x%x\n",
                val->inBuffer, val->outBufferLen, g_unIntMap[no].expectLen);
            TEST_PRINT_ERROR("outbuffer=%s, expect outbuffer=%s\n", val->outBuffer, g_unIntMap[no].expectResult);
            return TEEC_ERROR_GENERIC;
        }
    }
    return TEEC_SUCCESS;
    ...
    for (i = 0; i < (sizeof(g_unIntMap) / sizeof(g_unIntMap[0])); i++) {
        if (strncmp(val->inBuffer, g_unIntMap[i].name, val->inBufferLen) == 0) {
            *flag = 1;
            ...
            result = TestGetPropertyFromUnIntMap(context, session, val, i);
            return result;
        }
    }
    return TEEC_SUCCESS;
...
static TEEC_Result TestGetPropertyFromIntMap(TEEC_Context *context, TEEC_Session *session, TestData *val, int no)
{
    TEEC_Result result = Invoke_GetPropertyAsX(context, session, val);
    if (result != TEEC_SUCCESS || val->origin != TEEC_ORIGIN_TRUSTED_APP) {
        TEST_PRINT_ERROR("getProperty from Enumerator with %s is fail! result = 0x%x\n", val->inBuffer, result);
    ...
    if (val->cmd == CMD_TEE_GET_PROPERTY_AS_U32) {
        if (atoi(val->outBuffer) != (uint32_t)g_intMap[no].expectResult) {
            TEST_PRINT_ERROR("getProperty from Enumerator with %s is fail! out=0x%x, expect out=0x%x\n",
                val->inBuffer, atoi(val->outBuffer), (uint32_t)g_intMap[no].expectResult);
            return TEEC_ERROR_GENERIC;
        }
    } else {
        if (atoll(val->outBuffer) != g_intMap[no].expectResult) {
            TEST_PRINT_ERROR("getProperty from Enumerator with %s is fail! out=0x%llx, expect out=0x%llu\n",
                val->inBuffer, atoll(val->outBuffer), (unsigned long long)g_intMap[no].expectResult);
            return TEEC_ERROR_GENERIC;
        }
    }
    return TEEC_SUCCESS;
    ...
    for (i = 0; i < (sizeof(g_intMap) / sizeof(g_intMap[0])); i++) {
        if (strncmp(val->inBuffer, g_intMap[i].name, val->inBufferLen) == 0) {
            *flag = 1;
            ...
            result = TestGetPropertyFromIntMap(context, session, val, i);
            return result;
        }
    }
    return TEEC_SUCCESS;
...
TEE_TEST(TeeTcf1EnumTest, TEE_GetProperty_WithEnum_TEE_PROPSET_CURRENT_TA, Function | MediumTest | Level0)
{
    ...
    value.cmd = CMD_TEE_START_PROPERTY_ENUMERATOR;
    value.propSet = TEE_PROPSET_CURRENT_TA;
    Invoke_Operate_PropertyEnumerator(GetSession(), &value);

    do {
        value.cmd = CMD_TEE_GET_PROPERTY_NAME_ENUMERATOR;
        ...
        value.cmd = CMD_TEE_GET_NEXT_PROPERTY_ENUMERATOR;
        Invoke_Operate_PropertyEnumerator(GetSession(), &value);
        findFlag = 0;
...
TEE_TEST(TeeTcf1EnumTest, TEE_GetProperty_WithEnum_WithResetEnum, Function | MediumTest | Level0)
{
    ...
    value.cmd = CMD_TEE_RESET_PROPERTY_ENUMERATOR;
    value.propSet = TEE_PROPSET_IMPLEMENTATION;
    Invoke_Operate_PropertyEnumerator(GetSession(), &value);

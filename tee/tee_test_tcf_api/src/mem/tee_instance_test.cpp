...
TEE_TEST(TeeTcf2Test, TEE_Set_And_GetInstanceData_With_Success, Function | MediumTest | Level0)
{
    ...
    ret = Invoke_SetInstanceData(GetSession(), CMD_TEE_SET_INSTANCE_DATA, inBuf, caseId, &origin);
    ASSERT_EQ(ret, TEEC_SUCCESS);
    ASSERT_EQ(origin, TEEC_ORIGIN_TRUSTED_APP);

    ret = Invoke_GetInstanceData(GetSession(), CMD_TEE_GET_INSTANCE_DATA, outBuf, &outLen, &origin);
    ASSERT_EQ(ret, TEEC_SUCCESS);
    ASSERT_EQ(origin, TEEC_ORIGIN_TRUSTED_APP);

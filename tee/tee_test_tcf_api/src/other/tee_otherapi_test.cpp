...
TEE_TEST(TeeBasicTestFramework, TEE_PrintAPI, Function | MediumTest | Level0)
{
 ...
 */
TEE_TEST(TeeBasicTestFramework, TEE_GetAllKindsInfo, Function | MediumTest | Level0)
{
    ClientSessionMgr sess;
    uint32_t origin;
    TEEC_UUID testId = TCF_API_UUID_1;
...
TEE_TEST(TeeTcf2Ta2TaTest, TEE_InvokeTACommand_With_ShareMem, Function | MediumTest | Level0)
{
    ...
    ret = Invoke_InvokeTACommand(GetSession(), CMD_TEE_SHARE_MEM_API, ta2taSession, &value, &origin);
#ifndef TEST_STUB
    ASSERT_EQ(ret, TEEC_SUCCESS);
#else

...
TEE_TEST(TeeTcf2Ta2TaTest, TEE_OpenTASession_With_Success, Function | MediumTest | Level0)
{
    ...
    ret = Invoke_OpenTASession(GetSession(), CMD_TEE_OPEN_TA_SESSION, &ta2taSession, &value, &origin);
    ...
    ret = Invoke_CloseTASession(GetSession(), CMD_TEE_CLOSE_TA_SESSION, ta2taSession, &origin);
    ASSERT_EQ(ret, TEEC_SUCCESS);
    ASSERT_EQ(origin, TEEC_ORIGIN_TRUSTED_APP);
}
...
TEE_TEST(TeeTcf2Ta2TaTest, TEE_OpenTASession_With_MaxSession, Function | MediumTest | Level0)
{
    ...
    for (i = 0; i <= 6; i++) {
        ret = Invoke_OpenTASession(GetSession(), CMD_TEE_OPEN_TA_SESSION, &ta2taSession[i], &value, &origin);
        ASSERT_EQ(ret, TEEC_SUCCESS);
        ASSERT_EQ(origin, TEEC_ORIGIN_TRUSTED_APP);
        ASSERT_EQ(value.origin, TEEC_ORIGIN_TEE);
...
TEE_TEST(TeeTcf2Ta2TaTest, TEE_CloseTASession_With_MaxSession, Function | MediumTest | Level0)
{
    ...
    // ta2 has reach max session,so this time will open session fail
    value.inBufferLen = BIG_SIZE;
    ret = Invoke_OpenTASession(GetSession(), CMD_TEE_OPEN_TA_SESSION, &ta2taSession[NUM7], &value, &origin);
    ASSERT_EQ(ret, TEEC_ERROR_SESSION_MAXIMUM);
    ASSERT_EQ(origin, TEEC_ORIGIN_TRUSTED_APP);
    ASSERT_EQ(ta2taSession[NUM7], 0);

    // close one session of ta2
    ret = Invoke_CloseTASession(GetSession(), CMD_TEE_CLOSE_TA_SESSION, ta2taSession[NUM6], &origin);
    ASSERT_EQ(ret, TEEC_SUCCESS);
    ASSERT_EQ(origin, TEEC_ORIGIN_TRUSTED_APP);

    // ta2 has close one session, so this time will open session success
    ret = Invoke_OpenTASession(GetSession(), CMD_TEE_OPEN_TA_SESSION, &ta2taSession[NUM6], &value, &origin);
    ASSERT_EQ(ret, TEEC_SUCCESS);
    ASSERT_EQ(origin, TEEC_ORIGIN_TRUSTED_APP);
    ASSERT_EQ(value.origin, TEEC_ORIGIN_TEE);
    ASSERT_NE(ta2taSession[NUM6], 0);
}
...
TEE_TEST(TeeTcf2Ta2TaTest, TEE_InvokeTACommand_With_PARAM_TYPE_MEMREF, Function | MediumTest | Level0)
{
    ...
    ret = Invoke_InvokeTACommand(GetSession(), CMD_TEE_INVOKE_TA_COMMAND, ta2taSession, &value, &origin);
    ASSERT_EQ(ret, TEEC_SUCCESS);
    ASSERT_EQ(origin, TEEC_ORIGIN_TRUSTED_APP);
    ASSERT_EQ(value.origin, TEEC_ORIGIN_TEE);

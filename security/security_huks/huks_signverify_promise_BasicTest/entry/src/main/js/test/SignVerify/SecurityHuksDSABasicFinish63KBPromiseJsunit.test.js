...
function stringToUint8Array(str) {
    ...
    console.log(`test stringToUint8Array arr ${JSON.stringify(arr)}`);
    var tmpUint8Array = new Uint8Array(arr);
    console.log(`test stringToUint8Array tmpUint8Array ${JSON.stringify(tmpUint8Array)}`)
...
describe('SecurityHuksSignVerifyDSAPromiseJsunit', function () {
    ...
    it('testSignVerifyDSA001', 0, async function (done) {
        ...
        await publicSignVerifyFunc(srcKeyAlies, newSrcKeyAlies, HuksOptions, "finish", true)
        HuksOptions = {
            "properties": new Array(HuksSignVerify002.HuksKeyAlgDSA, HuksSignVerify002.HuksKeyDSAPurposeVERIFY, HuksSignVerify002.HuksTagDSADigestSHA1, HuksSignVerify002.HuksTagDSACOMMONSIZE),
            "inData": srcData63Kb,
        }
        await publicSignVerifyFunc(srcKeyAlies, newSrcKeyAlies, HuksOptions, "finish", false)
        done();
    })
})
...
let HuksCipher001 = {
    ...
    HuksKeyPurpose: {
        ...
        "value": HksKeyPurpose.HKS_KEY_PURPOSE_ENCRYPT | HksKeyPurpose.HKS_KEY_PURPOSE_DECRYPT
    },
    HuksKeyPurposeENCRYPT: { "tag": HksTag.HKS_TAG_PURPOSE, "value": HksKeyPurpose.HKS_KEY_PURPOSE_ENCRYPT },
    HuksKeyPurposeDECRYPT: { "tag": HksTag.HKS_TAG_PURPOSE, "value": HksKeyPurpose.HKS_KEY_PURPOSE_DECRYPT },
    HuksKeyAESSize128: { "tag": HksTag.HKS_TAG_KEY_SIZE, "value": HksKeySize.HKS_AES_KEY_SIZE_128 },
    HuksKeyAESSize192: { "tag": HksTag.HKS_TAG_KEY_SIZE, "value": HksKeySize.HKS_AES_KEY_SIZE_192 },
    HuksKeyAESSize256: { "tag": HksTag.HKS_TAG_KEY_SIZE, "value": HksKeySize.HKS_AES_KEY_SIZE_256 },
    HuksKeyAESPADDINGNONE: { "tag": HksTag.HKS_TAG_PADDING, "value": HksKeyPadding.HKS_PADDING_NONE },
    HuksKeyAESPADDINGPKCS7: { "tag": HksTag.HKS_TAG_PADDING, "value": HksKeyPadding.HKS_PADDING_PKCS7 },
    HuksKeyAESBLOCKMODE: { "tag": HksTag.HKS_TAG_BLOCK_MODE, "value": HksCipherMode.HKS_MODE_CBC },
    HuksKeyAESBLOCKMODECTR: { "tag": HksTag.HKS_TAG_BLOCK_MODE, "value": HksCipherMode.HKS_MODE_CTR },
    HuksKeyAESBLOCKMODEECB: { "tag": HksTag.HKS_TAG_BLOCK_MODE, "value": HksCipherMode.HKS_MODE_ECB },
    HuksKeyAESDIGESTNONE: { "tag": HksTag.HKS_TAG_DIGEST, "value": HksKeyDigest.HKS_DIGEST_NONE },
}
let defaultData = "0";
let srcData65 = "Hks_AES_Cipher_Test_00000000000000000000000000000000000000000000000000000_string";
...
async function publicGenerateKeyFunc(srcKeyAlies, genHuksOptionsNONECBC) {
    console.log(`test GenerateHuksOptions: ${JSON.stringify(genHuksOptionsNONECBC)}`);
    await huks.generateKey(srcKeyAlies, genHuksOptionsNONECBC).then((data) => {
...
async function finish(HuksOptions, isEncrypt) {
    await huks.finish(handle, HuksOptions).then((data) => {
        ...
        if (isEncrypt) {
            ...
            if (finishData === uint8ArrayToString(encryptedData)) {
                console.log(`test finish Decrypt success ${uint8ArrayToString(encryptedData)}`);
                console.log(`test finish Decrypt success ${uint8ArrayToString(finishData)}`);
                expect(data.errorCode == 0).assertTrue();
            } else {
                console.log(`test finish Decrypt fail ${uint8ArrayToString(encryptedData)}`);
                console.log(`test finish Decrypt fail ${uint8ArrayToString(finishData)}`);
                expect(null).assertFail();
            }
        }
    }).catch((err) => {
...
async function publicDeleteKeyFunc(srcKeyAlies, genHuksOptionsNONECBC) {
    await huks.deleteKey(srcKeyAlies, genHuksOptionsNONECBC).then((data) => {
...
describe('SecurityHuksCipherAESPromiseJsunit', function () {
    it('testCipherAES001', 0, async function (done) {
        ...
        genHuksOptions.properties.splice(3, 1, HuksCipher001.HuksKeyAESBLOCKMODE);
        genHuksOptions.properties.splice(4, 1, HuksCipher001.HuksKeyAESPADDINGNONE);
        let HuksOptions = {

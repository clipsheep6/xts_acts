...
let HuksCipher002 = {
    ...
    HuksKeyRSADIGESTSHA224: { "tag": HksTag.HKS_TAG_DIGEST, "value": HksKeyDigest.HKS_DIGEST_SHA224 },
    HuksKeyRSADIGESTSHA256: { "tag": HksTag.HKS_TAG_DIGEST, "value": HksKeyDigest.HKS_DIGEST_SHA256 },
    HuksKeyRSADIGESTSHA384: { "tag": HksTag.HKS_TAG_DIGEST, "value": HksKeyDigest.HKS_DIGEST_SHA384 },
    HuksKeyRSADIGESTSHA512: { "tag": HksTag.HKS_TAG_DIGEST, "value": HksKeyDigest.HKS_DIGEST_SHA512 },
}
let defaultData = "0";
...
async function publicGenerateKeyFunc(srcKeyAlies, genHuksOptionsNONC) {
    console.log(`test Generate HuksOptions: ${JSON.stringify(genHuksOptionsNONC)}`);
    await huks.generateKey(srcKeyAlies, genHuksOptionsNONC).then((data) => {
...
async function publicExportKeyFunc(srcKeyAlies, genHuksOptionsNONC) {
    ...
    await huks.exportKey(srcKeyAlies, genHuksOptionsNONC).then((data) => {
...
async function publicImportKeyFunc(srcKeyAlies, HuksOptions) {
    ...
    await huks.importKey(srcKeyAlies, HuksOptions).then((data) => {
...
async function finish(HuksOptions, isEncrypt) {
    ...
    console.log("test before finish HuksOptions outData: " + HuksOptions.outData.length);
    console.log("test before finish HuksOptions properties: " + JSON.stringify(HuksOptions.properties));
    ...
    await huks.finish(handle, HuksOptions).then((data) => {
        ...
        if (isEncrypt) {
            ...
            if (uint8ArrayToString(data.outData) === uint8ArrayToString(encryptedData)) {
                ...
                console.log(`test finish Encrypt fail ${uint8ArrayToString(data.outData)}`);
                expect(null).assertFail();
            } else {
                console.log(`test finish Encrypt success ${uint8ArrayToString(encryptedData)}`);
                console.log(`test finish Encrypt success ${uint8ArrayToString(data.outData)}`);
                expect(data.errorCode == 0).assertTrue();
            }
        } else {
            if (uint8ArrayToString(data.outData) === uint8ArrayToString(encryptedData)) {
                console.log(`test finish Decrypt success ${uint8ArrayToString(encryptedData)}`);
                console.log(`test finish Decrypt success ${uint8ArrayToString(data.outData)}`);
                expect(data.errorCode == 0).assertTrue();
            } else {
                console.log(`test finish Encrypt fail ${uint8ArrayToString(encryptedData)}`);
                console.log(`test finish Encrypt fail ${uint8ArrayToString(data.outData)}`);
...
async function publicDeleteKeyFunc(srcKeyAlies, genHuksOptionsNONC) {
    await huks.deleteKey(srcKeyAlies, genHuksOptionsNONC).then((data) => {
...
describe('SecurityHuksCipherRSAPromiseJsunit', function () {
    it('testCipherRSA001', 0, async function (done) {
        ...
        genHuksOptions.properties.splice(3, 1, HuksCipher002.HuksKeyRSABLOCKMODEECB);
        genHuksOptions.properties.splice(4, 1, HuksCipher002.HuksKeyRSAPADDINGNONE);
        genHuksOptions.properties.splice(5, 1, HuksCipher002.HuksKeyRSADIGESTSHA256);

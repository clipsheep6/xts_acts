...
describe('HKS_JSAPI_STRESS_TEST_SCENE_001', function () {
  ...
  function isKeyExistCallback(endTime, index, done){
    ...
    huks.isKeyExist(tmpAlias,emptyOption,function(err,data){
      ...
      if (currentCase == '00200' || currentCase == '01000') {
        index++;
        var currentTime = (new Date()).valueOf();
        console.info('HKS_STRESS_TEST_' + currentCase +' Time left ' + (endTime - currentTime) / 1000 + ' s,' + ' index: ' + index);
  ...
  it('HKS_JSAPI_STRESS_TEST_00900', 0, async function (done) {
    ...
    while (currentTime <= endTime) {
      ...
      console.info('HKS_JSAPI_STRESS_TEST_00900 Time left ' + (endTime - currentTime) / 1000 + ' s,' + ' index: ' + index);
      index++;
    }
    done();
  });

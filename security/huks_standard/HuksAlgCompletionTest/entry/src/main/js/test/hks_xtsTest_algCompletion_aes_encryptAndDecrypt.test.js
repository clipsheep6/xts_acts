...
describe('Hks_XtsTest_algCompletion_Aes_Encrypt_Decrypt', function () {
  ...
  async function encryptAndDecryptWithPromise (size, padding, mode, done, caseId){
    var genKeyOpt = makeGenerateKeyOption(
      huks.HksKeyAlg.HKS_ALG_AES, size, huks.HksKeyPurpose.HKS_KEY_PURPOSE_ENCRYPT | huks.HksKeyPurpose.HKS_KEY_PURPOSE_DECRYPT, padding, mode
    );
    var genKeyRet = await huks.generateKey(aliasForAES, genKeyOpt);
    expect(genKeyRet.errorCode).assertEqual(huks.HksErrorCode.HKS_SUCCESS);
    var isKeyExist = await huks.isKeyExist(aliasForAES, emptyOption);
    expect(isKeyExist).assertEqual(true);
    var encryptOpt = makeEncryptAndDecryptOption(
      huks.HksKeyAlg.HKS_ALG_AES, huks.HksKeyPurpose.HKS_KEY_PURPOSE_ENCRYPT, padding, mode, null, null, plainText
    );
    var encryptRet = await huks.encrypt(aliasForAES, encryptOpt);
    if (useLib == 'openssl' ||
    (useLib == 'mbedtls' &&
    (mode == huks.HksCipherMode.HKS_MODE_CBC || mode == huks.HksCipherMode.HKS_MODE_CTR || mode == huks.HksCipherMode.HKS_MODE_GCM))) {
      expect(encryptRet.errorCode).assertEqual(huks.HksErrorCode.HKS_SUCCESS);
      var decryptOpt = makeEncryptAndDecryptOption(
        huks.HksKeyAlg.HKS_ALG_AES, huks.HksKeyPurpose.HKS_KEY_PURPOSE_DECRYPT, padding, mode, null, null, encryptRet.outData
      );
      var decryptRet = await huks.decrypt(aliasForAES, decryptOpt);
      ...
      var deleteRet = await huks.deleteKey(aliasForAES, emptyOption);
      expect(deleteRet.errorCode).assertEqual(huks.HksErrorCode.HKS_SUCCESS);
      var isKeyExist = await huks.isKeyExist(aliasForAES, emptyOption);
      expect(isKeyExist).assertEqual(false);
    } else if (mode == huks.HksCipherMode.HKS_MODE_ECB && padding == huks.HksKeyPadding.HKS_PADDING_PKCS7) {
      expect(encryptRet.errorCode).assertEqual(huks.HksErrorCode.HKS_ERROR_NOT_SUPPORTED);
    } else {
      expect(encryptRet.errorCode).assertEqual(huks.HksErrorCode.HKS_ERROR_CRYPTO_ENGINE_ERROR);
    }
  ...
  function checkKeyIsExistAfterDeleteKey(done, caseId) {
    huks.isKeyExist(aliasForAES, emptyOption, function (err, data) {
  ...
  function deleteKeyWithCallback(done, caseId) {
    huks.deleteKey(aliasForAES, emptyOption, function (err, data) {
  ...
  function decryptWithCallback(size, padding, mode, ciphertext, done, caseId) {
    ...
    huks.decrypt(aliasForAES, decryptOpt, function (err, data) {
      expect(data.errorCode).assertEqual(huks.HksErrorCode.HKS_SUCCESS);
  ...
  function encryptWithCallback(size, padding, mode, done, caseId) {
    ...
    huks.encrypt(aliasForAES, encryptOpt, function (err, data) {
      if (useLib == 'openssl' ||
      (useLib == 'mbedtls' &&
      (mode == huks.HksCipherMode.HKS_MODE_CBC || mode == huks.HksCipherMode.HKS_MODE_CTR || mode == huks.HksCipherMode.HKS_MODE_GCM))) {
        expect(data.errorCode).assertEqual(huks.HksErrorCode.HKS_SUCCESS);
  ...
  function encryptAndDecryptWithCallback(size, padding, mode, done, caseId) {
    ...
    huks.generateKey(aliasForAES, genKeyOpt, function (err, data) {
      expect(data.errorCode).assertEqual(huks.HksErrorCode.HKS_SUCCESS);

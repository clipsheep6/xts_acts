...
static constexpr OHOS::HiviewDFX::HiLogLabel LABEL = {
    LOG_CORE, SECURITY_DOMAIN_PERMISSION, "DistributedPermissionUsedRecordsDisorderCppTest"};
const int STRESS_TEST_TIME = 60 * 60 * 6;
const int THREAD_COUNT = 15;
}  // namespace
class DistributedPermissionUsedRecordsDisorderCppTest : public testing::Test {
public:
    ...
    static void ExeAPIOrder(std::string &deviceId, int32_t ruid, int32_t duid);
    static void ExeAPIDisorder(std::string &deviceId, int32_t ruid, int32_t duid);
};

void DistributedPermissionUsedRecordsDisorderCppTest::SetUpTestCase(void)
{
    STDistibutePermissionUtil::Install(DPMS_THIRD_HAP_NAME);
}

void DistributedPermissionUsedRecordsDisorderCppTest::TearDownTestCase(void)
{
    STDistibutePermissionUtil::Uninstall(DPMS_THIRD_BUNDLE_NAME);
}

void DistributedPermissionUsedRecordsDisorderCppTest::SetUp(void)
{}

void DistributedPermissionUsedRecordsDisorderCppTest::TearDown(void)
{}

void DistributedPermissionUsedRecordsDisorderCppTest::TestFountion(std::string deviceId, int32_t ruid, int32_t duid)
{
    ...
    while (int(std::chrono::duration_cast<std::chrono::seconds>(std::chrono::system_clock::now() - starTime).count()) <=
           STRESS_TEST_TIME) {
        ExeAPIDisorder(deviceId, ruid, duid);
    }
}

void DistributedPermissionUsedRecordsDisorderCppTest::ExeAPIDisorder(std::string &deviceId, int32_t ruid, int32_t duid)
{
...
void DistributedPermissionUsedRecordsDisorderCppTest::StressFountion()
{
 ...
 * @tc.level : Level 3
 */
HWTEST_F(DistributedPermissionUsedRecordsDisorderCppTest, DPMS_PermissionUsedRecord_0100, TestSize.Level3)
{
    GTEST_LOG_(INFO) << "DPMS_PermissionUsedRecord_0100  start";
    auto starTime = std::chrono::system_clock::now();
    DistributedPermissionUsedRecordsDisorderCppTest::StressFountion();

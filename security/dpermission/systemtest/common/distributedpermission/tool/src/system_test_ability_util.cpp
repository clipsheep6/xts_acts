...
sptr<IAbilityManager> STAbilityUtil::GetAbilityManagerService2()
{
...
ErrCode STAbilityUtil::StartAbility2(const Want &want, sptr<IAbilityManager> &abilityMs, const time_t &delay)
{
    ErrCode result = OHOS::ERR_OK;
    abilityMs = GetAbilityManagerService2();
    if (abilityMs == nullptr) {
        result = OHOS::ERR_INVALID_VALUE;
        HILOG_ERROR("failed to get ability manager service");
        return result;
...
ErrCode STAbilityUtil::GetRunningProcessInfo(std::vector<RunningProcessInfo> &runningProcessInfo, sptr<IAppMgr> &appMs,
    const time_t &delay)
{
...
RunningProcessInfo STAbilityUtil::GetAppProcessInfoByName(const std::string &processName, sptr<IAppMgr> &appMs,
    const time_t &delay)
{
...
void STAbilityUtil::DeserializationStOperatorFromVector(StOperator &ParentOperator,
    std::vector<std::string> &vectorOperator)
{
...
int STAbilityUtil::RemoveStack(int id, sptr<AAFwk::IAbilityManager> &abilityMs, const time_t &backHmoeDelay,
...
int InstallToolStatusReceiver::TestWaitCompleted(Event &event, const std::string eventName, const int code,
    const int timeout)
{

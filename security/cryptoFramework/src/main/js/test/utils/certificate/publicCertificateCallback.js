...
async function verifyX509CrlCallback(certType) {
    var gInstance;
    await createX509CrlInstanceCallback(certType)
        .then(instance => {
            gInstance = instance;
            gInstance.getPublicKey((err, publicKey) => {
                if (publicKey == null) {
                    console.error("[callback] Crl getPublickey failed, publickey is null");
                    expect(null).assertFail();
                }
                console.warn("[callback] Crl getPublickey success");
                gInstance.verify(publicKey, (err1,) => {
                    if (err1) {
                        console.error("[callback] Crl certificate verify failed");
                        expect(err1.code == 0).assertFail();
                    } else {
                        console.warn("[callback] Crl certificate verify success");
                        expect(1 == 1).assertTrue();
                    }
                })
            })

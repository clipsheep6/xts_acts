...
async function verifyX509CrlPromise(certType) {
    var gInstance;
    await createX509CrlInstancePromise(certType)
        .then(instance => {
            gInstance = instance;
            ...
            if (publicKey == null) {
                console.error("[promise] Crl getPublickey failed! data is null ");
                return;
            }
            console.warn("[promise] Crl getPublickey success!");
            let verifyret = gInstance.verify(publicKey);
            return verifyret;
        }).then(verifyret => {
            expect(verifyret == null).assertTrue();
        }).catch(err => {
            console.error("[promise] Crl getPublickey failed! error  is: " + err);
            expect(null).assertFail();
        });
}

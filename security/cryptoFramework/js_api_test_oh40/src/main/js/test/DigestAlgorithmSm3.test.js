  ...
  describe("DigestAlgorithmSm3Jsunit", function () {
    ...
    it("Security_CryptoFramework_MDSM3_Func_0100", 0, async function (done) {
      await DigPromise.testMDDigestPromise("SM3").then((result) => {
        ...
        .catch((err) => {
          console.error("Security_CryptoFramework_MDSM3_Func_0100 catch err: " + err);
    ...
    it("Security_CryptoFramework_MDSM3_Func_0400", 0, async function (done) {
      ...
      try {
        await new Promise((resolve, reject) => {
          globalMd.update((err) => {
            if (err) {
              console.error("Security_CryptoFramework_MDSM3_Func_0400 catch 1 error is " + err);
              reject(err);
            } else {
      ...
      try {
        await new Promise((resolve, reject) => {
          globalMd.update(null, (err) => {
            if (err) {
              console.error("Security_CryptoFramework_MDSM3_Func_0400 catch 2 error is " + err);
              reject(err);
            } else {
              resolve("update success");
            }
    ...
    it("Security_CryptoFramework_HMACSM3_Func_0700", 0, async function (done) {
      ...
      try {
        let result = await new Promise((resolve, reject) => {
          ...
          console.log("HMAC= " + globalHMAC);
          console.log("HMAC algName is: " + globalHMAC.algName);
          globalsymKeyGenerator = cryptoFramework.createSymKeyGenerator("AES128");
          console.log("HMAC SymKeyGenerator algName:" + globalsymKeyGenerator.algName);

          globalsymKeyGenerator.generateSymKey().then((symKey) => {
            return globalHMAC.init(symKey);
          }).then((initData) => {
            console.log("HMAC Init success: " + initData);
            return globalHMAC.update(ginBlob);
          }).then((updateData) => {
            console.log("HMAC updateData success: " + updateData);
            return globalHMAC.doFinal();
          }).then((doFinalBlob) => {
            console.log("HMAC Digest result: " + uInt8ArrayToShowStr(doFinalBlob.data));
            ...
            console.error("HMAC TestHMACDigestCallback catch error: " + err);
            reject(err);
          });
        });
          expect(result).assertTrue();
    ...
    it("Security_CryptoFramework_HMACSM3_Func_0900", 0, async function (done) {
      ...
      let globalsymKeyGenerator = cryptoFramework.createSymKeyGenerator("SM4_128");
    ...
    /**
     * @tc.number Security_CryptoFramework_HMACSM3_Func_1100
     * @tc.name Obtain the calculate message authentication code algorithm name
     * @tc.desc the asyAlgoName is "SM3", Use the async Style of Interface
     */
    it("Security_CryptoFramework_HMACSM3_Func_1100", 0, async function (done) {
      try {

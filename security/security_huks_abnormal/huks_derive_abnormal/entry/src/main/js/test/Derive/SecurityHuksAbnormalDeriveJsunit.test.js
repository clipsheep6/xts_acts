...
async function publicDeriveInitFunc(srcKeyAlies, HuksOptions, Type) {
  await huks
    ...
    .then((data) => {
      ...
      console.log(`test init data ${JSON.stringify(data.errorCode)}`);
      console.log(`test init data ${JSON.stringify(Type)}`);
      handle = {
...
async function publicDeriveUpdateFunc(handle, HuksOptions, Type) {
  await huks
    ...
    .then((data) => {
      ...
      console.log(`test update data ${JSON.stringify(Type)}`);
      expect(data.errorCode == Type).assertTrue();
    })
    .catch((err) => {
  ...
  if (thirdInderfaceName == 'finish') {
    await huks
      ...
      .then((data) => {
        ...
        console.log(`test finish data ${JSON.stringify(data.errorCode)}`);
        console.log(`test finish data ${JSON.stringify(Type)}`);
        expect(data.errorCode == Type).assertTrue();
      })
      .catch((err) => {
    ...
    await huks
      ...
      .then((data) => {
        ...
        console.log(`test abort data ${JSON.stringify(data.errorCode)}`);
        console.log(`test abort data ${JSON.stringify(Type)}`);
        expect(data.errorCode == Type).assertTrue();
      })
      .catch((err) => {
...
async function publicDeriveDeleteFunc(srcKeyAlies, HuksOptions, Type) {
  await huks
    ...
    .then((data) => {
      ...
      console.log(`test deleteKey data ${JSON.stringify(data.errorCode)}`);
      console.log(`test deleteKey data ${JSON.stringify(Type)}`);
      expect(data.errorCode == Type).assertTrue();
    })
    .catch((err) => {
  ...
  beforeEach(function () {
    ...
    HuksOptionsFinish = {
      properties: new Array(
        ...
        HuksDeriveHKDF.HuksKeyBLOCK_MODEECB
      ),
      inData: srcData63Kb,
    };
  });
